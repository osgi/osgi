 ...
 * <p>
 * 
 * To make your own tests, subclass this class and implement the testmethods
 * that you need. Then make write your own {@link #getMethods() getMethods}that
 * returns a list of the method names that you want to run.
 * <p>
 * 
 * There are also some special methods that you can override if you have some
 * preparation code that you want to run. The methods are
 * <ul>
 ...
 * 
 * <p>
 * Assume you have two test-methods of your own, <code>testSomething()</code>
 * and <code>testSomethingElse()</code>. These methods are called in the
 * following order.
 * 
 * <pre>
 <code>
 ...
 *  
 */
public abstract class DefaultTestBundleControl extends Logger implements
		Runnable, BundleActivator, TestLogger {
	...
	private String[] getTestMethods() {
		...
		/*
		 * If there are no method list specified in the subclass, find all
		 * methods starting with "test" and call them in any order.
		 */
		if (methodNames == null) {
	 ...
	 * before all the testmethods. It should check so that everything needed to
	 * run the test is available and report if everything is ok or not. The
	 * default implementation returns true but should be overridden in most
	 * cases to do the proper checks.
	 * 
	 * @return <code>true</code> if all necessary preparations could be made,
	 ...
	 * returned by getMethods(). Override this to do things after all tests have
	 * been run.
	 */
	...
	/**
	 * The setState method is called before each and every testmethod Override
	 * this to set a specific state before each testmethod is called.
	 */
	protected void setState() throws Exception {
	}

	/**
	 * The clearState method can be used to undo whatever the setState did. It
	...
	public void run() {
		...
		try {
			...
			mainRegistry
					.registerService(TestLogger.class.getName(), this, null);
			if (webserver == null)
				...
				if (checkPrerequisites() == true) {
					...
					for (int i = 0; cont && i < methods.length; i++) {
						try {
							...
							Method m = getClass().getDeclaredMethod(methods[i],
									new Class[0]);
							...
							if (t instanceof AssertionFailedError) {
								...
								log("During execution of " + methods[i], ite
										.getTargetException());
								ite.getTargetException().printStackTrace();
								cont = getContinuationAfterError(ite
										.getTargetException());
							}
						}
						catch (Throwable e) {
					...
					/*
					 * Uninstall all bundles that have been installed during the
					 * tests
					 */
					link.send("ready");
				}
	...
	/**
	 * Return a flag to indicate if we should continue after this error or not.
	 */
	...
	/** * Convenience methods ** */
	/**
	 * Convenience method to convert an array to a String. The method converts
	 * each element in the array to a string so the usefulness of the output
	 * depends on if the elements has useful toString() methods.
	 * 
	 * @param array the array to be converted
	 * @return a String formatted as
	 *         <code>"{ element1string, element2string, ... }"</code>, or
	 *         "null" if <code>array</code> was <code>null</code>.
	 */
	...
	public Bundle installBundle(String bundleName) throws Exception {
		return mainRegistry.installBundle(bundleName);
	}

	public boolean isBundleInstalled(String bundleName) {
		return mainRegistry.isBundleInstalled(bundleName);
	}

	public void uninstallBundle(Bundle bundleName) throws Exception {
		mainRegistry.uninstallBundle(bundleName);
	}

	public void uninstallAllBundles() throws Exception {
		mainRegistry.uninstallAllBundles();
	}

	public Object getService(Class clazz) throws Exception {
		return mainRegistry.getService(clazz);
	}

	public Object getService(Class clazz, String filter) throws Exception {
		return mainRegistry.getService(clazz, filter);
	}

	public void ungetService(Object service) {
		mainRegistry.ungetService(service);
	}

	public void ungetAllServices() {
		mainRegistry.ungetAllServices();
	}

	public boolean serviceAvailable(Class clazz) {
		return mainRegistry.serviceAvailable(clazz);
	}

	public boolean securityNeeded(boolean needed) {
		return mainRegistry.securityNeeded(needed);
	}
	...
	public void unregisterService(Object service) {
		mainRegistry.unregisterService(service);
	}

	public void unregisterAllServices() {
		mainRegistry.unregisterAllServices();
	}
}
