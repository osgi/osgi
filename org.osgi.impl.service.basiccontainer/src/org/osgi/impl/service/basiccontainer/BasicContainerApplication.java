...
class BasicApplicationThread extends Thread
{
  ...
  public BasicApplicationThread( ApplicationHandle appHandle, BundleContext bc, ApplicationContext appContext )
  {
  ...
  {    
    try {
      BasicContainerDescriptor desc = (BasicContainerDescriptor)(appHandle.getAppDescriptor() );
   
      ClassLoader loader = new FileClassLoader( bc.getDataFile( desc.getId() ) );
      ...
      for( int i=0; keys.hasNext(); i += 2 )
      {
        ...
        args[i+1] = appContext.getLaunchArgs().get( args[i] ).toString();
      }
    
    
      Method mainMethod = main.getMethod( "main", new Class[] { args.getClass() } );
...
public class BasicContainerApplication implements Application 
{
  ...
  BasicContainerApplication( ApplicationContext ctx, ApplicationHandle handle, BundleContext bc )
  {
    appContext = ctx;
    appHandle = handle;
    this.bc = bc;
  ...
  public void startApplication() throws Exception
  {
    ...
    BasicApplicationThread appThread = new BasicApplicationThread( appHandle, bc, appContext );
    appThread.start();
  }
