...
public class BasicContainer implements ApplicationContainer
{
  ...
  public ApplicationDescriptor[] uninstallApplication(ApplicationDescriptor appDescriptor, boolean force) throws IOException, Exception
  {
    ...
    if( applicationIds.contains( desc.getId() ) )
    {
      ...
      System.out.println( "Can't uninstall application ID (" + desc.getId() + ")!" );
    return null;
  }
  ...
  private ApplicationDescriptor[] download( String id, InputStream inputStream, ApplicationDescriptor appDesc ) throws Exception
  {
    String dirName = id + "/";
    ...
    while( ( zipEntry = zipStream.getNextEntry()) != null )
    {
      ...
      else
      {
        ...
        BufferedOutputStream bostream = new BufferedOutputStream( outStream );
  ...
  private BasicContainerDescriptor [] registerApplication( String id, ApplicationDescriptor appDesc ) throws Exception
  {
    ...
    if( appDesc == null ) 
      desc = new BasicContainerDescriptor( appName, appCategory, appVersion, containerID, id, mainClass );
    else
    {
    ...
    ServiceRegistration serviceReg = bc.registerService("org.osgi.service.application.ApplicationDescriptor", desc, properties);
    ...
    if( installedApps.exists() )
    {
      ...
      if( !codedIds.equals("") )
      {
        int index = 0;
          ...
          System.out.println( "Application with id:" + name + " found!" );
  ...
  private void saveApplicationIds() throws Exception
  {
    ...
    for( int i=0; i != applicationIds.size(); i++ )
      stream.write( ( ( (i == 0) ? "" : "," ) + (String)applicationIds.get( i ) ).getBytes() );
