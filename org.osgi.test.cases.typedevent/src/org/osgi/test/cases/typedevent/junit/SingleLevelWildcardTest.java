...
public class SingleLevelWildcardTest {
	...
	class TypedHandlerWildcards {
		...
		public void testTypedHandlerWithSingleLevelWildcard() {
			TypedEventHandler<EventA> handler = mock(TypedEventHandler.class);
			...
			eventBus.deliver("custom/foo/topic", event);

			Mockito.verify(handler, Mockito.timeout(8000))
					.notify(eq("custom/foo/topic"),
							argThat(e -> "test".equals(e.a)));
		}

		@SuppressWarnings("unchecked")
		@Test
		@DisplayName("Typed handler should not receive events not matching single-level wildcard")
		public void testTypedHandlerWithSingleLevelWildcardNoMatch() {
			TypedEventHandler<EventA> handler = mock(TypedEventHandler.class);
			...
			eventBus.deliver("custom/topic", event);
