...
public class GetNodeVersion {
	 ...
	 *                  getNodeVersion to the correct plugin.
	 */
	private void testGetNodeVersion001() {
		...
		try {
			...
			int nodeVersion = session.getNodeVersion(TestReadOnlyDataPluginActivator.TEST_READ_ONLY_PLUGIN_ROOT);
			tbc.assertEquals("Asserts that DmtAdmin fowarded "+ TestReadOnlyDataPlugin.GETNODEVERSION
					+" to the correct plugin",TestReadOnlyDataPlugin.GETNODEVERSION_VALUE,nodeVersion);
	 ...
	 *                  DmtReadOnly#getNodeVersion correctly forwards the
	...
	private void testGetNodeVersion002() {
		...
		try {
			...
			session = tbc.getDmtAdmin().getSession(TestReadOnlyDataPluginActivator.TEST_READ_ONLY_PLUGIN_ROOT,
					DmtSession.LOCK_TYPE_EXCLUSIVE);
			session.getNodeVersion(TestReadOnlyDataPluginActivator.INTERIOR_NODE_EXCEPTION);
			...
			tbc.assertTrue("Asserts that DmtAdmin fowarded the DmtException with the correct message. ", e
					.getMessage().indexOf(TestReadOnlyDataPlugin.GETNODEVERSION)>-1);			
		} catch (Exception e) {
