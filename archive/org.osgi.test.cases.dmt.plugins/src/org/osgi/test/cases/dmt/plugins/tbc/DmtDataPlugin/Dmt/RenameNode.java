...
public class RenameNode {
	 ...
	 *                  renameNode to the correct plugin.
	 */
	private void testRenameNode001() {
		...
		try {
			...
			tbc.assertEquals("Asserts that DmtAdmin fowarded "+ TestDataPlugin.RENAMENODE+" to the correct plugin",TestDataPlugin.RENAMENODE,DmtTestControl.TEMPORARY);
			tbc.assertEquals("Asserts that DmtAdmin the parameter was fowarded to the correct plugin without modification",newName,DmtTestControl.PARAMETER_2);
		} catch (Exception e) {
	 ...
	 *                  Dmt#renameNode correctly forwards the 
