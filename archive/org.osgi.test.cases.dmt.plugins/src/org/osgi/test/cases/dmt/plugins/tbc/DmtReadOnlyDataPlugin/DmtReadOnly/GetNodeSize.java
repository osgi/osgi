 ...
 * @author Andre Assad
 * 
 * @methodUnderTest org.osgi.service.dmt.DmtReadOnly#getNodeSize
...
public class GetNodeSize {
	 ...
	 *                  getNodeSize to the correct plugin.
	 */
	private void testGetNodeSize001() {
		...
		try {
			...
			int nodeSize = session.getNodeSize(TestReadOnlyDataPluginActivator.LEAF_NODE);
			tbc.assertEquals("Asserts that DmtAdmin fowarded "+ TestReadOnlyDataPlugin.GETNODESIZE
					+" to the correct plugin",TestReadOnlyDataPlugin.GETNODESIZE_VALUE,nodeSize);
		} catch (Exception e) {
	 ...
	 *                  DmtReadOnly#getNodeSize correctly forwards the
	...
	private void testGetNodeSize002() {
		...
		try {
			...
			session = tbc.getDmtAdmin().getSession(TestReadOnlyDataPluginActivator.TEST_READ_ONLY_PLUGIN_ROOT,
					DmtSession.LOCK_TYPE_EXCLUSIVE);
			session.getNodeSize(TestReadOnlyDataPluginActivator.LEAF_NODE_EXCEPTION);
			
			tbc.failException("", DmtException.class);
		} catch (DmtException e) {
			
			tbc.assertEquals("Asserts that DmtAdmin fowarded the DmtException with the correct subtree: ", 
					TestReadOnlyDataPluginActivator.LEAF_NODE_EXCEPTION, e.getURI());			
			...
			tbc.assertTrue("Asserts that DmtAdmin fowarded the DmtException with the correct message. ", 
					e.getMessage().indexOf(TestReadOnlyDataPlugin.GETNODESIZE)>-1);
