/*
 * Copyright (c) The OSGi Alliance (2004). All Rights Reserved.
 *
 * Implementation of certain elements of the OSGi Specification may be subject
 * to third party intellectual property rights, including without limitation,
 * patent rights (such a third party may or may not be a member of the OSGi
 * Alliance). The OSGi Alliance is not responsible and shall not be held
 * responsible in any manner for identifying or failing to identify any or all
 * such third party intellectual property rights.
 *
 * This document and the information contained herein are provided on an "AS IS"
 * basis and THE OSGI ALLIANCE DISCLAIMS ALL WARRANTIES, EXPRESS OR IMPLIED,
 * INCLUDING BUT NOT LIMITED TO ANY WARRANTY THAT THE USE OF THE INFORMATION
 * HEREIN WILL NOT INFRINGE ANY RIGHTS AND ANY IMPLIED WARRANTIES OF
 * MERCHANTABILITY OR FITNESS FOR A PARTICULAR PURPOSE. IN NO EVENT WILL THE
 * OSGI ALLIANCE BE LIABLE FOR ANY LOSS OF PROFITS, LOSS OF BUSINESS, LOSS OF
 * USE OF DATA, INTERRUPTION OF BUSINESS, OR FOR DIRECT, INDIRECT, SPECIAL OR
 * EXEMPLARY, INCIDENTIAL, PUNITIVE OR CONSEQUENTIAL DAMAGES OF ANY KIND IN
 * CONNECTION WITH THIS DOCUMENT OR THE INFORMATION CONTAINED HEREIN, EVEN IF
 * ADVISED OF THE POSSIBILITY OF SUCH LOSS OR DAMAGE.
 *
 * All Company, brand and product names may be trademarks that are the sole
 * property of their respective owners. All rights reserved.
 */

/* 
 * REVISION HISTORY:
 *
 * Date          Author(s)
 * CR            Headline
 * ============  ==============================================================
 * Jan 31, 2005  Andre Assad
 * 1             Implement MEG TCK
 * ============  ==============================================================
 * Feb 11, 2005  Andre Assad
 * 1             Updates after formal inspection (BTC_MEG_TCK_CODE-INSPR-001)
 * ============  ==============================================================
 * Mar 01, 2005  Luiz Felipe Guimaraes
 * 1             Updates 
 * ============  ==============================================================
 */

package org.osgi.test.cases.dmt.main.tbc.DmtAcl;

import org.osgi.test.cases.dmt.main.tbc.DmtTestControl;
import org.osgi.service.dmt.DmtAcl;

/**
 * @author Andre Assad
 * 
 * @methodUnderTest org.osgi.service.dmt.DmtAcl#deletePermission
 * @generalDescription This Test Class Validates the implementation of
 *                     <code>deletePermission<code> method, according to MEG reference
 *                     documentation (rfc0085).
 */
public class DeletePermission {
	private DmtTestControl tbc;

	private static final String ACL_DEFAULT = "Add=" + DmtTestControl.PRINCIPAL
			+ "&Delete=" + DmtTestControl.PRINCIPAL + "&Exec="
			+ DmtTestControl.PRINCIPAL + "&Get=*";

	public DeletePermission(DmtTestControl tbc) {
		this.tbc = tbc;
	}

	public void run() {
		testDeletePermission001();
		testDeletePermission002();
		testDeletePermission003();
		testDeletePermission004();
		testDeletePermission005();
		testDeletePermission006();
		testDeletePermission007();
	}

	/**
	 * @testID testDeletePermission001
	 * @testDescription This method asserts that a principal permission is
	 *                  correctly deleted from its Acl.
	 */
	private void testDeletePermission001() {
		try {
			tbc.log("#testDeletePermission001");
			DmtAcl dmtAcl = new DmtAcl(ACL_DEFAULT);

			dmtAcl = dmtAcl.deletePermission(DmtTestControl.PRINCIPAL,
					DmtAcl.DELETE);

			tbc.assertEquals("Asserting deleted permission", DmtAcl.ADD
					| DmtAcl.EXEC | DmtAcl.GET, dmtAcl
					.getPermissions(DmtTestControl.PRINCIPAL));
		} catch (Exception e) {
			tbc.fail("Unexpected exception: " + e.getClass().getName());
		}
	}

	/**
	 * @testID testDeletePermission002
	 * @testDescription Test that more than one permission can be deleted in the
	 *                  same time from a principal.
	 */
	private void testDeletePermission002() {
		try {
			tbc.log("#testDeletePermission002");
			DmtAcl dmtAcl = new DmtAcl(ACL_DEFAULT);

			dmtAcl = dmtAcl.deletePermission(DmtTestControl.PRINCIPAL,
					DmtAcl.DELETE | DmtAcl.EXEC);

			tbc.assertEquals("Asserting deleted permission", DmtAcl.ADD
					| DmtAcl.GET, dmtAcl
					.getPermissions(DmtTestControl.PRINCIPAL));
		} catch (Exception e) {
			tbc.fail("Unexpected exception: " + e.getClass().getName());
		}
	}

	/**
	 * @testID testDeletePermission003
	 * @testDescription Asserts that an IllegalArgumentException is thrown
	 *                  whenever an invalid permission is deleted from a
	 *                  principal.
	 */
	private void testDeletePermission003() {
		try {
			tbc.log("#testDeletePermission003");
			DmtAcl dmtAcl = new DmtAcl(ACL_DEFAULT);
			dmtAcl = dmtAcl.deletePermission(DmtTestControl.PRINCIPAL, 2005);
			tbc.failException("#", IllegalStateException.class);
		} catch (IllegalArgumentException e) {
			tbc.pass("IllegalArgumentException correctly thrown");
		} catch (Exception e) {
			tbc.fail("Expected IllegalArgumentException and "
					+ e.getClass().getName() + " was thrown");
		}
	}

	/**
	 * @testID testDeletePermission004
	 * @testDescription Asserts that an IllegalArgumentException is thrown
	 *                  whenever a valid permission is deleted from an invalid
	 *                  principal.
	 */
	private void testDeletePermission004() {
		try {
			tbc.log("#testDeletePermission004");
			DmtAcl dmtAcl = new DmtAcl(ACL_DEFAULT);
			dmtAcl = dmtAcl.deletePermission(DmtTestControl.INVALID,
					DmtAcl.EXEC);
			tbc.failException("#", IllegalStateException.class);
		} catch (IllegalArgumentException e) {
			tbc.pass("IllegalArgumentException correctly thrown");
		} catch (Exception e) {
			tbc.fail("Expected IllegalArgumentException and "
					+ e.getClass().getName() + " was thrown");
		}
	}

	/**
	 * @testID testDeletePermission005
	 * @testDescription Tests if this method creates a new instance of DmtAcl
	 *                  without removing any permissions from the existing
	 *                  DmtAcl
	 * 
	 */
	private void testDeletePermission005() {
		try {
			tbc.log("#testDeletePermission005");
			DmtAcl dmtAcl = new DmtAcl("Add=" + DmtTestControl.PRINCIPAL
					+ "&Delete=" + DmtTestControl.PRINCIPAL + "&Exec="
					+ DmtTestControl.PRINCIPAL + "&Get=*");

			DmtAcl dmtAcl2 = dmtAcl.deletePermission(DmtTestControl.PRINCIPAL,
					DmtAcl.EXEC);

			tbc.assertEquals("Assert " + DmtTestControl.PRINCIPAL
					+ " permission", DmtAcl.GET | DmtAcl.ADD | DmtAcl.EXEC
					| DmtAcl.DELETE, dmtAcl
					.getPermissions(DmtTestControl.PRINCIPAL));

			tbc.assertEquals(
					"Assert that '*' grants permission to all principals",
					DmtAcl.GET, dmtAcl2
							.getPermissions(DmtTestControl.PRINCIPAL_2));

			tbc.assertEquals("Assert " + DmtTestControl.PRINCIPAL
					+ " permission", DmtAcl.GET | DmtAcl.ADD | DmtAcl.DELETE,
					dmtAcl2.getPermissions(DmtTestControl.PRINCIPAL));

			tbc.assertEquals(
					"Assert that '*' grants permission to all principals",
					DmtAcl.GET, dmtAcl
							.getPermissions(DmtTestControl.PRINCIPAL_2));
		} catch (Exception e) {
			tbc.fail("Unexpected exception: " + e.getClass().getName());
		}
	}

	/**
	 * @testID testDeletePermission006
	 * @testDescription Asserts that an IllegalArgumentException is thrown when
	 *                  a globally granted permission is revoked from a specific
	 *                  principal
	 */
	private void testDeletePermission006() {
		try {
			tbc.log("#testDeletePermission006");
			DmtAcl dmtAcl = new DmtAcl(ACL_DEFAULT);

			dmtAcl = dmtAcl.deletePermission(DmtTestControl.PRINCIPAL,
					DmtAcl.GET);
			tbc.failException("#", IllegalArgumentException.class);
		} catch (IllegalArgumentException e) {
			tbc.pass("IllegalArgumentException correctly thrown");
		} catch (Exception e) {
			tbc.fail("Expected IllegalArgumentException and "
					+ e.getClass().getName() + " was thrown");
		}
	}

	/**
	 * @testID testDeletePermission007
	 * @testDescription Asserts if "*" revokes permissions from all principals.
	 */
	private void testDeletePermission007() {
		try {
			tbc.log("#testDeletePermission007");
			String[] principal = { DmtTestControl.PRINCIPAL,
					DmtTestControl.PRINCIPAL_2, "*" };
			int[] perm = { DmtAcl.GET, DmtAcl.EXEC, DmtAcl.ADD };

			DmtAcl dmtAcl = new DmtAcl(principal, perm);
			dmtAcl = dmtAcl.deletePermission("*", DmtAcl.ADD);
			boolean passed = false;

			if (dmtAcl.getPermissions(principal[0]) == (DmtAcl.GET)) {
				if (dmtAcl.getPermissions(principal[1]) == (DmtAcl.EXEC)) {
					passed = true;
				}
			}
			tbc.assertTrue(
					"Asserts if '*' revokes permissions from all principals.",
					passed);

		} catch (Exception e) {
			tbc.fail("Unexpected exception: " + e.getClass().getName());
		}
	}

}
