    ...
    implements org.osgi.service.application.midlet.MidletDescriptor.Delegate
{
    ...
    public void init(BundleContext bc, Properties props, Map names, Map icons, String defaultLang, String startClass, Bundle bundle, 
        ...
        throws Exception
    {
        ...
        this.bundle = bundle;
        if(names.size() == 0 || icons.size() == 0 || !props.containsKey("application.bundle.id") || !props.containsKey("service.pid") || !props.containsKey("application.version"))
    ...
    public Map getPropertiesSpecific(String locale)
    {
        ...
        properties.put("application.version", props.getProperty("application.version"));
        properties.put("application.vendor", props.getProperty("application.vendor"));
        String singleton = props.getProperty("application.singleton");
    ...
    public ApplicationHandle launchSpecific(Map args)
        throws Exception
    {
        ...
        if(midlet == null)
            ...
            return createMidletHandleByReflection(midlet, args, refs);
    }
    ...
    public MidletHandle createMidletHandleByReflection(MIDlet midlet, Map args, Vector refs)
    {
        try
        {
            Class midletHandleClass = org.osgi.service.application.midlet.MidletHandle.class;
            Constructor constructor = midletHandleClass.getDeclaredConstructor(new Class[] {
                java.lang.String.class, org.osgi.service.application.midlet.MidletDescriptor.class
            });
            constructor.setAccessible(true);
            MidletHandle midletHandle = (MidletHandle)constructor.newInstance(new Object[] {
                createNewInstanceID(pid), midletDescriptor
            });
            Field delegate = midletHandleClass.getDeclaredField("delegate");
            delegate.setAccessible(true);
            MidletHandleImpl midHnd = (MidletHandleImpl)delegate.get(midletHandle);
            midHnd.init(bc, midletContainer, midlet, refs);
            midHnd.startHandle(args);
            return midletHandle;
        }
        catch(Exception e)
        {
            MidletContainer.log(bc, 1, "Exception occurred at creating midlet handle!", e);
        }
        return null;
    }
