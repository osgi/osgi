...
public class MidletContainer
    implements BundleListener, EventHandler
{

    public MidletContainer(BundleContext bc)
        throws Exception
    {
        this.bc = bc;
    ...
    public void installApplication(long bundleID)
    ...
    public MIDlet createMidletInstance(MidletDescriptorImpl appDesc, Vector serviceVector, boolean resume)
        throws Exception
    {
        ...
        ClassLoader loader = new MIDletClassLoader(getClass().getClassLoader(), appBundle, mainClass.getProtectionDomain());
        Class midletClass = loader.loadClass(appDesc.getStartClass());
        Constructor constructor = midletClass.getDeclaredConstructor(new Class[0]);
        ...
        for(int j = 0; j != deps.dependentFields.length; j++)
        {
            ...
            ServiceReference ref = bc.getServiceReference(deps.requiredService[i]);
        ...
        throws Exception
    {
        ...
        if(desc == null)
            throw new Exception("Application wasn't installed onto the meglet container!");
        else
            return desc;
    }

    private int getApplicationIndex(MEGBundleDescriptor desc, ApplicationDescriptor appDesc)
        throws Exception
    {
    ...
    private String checkDependencies(MidletDescriptorImpl appDesc, Dependencies deps)
    {
        try
        {
            for(int i = 0; i != deps.requiredService.length; i++)
            {
                ...
                {
                    ServiceReference refs[] = bc.getServiceReferences(serviceToCheck, null);
                    if(refs == null || refs.length == 0)
                        return "Dependencies failed because of a missing service(" + serviceToCheck + ")!";
                }
            }
        ...
        {
            log(bc, 1, "Exception occurred at checking the dependencies of a meglet!", e);
        }
        return "Exception occurred at checking the dependencies!";
    }

    private boolean checkSingletonity(MidletDescriptorImpl appDesc, boolean resume)
    {
        if(!appDesc.isSingleton())
            return true;
        try
        {
            ServiceReference appList[] = bc.getServiceReferences("org.osgi.service.application.ApplicationHandle", "(application.descriptor=" + appDesc.getPID() + ")");
        ...
        {
            log(bc, 1, "Exception occurred at checking the singletonity of a meglet!", e);
        }
        return false;
    }
    ...
    public boolean isLaunchable(MidletDescriptorImpl appDesc)
    {
        ...
        {
            log(bc, 1, "Exception occurred at checking if the meglet is launchable!", e);
        }
        return false;
    }
    ...
    private void checkAutoStart(long bundleID, boolean isInstalling)
    {
        ...
        {
            log(bc, 1, "Exception occurred at checking trying to autostart a meglet!", e);
        }
    }

    public void unregisterAllApplications()
        throws Exception
    {
        for(int i = 0; i != bundleIDs.size(); i++)
    ...
    void checkAndRegister(Bundle b)
    {
        try
        {
            ...
            if(map != null && map.containsKey("MIDlet-Name") && map.containsKey("MIDlet-Version") && map.containsKey("MIDlet-Vendor") && map.containsKey("MIDlet-1") && map.containsKey("MicroEdition-Profile") && map.containsKey("MicroEdition-Configuration"))
    ...
    private ApplicationDescriptor[] parseManifestHeaders(long bundleID)
    {
        try
        {
            Map manifest = convertObsoleteDictionaryToMap(bc.getBundle(bundleID).getHeaders());
            ...
            int midletCounter = 0;
            String midletProps;
            while((midletProps = (String)manifest.get("MIDlet-" + ++midletCounter)) != null) 
            {
                ...
                if(t.hasMoreTokens())
                {
                    ...
                    if(t.hasMoreTokens())
                    {
                        ...
                        if(t.hasMoreTokens())
                        {
                            ...
                            props.setProperty("application.vendor", MIDletVendor);
                            props.setProperty("application.singleton", "true");
                            props.setProperty("application.visible", "true");
                            props.setProperty("application.version", MIDletVersion);
                            props.setProperty("application.autostart", "false");
                            String defaultLang = Locale.getDefault().getLanguage();
                            names.put(defaultLang, MIDletName);
                            icons.put(defaultLang, MIDletIcon);
                            String uniqueID = MIDletName + "-" + MIDletSuiteName + "-" + MIDletVersion;
                            props.put("service.pid", uniqueID);
                            EventSubscribe subscribe = new EventSubscribe();
                            Dependencies deps = new Dependencies();
                            String dependentFields = (String)manifest.get("Dependent-Field-" + midletCounter);
                            if(dependentFields != null)
                            {
                                LinkedList depVector = new LinkedList();
                                for(StringTokenizer tokens = new StringTokenizer(dependentFields, ","); tokens.hasMoreTokens(); depVector.add(tokens.nextToken().trim()));
                                deps.dependentFields = new String[depVector.size()];
                                deps.requiredService = new String[depVector.size()];
                                int counter = 0;
                                while(depVector.size() != 0) 
                                    deps.dependentFields[counter++] = (String)depVector.removeFirst();
                                Bundle appBundle = bc.getBundle(bundleID);
                                ClassLoader loader = new MIDletClassLoader(getClass().getClassLoader(), appBundle, getClass().getProtectionDomain());
                                Class startClass = loader.loadClass(MIDletStartClass);
                                for(int i = 0; i != deps.dependentFields.length; i++)
                                {
                                    Field field = startClass.getDeclaredField(deps.dependentFields[i]);
                                    deps.requiredService[i] = field.getType().getName();
                                    if(deps.requiredService[i].equals(org.osgi.framework.BundleContext.class.getName()))
                                        deps.requiredService[i] = null;
                                }

                            }
                            eventVector.add(subscribe);
                            appVector.add(createMidletDescriptorByReflection(props, names, icons, defaultLang, MIDletStartClass, bc.getBundle(bundleID)));
    ...
    public void handleEvent(Event event)
    {
        try
        {
            for(Enumeration megBundles = bundleHash.keys(); megBundles.hasMoreElements();)
            {
                ...
                for(int i = 0; i != bundleDesc.eventSubscribes.length; i++)
                    if(bundleDesc.eventSubscribes[i] != null && bundleDesc.eventSubscribes[i].eventTopic != null)
                    {
                        ...
                        {
                            org.osgi.framework.Filter topicFilter = bc.createFilter("(event.topics=" + bundleDesc.eventSubscribes[i].eventTopic[j] + ")");
                            if(event.matches(topicFilter))
                                ...
                                case 3: // '\003'
                                case 4: // '\004'
                                    String pid = (String)bundleDesc.applications[i].getProperties("").get("service.pid");
                                    ServiceReference references[] = bc.getServiceReferences("org.osgi.service.application.ApplicationHandle", "(application.descriptor=" + pid + ")");
                                    if(references == null || references.length == 0)
                                        break;
                                    for(int k = 0; k != references.length; k++)
                                    {
                                        MidletHandle handle = (MidletHandle)bc.getService(references[k]);
                                        switch(bundleDesc.eventSubscribes[i].eventAction[j])
                                        {
                                        case 2: // '\002'
                                            handle.destroy();
    ...
    {
        System.out.println("Serverity:" + severity + " Message:" + message + " Throwable:" + throwable);
    ...
    {
        MEGBundleDescriptor desc = (MEGBundleDescriptor)bundleHash.get(new Long(megDesc.getBundleId()));
        if(desc == null)
            return null;
        for(int i = 0; i != desc.applications.length; i++)
    ...
    public MidletDescriptor createMidletDescriptorByReflection(Properties props, Hashtable names, Hashtable icons, String defaultLang, String startClass, Bundle bundle)
    {
        ...
        try
        {
            ...
            Constructor constructor = midletDescriptorClass.getDeclaredConstructor(new Class[] {
                java.lang.String.class
            });
            constructor.setAccessible(true);
            MidletDescriptor megletDescriptor = (MidletDescriptor)constructor.newInstance(new Object[] {
                pid
            });
            Field delegate = midletDescriptorClass.getDeclaredField("delegate");
            delegate.setAccessible(true);
            MidletDescriptorImpl megDesc = (MidletDescriptorImpl)delegate.get(megletDescriptor);
    ...
    private Map convertObsoleteDictionaryToMap(Dictionary dict)
    {
        ...
        if(dict == null)
            return null;
        String key;
        for(Enumeration enum = dict.keys(); enum.hasMoreElements(); convertedTable.put(key, dict.get(key)))
