...
public class MEGApplicationDescriptor implements ApplicationDescriptor 
{
  ...
  MEGApplicationDescriptor( BundleContext bc, Properties props, Map names, Map icons, String startClass ) throws Exception
  {
    this.bc = bc;
    ...
    if( names.size() == 0 || icons.size() == 0 || !props.containsKey( "bundle_id" ) ||
  ...
  {
    String uniqueID = getContainerID() + "-" + getCategory() + "-" + getName() + "-" + getVersion() + "-" +
  ...
  {    
    ...
    while( icn.hasMoreElements() )
    {
      ...
      {
        properties.put( "localized_icon_" + key.substring( 3 ) + "_path", icons.get( key ) );
        iconFound = true;
      }
    }
    if( !iconFound )
      properties.put( "localized_icon_" + firstIcon.substring( 3 ) + "_path", icons.get( firstIcon ) );
  ...
  public Map getContainerProperties (String locale)
  {
    try {
      ServiceReference []MEGContainerList = bc.getServiceReferences(
         "org.osgi.service.application.ApplicationContainer", "(application_type=MEG)" );
      ...
      for( int i=0; i != propKeys.length; i++ )
        containerProps.put( propKeys[i], MEGContainerList[ 0 ].getProperty( propKeys[ i ] ) );
