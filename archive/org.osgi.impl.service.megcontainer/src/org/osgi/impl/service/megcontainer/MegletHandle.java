...
public class MegletHandle implements ApplicationHandle {
	...
	void setStatus( int status ) {
		...
		if( appHndServiceID != null )
			props.put( "application_instance.id", new Long( appHndServiceID.longValue() ) );
		...
		case ApplicationHandle.NONEXISTENT:
			if( lastStatus == ApplicationHandle.STOPPING )
				topic += "stopped";
			else if( lastStatus == ApplicationHandle.NONEXISTENT )
				topic += "starting";
			break;
	...
	boolean sendEvent( Event event, boolean asynchron ) {
		ServiceReference eventRef = bc.getServiceReference( "org.osgi.service.event.EventAdmin" );
		if (eventRef != null) {
	...
	public void destroyApplication() throws Exception {
		...
		AccessController.checkPermission( new ApplicationAdminPermission(
			appDesc.getApplicationPID(), ApplicationAdminPermission.MANIPULATE ) );
	...
	void registerAppHandle() throws Exception {
		...
		appHndServiceID = (Long)serviceReg.getReference().getProperty( Constants.SERVICE_ID );
		...
		if( appDescRefs != null && appDescRefs.length != 0 )
			appDescServiceID = (Long)appDescRefs[ 0 ].getProperty( Constants.SERVICE_ID );
	}
