 ...
 * <p>
 * When a tree access command is called on the Dmt Admin service, it must
 ...
 * given node (e.g. the target of <code>getChildNodeNames</code> is an interior
 * node), etc.  All errors described by the error codes
 * {@link DmtException#INVALID_URI}, 
 * {@link DmtException#URI_TOO_LONG},
 ...
 * reaches the plugin.
 * <p>
 * If the plugin provides meta-data for a node, the Dmt Admin service must also
 * check the constraints specified by it, as described in {@link MetaNode}.  
 * If the plugin does not provide meta-data, it must perform the necessary 
 * checks for itself and use the {@link DmtException#METADATA_MISMATCH} error 
 * code to indicate such discrepancies.
 * <p>
 * The Dmt Admin also ensures that the targeted nodes exist before calling the
...
public interface ReadableDataSession {
     ...
     * session was invalidated due to an exception during commit or rollback,
     * it is guaranteed that no methods are called on the plugin until it is 
     ...
     * removed by the Dmt Admin before returning it to the client.
     * 
     ...
     *         <li><code>METADATA_MISMATCH</code> if the information could not
     *         be retrieved because of meta-data restrictions
     *         <li><code>FEATURE_NOT_SUPPORTED</code> if the Title property is
     *         not supported by the plugin
     *         <li><code>DATA_STORE_FAILURE</code> if an error occurred while
     *         accessing the data store
     *         <li><code>COMMAND_FAILED</code> if some unspecified error is 
