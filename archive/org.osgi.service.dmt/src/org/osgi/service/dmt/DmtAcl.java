...
/**
 * The DmtAcl class represents structured access to DMT ACLs. Under OMA DM the
 * ACLs are defined as strings with an internal syntax.
 * <p>
 * The methods of this class taking a principal as parameter accept remote
 * server IDs (as passed to <code>DmtFactory.getTree</code>), as well as "
...
public class DmtAcl {
	...
	//----- Private constants -----//
	private static final int[]		PERMISSION_CODES	= new int[] {ADD,
			DELETE, EXEC, GET, REPLACE					};
	private static final String[]	PERMISSION_NAMES	= new String[] {"Add",
			"Delete", "Exec", "Get", "Replace"			};
	private static final int		ALL_PERMISSION		= ADD | DELETE | EXEC
																| GET | REPLACE;
	...
	/**
	 * Creates an instance of the ACL that represents the same permissions as
	 * the parameter ACL object.
	 * 
	...
	//----- Public methods -----//
	/**
	 * Add a specific permission to a given principal. The already existing
	...
	/**
	 * Revoke a specific permission from a given principal. Other permissions of
	 * the principal are not affected.
	 * 
	 * @param principal The entity from which a permission should be revoked.
	...
	private void deleteFromAll(int perm) {
		...
		while (i.hasNext()) {
			Map.Entry entry = (Map.Entry) i.next();
			setPrincipalPermission((String) entry.getKey(), ((Integer) entry
					.getValue()).intValue()
					& ~perm);
		}
	}
	...
	// TODO find out if ACL string (commands and/or server IDs) should be case
	// insensitive
		...
		for (int i = 0; i < aclEntries.length; i++) {
			...
			for (int j = 0; j < serverIds.length; j++) {
				...
				if (serverIds[j].equals(ALL_PRINCIPALS))
					...
					Integer n = (Integer) principalPermissions
							.get(serverIds[j]);
					int oldPermission = (n != null) ? n.intValue() : 0;
					principalPermissions.put(serverIds[j], new Integer(
