 ...
 * the <code>execRootURIs</code> registration parameter.
 */
public interface DmtExecPlugIn {
	 ...
	 * which handles the request. This operation corresponds to the EXEC command
	 * in OMA DM. The semantics of an EXEC operation and the data parameters it
	 * takes depend on the definition of the managed object on which the command
	 * is issued.
	 * 
	 * @param session A reference to the session in which the operation was
	 *        issued. Session information is needed in case an alert should be
	 *        sent back from the plugin.
	 * @param nodeUri The node to be executed.
	 * @param data The data of the EXEC operation. The format of the data is not
	 *        specified, it depends on the definition of the managed object (the
	 *        node).
	 * @throws DmtException
	 */
