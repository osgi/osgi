...
public class PermissionAdminImpl implements PermissionAdmin {
	...
	final private PermissionInfo[]	implicitDefaultPermissions						= new PermissionInfo[] {new PermissionInfo(
	 ...
	 *        permissions.
	 * @param perms The permissions to be assigned, or <tt>null</tt> if the
	...
	public synchronized PermissionInfo[] getDefaultPermissions() {
		return explicitDefaultPermissions == null ? null
				: (PermissionInfo[]) explicitDefaultPermissions.clone();
	}
	...
	//
	/**
	 * Gets the permissionCollection assigned to the bundle with the specified
	 * id. The collection contains the configured permissions for the bundle
	 * location plus implict gratnet permissions (i.e FilePermission for the
	 * data area).
	 * 
	 * @param bid The bundle id whose permissions are to be returned.
	 * 
	 * @return The permissions assigned to the bundle with the specified
	 *         location, or the default permissions if that bundle has not been
	...
	/**
	 * Gets the permissionCollection assigned to the bundle with the specified
	 * id. We return a permission wrapper so that we can change it dynamicly.
	 * 
	...
	synchronized PermissionCollection getPermissionCollection(BundleImpl bundle) {
		PermissionCollection pc = (PermissionCollection) cache
				.get(bundle.location);
		if (pc == null) {
	...
	private void save() {
		...
		if (explicitDefaultPermissions != null) {
			...
			props.setProperty(p + "size", Integer
					.toString(explicitDefaultPermissions.length));
			for (i = 0; i < explicitDefaultPermissions.length; i++) {
				props.setProperty(p + i, explicitDefaultPermissions[i]
						.toString());
			}
		}
			...
			try {
				...
				if (s != null) {
					...
					for (int i = 0; i < size; i++) {
						explicitDefaultPermissions[i] = new PermissionInfo(
								(String) props.remove(p + i));
					}
				}
				while (!props.isEmpty()) {
					...
					if (loc != null) {
						int size = Integer.parseInt((String) props.remove(p
								+ "size"));
						PermissionInfo[] pi = new PermissionInfo[size];
						for (int i = 0; i < size; i++) {
							pi[i] = new PermissionInfo((String) props.remove(p
									+ i));
						}
