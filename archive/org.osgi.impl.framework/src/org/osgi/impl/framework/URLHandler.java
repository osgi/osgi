...
/**
 * This class implements RFC 44 as a stand alone bundle.  Note, that this
 * bundle cannot be stopped once started since it registers content handlers
 * with the JVM!
 * <p>
 * This bundle bridges the factories with the service registry.
 */
public class URLHandler implements URLStreamHandlerFactory, ContentHandlerFactory
{
    ...
    /** Registers the factories.  Also sets up prefix arrays for finding
     *  built-in handlers. */
    ...
    /** Fix mimetype to correspond to the class name lookup rules of 
     *  rfc 44 section 3.3. */
	...
	ContentHandler handler = 
	    (ContentHandler)contentHandlerCache.get(mimetype);
	...
	URLStreamHandlerProxy proxy = new URLStreamHandlerProxy(framework, protocol);
	// Unlike content handlers, we want to return null if a handler
	// doesn't exist for the protocol.
	if (proxy.exists()) return proxy;
	proxy = null;
	return null;
    }
}
