...
public class Archive {
	...
	File				file;
	/**
	 * URL used for CodeSource, set to bundle location if it is a valid URL
	 * otherwise set to URL of archive file.
	 */
	URL					codeUrl;
	/**
	...
	/**
	 * Create an Archive based on contents of an InputStream, the archive is
	 * saved as local copy in the specified directory.
	 * 
	...
	Archive(File dir, URL url) throws IOException {
		...
		if (file == null) {
			throw new IOException("No saved jar file found in: "
					+ dir.getAbsolutePath());
		}
		jar = new JarFile(file);
		codeUrl = url != null ? url : file.toURL();
	 ...
	 * The new archive is saved in a subdirectory below local copy of the
	 * existing Archive.
	 * 
	...
	/**
	 * Create a new Archive that will later replace the archive sent as a
	 * parameter. This is used for updating a bundle. The bundle data is saved
	 * in a temporary file until we now that the contents is okay.
	 * 
	...
	Archive(Archive a, InputStream is) throws IOException {
		try {
			int c = Integer.parseInt(a.file.getName().substring(
					ARCHIVE.length()));
			c++;
	...
	private FileTree getSubFileTree(Archive archive) {
		return new FileTree(archive.file.getParentFile(), SUBDIR
