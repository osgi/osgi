...
class BundlePackages {
	 ...
	 */
	BundlePackages(BundleImpl b, String exportStr, String importStr,
			String dimportStr) {
		this.bundle = b;
		try {
			...
			while (i.hasNext()) {
				Map e = (Map) i.next();
				PkgEntry p = new PkgEntry((String) e.get("key"), (String) e
						.get(Constants.PACKAGE_SPECIFICATION_VERSION), bundle);
				exports.add(p);
		...
		try {
			...
			wloop: while (i.hasNext()) {
				Map e = (Map) i.next();
				PkgEntry pe = new PkgEntry((String) e.get("key"), (String) e
						.get(Constants.PACKAGE_SPECIFICATION_VERSION), bundle);
			...
			while (i.hasNext()) {
				...
				if (key.equals("*")) {
					...
					if (key.endsWith(".*")) {
						...
						if (key.endsWith(".")) {
							b.framework.listeners.frameworkError(b,
	...
	synchronized void registerPackages() {
		bundle.framework.packages.registerPackages(exports.iterator(), imports
				.iterator());
	}
	...
	synchronized boolean unregisterPackages(boolean force) {
		Iterator i = (okImports != null ? okImports.values() : imports)
				.iterator();
		if (bundle.framework.packages.unregisterPackages(exports.iterator(), i,
				force)) {
			okImports = null;
			return true;
		}
	 ...
	 * 
	 * @return true if we resolved all packages. If we failed return false.
	 *         Reason for fail can be fetched with getResolveFailReason().
	 */
	synchronized boolean resolvePackages() {
		...
		List m = bundle.framework.packages.checkResolve(bundle, imports
				.iterator());
		if (m != null) {
	...
	/**
	 * If bundle packages has been resolved look for a bundle that provides the
	 * requested package. If found, check we import it. If not imported, check
	 ...
	 * 
	 * @param pkgs List over all packages to check. Data is Map.Entry with a
	 *        List of package names as key and a Map of parameters as value.
