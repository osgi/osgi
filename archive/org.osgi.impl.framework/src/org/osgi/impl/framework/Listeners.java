 ...
 * @version $Revision$
 */
public class Listeners implements BundleListener, FrameworkListener,
		ServiceListener {
	/**
	...
	/**
	 * Remove bundle listener from current framework. Silently ignore if
	 * listener doesn't exist. If listener is registered more than once remove
	 * one instances.
	 * 
	...
	/**
	 * Remove framework listener from current framework. Silently ignore if
	 * listener doesn't exist. If listener is registered more than once remove
	 * all instances.
	 * 
	 * @param bundle Who wants to remove listener.
	...
	/**
	 * Add a service listener with filter to current framework. If no filter is
	 * wanted, call with filter param set to null.
	 * 
	 * @param bundle Who wants to add listener.
	 * @param listener Object to add.
	 * @param filter LDAP String used for filtering event before calling
	 *        listener.
	 */
	void addServiceListener(Bundle bundle, ServiceListener listener,
	...
	/**
	 * Remove service listener from current framework. Silently ignore if
	 * listener doesn't exist. If listener is registered more than once remove
	 * all instances.
	 * 
	 * @param bundle Who wants to remove listener.
	...
	//
	/**
	 * Receive notification that a bundle has had a change occur in it's
	 * lifecycle.
	 * 
	...
	public void serviceChanged(final ServiceEvent evt) {
		ServiceReferenceImpl sr = (ServiceReferenceImpl) evt
				.getServiceReference();
		...
		for (Enumeration e = sl.elements(); e.hasMoreElements();) {
			...
			try {
				if (l.filter == null
						|| LDAPQuery.query(l.filter, sr.getProperties())) {
					for (int i = 0; i < c.length; i++) {
	...
	//
	/**
	 * Remove all listener registered by a bundle in specified list within the
	 * current framework. Silently ignore if listener doesn't exist.
	 * 
	 ...
	 * Silently ignore if listener doesn't exist. If listener is registered more
	 * than once remove all instances.
	 * 
	 * @param l Which list to remove from bundle, framework or service.
...
/**
 * Data structure for saving listener info. Contains bundle who registered
 * listener, listener object and if a service listener any associated filter.
 */
