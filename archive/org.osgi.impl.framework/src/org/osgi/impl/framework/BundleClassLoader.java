...
class BundleClassLoader extends ClassLoader {
	...
	/**
	 * Find bundle class to load. First check if this load comes from an
	 * imported package. Otherwise load class from out bundle.
	 * 
	...
	//
	/**
	 * Load of class from our bundle. First check if it is already loaded. Then
	 * try all archives in this bundles classpath.
	 */
		...
		if (c == null) {
			...
			for (Enumeration e = classArchives.elements(); e.hasMoreElements();) {
				...
				try {
					...
					bpkgs.bundle.framework.listeners.frameworkError(
							bpkgs.bundle, ioe);
				}
			}
			throw new ClassNotFoundException(name);
	...
	InputStream getInputStream(URL burl) throws IOException {
		if (BundleURLStreamHandler.PROTOCOL.equals(burl.getProtocol())) {
			...
			return ((Archive) classArchives.elementAt(i)).getInputStream(burl
					.getFile());
		}
		else {
	...
	/**
	 * Close down this classloader. We don't give out any new classes. Perhaps
	 * we should block all classloads.
	 */
	void close() {
		classArchives.clear();
		nativeLibrary.clear();
	...
	/**
	 * Close down this classloader and all its archives. Purge all archives.
	 *  
	 */
	void purge() {
	...
	Enumeration findBundleResources(final String name) {
		...
		for (int i = 0; i < classArchives.size(); i++) {
			try {
				InputStream test = ((Archive) classArchives.elementAt(i))
						.getInputStream(name);
				if (test != null) {
					final int idx = i;
					answer.add(AccessController
