...
import org.osgi.impl.service.upnp.cd.ssdp.*;
...
public class SubscriptionCheck extends ErrorCheck{
	...
	public SubscriptionCheck(DataOutputStream dos , Hashtable headers , BundleContext context) {
		this.dos = dos;
	...
	// This mehod will be called by processor object to process the given subscription. Checks
 	// for all valid headers, if valid, creates a new subscription along with sid, and sends it
 	// back to the subscriber. Intial events will be delivered on a separate thread.
 	public void subscribe() {
		...
		if(!availableCheck) {
			writeData(GenaConstants.GENA_SERVER_VERSION + GenaConstants.GENA_ERROR_503);
			return;
		}
		...
		Subscription subscription = createNewSubscription(subscriptionId ,publisherUrl);
		...
		//System.out.println("####----> cd->event->SubscriptionCheck-> send initial event message????");
		...
		if( ! EventRegistry.checkIsListenerAvailable(udn+serviceid)) {
			Filter filter = null;
			GenaEventListener listener = new GenaEventListener(subscriptionId,upnpservice);
			try {
				filter = context.createFilter("(&("+UPnPDevice.UDN+"="+udn+")("+UPnPService.ID+"="+serviceid+"))");
			}catch(Exception e) {System.out.println(e.getMessage());}
			Hashtable description = new Hashtable();
			ServiceRegistration registration = null;
			description.put("upnp.filter" , filter);
			try {
				registration = context.registerService("org.osgi.service.upnp.UPnPEventListener",listener ,description);
			}catch(Exception e) {
				e.printStackTrace();
			}
 	...
 	// Checks whether the subscription eventing url is available or not. It not available sends
		...
		if(upnpservice!= null) {	
			...
			for(int i=0 ;i<stateVariables.length; i++) {
				if(stateVariables[i].sendsEvents()) {
					...
					if (variableObject == null) serviceStates.put(variableName, "null");
					else serviceStates.put(variableName, variableObject);					
				}
			}			
		}
		return check_result;
	...
	Subscription createNewSubscription(String subscriptionId ,String publisherPath) {		
		String callback = (String)headers.get("callback");
		String hostString = (String)headers.get("host");
		String publisherpath =publisherPath;
		Subscription subscription = new Subscription(subscriptionId.trim(), publisherpath,0,
		null, hostString,callback);
	...
	// This method checks for valid callback urls along with notification type header.
	public String invalid_CALLBACK() {
		String nt = (String)headers.get("nt");
		String callback = (String)headers.get("callback");
		if(callback == null) { 
			return GenaConstants.GENA_SERVER_VERSION + GenaConstants.GENA_ERROR_412;
		}
		if(nt == null ) {
			 return GenaConstants.GENA_SERVER_VERSION + GenaConstants.GENA_ERROR_400;
		}
