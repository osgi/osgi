...
public class SSDPComponent implements SSDPConstants{
	...
	public SSDPComponent(String cache, BundleContext bc, String genaIP, EventAccessForExporter evRegistry) throws Exception{
		cacheControl 		= cache;
		...
		server				= new String(osname + "/" + osver + " UPNP/1.0 SAMSUNG-UPnP-STACK/1.0");		
		allDeviceDetails	= new Hashtable(10);
		HttpService httpService	= null;
		try{        					
			ServiceReference sr		= bc.getServiceReference("org.osgi.service.http.HttpService");		
			if (System.getProperty("org.osgi.service.http.port") != null) 	
				baseURL  = genaIP.substring(0, genaIP.indexOf(":")+1) + System.getProperty("org.osgi.service.http.port"); 						
			else 
	...
	// This method starts the Mulicast listener. This listener getts all ssdp multicast messages
	...
	// This method reads device details of CD Bundle device and all exported devices. It calls
 	// sendNotifyBye() method  by passing the required details.	
	void readDevicesForNotifyBye(){		
		for(Enumeration enum = allDeviceDetails.elements(); enum.hasMoreElements();){ 
	...
	void sendNotifyBye(String type, String uuid, String deviceType, String serviceType){		
		String 	usn			= null;
		String 	nt 			= null;					
		if(type.equals("root")){			
	...
	String makeMsearchResponse(String dateValue, String st, String usn, String location){
		msearchResponse = new String(
									MSEARCH_RESP + "\r\n" + 
									CACHE + " max-age = " + cacheControl + "\r\n" + 
									DATE1 + " " + dateValue + "\r\n" + 
									EXT + "\r\n" +
									LOC + " " + location +"\r\n"+ 
									SERVER + " " + server + "\r\n" +
	...
	// This method add a new DeviceDetails object to the allDeviceDetails list. This list used 
	...
	// This method removes a DeviceDetails object fom the allDeviceDetails list. This list used 
 	// for sending notiyfy messages and msearch responses for all devices.	
