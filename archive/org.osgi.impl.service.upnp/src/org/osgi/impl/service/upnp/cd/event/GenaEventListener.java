...
public class GenaEventListener implements UPnPEventListener{
	...
	public GenaEventListener(String subscriptionId ,UPnPService upnpservice) {
	 ...
	 // when ever a statevariable is changed  , this method will be called and inturn will
     // send the new changed evented statevariables list to all the subscribers who are 
     // subscribed for this .
	public void notifyUPnPEvent(String deviceId,String serviceId, Dictionary events) {	

		if(upnpservice != null) {
			Hashtable eventedVariables = checkSendEvents((Hashtable)events ,upnpservice);
			String eventsuburl = deviceId + serviceId;
			String tmpEventUrl = "";
			eventsuburl = eventsuburl.replace(':', '-');
			for(Enumeration enum =EventRegistry.getSubscriberIds();enum.hasMoreElements();) {
				String element = (String) enum.nextElement();
				Subscription subscription = (Subscription)EventRegistry.getSubscriber(element);
			...
			for(int i = 0 ; i < variables.length ; i++) {
				if(variables[i].getName().equals(variablename)) {
					if(variables[i].sendsEvents()) {
						 eventedVariables.put(variablename,statevariables.get(variablename));
					}
				}
			}
		}
