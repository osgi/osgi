...
public class EventRegistry implements EventAccessForExporter{
	// This table holds the subscriber list. This table maintains all the subscription info against the
    ...
    // This method inserts the listener mapped with serviceid in idmapslistener table
     public synchronized static void insertListeners(String serviceId,ServiceRegistration registration) {
     	idMapsListener.put(serviceId, registration);
     }
     
     // This method checks whether a particular registration object is available against the udn.
     ...
     public synchronized static void insertSubscriber(String subscriptionId, Subscription subscription) {
     	subscribersList.put(subscriptionId, subscription);
     }
   
     // This method retrieves the subscription object against the given subscription id
	...
	// This method removes the udn from the table along with making the listener unregister
    // from the framework. If any subscriber subscribers for this service's eventing url,
    // that subscriber will also be removed from the list.
    public synchronized void removeServiceId(String serviceId) {
		ServiceRegistration registration = (ServiceRegistration)idMapsListener.get(serviceId);
		...
		for(Enumeration enum =subscribersList.keys();enum.hasMoreElements();) {
				...
				Subscription subscription = (Subscription)subscribersList.get(subscriptionId);
	...
	// when a particular subscriber unsubscribes, his subscription  for the particular device
    // service event url has also been removed from the idlistenerlist provided no other 
    // subscriber is subscribing for that event url. This has to be done because of unncessary
    // keeping of listener with framework.
	public static void checkForServiceId(String serviceId) {
		boolean removeListener = true;
		for(Enumeration enum = subscribersList.elements();enum.hasMoreElements();) {
			if(((Subscription)enum.nextElement()).getServiceId().equals(serviceId)) {
	...
	public void releaseAllListeners() {
		for(Enumeration enum = idMapsListener.keys();enum.hasMoreElements();) {
			...
			ServiceRegistration registration = (ServiceRegistration)idMapsListener.get(eventUrl);
			try {
				registration.unregister();
				registration = null;
