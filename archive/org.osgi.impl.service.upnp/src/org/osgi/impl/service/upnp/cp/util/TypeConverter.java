...
public abstract class TypeConverter{
    ...
    public Object convertToJavaType(String value) throws WrongSoapValueException, IllegalArgumentException, Exception {
        ...
        try {    
            ...
            throw new WrongSoapValueException("Original message: " + e.getMessage() + "\n" 
                                            + "Details: Cannot convert '" + value + "' into type '" + getType() + "' ");
        }
        // other exception e.g. IndexOutOfRangeException will be directly thrown to a callee         
        return o;
    }

    abstract public String convertToString(Object value) throws IllegalArgumentException;    
    abstract protected Object convertToJavaTypeBySubchild(String value) throws Exception;
}
...
/**
 * 
 * Same as DoubleConverter but no more than 14 digits to the left of the decimal point and no more than 4 to the right. 
 * 
 * if there are more than 14 digits to the left of point, it throws IllegalArgumentException
 * if there are more than 4 digits to the right, it just throw the rest away
 * 
 * */
    ...
    public String convertToString(Object value) throws IllegalArgumentException   {
        if (value instanceof Double) {
            ...
            if (Long.toString(leftOfPoint).length() > 14)
                throw new IllegalArgumentException("There should be no more than 14 digits to the left of point but value was " + value);
...
class ISO8601Converter extends TypeConverter  {
    private String formatStrings[] =  {
            // see http://www.cl.cam.ac.uk/~mgk25/iso-time.html for ISO 8601
            "yyyy-MM-dd",   "yyyyMMdd",
            "yy-MM-dd",     "yyMMdd",
            "yyyy-MM",      "yyyyMM",
            "yyyy-'W'ww",   "yyyy'W'ww",
            // REDTAG: don't know how to solve this...  1997-W01-2 or 1997W012      yyyy-'W'ww-? or yyyy'W'ww?   ??
            "yy'W'ww",
            "yyyy-DDD",     "yyyyDDD",
        };
    ...
    public ISO8601Converter(String typeName) {
        ...
        for (int i = 0; i < formatStrings.length; i++) {
            dateFormatters[i] = (SimpleDateFormat) DateFormat.getDateTimeInstance();
...
/**
 * if value is "" then it returns byte[] with its length = 0
 * if value has type of "33-3a-5a", it return byte[] containing {33, 3a, 5a}
 */
...
class BinHexConverter extends TypeConverter  {
    ...
    public Object convertToJavaTypeBySubchild(String value) throws Exception {
        ...
        for(int i = 0; tokenizer.hasMoreTokens(); i++) {
            ...
            if (token.length() != 2)
                throw new WrongSoapValueException("the length of token '" + token + "' should be 2 or more");   
    ...
    public String convertToString(Object value) throws IllegalArgumentException  {
        if (value instanceof byte[]) {
            ...
            StringBuffer strBuf = new StringBuffer(ConverterUtil.get00toFF(bytes[0]));
...
class ConverterUtil {
    ...
    public static String get00toFF(int value) {
        return value < 0x10 ? "0" + Integer.toString(value, 16) 
