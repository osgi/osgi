...
public class Description {			
	...
	// This method returns the document object based on the xml file.
	public Document getDocument(String cdURL, BundleContext bc) throws Exception {
    	cdPath = cdURL; 		
		URL ur1 = null;
		try {
			ur1 = new URL(cdURL);			
		...
		if(XMLfileContents.startsWith("ERROR")) {
			return null;
		} else if(XMLfileContents.length() != 0){
        	try {        		
	...
	// This method sends the message to the CD.
  	void sendAgain(Socket sock1, String fileN,String ipaddr,int portN) throws Exception {		
	...
	void receiveData(Socket sock,String fileN, String ipaddr, int portN) throws Exception{			
		InputStream din = sock.getInputStream();		
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        /*int cctr=din.read();        
        XMLfileContents = XMLfileContents + (char)cctr;        
        while(cctr!=-1){
        	cctr = din.read();        	
        	XMLfileContents = XMLfileContents + (char)cctr;
        } */
	...
	public String makeDescriptionRequest(String ipaddr, int portN, String filePath){			
