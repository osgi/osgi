...
public class UPnPDeviceImpl implements UPnPDevice{
	...
	UPnPDeviceImpl(UPnPBaseDriver basedriver, RootDevice deviceinfo, Properties props, BundleContext bc){
    ...
    // This method sets all the service and icon details to the services and the icons object.     
	void extractDeviceInfo(){
		ServiceInfo[] serviceinfo = deviceinfo.getServices();
		if(serviceinfo != null){
			for(int i = 0; i < serviceinfo.length; i++){
				if( serviceinfo[i] != null ){
					UPnPServiceImpl  upnpservice = new UPnPServiceImpl(serviceinfo[i], basedriver, devid, devtype, bc);
		...
		if(iconss != null){
			...
			for(int j = 0; j < icons.length; j++){
				UPnPIconImpl  upnpicon = new UPnPIconImpl(iconss[j], basedriver.deviceinfo.getURLBase());
				icons[j] = upnpicon;
			}
		}
	}

    // This method returns the UPnPService object based on the given name of the service.
