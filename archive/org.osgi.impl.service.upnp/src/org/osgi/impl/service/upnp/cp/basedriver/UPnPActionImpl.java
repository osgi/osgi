...
public class UPnPActionImpl implements UPnPAction  {
	...
	UPnPActionImpl(String name, String curl, String stype, String host, UPnPBaseDriver basedriver){
		this.name			= name;
		controlurl			= curl;
		servicetype			= stype;
		this.host			= host;
	...
	// This constructor creates the UPnPAction object with the given parameters.
	UPnPActionImpl(Action action, UPnPBaseDriver basedriver, UPnPServiceImpl upnpservice){
		this.action			= action;
		this.basedriver 	= basedriver;
		...
		if(args != null){
			...
			for(int k = 0; k < args.length; k++){
				if(args[k].getDirection().equals("in")){
					i++;
				}else if(args[k].getDirection().equals("out")){
					j++;
				}
			}
			...
			for(int l = 0; l < args.length; l++){			
				if(args[l].getDirection().equals("in")){
					...
					m++;
				}else if(args[l].getDirection().equals("out")){
   		...
   		for (int i = 0; i< outputargs.length; i++) {
   			for(int j = 0; j < args.length; j++){    		
    			...
    			if(outputargs[i].equals(argname)){
   					...
   					UPnPStateVariable USV = (UPnPStateVariable)upnpservice.getStateVariable(stName);   					
   ...
   public Dictionary invoke(Dictionary dict) throws Exception{    
    	...
    	Dictionary outparms = basedriver.control.sendControlRequest(controlurl, host, servicetype, name, inparms, true);        	
