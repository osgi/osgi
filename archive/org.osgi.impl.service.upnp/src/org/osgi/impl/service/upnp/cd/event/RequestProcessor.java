...
public class RequestProcessor {
	...
	// Constructor which accepts the input stream from the client and initializes all the
	// variables.
	...
	// function used for separating the given string into multiple tokens and returns
	...
	// This method parses the given input stream and retrieves all the values from the input
	// stream and stores in appropriate variables. Stores all the header values in headers
	...
	// This method extracts the content body which is been send with  the request.
	private int extractContentBody (){
		...
		for (int l1 = 0; l1 < contentLength; l1 += i2) {
			try {				 
				if((in.available() < contentLength)  || (in.available() >contentLength)) {
					setErrorMessage("Length Required");
					return 411;
				}else {
	...
	// This method checks whether request has valid content type header and content-length header
	...
	// This method extracts all the headers from the request and stores it in a table (headers).
	...
	public void setCharset() {
		...
		if(content_type != null) {
			...
			if(result != -1) {
				charSet = content_type.substring((result + "charset".length()+1)).trim();
			}
		}
	}

 	
 	// Returns a line from the input stream as a string. When end of the stream is reached, 
	// returns null.
