...
public class SubscriptionAlive extends Thread {
	...
	// run method of the thread which keeps on polling event registry for all the subscriptions 
	// whose timeout period expires. Removes the subscription object if timeout expires and 
	// notifies event registry  to check wehther the servicelistener related with this subscription can
	...
	public void run() {		
		while(active) {
			...
			for(; subscriberIds.hasMoreElements(); ) {
				...
				Subscription sc = (Subscription)EventRegistry.getSubscriber(subscriberId);
				if(! sc.getInfinite()) {
	...
	// Method which will be called from httpserver when the server shuts down. Since this threads
	// continously runs, there should be a way to come out of the loop.Sets the variable to false.
