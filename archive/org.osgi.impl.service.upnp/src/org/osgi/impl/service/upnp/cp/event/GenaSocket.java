...
public class GenaSocket {
    ...
    * throws GenaException with the error message.    
    */   
   ...
   // This method checks for the subscription id. And if the subscription id is invalid.
   ...
   // This method checks for the timeout duration. And if the timeout duration is invalid
	void checkTimeoutDuration() throws Exception{
		...
		if(! receivedTimeout.equals("infinite")) {
			int result = receivedTimeout.indexOf("-");
   			timeDuration = Integer.parseInt(receivedTimeout.substring(result + 1));
