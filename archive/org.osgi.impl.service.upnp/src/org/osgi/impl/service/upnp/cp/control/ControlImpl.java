...
public class ControlImpl implements Control, SOAPConstants {
	...
	public Dictionary sendControlRequest(String path, String host, String serviceType, String actionName, Dictionary parameters, boolean first) throws Exception {
    	String soapRequest;     	
    	if (first) { 
    		soapRequest = maker.createControlRequest(SOAPConstants.post, path, host, serviceType, actionName, parameters);	
    	} else { 
    		soapRequest = maker.createControlRequest(SOAPConstants.mpost, path, host, serviceType, actionName, parameters);	    	
    	}
		...
		if (i != -1) {
			...
			if (firstHeader.endsWith(ERROR_405) && first) 	{				
				return sendControlRequest(path, host, serviceType, actionName, parameters, false);								
			}else if (firstHeader.endsWith(RES_OK)) {				
