...
public class ErrorCheck {
	...
	// This method will be called by the subscription check and renwal check to check ,
	// headers.
	boolean checkHeaders(Hashtable headers) {
		if(headers.get("sid")!= null) {
			if(headers.get("nt")!=null || headers.get("callback")!=null) {
				result = GenaConstants.GENA_SERVER_VERSION + GenaConstants.GENA_ERROR_400;
				return false;
			}
		}
	return true;
	...
	// This method will be called by the subscription check and renwal check to check ,
	// timeoutheadr, converts it to milli seconds and will be  stored with the subscription object.
	void  setTime(String timeOut ,Subscription subscription) {
		String timeconvert = null;
   		...
   		}else if( timeOut.equals("infinite")) {
   			timeconvert = timeOut;
   		}
   		...
   		if(timeconvert.trim().equals("infinite")) {
			...
			newtime = new Integer(Integer.parseInt(timeconvert)*1000).longValue();
			if(newtime < 1800000 ) {
				timeOut = "Second-1800";
				newtime = 1800000;
			}
   	...
   	// This method will be called by the subscription check and renwal check to form ,
	// subscription okay message which will be send back to the subscribers.
   	String formSubscription_Okay_Message(Hashtable headers, Subscription subscription) {
		
		String message = 
			GenaConstants.GENA_SERVER_VERSION + GenaConstants.GENA_OK_200 + "\r\n" + 
			"DATE: " + getDate()+ "\r\n" + 
			"SERVER: "+ System.getProperty("os.name")+"/"+ System.getProperty("os.version") + " UPnP/1.0 SamsungUPnP/1.0\r\n" + 
			"SID: "+ subscription.getSubscriptionId()+ "\r\n" + 
			"TIMEOUT: "+ subscription.getTimeout()+ "\r\n\r\n";	
		
		return message;
	}
	
	// This method will be called by the subscription check and renwal check to get ,
