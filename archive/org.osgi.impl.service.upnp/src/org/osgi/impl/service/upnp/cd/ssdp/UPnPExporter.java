 ...
 public class UPnPExporter implements ServiceTrackerCustomizer, HttpContext{	
	...
	public void startExporter(){
		...
		try{			
			...
			filter = bc.createFilter("(&(objectclass=" + UPnPDevice.class.getName() + ")(UPnP.export=*))");		
		...
		if(name != null){
			if(name.startsWith("/samsungcd/")){
				...
				if(resources.get(name) != null){
					return "text/xml";
				}else if(iconsTable.get(name) != null){
					UPnPIcon icon = (UPnPIcon)iconsTable.get(name);
	...
	// This method called by service tracker when ever it tracks the new UPnPDevice Service added
	...
	// This method called by service tracker when ever it tracks the new UPnPDevice Service removed
    public void removedService(ServiceReference sr, Object obj){
    	String udn = (String)sr.getProperty("UPnP.device.UDN");    	
    	removeDevice(udn);
	}
    
    // This method called by service tracker when ever the new UPnPDevice Service modified
    public void modifiedService(ServiceReference sr, Object obj){
	}
	
	// This method removes all databases information and sends NOTIFY BYE meessages of perticuler device.	
