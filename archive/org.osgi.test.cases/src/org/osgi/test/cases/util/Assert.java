 ...
 * 
 * <p>
 * This class was inspired by the Assert class in the JUnit test framework.
 */
public abstract class Assert {
	/**
	 * Passes a test. Default implementation does nothing at all, but could be
	 * overridden to i.e. log all the good things that happen
	 */
	public void pass(String message) {
	}

	/**
	 * Fails a test with the given message. Throws an AssertionFailedError by
	 * default, but could be overridden to do something else.
	 */
	 ...
	 */
	public void assertEqualProperties(String message, Dictionary expected,
			Dictionary actual) {
		if (expected == actual) {
	...
	public void assertEquals(String message, double expected, double actual,
		...
		if (Double.isInfinite(expected)) {
			...
			if (!(Math.abs(expected - actual) <= epsilon)) { // Because
															 // comparison with
															 // NaN always
															 // returns false
				failNotEquals(message, new Double(expected), new Double(actual));
	...
	/**
	 * Asserts that two objects does not refer to the same object. If they are
	 ...
	 * specifies the expected class. If the parameter <code>got</code> is of
	 * the wanted type (or a subtype of the wanted type), the classname of
