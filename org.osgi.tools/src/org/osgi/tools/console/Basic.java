...
public class Basic implements CommandProvider
{
	...
	Basic( Console console, BundleContext context ) {
		...
		try {
			...
			InputStream is = getClass().getResourceAsStream("console.properties");
			if (is != null) {
				props.load(is);
				is.close();
			}
	...
	{
		ServiceReference[] refs = _context.getServiceReferences( null, "(service.id=" + intp.nextArgument() + ")" );
	...
	public Object _framework( CommandInterpreter intp ) {
		...
		sb.append( "Vendor                 " + _context.getProperty( "org.osgi.framework.vendor" ) + "\r\n");
		sb.append( "Version                " + _context.getProperty( "org.osgi.framework.version" ) + "\r\n");
		sb.append( "OS name                " + _context.getProperty( "org.osgi.framework.os.name" ) + "\r\n");
		sb.append( "OS version             " + _context.getProperty( "org.osgi.framework.os.version") + "\r\n");
		sb.append( "Processor              " + _context.getProperty( "org.osgi.framework.processor" ) + "\r\n");
	...
	PackageAdmin getPackageAdmin() {
		ServiceReference    ref = _context.getServiceReference( PackageAdmin.class.getName() );
		if ( ref == null )
	...
	/*    public Object _quit(  CommandInterpreter intp ) throws Exception
	{
			String error = intp.nextArgument();
			int n = 0;
			if ( error != null )
				n = Integer.parseInt( error );
	...
	*/
	...
	public Object _log(  CommandInterpreter intp ) throws Exception {
		...
		ServiceReference ref = _context.getServiceReference( LogReaderService.class.getName() );
		if ( ref == null)
		...
		for ( Enumeration e=rdr.getLog(); e.hasMoreElements(); ) {
			...
			{
			case LogService.LOG_DEBUG:      if ( (mask & 1) != 0 ) result.addElement( entry ); break;
			case LogService.LOG_INFO:       if ( (mask & 2) != 0 ) result.addElement( entry ); break;
			case LogService.LOG_WARNING:    if ( (mask & 4) != 0 ) result.addElement( entry ); break;
			case LogService.LOG_ERROR:      if ( (mask & 8) != 0 ) result.addElement( entry ); break;
			default: 
	...
	public Object _global( CommandInterpreter intp ) {
		...
		if ( name == null || value == null )
			return intp.error( "Name and value should be non null, name=" + name + " and value=" + value );
	...
	//
	//	Sets the console to binary mode. Lines are terminated with CTRL-A instead of \r\n.
	//  
	...
	public Object _login( CommandInterpreter intp ) throws Exception {
		...
		if (user == null || pass == null || _username == null || _password == null) {
	...
	public Object _listresources( CommandInterpreter intp ) throws Exception {
		...
		if ( prefix == null )
			prefix = "";
		try {
			Method m = b.getClass().getMethod("getEntryPaths", new Class[] { String.class } );
			Vector v = new Vector();
			for ( Enumeration e= (Enumeration) m.invoke(b,new Object[] { prefix }); e.hasMoreElements(); ) {
			...
			throw new RuntimeException("This framework does not support enumerating resources");
		}
	}
	...
	public Object _showresource( CommandInterpreter intp ) throws Exception {
		...
		try {
			Method m = b.getClass().getMethod("getEntry", new Class[] { String.class } );
