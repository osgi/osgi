        ...
        Thread
{
    ...
    public static String toString( int n, LogEntry l ) {        
        ...
        switch( l.getLevel() ) {
        case LogService.LOG_ERROR:      level="ERROR  ";    break;
        case LogService.LOG_WARNING:    level="WARNING";    break;
        case LogService.LOG_INFO:       level="INFO   ";    break;
        case LogService.LOG_DEBUG:      level="DEBUG  ";    break;
        }
    ...
    public static String toString( int n, Bundle b ) {
        return spaces(n) + justify(b.getBundleId()+" ", 4, 'L') + " " + justify( status(b), 12, 'L' )+ justify(getName(b), 60, 'L');
    }
    ...
    public static String toString( int n, Thread t ) {
        if ( t != null )
            return spaces(n) + (t.isAlive() ? "alive " : "dead  ") + t.getName();               
        return null;
    }

    /**
	 * -11   ACTIVE      file:C:\framework\.\load\org.osgi.impl.service.automation.ja
	 * objectClass  org.osgi.service.automation.WireAdmin
	 * service.id   9
	 * 
	 */
    ...
    String getLine(boolean _echo, boolean _break) throws IOException
    {
        ...
        outer: while ( c!= -1 )
        {
            ...
            case 0x01:
                if ( _history.size() > 0 ) {
                    String l = (String) _history.elementAt( _history.size()-1 );
                    _wrt.print( "\r\n>" + l + "\r\n" );
                    return l;
                }
                break;
    ...
    public void run() {
        ...
        try {
            ...
            while ( _continue ) {
				boolean		mustLogin = System.getProperty("org.osgi.tools.console.mustlogin")!=null;
                ...
                if ( result != null ) {
                    ...
                    //for ( Enumeration e=_targets.elements(); translation==null && e.hasMoreElements(); )
                    //{
                    //  CommandProvider p = (CommandProvider) e.nextElement();
                    //  System.out.println( "Will translate " + p );
                    //  translation = p.toString( result );
                    //  System.out.println( "Hase done translate " + p + " " + translation );
                    //}
    ...
    static String justify( String s, int w, int type ) {
        ...
        {
            switch(type) {
            case 'L': return s + spaces( diff ); 
            case 'C':  return spaces( diff/2 ) + s + spaces( w - s.length() - diff/2);
            case 'R':   return spaces( diff ) + s;
            }
        }
        if ( w > 0 )
	...
	Object setTimer() {
		...
		try {
			...
			TimerTask	task = new TimerTask() {
				...
				public void run() {
					if ( cnt++ == 0 )
						...
						currentThread.stop(new IllegalStateException("Command takes too long"));
					else if ( cnt == 3 )
						currentThread.stop();
					else if ( cnt >= 4 )
						this.cancel();
			...
			((Timer)timer).scheduleAtFixedRate( task, 30000, 10000 );
			return task;
		}
		catch( Error e ) {}
		*/
