...
class Context implements CommandInterpreter {
	...
	public boolean nextLine() {
		...
		try {
			_st = new StringTokenizer( _handler.getNextLine(true, false), " \t\"\'^={}[]", true );
			nextArgument();
			return true;
		}
		catch( Exception e ) {
	...
	public String getUser() { return _handler.getUser(); }
	...
	public Object execute( String line ) {
		...
		catch( IllegalAccessException ite ) { ite.printStackTrace(); }
		catch( RuntimeException ite ) {
			ite.printStackTrace();
			return "ERROR: Unexpected exception " + ite + ":" + ite.getMessage();
		}
		catch( InvocationTargetException ite ) {
			ite.getTargetException().printStackTrace();
			if ( ite.getTargetException() instanceof BundleException ) {
				BundleException be = (BundleException) ite.getTargetException();
				return "ERROR: Bundle Exception " + cmd + " ("  + be.getNestedException() + " )";
			}
			return "ERROR: Command resulted in exception " + ite.getTargetException();
		}
		catch( Exception ee ) {
