...
public class EclipseProject {
	...
	public EclipseProject(File eclipseProject,
			boolean root) {
	...
	private class Handler extends DefaultHandler {
		public void startElement(String uri, String localName, String tag,
			...
			for (int i = 0; i < size; i++) {
				...
				} else if (key.equals("sourcepath")) {
					// we don't need this attribute
				} else if (key.equals("exported")) {
					exported = Boolean.valueOf(value).booleanValue();
				} else if (key.equals("excluding")) {
					// we don't need this attribute
				} else {
			...
			if ("src".equals(kind)) {
				...
				/*
				 * var is a dir or jar/zip file the first component of which is
				 * a variable name.
				 */
				if (root || exported) {
					...
					if (value == null) {
						throw new SAXParseException("Undefined property \""
								+ value + "\"", locator);
					}
				...
				/*
				 * output is the output folder within the current eclipse dir.
				 */
				path = new File(eclipseProject, path).getAbsolutePath();
				setBindir(path);
				addClasspath(path);
			} else if ("con".equals(kind)) {
				/*
				 * con is a container. Don't know how to find its value :-(
				 */
			} else {
				throw new SAXParseException("Unexpected attribute value \""
						+ kind + "\"", locator);
			}
		}
	}
