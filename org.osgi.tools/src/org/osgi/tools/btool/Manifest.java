...
public class Manifest extends Hashtable {
		...
		do {
			...
			while ( st.ttype == ';' ) {
				...
				&& pair[1] != null )
					...
					error( "Invalid attribute for package " + pair[0] + "=" + pair[1] );
			}
			v.addElement( p );
		} while ( st.ttype == ',' );
	...
	Native[] getNative( String line ) throws IOException
	{
		...
		do {
			...
			do {
				...
				else if ( pair[0].equalsIgnoreCase( "processor" ) )
					spec.processor = pair[1];
				else if ( pair[0].equalsIgnoreCase( "osname" ) )
					spec.osname = pair[1];
				else if ( pair[0].equalsIgnoreCase( "osversion" ) )
					spec.osversion = versionToLong(pair[1]);
				else if ( pair[0].equalsIgnoreCase( "language" ) )
					spec.language = pair[1];
				else
					error( "Unknown attribute for native code : " + pair[0] + "=" + pair[1] );
	...
	public Package [] getImports() { return _imports; }
	public Package [] getExports()  { return _exports; }
	public String getActivator() { return _activator; }
	...
	String getValue(String key) { return (String) super.get(key.toLowerCase()); }
}
