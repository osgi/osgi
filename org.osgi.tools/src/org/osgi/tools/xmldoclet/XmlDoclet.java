...
public class XmlDoclet extends Doclet {
	...
	void print( PackageDoc pack ) {
		...
		pw.println( "  <package name='" + pack.name() + "' fqn='" + pack.name()+ "' qn='" + pack.name() + "'>" );
		printComment( pack );		
	...
	void print( ClassDoc clazz ) {
		...
		pw.println( "  <class name='" + name 
			...
			+ "' modifiers='" + clazz.modifiers() + (clazz.isClass()? " class" : "" )
				...
				for ( int i=0; i<interfaces.length; i++ ) {
					if ( ht.get( interfaces[i] ) == null ) { 
						pw.println( "<implements name='" + interfaces[i].name() 
							...
							+ "' package='" + interfaces[i].containingPackage().name() 
							+ "' local='" + (ptr= clazz)
							+ "'/>" );	
						ht.put( interfaces[i], "" );
					}
	...
	void print( ConstructorDoc cnst ) {
		pw.println( "    <method name='" + cnst.name() 
			+ "' fqn='" +  cnst.qualifiedName() 
			+ "' qn='" +  cnst.containingClass().name()  + "." + cnst.name() + flatten(cnst.signature())
			+ "' package='" + cnst.containingPackage().name() 
			+ "' modifiers='" + cnst.modifiers()
			+ "' signature='" + cnst.signature()
	...
	void print( MethodDoc cnst ) {
		pw.println( "    <method name='" + cnst.name() 
			...
			+ "' package='" + cnst.containingPackage().name() 
			+ "' modifiers='" + cnst.modifiers()
			+ "' typeName='" + cnst.returnType().typeName()
			+ "' qualifiedTypeName='" + cnst.returnType().qualifiedTypeName()
			+ "' dimension='" + cnst.returnType().dimension()
			+ "' signature='" + cnst.signature()
			+ "' flatSignature='" + flatten(cnst.signature())
			+ "'>" );
		printMember( cnst );
	...
	void print( FieldDoc cnst ) {
		...
		pw.println( "    <field name='" + cnst.name() 
			+ "' fqn='" + cnst.qualifiedName() 
			+ "' qn='" +  cnst.containingClass().name()  + "." + cnst.name()
			+ "' package='" + cnst.containingPackage().name() 
			+ "' modifiers='" + cnst.modifiers()
	...
	void print( Parameter param ) {
		pw.println( "<parameter name='" + param.name()
			+ "' dimension='" + param.type().dimension()
			+ "' typeName='" + param.type().typeName()
			+ "' fqn='" + param.type().qualifiedTypeName()
			+ "'/>" );
	}		
	...
	void printThrows( StringBuffer sb, ThrowsTag tag ) {
		sb.append( "   <throws name='" 
			...
			+ ( tag.exception() != null ? "' exception='" + tag.exception().qualifiedName() : "" )  
			+ "'>" );
	...
	void printX( StringBuffer sb, Tag tag ) {
		if ( tag.kind().equals("Text") ) {
			...
			sb.append( "<" + tag.kind().substring(1) + ">" + html( toString(tag.inlineTags())) + "</" + tag.kind().substring(1) + ">" );
	}
