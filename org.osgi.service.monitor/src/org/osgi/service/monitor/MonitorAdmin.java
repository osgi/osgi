 ...
 * {@code StatusVariable} query requests and measurement job control requests.
 * <p>
 ...
 * {@code ServicePermissions} can query the list of {@code Monitorable} services
 * from the service registry and then query the list of {@code StatusVariable}
 * names from the {@code Monitorable} services. This way all services which
 * publish {@code StatusVariable}s will be returned regardless of whether they
 * do or do not hold the necessary {@code MonitorPermission} for publishing
 * {@code StatusVariable}s. By using the {@code MonitorAdmin} to obtain the
 * {@code StatusVariable}s it is guaranteed that only those {@code Monitorable}
 * services will be accessed who are authorized to publish
 * {@code StatusVariable}s. It is the responsibility of the {@code MonitorAdmin}
 ...
 * parameters must not be {@code null}, and the IDs they contain must conform to
 * their respective definitions in {@link Monitorable} and
...
public interface MonitorAdmin {
	...
	/**
	 * Returns a {@code StatusVariable} addressed by its full path. The entity
	 * which queries a {@code StatusVariable} needs to hold
	 * {@code MonitorPermission} for the given target with the {@code read}
	 * action present.
	 * 
	 ...
	 *         {@code null} or otherwise invalid, or points to a non-existing
	 *         {@code StatusVariable}
	 * @throws java.lang.SecurityException if the caller does not hold a
	 *         {@code MonitorPermission} for the {@code StatusVariable}
	 *         specified by {@code path} with the {@code read} action present
	 */
	public StatusVariable getStatusVariable(String path) throws IllegalArgumentException, SecurityException;

	/**
	 * Returns the names of the {@code Monitorable} services that are currently
	 * registered. The {@code Monitorable} instances are not accessible through
	 * the {@code MonitorAdmin}, so that requests to individual status variables
	 * can be filtered with respect to the publishing rights of the
	 * {@code Monitorable} and the reading rights of the caller.
	 * <p>
	 * The returned array contains the names in alphabetical order. It cannot be
	 * {@code null}, an empty array is returned if no {@code Monitorable}
	 ...
	 * {@code Monitorable} instance. The {@code StatusVariables} will hold the
	 ...
	 * {@code Monitorable}.
	 * 
	 * @param monitorableId the identifier of a {@code Monitorable} instance
	 * @return a list of {@code StatusVariable} objects published by the
	 *         specified {@code Monitorable}
	 * @throws java.lang.IllegalArgumentException if {@code monitorableId} is
	 *         {@code null} or otherwise invalid, or points to a non-existing
	 *         {@code Monitorable}
	 */
	 ...
	 * {@code Monitorable} instance. Only those status variables are listed
	 ...
	 * alphabetical order. It cannot be {@code null}, an empty array is returned
	 * if no (authorized and readable) Status Variables are provided by the
	 * given {@code Monitorable}.
	 * 
	 * @param monitorableId the identifier of a {@code Monitorable} instance
	 * @return a list of {@code StatusVariable} objects names published by the
	 *         specified {@code Monitorable}
	 * @throws java.lang.IllegalArgumentException if {@code monitorableId} is
	 *         {@code null} or otherwise invalid, or points to a non-existing
	 *         {@code Monitorable}
	...
	/**
	 * Switches event sending on or off for the specified {@code StatusVariable}
	 * s. When the {@code MonitorAdmin} is notified about a
	 * {@code StatusVariable} being updated it sends an event unless this
	 * feature is switched off. Note that events within a monitoring job can not
	 * be switched off. The event sending state of the {@code StatusVariables}
	 * must not be persistently stored. When a {@code StatusVariable} is
	 ...
	 * wildcard is that it stands for any matching {@code StatusVariable} at the
	 * time of the method call, it does not affect the event sending status of
	 * {@code StatusVariable}s which are not yet registered. As an example, when
	 * the {@code switchEvents("MyMonitorable/*", false)} method is executed,
	 * event sending from all {@code StatusVariables} of the MyMonitorable
	 * service are switched off. However, if the MyMonitorable service starts to
	 * publish a new {@code StatusVariable} later, it's event sending status is
	 * on by default.
	 * 
	 ...
	 *         {@code MonitorPermission} with the {@code switchevents} action or
	 *         if there is any {@code StatusVariable} in the {@code path} field
	...
	/**
	 * Issues a request to reset a given {@code StatusVariable}. Depending on
	 * the semantics of the {@code StatusVariable} this call may or may not
	 * succeed: it makes sense to reset a counter to its starting value, but
	 * e.g. a {@code StatusVariable} of type String might not have a meaningful
	 * default value. Note that for numeric {@code StatusVariable}s the starting
	 * value may not necessarily be 0. Resetting a {@code StatusVariable}
	 * triggers a monitor event if the {@code StatusVariable} supports update
	 * notifications.
	 * <p>
	 * The entity that wants to reset the {@code StatusVariable} needs to hold
	 * {@code MonitorPermission} with the {@code reset} action present. The
	 * target field of the permission must match the {@code StatusVariable} name
	 * to be reset.
	 * 
	 * @param path the identifier of the {@code StatusVariable} in
	 *        [Monitorable_id]/[StatusVariable_id] format
	 * @return {@code true} if the {@code Monitorable} could successfully reset
	 *         the given {@code StatusVariable}, {@code false} otherwise
	 * @throws java.lang.IllegalArgumentException if {@code path} is
	 *         {@code null} or otherwise invalid, or points to a non-existing
	 *         {@code StatusVariable}
	 * @throws java.lang.SecurityException if the caller does not hold
	 *         {@code MonitorPermission} with the {@code reset} action or if the
	 *         specified {@code StatusVariable} is not allowed to be reset as
	 *         per the target field of the permission
	 */
	public boolean resetStatusVariable(String path) throws IllegalArgumentException, SecurityException;

	/**
	 * Returns a human readable description of the given {@code StatusVariable}.
	 * The {@code null} value may be returned if there is no description for the
	 * given {@code StatusVariable}.
	 * <p>
	 ...
	 * @param path the full path of the {@code StatusVariable} in
	 *        [Monitorable_ID]/[StatusVariable_ID] format
	 * @return the human readable description of this {@code StatusVariable} or
	...
	/**
	 * Starts a time based {@code MonitoringJob} with the parameters provided.
	 * Monitoring events will be sent according to the specified schedule. All
	 * specified {@code StatusVariable}s must exist when the job is started. The
	 * initiator string is used in the {@code mon.listener.id} field of all
	 * events triggered by the job, to allow filtering the events based on the
	 * initiator.
	 * <p>
	 * The {@code schedule} parameter specifies the time in seconds between two
	 ...
	 * {@code StatusVariable}s with the {@code startjob} action present. If the
	 * permission's action string specifies a minimal sampling interval then the
	 * {@code schedule} parameter should be at least as great as the value in
	 * the action string.
	 * 
	 ...
	 *         {@code StatusVariable} names contains an invalid or non-existing
	 *         {@code StatusVariable}; if {@code initiator} is {@code null} or
	 *         empty; or if the {@code schedule} or {@code count} parameters are
	 *         invalid
	 * @throws java.lang.SecurityException if the caller does not hold
	...
	/**
	 * Starts a change based {@code MonitoringJob} with the parameters provided.
	 * Monitoring events will be sent when the {@code StatusVariable}s of this
	 * job are updated. All specified {@code StatusVariable}s must exist when
	 * the job is started, and all must support update notifications. The
	 * initiator string is used in the {@code mon.listener.id} field of all
	 * events triggered by the job, to allow filtering the events based on the
	 * initiator.
	 * <p>
	 * The {@code count} parameter specifies the number of changes that must
	 * happen to a {@code StatusVariable} before a new notification is sent,
	 * this must be a positive integer.
	 * <p>
	 * The entity which initiates a {@code MonitoringJob} needs to hold
	 * {@code MonitorPermission} for all the specified target
	 * {@code StatusVariable}s with the {@code startjob} action present.
	 * 
	 * @param initiator the identifier of the entity that initiated the job
	 * @param statusVariables the list of {@code StatusVariable}s to be
	 *        monitored, with each {@code StatusVariable} name given in
	 ...
	 * @throws java.lang.IllegalArgumentException if the list of
	 *         {@code StatusVariable} names contains an invalid or non-existing
	 *         {@code StatusVariable}, or one that does not support
	 *         notifications; if the {@code initiator} is {@code null} or empty;
	 *         or if {@code count} is invalid
	 * @throws java.lang.SecurityException if the caller does not hold
	 *         {@code MonitorPermission} for all the specified
	 *         {@code StatusVariable}s, with the {@code startjob} action present
	 */
	...
	/**
	 * Returns the list of currently running {@code MonitoringJob}s. Jobs are
	 ...
	 * have {@code MonitorPermission} with the proper {@code startjob} action
	 * for all the Status Variables monitored by a job, then that job will be
	 * silently omitted from the results.
	 * <p>
	 * The returned array cannot be {@code null}, an empty array is returned if
