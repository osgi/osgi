 ...
 * {@code StatusVariable}, the action can be {@code read}, {@code publish},
 * {@code reset}, {@code startjob}, {@code switchevents}, or the combination of
 * these separated by commas. Action names are interpreted case-insensitively,
 * but the canonical action string returned by {@link #getActions()} uses the
 * forms defined by the action constants.
 * <p>
 * If the wildcard {@code *} appears in the actions field, all legal monitoring
...
public class MonitorPermission extends Permission {
	...
	/**
	 * Holders of {@code MonitorPermission} with the {@code read} action present
	 * are allowed to read the value of the {@code StatusVariable}s specified in
	 * the permission's target field.
	 */
	public static final String	READ				= "read";

	/**
	 * Holders of {@code MonitorPermission} with the {@code reset} action
	 * present are allowed to reset the value of the {@code StatusVariable}s
	 * specified in the permission's target field.
	 */
	public static final String	RESET				= "reset";

	/**
	 * Holders of {@code MonitorPermission} with the {@code publish} action
	 * present are {@code Monitorable} services that are allowed to publish the
	 * {@code StatusVariable}s specified in the permission's target field. Note,
	 * that this permission cannot be enforced when a {@code Monitorable}
	 * registers to the framework, because the Service Registry does not know
	 * about this permission. Instead, any {@code StatusVariable}s published by
	 * a {@code Monitorable} without the corresponding {@code publish}
	 * permission are silently ignored by {@code MonitorAdmin}, and are
	 * therefore invisible to the users of the monitoring service.
	 */
	public static final String	PUBLISH				= "publish";

	/**
	 * Holders of {@code MonitorPermission} with the {@code startjob} action
	 ...
	 * form: {@code startjob:n}. This allows the holder of the permission to
	 * initiate time based jobs with a measurement interval of at least
	 * {@code n} seconds. If {@code n} is not specified or 0 then the holder of
	...
	/**
	 * Holders of {@code MonitorPermission} with the {@code switchevents} action
	 * present are allowed to switch event sending on or off for the value of
	 * the {@code StatusVariable}s specified in the permission's target field.
	 */
	...
	private static final int	ALL_FLAGS			= READ_FLAG | RESET_FLAG | PUBLISH_FLAG | STARTJOB_FLAG | SWITCHEVENTS_FLAG;
	...
	/**
	 * Create a {@code MonitorPermission} object, specifying the target and
	 * actions.
	 * <p>
	 * The {@code statusVariable} parameter is the target of the permission,
	 ...
	 * wildcard {@code *} in the target string. The wildcard is allowed in both
	 ...
	 *         {@code null}, or invalid with regard to the constraints defined
							...
							if (action.equalsIgnoreCase(SWITCHEVENTS)) {
								...
								if (action.toLowerCase().startsWith(STARTJOB)) {
									...
									if (action.length() != slen) {
										...
										try {
											minJobInterval = Integer.parseInt(action.substring(slen + 1));
	...
	private void checkId(String id, String idName) throws IllegalArgumentException {
		...
		if (nameBytes.length > StatusVariable.MAX_ID_LENGTH)
			throw new IllegalArgumentException(idName + " is too long (over " + StatusVariable.MAX_ID_LENGTH + " bytes in UTF-8 encoding).");
