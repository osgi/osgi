...
public class PackageStateMBeanTestCase extends MBeanGeneralTestCase {
	...
	public void testGetExportingBundle() throws IOException {
		...
		String bundleVersion = (String)testBundle1.getHeaders().get("Bundle-Version");
		
		long mBeanBundleId = pMBean.getExportingBundle(EXPORTED_PACKAGE, bundleVersion);
		ExportedPackage[] exportedPackages = pAdmin.getExportedPackages(EXPORTED_PACKAGE);
		
		boolean found = false;
		for(ExportedPackage exportPack : exportedPackages) {
			if (exportPack.getExportingBundle().getBundleId() == mBeanBundleId) {
				found = true;
				break;
			}
		}
	...
	public void testGetImportingBundles() throws IOException {
		...
		String bundleVersion = (String)testBundle2.getHeaders().get("Bundle-Version");
		long[] bundleIds = pMBean.getImportingBundles(EXPORTED_PACKAGE, bundleVersion);
		boolean found = false;
	...
	public void testIsRemovalPending() throws IOException {
		...
		boolean mBeanRemovalPending = pMBean.isRemovalPending(EXPORTED_PACKAGE, bundleVersion);
		...
		assertTrue(
				"received inconsistent state information for bundle exporting package "
						+ EXPORTED_PACKAGE + "." 
						+ testBundle2.getSymbolicName(), mBeanRemovalPending == bundleStateRemovalPending);
