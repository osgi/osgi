...
public class PositionControl extends DefaultTestBundleControl {
	...
	/**
	 * Tests <tt>Position</tt> class constructors by displaying
	...
	public void testPositionConstructor() {
		...
		Measurement[] m = {
			new Measurement(1.0, 0.1, Unit.rad),         // 0 rad with error
			new Measurement(1.0, 0.0, Unit.rad),         // 1 rad with error=0
			new Measurement(1.0, Double.NaN, Unit.rad),  // 2 rad with error NaN
			...
			new Measurement(1.0, Double.NaN, Unit.m_s),  //10 m_s with error NaN
		...
		assertPosition(" Position with errors and correct units:",pos,m[0],m[0],m[4],m[8],m[0]);

		// Position with errors=0 and correct units
		pos = new Position(m[1],m[1],m[5],m[9],m[1]);
		assertPosition(" Position with errors=0 and correct units:",pos,m[1],m[1],m[5],m[9],m[1]);

		// Position with errors NaN and correct units
		pos = new Position(m[2],m[2],m[6],m[10],m[2]);
		assertPosition(" Position with errors NaN and correct units:",pos,m[2],m[2],m[6],m[10],m[2]);

		// Position without errors and correct units
		pos = new Position(m[3],m[3],m[7],m[11],m[3]);
		assertPosition(" Position without errors but correct units:",pos,m[3],m[3],m[7],m[11],m[3]);
		...
		assertPosition("Position with null-parameters:",pos,null,null,null,null,null);

        // Flip around the World test
        Position myOffice = new Position(
                new Measurement(52.1369*degrees, 0.0, Unit.rad), // Latitude
        ...
        Position myOfficeToo = new Position(
                new Measurement((180.0-52.1369)*degrees, 0.0, Unit.rad), // Latitude
     ...
     void assertPosition(String text, Position pos,
                         Measurement lat, Measurement lon, Measurement alt,
     ...
     {
         assertEquals(text+" Latitude",xpos.getLatitude().getValue(),pos.getLatitude().getValue(), 0.00001);
         assertEquals(text+" Longitude",xpos.getLongitude().getValue(),pos.getLongitude().getValue(), 0.00001);
         assertEquals(text+" Altitude",xpos.getAltitude().getValue(),pos.getAltitude().getValue(), 0.00001);
         assertEquals(text+" Speed",xpos.getSpeed().getValue(),pos.getSpeed().getValue(), 0.00001);
         assertEquals(text+" Track",xpos.getTrack().getValue(),pos.getTrack().getValue(), 0.00001);

         assertEquals(text+" Latitude Error",xpos.getLatitude().getError(),pos.getLatitude().getError(), 0.00001);
         assertEquals(text+" Longitude Error",xpos.getLongitude().getError(),pos.getLongitude().getError(), 0.00001);
         assertEquals(text+" Altitude Error",xpos.getAltitude().getError(),pos.getAltitude().getError(), 0.00001);
         assertEquals(text+" Speed Error",xpos.getSpeed().getError(),pos.getSpeed().getError(), 0.00001);
         assertEquals(text+" Track Error",xpos.getTrack().getError(),pos.getTrack().getError(), 0.00001);

         assertEquals(text+" Latitude Unit",xpos.getLatitude().getUnit(),pos.getLatitude().getUnit());
         assertEquals(text+" Longitude Unit",xpos.getLongitude().getUnit(),pos.getLongitude().getUnit());
         assertEquals(text+" Altitude Unit",xpos.getAltitude().getUnit(),pos.getAltitude().getUnit());
         assertEquals(text+" Speed Unit",xpos.getSpeed().getUnit(),pos.getSpeed().getUnit());
         assertEquals(text+" Track Unit",xpos.getTrack().getUnit(),pos.getTrack().getUnit());
     }


	/**
	 * Test the boundary conditions of the lat/lon
	 * normalization. We do this in degrees so the input
	 * is easier to verify.
	 */
	public void testLatLon() {
		...
		assertLatLon( "+/- lat 91   ",	    0,     91,   -180,	 89 ); // Should flip to other side of world
		assertLatLon( "+/- lat 89   ",	    0,     89,      0,	 89 );
		assertLatLon( "+/- lat -91  ",	    0,    -91,   -180,	-89 ); // Should flip to other side of world
		assertLatLon( "+/- lat -89  ",	    0,    -89,      0,	-89 );
		assertLatLon( "+/- lat 89   ",	    0,     89,      0,	 89 );
		assertLatLon( "+/- lat/lon  ",    200,    -99,     20,	-81 ); // Should flip to other side of world
	...
	void assertLatLon( String test, int lon, int lat, int xlon, int xlat ) {
		...
		assertEquals( test + " Longitude ", xlon, Math.round(pos.getLongitude().getValue()/degrees) );
		assertEquals( test + " Latitude  ", xlat, Math.round(pos.getLatitude().getValue()/degrees) );
	}
	...
	void assertTrack( double in, double out ) {
		Position p = new Position( null, null, null, null, new Measurement(in*degrees,0,Unit.rad) );
