...
import org.osgi.impl.service.dmt.dispatcher.old.NodeUtil;
...
public class DmtSessionImpl implements DmtSession {
	...
	DmtSessionImpl(String principal, String subtreeUri, int lockMode,
        ...
        subtreeNode = NodeUtil.validateAndNormalizeAndCreateNode(subtreeUri);
	...
	public synchronized void renameNode(String nodeUri, String newNodeName) 
        ...
        String newName = NodeUtil.validateAndNormalizeNodeName(newNodeName);
    ...
    private Node getRootForPlugin(Plugin plugin, Node node) {
    ...
    private Node makeAbsoluteUri(String nodeUri) throws DmtException {
        Node node = NodeUtil.validateAndNormalizeAndCreateNode(nodeUri);
		if (node.isAbsolute()) {
    ...
    static void init_acls() {
        acls = new Hashtable();
        acls.put(NodeUtil.ROOT_NODE, new Acl("Add=*&Get=*&Replace=*"));
    }
    ...
    private void postOSGiEvent(DmtEventCore dmtEvent) {
        ...
        if(nodes != null)
            properties.put("nodes", 
                    NodeUtil.getUriArray((Node[]) nodes.toArray(new Node[0])));
        
        List newNodes = dmtEvent.getNewNodes();
        if(newNodes != null)
            properties.put("newnodes", 
                    NodeUtil.getUriArray((Node[]) newNodes.toArray(new Node[0])));
