 ...
 * 
 * This implements the default algorithm specified. It is used when no other
 * DriverSelector service is found in the framework registry.
 * <p>
 * This object is not intended to be ever registered in the framework registry.
 * 
 * @author $Author$
 * @version $Revision$
 *  
...
class DefaultDriverSelector implements DriverSelector {
	public int select(ServiceReference device, Match[] matches) {
		...
		for (int i = 0; i < matches.length; i++) {
			...
			if (osid == null)
				...
				try {
					...
					Log
							.error("Illegal number format in service ID of "
									+ cd, e);
				}
			if (osrank != null) {
				try {
					...
					Log.error("Illegal number format in service ranking of "
							+ cd, e);
				}
			}
			if (cm > maxmatch
					|| (cm == maxmatch && (crank > brank || (crank == brank && csid < bsid)))) {
				maxmatch = cm;
				best = i;
				bsid = csid;
				brank = crank;
