...
public class DriverRef implements Match {
	...
	String				id;
	/**
	 * Driver bundle. If non-null, the bundle should be stopped and uninstalled
	 * at cleanup.
	 */
	Bundle				bundle;
	/**
	 ...
	 */
	DriverRef(BundleContext bc, String id, ServiceReference driverService,
			boolean trans) {
		this.bc = bc;
	...
	DriverRef(BundleContext bc, ServiceReference driverService) {
	...
	/**
	 * Compute the match value of the driver. Subsequent matches return the same
	 * value or <code>Device.MATCH_NONE</code> if the driver is disabled.
	 * 
	 * @param deviceSR Service reference to the device
	 * 
	 * @return Match value
	...
	/**
	 * Attach to the device (return null), or refer to another driver. If the
	 * attach fails to complete an exception is thrown. Regardless of the
	 * outcome, the DriverRef is disabled and will not participate in future
	 * matches. This is achieved by setting its match value to
