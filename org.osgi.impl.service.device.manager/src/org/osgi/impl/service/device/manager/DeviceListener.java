 ...
 *
 * Registered devices are put in a queue, where a thread
 * processes one after another and tries to find a matching
 * driver.
 *
 * @see DeviceRef
 *
...
public class DeviceListener implements ServiceListener, Runnable {
	...
	this.bc = bc;
	this.dm = dm;
	driverLocators = new ServiceTracker (bc, dm.locatorClass, null);
	driverLocators.open();

	driverSelector = new ServiceTracker (bc, dm.selectorClass, null);
	driverSelector.open();	
    }
	...
	case ServiceEvent.REGISTERED:
		...
		deviceQueue.add(new DeviceRef(bc,sr));
		 Log.info(sr+": device added to queue (deviceID="+sr.getProperty("deviceID")+")");
		 deviceQueue.notify();
	    }
	    break;
     ...
     *
     * This is the main
     * loop which takes devices from the queue and processes them
     */
    ...
    void doLocators(Hashtable   instDrivers,
		    DeviceRef dev) {
	...
	for(int i=0;i<locators.length;i++) {
		...
		for(int j=0;j<drv.length;j++) {
			...
			Log.debug ("loading driver "+drv[j]+" from locator "+locator); 
			try {
				...
				Log.warn ("Driver Locator failed to provide InputStream for driver "+id);
			} catch (Exception e) {
    ...
    /**
     * This method is invoked for every DeviceReference taken from the 
     * queue, or when a device became idle.
     * <br>
	...
	synchronized (dm.queueLock) {
	    ...
	    do {
	        DriverRef[] drivers = (DriverRef[])allDrivers.
		    values().toArray(new DriverRef[0]);
		for (int i=0; i<drivers.length; i++) 
		    drivers[i].match (deviceRef.getServiceReference());
		
		DriverSelector ds=(DriverSelector)driverSelector.getService();
		if (ds==null)
		    ds=new DefaultDriverSelector();
		int chosen = DriverSelector.SELECT_NONE;
		try {
		    chosen = ds.select(deviceRef.getServiceReference(),drivers);
		} catch (Exception e) {
