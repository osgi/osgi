 ...
 * 
 * Registered drivers are put in a queue, where a thread processes one after
 * another and runns a matching with currently idle devices.
 * 
 * @see DriverRef
 * 
 * @author $Author$
...
public class DriverListener implements ServiceListener, Runnable {
	...
	Vector				transientDrivers	= new Vector(); // contains
	 ...
	 * 
	 * Registered drivers are put in a queue, where a thread processes one after
	 * another and tries to find idle devices to attach to them.
	 * </P>
	 * transient drivers which are installed during a device manager matching
	 * ans attaching phase and which are subject to removal (so called
	 * "transient" drivers) are ignore and not further processed.
	 * 
	...
	public void serviceChanged(ServiceEvent se) {
		...
		switch (se.getType()) {
			...
			case ServiceEvent.UNREGISTERING :
				...
				if (!transientDrivers.contains(sr.getBundle())) {
					dm.handleIdleDevices(); // according to spec 5.2.5 take care
					// of ALL idle devices here.
				}
				else {
					Log.info(sr + ": transient driver - ignored!");
				}
	 ...
	 * run
	 * 
	 * This is the main loop which takes drivers from the queue and processes
	 * them
	 */
	public void run() {
		while (!quit) {
			DriverRef dr = null;
			synchronized (dm.queueLock) { // prevent running this algorithm
				// while the
	...
	Hashtable getAllDrivers(Hashtable oldDrivers, boolean trans) {
		...
		if (oldDrivers != null)
			drivers = oldDrivers;
		try {
			sra = bc.getServiceReferences(DeviceManagerImpl.driverClass, null);
		}
		...
		for (int i = 0; i < sra.length; i++) {
			...
			if (id != null) {
				...
				Log.warn("No DRIVER_ID set for driver service " + sra[i]
						+ " from bundle: " + sra[i].getBundle().getLocation()
						+ " --> IGNORING");
			}
		}
		return drivers;
	...
	/**
	 * This method is invoked for every DriverReference taken from the queue.
	 * 
