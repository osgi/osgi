 ...
 * <p>
 * The Device Manager does not register itself any services with the framework.
 * Instead it reacts to Device and Driver services being registered.
 * </p>
 * <p>
...
public class DeviceManagerImpl implements BundleActivator {
	...
	public void start(BundleContext bc) {
		...
		try {
			bc.addServiceListener(deviceListener, "(|(objectClass="
					+ deviceClass + ")" + "(" + deviceCategoryKey + "=*))");
		}
		...
		driverSweeper.start();
		try {
			bc.addServiceListener(driverListener, "(objectClass=" + driverClass
					+ ")");
		}
		catch (InvalidSyntaxException e) {
	 ...
	 * 
	 * It will take a list of old drivers, and only add newly found drivers, not
	 * present in the old list.</BR> The newly added drivers will be flagged as
	 * "transient" as specified by the given flag. - This information is later
	 * consulted as to whether these drivers are candidates for removing.
	 * 
	 * @param oldDrivers The list of known Drivers
	 * @param trans whether to flag the newly found drivers as "transient"
	 */
	Hashtable getAllDrivers(Hashtable oldDrivers, boolean trans) {
	 ...
	 * 
	 * A "idle" device bundle is a bundle registering a device service, which no
	 * other bundle registering a driver service has a dependency on.
	 */
	synchronized ServiceReference[] findIdleDevices() {
		...
		try {
			allDevices = bc.getServiceReferences(null, "(|(objectClass="
					+ deviceClass + ")(" + deviceCategoryKey + "=*))");
		}
		catch (InvalidSyntaxException e) {
		...
		for (int i = 0; i < allDrivers.length; i++) {
			ServiceReference sr[] = allDrivers[i].getBundle()
					.getServicesInUse();
			if (sr != null) {
		...
		if (idleDevices.size() == 0)
			return null;
		return ((ServiceReference[]) (idleDevices
				.toArray(new ServiceReference[1])));
	}

	/**
	 * Take care of idle devices.
	 * 
	 * simulate as if any idle device would have just been detected as a new
	 * device
	 */
	synchronized void handleIdleDevices() {
		...
		for (int i = 0; i < idleDevices.length; i++) {
			Log
					.debug("trying to find driver for idle device:"
							+ idleDevices[i]);
