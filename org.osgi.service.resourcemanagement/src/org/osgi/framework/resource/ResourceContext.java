...
/**
 * Logical entity for resource accounting. A resource context has a group of member bundles, 
 * and a bundle can be a member of 1 or 0 resource contexts.
 * 
 * <p>
 * Management agents can use the {@link ResourceManager#createContext(String, ResourceContext)}} 
 * method to create ResourceContext instances.
 * <p>
 * Management agents can use the {@link #getMonitor(String)} method to get {@link ResourceMonitor}
 * instances for the supported resource types. These instances can then be used to monitor the usage
 * of the resources, or the set usage limits.
 * <p>
 * The ResourceContext object associated with a bundle can be obtained by calling
 * {@link Bundle#adapt(Class) bundle.adapt(ResourceContext.class)} on the bundle.
 * 
 * @ThreadSafe
 * @noimplement
 */
public interface ResourceContext {
  ...
  /**
   * Returns the name of the resource context. Resource context names are unique
   * within a framework instance.
   ...
   * @return An array of {@link Bundle} objects, or an empty array if no bundles
   * are currently members of this context
   */
  ...
  /**
   * Removes the bundle from this resource context. If a <code>destination</code>
   * context is specified, the bundle will be added in it.
   * <p>
   * Resources previously allocated by this bundle will not be removed from the
   * resource context. The change applies only for future allocations.
   * <p>
   * A {@link ResourceEvent} with type {@link ResourceEvent#BUNDLE_REMOVED} will be sent.
   *        
   ...
   * If the ResourceManager implementation does not support this resource
   * type, null is returned
   * @param resourceType The resource type, for which a resource monitor 
   * is requested
   * @return A ResourceMonitor instance, or null, if this resource type is
   * not supported
   */
