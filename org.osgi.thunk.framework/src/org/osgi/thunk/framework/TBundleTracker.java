...
class TBundleTracker<S> implements TBundleTrackerCustomizer<S> {
	 ...
	 */
	protected final TBundleContext		context;

	/**
	 * The <code>BundleTrackerCustomizer</code> object for this tracker.
	 */
	final TBundleTrackerCustomizer<S>	customizer;
	 ...
	 * @see Bundle#getState()
	 */
	public TBundleTracker(TBundleContext context, int stateMask,
			TBundleTrackerCustomizer<S> customizer) {
	...
	public void open() {
		...
		synchronized (this) {
			...
			synchronized (t) {
				...
				List<TBundle> bundles = new ArrayList<TBundle>(Arrays
						.asList(context.getBundles()));
				for (Iterator<TBundle> i = bundles.iterator(); i.hasNext();) {
					TBundle bundle = i.next();
	 ...
	 * @see TBundleTrackerCustomizer#addingBundle(Bundle, BundleEvent)
	 */
	public S addingBundle(TBundle bundle, TBundleEvent event) {
	 ...
	 * @see TBundleTrackerCustomizer#modifiedBundle(Bundle, BundleEvent, Object)
	 */
	public void modifiedBundle(TBundle bundle, TBundleEvent event, S object) {
		/* do nothing */
	}
	 ...
	 *        with the call to this method.
	 * @param object The customized object for the specified bundle.
	 * @see TBundleTrackerCustomizer#removedBundle(Bundle, BundleEvent, Object)
	 */
	public void removedBundle(TBundle bundle, TBundleEvent event, S object) {
		/* do nothing */
	}
	 ...
	 *         being tracked.
	 */
	public S getObject(TBundle bundle) {
		final Tracked t = tracked();
		if (t == null) { /* if BundleTracker is not open */
			return null;
		}
	 ...
	 */
	public void remove(TBundle bundle) {
		final Tracked t = tracked();
		if (t == null) { /* if BundleTracker is not open */
			return;
		}
	...
	class Tracked extends AbstractTracked<TBundle, TBundleEvent, S> implements
		 ...
		 */
		public void bundleChanged(final TBundleEvent event) {
			/*
			...
			final TBundle bundle = event.getBundle();
		 ...
		 */
		S customizerAdding(final TBundle item, final TBundleEvent related) {
			return customizer
		 ...
		 */
		void customizerModified(final TBundle item, final TBundleEvent related,
				final S object) {
		 ...
		 * @param object Customized object for the tracked item.
		 */
		void customizerRemoved(final TBundle item, final TBundleEvent related,
				final S object) {
