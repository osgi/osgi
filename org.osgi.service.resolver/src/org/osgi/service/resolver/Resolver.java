...
public interface Resolver {
	 ...
	 * resources and wires are included. To get the complete resolution the
	 * caller can merge the start state and the delta using something like the
	 * following:
	 *
	 * <p>
	 * TODO This code example needs fixing
	 *
	 * <pre>
	 * Map&lt;Resource, Wiring&gt; delta = resolver.resolve(env, resources, null);
	 * Map&lt;Resource, Wiring&gt; wiring = env.getWiring();
	 * 
	 * for (Map.Entry&lt;Resource, Wiring&gt; e : delta.entrySet()) {
	 * 	Resource res = e.getKey();
	 * 	Wiring newWiring = e.getValue();
	 * 
	 * 	Wiring currentWires = wiring.get(res);
	 ...
	 * 
	 * @return the new resources and wirings required to satisfy the
	 *         requirements
	 * 
	 ...
	 */
	Map<Resource, Wiring> resolve(Environment environment,
