...
/**
 * Tests Measurement utility class. It tests as well two other classes
 * related to Measurement class that is to say State and Unit.
 *
 * <p>A <tt>Measurement</tt> object is used for maintaining the tuple
 * of value, error, unit and timestamp.
...
public class MeasurementControl extends DefaultTestBundleControl {
    ...
    private final static Unit [] allUnits = new Unit[] {    Unit.m, Unit.s, Unit.kg, Unit.K, Unit.A, Unit.mol, Unit.cd, Unit.rad,
                                                            Unit.m_s, Unit.m_s2, Unit.m2, Unit.m3, Unit.Hz, Unit.N, Unit.Pa, Unit.J,
                                                            Unit.W, Unit.C, Unit.V, Unit.F, Unit.Ohm, Unit.S, Unit.Wb, Unit.T, Unit.lx,
                                                            Unit.Gy, Unit.kat, Unit.unity};
    ...
    static String[] methods = new String[] {
        ...
        "testStateComparison",
        "testMeasurementConstructor",
        "testMeasurementComparison",
        "testMeasurementAddition",
        "testMeasurementSubstraction",
        "testMeasurementMultiplication",
        "testMeasurementDivision",
        "testMeasurementFromString",
        "testUnitFromString"
    };
    ...
    public void testUnitComparison() {
        ...
        assertTrue("Unit.equals() is wrong", !Unit.kg.equals(new State(10, "test")));
    }

    /**
	 * Tests all <tt>State</tt> class constructor by displaying
	 * object contents after creation.
	 */
    public void testStateConstructor() {

        // Construct a State with all possible constructors
        State[] states = {
                new State(1, null),                 // test arg null
                new State(2, "firstState"),
                new State(3, null, 1000),           // test arg null
    ...
    public void testStateComparison() {
        ...
        assertEquals("State.equals() is wrong", refState.hashCode(), identicalState.hashCode());
    ...
    /**
	 * Tests <tt>Measurement</tt> class constructors by displaying
	 * object contents after creation.
	 */
    public void testMeasurementConstructor() {
        ...
        Measurement[] mesures = {
                ...
                new Measurement(101.0, Unit.m),
                new Measurement(102.0, null),                 // test arg null
    ...
    /**
	 * Tests <tt>Measurement</tt> class comparison methods. There are two
	 * ways to compare two <tt>Measurement</tt> objects by using equals() or
	 * compareTo() methods.
	 *
	 * The compareTo() method is using the following rules: it returns a negative
	 * integer, zero, or a positive integer if the this object is less than,
	 * equal to, or grater than the specified object.
	 *
	 * compareTo() returns zero if there is some x such that
    ...
    public void testMeasurementComparison() {
        ...
        assertEquals("Measurement.equals() is wrong", mes1.hashCode(), mes2.hashCode());
        // Equality with the same object
        assertTrue("Measurement.equals() is wrong", mes1.equals(mes1));
        // Mesures differents due to value
        assertTrue("Measurement.equals() is wrong", !mes1.equals(new Measurement(101.0, 3.0, Unit.m, 1002)));
        // Mesures differents due to error
        assertTrue("Measurement.equals() is wrong", !mes1.equals(new Measurement(100.0, 1.0, Unit.m, 1003)));
        // Mesures differents due to unit
        assertTrue("Measurement.equals() is wrong", !mes1.equals(new Measurement(100.0, 3.0, Unit.kg, 1004)));
        ...
        assertTrue("Measurement.compareTo() is wrong", (mes1.compareTo(mes1)) == 0);
        assertTrue("Measurement.compareTo() is wrong", (mes1.compareTo(new Measurement(100.0, 1.0, Unit.m, 1005))) == 0);
        assertTrue("Measurement.compareTo() is wrong", (mes1.compareTo(new Measurement(100.0, 3.0, Unit.m, 1006))) == 0);
        assertTrue("Measurement.compareTo() is wrong", (mes1.compareTo(new Measurement(95.0, 2.0, Unit.m, 1007))) == 0);
        assertTrue("Measurement.compareTo() is wrong", (mes1.compareTo(new Measurement(104.0, 2.0, Unit.m, 1008))) == 0);

        // mes1 is less than the mesure specified as parameter
        assertTrue("Measurement.compareTo() is wrong", (mes1.compareTo(new Measurement(105.0, 1.9, Unit.m, 1009))) < 0);

        // mes1 is greater than the mesure specified as parameter
        assertTrue("Measurement.compareTo() is wrong", (mes1.compareTo(new Measurement(94.0, 2.0, Unit.m, 1010))) > 0);
        ...
        try {
            ...
            assertException("CompareTo an uncompatible unit", ArithmeticException.class, e);
        }
        ...
        try {
            ...
            // Check that we got the correct exception
            assertException("CompareTo a different class", ClassCastException.class, e);
        }
    }
    ...
    public void testMeasurementMultiplication() {
        // Test the use of the three different methods to perform a multiplication
    ...
    /**
	 * Tests the three differents methods to compute <tt>Measurement</tt> object.
	 * The first one has the value (double) as parameter
	 * The second one has the value (double) and the unit (<tt>Unit</tt>) as parameters
	 * The third one has another <tt>Measurement</tt> object as parameter
	 * The fourth method is testing the operation with a 0 value.
	 *
    ...
    private void testMeasurementComputation(String operation) {
        ...
        MethodCall[] methods = {
                ...
                new MethodCall(operation, Measurement.class, new Measurement(100.0, 1.0, Unit.kg, 1000)),
                new MethodCall(operation, Measurement.class, new Measurement(0.0, 1.0, Unit.kg, 1001))
            };
        ...
        for(int i = 0; i < methods.length; i++) {
            try {
                calcMes = (Measurement)methods[i].invoke(new Measurement(100.0, 1.0, Unit.kg, 1002));
            }
            catch(Throwable e) {

                // Check that we got the correct exception
                assertException(methods[i].getName(), ArithmeticException.class, e);
            }
	 ...
	 * operations.
	 *
	 * The ouput is displayed in two differents lists: the list of compatible unit
	 * and the list of incompatible unit. The operation with an incompatible unit
	 * generates an <tt>ArithmeticException</tt>
	 *
	 * @param operation The operation to perform
	 */
    private void testUnitCompatibility(String operation) {
        ...
        for (int i=0; i<allUnits.length; i++) {
            ...
            method = new MethodCall(operation, Measurement.class, new Measurement(100.0, 1.0, allUnits[i], 1000+i));
            ...
            for (int j=0; j<allUnits.length; j++) {
                try {
                    calcMes = (Measurement)method.invoke(new Measurement(100.0, 1.0, allUnits[j], 1000+j));
                    compatibleUnit[k++] = allUnits[j];
                }
                catch(Throwable e) {
                    // Check that we got the correct exception
                    assertException(method.getName() + " " + allUnits[i] + " to " + allUnits[j], ArithmeticException.class, e);
    ...
    /**
	 * Tests error computation while performing one of the four operations available
	 * in <tt>Measurement</tt> class.
	 *
	 ...
	 * Eb is the absolute positive error in a value b. c is a constant double value
	 * without error.
	 *
	 * Calculation  Function            Error
	 * -----------  --------            -----
	 * a x b        mul(Measurement)    |Ea x b| + |a x Eb|
	 * a / b        div(Measurement)    (|Ea x b| + |a x Eb|) / b^2
	 * a + b        add(Measurement)    Ea + Eb
	 * a - b        sub(Measurement)    Ea + Eb
	 * a x c        mul(double)         |Ea x c|
	 * a / c        div(double)         |Ea / c|
	 * a + c        add(double)         Ea
	 * a - c        sub(double)         Ea
	 *
    ...
    private void testErrorComputation(String operation) {
        MethodCall[] methods = {
                new MethodCall(operation, double.class, new Double(-100.0)),
                new MethodCall(operation, Measurement.class, new Measurement(200.0, -1.1, Unit.kg, 1000))
            };
        Measurement calcMes = null;
        ...
        for(int i = 0; i < methods.length; i++) {
            try {
                calcMes = (Measurement)methods[i].invoke(new Measurement(300.0, 1.2, Unit.kg, 1001));
            }
            catch(Throwable e) {
            ...
            log("Error computed by " + methods[i].getName() + " = " + calcMes.getError());
        }
    }
	 ...
	 *
	 * The first part of this method is performing operation recursively
	 * on the same unit in order to test exponent.
	 * The second part is performing operation recursively with all units.
	 *
	 * @param operation The operation to perform
    ...
    private void testUnitComputation(String operation, int exponentLimit) {
        ...
        method = new MethodCall(operation, Measurement.class, new Measurement(100.0, 1.0, Unit.kg, 1000));
        ...
        for (int i=0; i<allUnits.length; i++) {
            try {
                // Log the unit computed
                message = operation + " " + allUnits[i] + " to " + calcMes.getUnit();
                method = new MethodCall(operation, Measurement.class, new Measurement(100.0, 1.0, allUnits[i], 1000));
                calcMes = (Measurement)method.invoke(calcMes);
                message += " = ";
                message += calcMes.getUnit();
                log(message);
	...
	public void testMeasurementFromString() throws Exception {
		assertEquals( "1 kg:0", new Measurement(1,0,Unit.kg), new Measurement("1 kg:0"));
		assertEquals( "1 kg", new Measurement(1,0,Unit.kg), new Measurement("1 kg"));
		assertEquals( "3.5e10 kg:0", new Measurement(3.5e10,0,Unit.kg), new Measurement("3.5e10 kg:0"));
		assertEquals( "3.5e10 kg:3.5e10", new Measurement(3.5e10,0,Unit.kg), new Measurement("3.5e10 kg 0"));
		assertEquals( "3.5e10 kg:3.5e10", new Measurement(3.5e10,3.5e10,Unit.kg), new Measurement("3.5e10 kg:3.5e10"));
		assertEquals( "3.5e10 kg 3.5e10", new Measurement(3.5e10,3.5e10,Unit.kg), new Measurement("3.5e10 kg 3.5e10"));
		assertEquals( "3.5e10 Hz 3.5e10", new Measurement(3.5e10,3.5e10,Unit.Hz), new Measurement("3.5e10 Hz 3.5e10"));
		assertEquals( "3.5e10 1/s 3.5e10", new Measurement(3.5e10,3.5e10,Unit.Hz), new Measurement("3.5e10 1/s 3.5e10"));
	}
	...
	public void testUnitFromString() {
		...
		Measurement m = new Measurement(1,0,Unit.rad).div(new Measurement(1,0,Unit.s));
		...
		try {
			...
			assertException("Exponent 5 is allowed",IllegalArgumentException.class,e);
		}
		try {
