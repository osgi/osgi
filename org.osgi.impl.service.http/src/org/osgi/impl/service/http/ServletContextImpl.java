...
//  ********************     ServletContextImpl     ********************
/**
 ** This class implements the ServletContext interface defined by the servlet API.
 ** There is a 1:1 mapping between HttpContext and this class, i.e. servlets registered
 ** with the same HttpContext will also share ServletContext
 **
...
public final class ServletContextImpl implements ServletContext {
  ...
  int refCnt = 0;
  private final static Vector whenDeprecated = new Vector(0);		// empty vector
  private final Hashtable attributes = new Hashtable();
  
  public ServletContextImpl(HttpServer httpServer, HttpContext httpContext, Log log, AccessControlContext acc) {
  ...
  /**
   ** Return the mime type for a file. First query the servlet's HttpContext for a mime type.
   ** If the HttpContext return null, query HttpServer for a mime-type (which guarantees a 
   ** default mime type of no mime type is set for this file type.
   ** @param file name of the file whose mime type is required
   */
  public String getMimeType(final String file) {
    String mimeType = null;
    try {
      mimeType = (String) AccessController.doPrivileged(new PrivilegedExceptionAction() {
  ...
  /**
   ** The servlet engine denies access to other servlet's ServletContext object.
   ** This is OK according to the Servlet API spec.
   **
  ...
  public RequestDispatcher getRequestDispatcher(String path) {
    ...
    return (aliasMatch != null) ? new RequestDispatcherImpl(aliasMatch) : null;
  }
