...
public abstract class AliasRegistration {
  ...
  public AliasRegistration(HttpServiceImpl serviceImpl, String alias, HttpContext httpContext)
    throws NamespaceException, IllegalArgumentException
  {
    ...
    if (alias.charAt(0) != '/')
      throw new IllegalArgumentException("Alias must begin with slash ('/') - " + alias);

    // and not end with '/'
    if (alias.equals("/"))
      alias = "";
    else if (alias.endsWith("/"))
      throw new IllegalArgumentException("Alias must not end with slash ('/') - " + alias);

    this.alias = alias;
    this.serviceImpl = serviceImpl;
    this.httpContext = httpContext;
    this.acc = AccessController.getContext();
    this.log = new Log("AliasRegistration_bid#" + serviceImpl.getBundle().getBundleId() + " " + alias);
  }
  ...
  public abstract void handleRequest(HttpServletRequest request, HttpResponse response) throws IOException;
