...
//  ********************     HttpServer     ********************
/**
 ** This is the actual HTTP server. The server listens to a
 ** port, typically 80 or 8080 for HTTP requests. When a request is
 ** received an HttpTransaction is created to deal with the request.
 ** <p>
 ** The server extends Thread and the thread method run() hangs on a
 ** call to serverSock.accept() until the server is stoped.
 **
...
{
  private ServerSocket serverSock;      // The server socket we are listening to
  private boolean done = false;         // Flag indicating when to stop
  private Hashtable registrations;      // alias -> AliasRegistration
  private Vector servlets;              // registered servlets
  private Bundle bundle;                // The HttpServers' own bundle object
  private ThreadGroup threadGroup = new ThreadGroup("HttpTransactionManager");
  ...
  public HttpServer(Bundle bundle) {
    ...
    String httpHostname = System.getProperty("org.osgi.service.http.hostname");
    ...
    if (userPort != null) {
      ...
      if (!bindPort(Integer.parseInt(userPort))) {
        log.error("Cannot use port specified by property osr.osgi.service.http.port");
        throw new IllegalArgumentException("Cannot use port specified by property org.osgi.service.http.port [" + userPort + "]");
      }
    }
    else {
      ...
      if (!bindPort(port = Integer.parseInt(HttpConfig.getProperty("port.primary", "80")))) {
        log.warn("Failed to use use server socket primary port, trying secondary port");

        // Try secondary port
        if (!bindPort(port = Integer.parseInt(HttpConfig.getProperty("port.secondary", "8080")))) {
  ...
  public void run() {
    ...
    try {
      while (!done) {
        Socket client = serverSock.accept(); // Listen for incoming requests.
        (new HttpTransaction(threadGroup, this, threadCount++, client)).start();
      }
    }
    catch (IOException e) {
      if (!done) {
        log.error("FATAL - Communication error in HttpServer, exiting", e);
    ...
    throws NamespaceException
  {
    synchronized(registrations) {
      if (registrations.containsKey(reg.alias))
        throw new NamespaceException("Alias already registered: " + reg.alias);
  ...
  void removeBundleRegistrations(Bundle bundle) {
    ...
    int i = 0;
    for (Enumeration enum = registrations.elements(); enum.hasMoreElements(); i++) {
