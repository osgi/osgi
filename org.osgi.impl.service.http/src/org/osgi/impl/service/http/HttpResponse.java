 ...
 ** when they create the HTTP response. E.g a doGet() or doPost() method
 ** in a servlet.
 **
...
public final class HttpResponse implements HttpServletResponse {
  ...
  private ByteArrayOutputStream outBuffer = new ByteArrayOutputStream(1024);
  ...
  HttpResponse(HttpServer httpServer, String transId, DataOutputStream dataOut, HttpRequest request) 
  {
  ...
  /**
   ** Creates the response which is sent back to the agent 
   ** making the request.
  ...
  void createResponse() throws IOException {
    ...
    if (aliasMatch == null) {
      ...
      sendFancyError(HttpServletResponse.SC_NOT_FOUND, 
                     "The requested URI does not exist",
                     "The requested URI does not exist: " + 
                     "<code>" + requestURI + "</code><p>" + 
    ...
    try {
      ...
      Boolean access;
      try {
        access = (Boolean) AccessController.doPrivileged(new PrivilegedExceptionAction() {
          public Object run() throws Exception {
            return new Boolean(aliasMatch.aliasReg.getHttpContext().handleSecurity(request,HttpResponse.this));
          }
  ...
  void sendResponse(String requestMethod) throws IOException {
    log.debug("Sending response: " + PROTO_VERSION + " " + statusCode + " " + statusMsg);
    log.debug("  Content-Type: " + contentType);
    log.debug("  Content-Length: " + outBuffer.size());
    
    dataOut.writeBytes(PROTO_VERSION + " " + statusCode + " " + statusMsg + "\r\n");
    
    dataOut.writeBytes("Date: " + responseDateFormat.format(gregCal.getTime()) + "\r\n");
    dataOut.writeBytes("MIME-Version: 1.0\r\n");
    dataOut.writeBytes("Server: OSGi reference implementation - HTTP Server (Java)\r\n");
    dataOut.writeBytes("Connection: close\r\n"); // Not supporting Keep-Alive
    ...
    if (requestMethod.equalsIgnoreCase("head")) // Clear message body for HEAD requests
      outBuffer.reset();
  ...
  private void sendCookie(Cookie cookie) 
    throws IOException 
  {
    String attrValue;
    int maxAge;
  ...
  void sendFancyError(int code, String hdr, String body, boolean see) {
    ...
    out.println("<html><head><title>Gatespace OSG platform HTTP server</title></head>");
    ...
    out.println(body);
    if (see) 
      out.println("<p><em>See also the HTTP server's log for more information</em><p>");
  ...
  public synchronized ServletOutputStream getOutputStream() throws IOException {
    ...
    return (sos == null) ? sos = new ServletOutputStreamImpl(outBuffer) : sos;
  }
  ...
  public void setDateHeader(String name, long date) {
    SimpleDateFormat s = new SimpleDateFormat("EEE, dd MMM yyyy HH:mm:ss zzz");
  ...
  public String encodeURL (String url) {
    if (request.isRequestedSessionIdValid() && request.isRequestedSessionIdFromURL()) {
      if (url.indexOf(HttpConfig.OSGIREF_SESSION) == -1) {
        url += (url.indexOf('?') == -1) ? "?" : "&";
        url += HttpConfig.OSGIREF_SESSION + "=" + request.getRequestedSessionId();
      }
    }
    return url;
  }
