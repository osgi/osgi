...
public final class ResourceRegistration extends AliasRegistration {
  ...
  public ResourceRegistration(HttpServiceImpl serviceImpl, 
			      String alias, 
			      String name,
    ...
    throws NamespaceException
  {
    ...
    if (name.endsWith("/")) 
      throw new IllegalArgumentException("The name must not end with slash ('/') - " + name);
  ...
  public void handleRequest(HttpServletRequest request, HttpResponse response) throws IOException {
    InputStream is;
    URL url;
    String reqURI = request.getRequestURI();
    if (response.aliasMatch.url == null || (is = response.aliasMatch.url.openStream()) == null) {
      log.error("Strange, cannot get bundle's resource : " + reqURI);
      response.sendFancyError(HttpServletResponse.SC_NOT_FOUND, 
			      ...
			      "to some internal problems.",
			      true);
      return;
    }
    
    String mimeType = getMimeType(reqURI);
    response.setContentType(mimeType != null ? mimeType : serviceImpl.getHttpServer().getMimeType(reqURI));
    ...
    try {
      ...
      return (URL) AccessController.doPrivileged(new PrivilegedExceptionAction() {
        public Object run() throws Exception {
          return httpContext.getResource(name + resource.substring(alias.length()));
        }
      }, acc);
    }
