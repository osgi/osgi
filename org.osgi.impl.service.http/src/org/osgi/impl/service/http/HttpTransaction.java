 ...
 ** an HttpTransaction is created to deal with the request. The task of parsing the
 ** HTTP request and to generete the HTTP response is delegated to the HttpRequest and
...
public final class HttpTransaction extends Thread {
  ...
  HttpTransaction(ThreadGroup parentGroup, HttpServer httpServer, int threadCount, Socket client) {
    ...
    if ("true".equals(HttpConfig.getProperty("http.network.dnslookup", "true")))
  ...
  public void run() {
    ...
    try {
      dataOut = new DataOutputStream(new BufferedOutputStream(client.getOutputStream(), 1024));
      inBuf = new BufferedInputStream(client.getInputStream());
      
      HttpRequest request = new HttpRequest(httpServer, this, inBuf);
      HttpResponse response = new HttpResponse(httpServer, transId, dataOut, request);
	...
	response.sendError(status_code);
      ...
      log.debug("Stopping HttpTransaction: " + getName() + " - client connection timed out");
    }	  
    catch (IOException e) {
