...
public final class HttpRequest implements HttpServletRequest {
  ...
  private HttpResponse response;	// The respone object connected to this request
  ...
  AliasRegistration aliasReg;	// The reg this request resolves to, set in HttpResponse
  String requestURI = null;	// The request URI, can be reset when redirecting


  HttpRequest(HttpServer httpServer, HttpTransaction transaction, BufferedInputStream inBuf)
  {
  ...
  int parseRequest() throws IOException {
    ...
    if ((line = getLine()) == null || line.length() == 0)
      throw new InterruptedIOException("Failed to read from client's input stream");
    ...
    if (requestProtocol.length() == 0)
      requestProtocol = "HTTP/0.9"; // Assume HTTP/0.9 if protocol is missing
    ...
    if (headerValue != null) {
      ...
      while (tok.hasMoreTokens()) {
        ...
        if (tokStr.equals("="))
          isValue = true;
        else if (";,".indexOf(tokStr) != -1)
          isValue = false;
        else {
          if (prevTokStr.equals("$Version") && isValue)
            version = Integer.parseInt(tokStr);
          else if (prevTokStr.equals("$Path") && isValue)
            cookie.setPath(tokStr);
          else if (prevTokStr.equals("$Domain") && isValue)
  ...
  public synchronized ServletInputStream getInputStream() throws IOException {
    ...
    return (sis == null) ? sis = new ServletInputStreamImpl(new ByteArrayInputStream(outBuffer.toByteArray())) : sis;
  }
  ...
  public synchronized BufferedReader getReader() throws IOException {
    ...
    return (reader == null) ?
      reader = new BufferedReader(new InputStreamReader(new ByteArrayInputStream(outBuffer.toByteArray()))) :
      reader;
  }
  ...
  public long getDateHeader(String name) throws IllegalArgumentException {
    ...
    fmt = new SimpleDateFormat("EEE, dd MMM yyyy HH:mm:ss zzz",Locale.ENGLISH);
    try {
    ...
    fmt = new SimpleDateFormat("EEEEEE, dd-MMM-yy HH:mm:ss zzz",Locale.ENGLISH);
    try {
      date = fmt.parse(value);
      return date.getTime();
    }
  ...
  public HttpSession getSession (boolean create) {
    ...
    HttpSession session = httpServer.servletEngine.getHttpSession(sessionId);
  ...
  public boolean isRequestedSessionIdValid () {
    HttpSessionImpl session = httpServer.servletEngine.getHttpSession(sessionId);
    if (session != null)
