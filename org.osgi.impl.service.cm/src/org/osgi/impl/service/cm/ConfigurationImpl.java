...
/**
 * ConfigurationImpl implements a configuration, holds its properties, 
 * manages load and save to persistent storage.
 *
...
public class ConfigurationImpl 
   ...
   * registration with the proper pid, then the value is false, otherwise - true.
   */
  ...
  /**
   * Constructs a ConfigurationImpl object, initially setting its active flag to false.
   *
  ...
  public void delete() throws IOException {
    ...
    synchronized (ServiceAgent.storage) {
      ...
      if (sRef != null) {
        CMEventManager.addEvent(new CMEvent(this, null, CMEvent.DELETED, sRef));
      }
      deleted = true;
    }
  }
  ...
  /**
   * Updates configuration with new properties. Activates 
   * configuration when invoked for the first time.
   *
   * @param   properties  configuration set to associate with
   *
   * @exception   IOException  if storage fails.
   * @exception   IllegalStateException  if configuration already deleted.   
   * @exception   IllegalArgumentException if properties contain wrong data types.
   */
  public void update(Dictionary properties) 
    ...
    if (!isCorrectType(properties)) {
      throw new IllegalArgumentException("Properties contain incorrect type!");
    }
  ...
  /**
   * Two configurations are considered equal when their pids are
   * equal.
   *
   * @param   o  Another configuration object is expected
   * @return     true if the parameter is a Configuration object 
   *             with the same pid; false otherwise
   */
  ...
  /**
   * Processes an update call back to a ManagedService(Factory). If the new properties are 
   * not null, after the automatic properties are added, they are persistently stored.
   * Null properties are not stored. If there is a target ManagedService(Factory) 
   * present in the service registry, the Configuration Plugins are informed 
   * and the target service is updated.
   * 
   * @param   sRef  reference to a ManagedService(Factory). If null, 
   *                the method searches for a proper ManagedService(Factory).
   * 
   * @param   newProperties  properties which will be used for the update. If null, 
   *                         the current configuration properties are used.
   * @exception   IOException  If storage operation fails with I/O error
   */
  protected void callBackManagedService(ServiceReference sRef, Dictionary newProperties) throws IOException {
    synchronized (ServiceAgent.storage) {
      if (sRef == null) {
    ...
    if (sRef != null) {
      //correctness check will not be made as plugins are trusted entities
      //they should not corrupt properties
      CMDictionary pprops = (props != null)? (CMDictionary)props.clone(): null;
      informPlugins("(&" + target + "(&(service.cmRanking=*)(!(service.cmRanking>=0))))", sRef, pprops);
      informPlugins("(&" + target + "(|(!(service.cmRanking=*))(&(service.cmRanking>=0)(service.cmRanking<=1000))))", sRef, pprops);
      informPlugins("(&" + target + "(&(service.cmRanking=*)(!(service.cmRanking<=1000))))", sRef, pprops);
      CMEventManager.addEvent(new CMEvent(this, pprops, CMEvent.UPDATED, sRef));
    }
  }
  ...
  private static boolean isCorrectArray(String className, Object arr) {
    ...
    if (i > 0) {
      /* look for L - specifying object arr - - they look like this: "[[[Ljava.lang.String;" */
    ...
    if (className.equals(Vector.class.getName())) {
      ...
      if (v.length > 0) {
        for (i = 0; i < v.length; i++) {
          if (v[i] != null && !isCorrectVector(v[i])) return false;
        }
      }
      return true;
    }
  ...
  private static boolean isCorrectType(Dictionary props) {
    ...
    while (e.hasMoreElements()) {
      ...
      if (!isCorrectObject(props.get(key))) return false;
    }
    return true;
  }
  ...
  private static boolean isCorrectVector(Vector v) {
    ...
    while (e.hasMoreElements()) {
      if (!isCorrectObject(e.nextElement())) return  false;
    }
    return true;
  }
  ...
  private void informPlugins(String filter, ServiceReference msRef, CMDictionary props) {
    try {
      ServiceReference[] sRefs = 
        ServiceAgent.bc.getServiceReferences(ConfigurationPlugin.class.getName(), filter);

      if (sRefs == null) return;
      if (sRefs.length > 1) {
        sRefs = sort(sRefs);
      }
      ...
      for (int i = 0; i < sRefs.length; i++) {
        try {
          plugin = (ConfigurationPlugin) ServiceAgent.bc.getService(sRefs[i]);          
          plugin.modifyConfiguration(msRef, props);
          ServiceAgent.bc.ungetService(sRefs[i]);
        } catch (Throwable th) {
          Log.log(1, "[CM]A ConfigurationPlugin failed to modify a configuration!", th);
        }
      }  
    } catch (InvalidSyntaxException ise) {
      Log.log(1, "[CM]Unexpected error, while calling ConfigurationPlugins!", ise);
    }    
  }
  ...
  private ServiceReference[] sort(ServiceReference[] sRefs) {
    ...
    for (int i=0; i<sRefs.length; i++) {
      ...
      if (result.isEmpty()) {
        ...
        minRank = ranking;
        maxRank = ranking;
      } else if (ranking <= minRank) {
        result.insertElementAt(sRefs[i], 0);
        minRank = ranking;
      } else if (ranking >= maxRank) {
        result.addElement(sRefs[i]);
        maxRank = ranking;
      } else {
        for (int j=1; j<i; j++) {
          if (ranking <= getRanking((ServiceReference) result.elementAt(j))) {
  ...
  private int getRanking(ServiceReference sRef) {
    try {
      ...
      if (temp != null) return temp.intValue();
    } catch (ClassCastException cce) {      
    }
    return 0;
  }
