...
/**
 * The class represents a friendly wrapper of DataBase class,
 * providing methods for getting, storing, deleting configurations.
 *
...
public class ConfigurationStorage extends DataBase {
  ...
  /**
   * Stores the config in database. The object is serialized and passed 
   * to DataBase to be persistently stored.
   *
   * @param   config  object to store.
   * @param   create  indicates if config is newly created.
   * @exception   IOException  if an I/O error occurs.
   */
  protected void storeConfig(ConfigurationImpl config, boolean create) throws IOException{
    if (create) {
      if (config.fPid != null) {
        putFactoryConfig(config);
      }
  ...
  private ConfigurationImpl makeConfig(byte[] configBytes) {
    ...
    try {    
      ...
      Log.log(1, "[CM]Error while trying to read Configuration object.", sce);
    } catch (IOException ioe) {
      Log.log(1, "[CM]Error while trying to read Configuration object.", ioe);
    } catch (ClassNotFoundException cnfe) {
      Log.log(1, "[CM]Error while trying to read Configuration object.", cnfe);
    } finally {
      try {
        barr.close();
      } catch (IOException ioe) {
