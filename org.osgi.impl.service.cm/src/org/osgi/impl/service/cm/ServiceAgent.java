...
/**
 * The class acts as an agent between CM and framework registry.
 * It listens for service events from ManagedServices, ManagedServiceFactories. 
...
public class ServiceAgent 
  ...
  /**
   * Inits variables, checks for already available ManagedServices, 
   * and ManagedServiceFactories. Those which have corresponding 
   * Configuration objects are associated to them. 
   *
  ...
  /**
   * When a bundle is uninstalled, the configurations that are bound to
   * must be either unbound or deleted.
   */
        ...
        while (e.hasMoreElements()) {
          ...
          if (location.equals(c.location)) {
            try {
              ...
              Log.log(1, "[CM]Error while deleting or unbinding a configuration with pid " + c.pid  + ".", ioe);
            }
          }
        }
      }
  ...
  /**
   * Invoked by the framework, when a service state changes. 
   * When a ManagedService or ManagedServiceFactory registers, 
   * available Configuration objects are inspected for pid match - if 
   * the corresponding Configuration is found - it is assigned its ManagedService or 
   * ManagedServiceFactory. When a ManagedService(Factory) unregisters - 
   * the corresponding Configuration is informed.
   * 
  ...
  public void serviceChanged(ServiceEvent sEv) {
    if (sEv.getType() == sEv.REGISTERED) {
      ...
      String[] oClasses = (String[])sRef.getProperty(Constants.OBJECTCLASS);
      for (int i=0; i<oClasses.length; i++) {
        if (oClasses[i].equals(ManagedService.class.getName())) {
          processService(sRef, true);
        } else if (oClasses[i].equals(ManagedServiceFactory.class.getName())) {
  ...
  /**
   * Searches for a ManagedService(Factory), which has the same service.pid as the 
   * configuration's pid(factory pid). If the configuration already has been bound to 
   * a bundle location, then the ManagedService(Factory) must be registered by a bundle
   * with the same location. Otherwise - if the configuration has not been bound to a 
   * location, yet, then it is given the ManagedService(Factory)'s bundle's location.
   *
   * @param   config  a Configuration
   * @param   toStore  if the configuration is assigned a bundle location, should it be stored
   * @return  target ManagedService(Factory)'s service reference.
   */
  protected static ServiceReference searchForService(ConfigurationImpl config, boolean toStore) {
    ServiceReference[] sRefs = null;
    try {
      if (config.fPid == null) {
        sRefs = 
          bc.getServiceReferences(ManagedService.class.getName(), "("+Constants.SERVICE_PID+"="+config.pid+")");
      } else {
        sRefs = 
          bc.getServiceReferences(ManagedServiceFactory.class.getName(), "("+Constants.SERVICE_PID +"="+config.fPid+")");
      }
    } catch (InvalidSyntaxException ise) {
      /* syntax is valid*/
    }
    if (sRefs == null) return null;
  ...
  private static boolean checkLocation(ConfigurationImpl config, Bundle bundle, boolean storeLocation) {
    final Bundle b = bundle;
    String locationToCheck = (String) AccessController.doPrivileged(new PrivilegedAction(){
      public Object run() {
        return b.getLocation();
      }
    });
    if (config.location == null) {
      ...
      return true;
    } else if (locationToCheck.equals(config.location)) {
      return true;
    } else {
      if (config.fPid != null) {
        Log.log(1, "[CM]Bundle with location: " + locationToCheck + " is trying to forge pid: " + config.fPid);
      } else {
        Log.log(1, "[CM]Bundle with location: " + locationToCheck + " is trying to forge pid: " + config.pid);
      }
      return false;
    }
  }
  ...
  private void informManagedServices(boolean isMS) throws InvalidSyntaxException {
    ServiceReference[] sRefs = null;
    if (isMS) {
      sRefs = bc.getServiceReferences(ManagedService.class.getName(), null);
    } else {
      sRefs = bc.getServiceReferences(ManagedServiceFactory.class.getName(), null);
    }
    if (sRefs != null) {
      for (int i=0; i< sRefs.length;i++) {
  ...
  private void processService(ServiceReference sRef, boolean isMS) {
    ...
    if (isMS) {
      ConfigurationImpl config = 
        (ConfigurationImpl) storage.configs.get(pid);
      if (config != null && 
          checkLocation(config, sRef.getBundle(), true)) {
        try {    
        ...
        CMEventManager.addEvent(new CMEvent(null, null, CMEvent.UPDATED, sRef));
      } 
    } else {
