...
/**
 * BundleActivator class - it is addressed by the framework, 
 * when the CM bundle needs to be started or stopped.
 * 
 * @author Open Services Gateway Initiative
 * @version $Revision$
 */
public class CMActivator 
  implements BundleActivator, 
             ServiceFactory {
  
  /**
   * Utility reference to Log class, 
   * necessary for communication with Log Service.
   */
  protected Log log;  
   ...
   * 
   * All players in Config Management are created and started, 
   * this including: CM Storage, CMEventManager, ServiceAgent.
   *
   * @param   bc  context of this bundle, allowing it to access 
   *              the framework.
   * @exception   BundleException  If storage fails to load, or any  
  ...
  public void start(BundleContext bc) throws BundleException {
    ...
    try {
      ...
      cmReg = bc.registerService(ConfigurationAdmin.class.getName(), this, null);
    } catch (Exception e) {
  ...
  /**
   * Method is called by the framework, when the bundle should be stopped,
   * so naturally, the resources got by CM, here are released: 
   * storage is closed, listeners removed, event manager informed to stop,
   * CM service unregistered.
   *
  ...
  /**
   * Returns a ConfigurationAdminImpl object, created for each requesting bundle.
   * 
  ...
  /**
   * Does nothing, as the CM bundle does not keep references to ConfigurationAdminImpl objects.
   *
   * @param   callerBundle  bundle requesting to unget ConfigurationAdmin service.
