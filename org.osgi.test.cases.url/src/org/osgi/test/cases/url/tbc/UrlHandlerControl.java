...
public class UrlHandlerControl extends DefaultTestBundleControl {
	...
	public void prepare(){
		...
		urlServiceProperty1.put(URLConstants.URL_HANDLER_PROTOCOL,new String[]{protocol1,protocol2}); 
		urlServiceProperty1.put(Constants.SERVICE_RANKING,new Integer(10)); 
		
		urlServiceProperty2=new Hashtable();
		urlServiceProperty2.put(URLConstants.URL_HANDLER_PROTOCOL,new String[]{protocol1}); 
		...
		contentServiceProperty1.put(URLConstants.URL_CONTENT_MIMETYPE ,new String[]{"osgi/test"}); 
		contentServiceProperty1.put(Constants.SERVICE_RANKING,new Integer(10)); 
		
		contentServiceProperty2=new Hashtable();
		contentServiceProperty2.put(URLConstants.URL_CONTENT_MIMETYPE ,new String[]{"osgi/test"}); 
	...
	public void testAccessingUrlWhenHandlerRegistered()throws Exception{
		...
		//register services
		urlHandlerSerReg1=getContext().registerService(URLStreamHandlerService.class.getName(),urlHandler1,urlServiceProperty1);
		urlHandlerSerReg2=getContext().registerService(URLStreamHandlerService.class.getName(),urlHandler2,urlServiceProperty2);
		
		//test protocol1
		URL url=new URL(protocol1+"://test");
		URLConnection con=url.openConnection();
		URLConnection reqd=urlHandler1.getConnectionObject();
		assertEquals("Opening connection after registering handler for "+protocol1,reqd,con);
		
		//test protocol2
		url=new URL(protocol2+"://test");
		con=url.openConnection();
		reqd=urlHandler1.getConnectionObject();
		assertEquals("Opening connection after registering handler for "+protocol2,reqd,con);
	...
	public void testContentHandlerAfterRegistering()throws Exception{
		...
		//register services
		contentHandlerSerReg1=getContext().registerService(ContentHandler.class.getName(),contentHandler1,contentServiceProperty1);
		contentHandlerSerReg2=getContext().registerService(ContentHandler.class.getName(),contentHandler2,contentServiceProperty2);
	...
	//******************************************************
	/**
	 * Test whether the content handler returns the "inputstream" when ther
	 * is no assosiated content handler present
	 */
	...
	public void testUrlHandlerWhenHandlerIsUnregistered()throws Exception{
		...
		assertException("Expecting a Malformed Url Exception",required,exThrown);
		...
		assertException("Expecting a IllegalState Exception",required,exThrown);
	}
	//******************************************************
