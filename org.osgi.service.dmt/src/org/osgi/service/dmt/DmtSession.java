 ...
 * <p>
 * Each method of {@code DmtSession} that accesses the tree in any way can throw
 * {@code DmtIllegalStateException} if the session has been closed or
...
public interface DmtSession {
	/**
	 * Sessions created with {@code LOCK_TYPE_SHARED} lock allows read-only
	 * access to the tree, but can be shared between multiple readers.
	 */
	int LOCK_TYPE_SHARED = 0;

	/**
	 * {@code LOCK_TYPE_EXCLUSIVE} lock guarantees full access to the tree, but
	...
	/**
	 * Get the root URI associated with this session. Gives "{@code .}" if the
	 ...
	 *             <ul>
	 *             <li>{@code METADATA_MISMATCH} if the operation failed because
	 *             of meta-data restrictions <li>{@code CONCURRENT_ACCESS} if it
	 *             is detected that some modification has been made outside the
	 *             scope of the DMT to the nodes affected in the session's
	 *             operations <li> {@code TRANSACTION_ERROR} if an error
	 *             occurred during the commit of any of the underlying plugins
	 *             <li>{@code DATA_STORE_FAILURE} if an error occurred while
	 *             accessing the data store <li> {@code COMMAND_FAILED} if some
	 *             unspecified error is encountered while attempting to complete
	 *             the command
	 *             </ul>
	 ...
	 *             execute the underlying management operation
	 */
	 ...
	 * 
	 * @throws DmtException
	 *             with the error code {@code ROLLBACK_FAILED} in case the
	 *             rollback did not succeed
	 * @throws DmtIllegalStateException
	 *             if the session was not opened using the
	 *             {@code LOCK_TYPE_ATOMIC} lock type, or if the session is
	 *             already closed or invalidated
	 ...
	 * {@code DmtSession} must first persist the changes made to the DMT by
	 * calling {@code commit()} on all (transactional) plugins participating in
	 * the session. See the documentation of the {@link #commit()} method for
	 * details and possible errors during this operation.
	 * <p>
	 * The state of the session changes to {@code DmtSession.STATE_CLOSED} if
	 ...
	 *             with the following possible error codes:
	 *             <ul>
	 *             <li>{@code METADATA_MISMATCH} in case of atomic sessions, if
	 *             the commit operation failed because of meta-data restrictions
	 *             <li> {@code CONCURRENT_ACCESS} in case of atomic sessions, if
	 *             the commit operation failed because of some modification
	 *             outside the scope of the DMT to the nodes affected in the
	 *             session <li> {@code TRANSACTION_ERROR} in case of atomic
	 *             sessions, if an underlying plugin failed to commit <li>
	 *             {@code DATA_STORE_FAILURE} if an error occurred while
	 *             accessing the data store <li> {@code COMMAND_FAILED} if an
	 *             underlying plugin failed to close, or if some unspecified
	 ...
	 * @param data
	 *            the parameter of the execute operation, can be {@code null}
	 * @throws DmtException
	 *             with the following possible error codes:
	 *             <ul>
	 *             <li>
	 ...
	 *             {@code DATA_STORE_FAILURE} if an error occurred while
	 *             accessing the data store <li>{@code COMMAND_FAILED} if the
	 *             URI is not within the current session's subtree, if no
	 *             DmtExecPlugin is associated with the node and the DmtAdmin
	 *             can not execute the node, or if some unspecified error is
	 *             encountered while attempting to complete the command
	 *             </ul>
	 * @throws DmtIllegalStateException
	 *             if the session was opened using the {@code LOCK_TYPE_SHARED}
	 *             lock type, or if the session is already closed or invalidated
	 * @throws SecurityException
	 *             if the caller does not have the necessary permissions to
	 *             execute the underlying management operation, or, in case of
	 *             local sessions, if the caller does not have
	 ...
	 *            notifications sent in response to it, can be {@code null} if
	 *            not needed
	 * @param data
	 *            the parameter of the execute operation, can be {@code null}
	 * @throws DmtException
	 ...
	 *             accessing the data store <li>{@code COMMAND_FAILED} if the
	 *             URI is not within the current session's subtree, if no
	 *             DmtExecPlugin is associated with the node, or if some
	 *             unspecified error is encountered while attempting to complete
	 *             the command
	 *             </ul>
	 * @throws DmtIllegalStateException
	 ...
	 * {@code Acl} object does not take inheritance into account, it gives the
	 * ACL specifically given to the node.
	 * 
	 * @param nodeUri
	 *            the URI of the node
	 * @return the Access Control List belonging to the node or {@code null} if
	 *         none defined
	 * @throws DmtException
	 *             with the following possible error codes:
	 *             <ul>
	 ...
	 *             <li>{@code COMMAND_FAILED} if the URI is not within the
	 ...
	 *             does not allow the {@code Replace} operation for the
	 *             associated principal <li>{@code COMMAND_NOT_ALLOWED} if the
	 *             command attempts to set the ACL of the root node not to
	 ...
	 * @throws SecurityException
	 *             in case of local sessions, if the caller does not have
	 *             {@code DmtPermission} for the node or its parent (see above)
	 ...
	 * @param recursive
	 *            {@code false} if only a single node is copied, {@code true} if
	 ...
	 *             {@link #getMetaNode(String)}) <li>{@code NODE_ALREADY_EXISTS}
	 *             if {@code newNodeUri} points to a node that already exists
	 *             <li>{@code PERMISSION_DENIED} if the session is associated
	 *             with a principal and the ACL of the copied node(s) does not
	 *             allow the {@code Get} operation, or the ACL of the parent of
	 *             the target node does not allow the {@code Add} operation for
	 *             the associated principal <li>{@code COMMAND_NOT_ALLOWED} if
	 *             {@code nodeUri} is an ancestor of {@code newNodeUri}, or if
	 *             any of the implied retrieval or update operations are not
	 *             allowed <li>{@code METADATA_MISMATCH} if any of the meta-data
	 *             constraints of the implied retrieval or update operations are
	 *             violated <li>{@code TRANSACTION_ERROR} in an atomic session
	 *             if the underlying plugin is read-only or does not support
	 *             atomic writing <li>{@code DATA_STORE_FAILURE} if an error
	 *             occurred while accessing the data store <li>
	 *             {@code COMMAND_FAILED} if either URI is not within the
	 *             current session's subtree, or if some unspecified error is
	 *             encountered while attempting to complete the command
	 *             </ul>
	 * @throws DmtIllegalStateException
	 ...
	 *             action present, or for the parent of the target node with the
	 *             Add action
	 */
	 ...
	 * creating it. The node must have {@code MetaNode.CMD_ADD} access type, it
	 ...
	 *             {@code PERMISSION_DENIED} if the session is associated with a
	 *             principal and the ACL of the parent node does not allow the
	 *             {@code Add} operation for the associated principal <li>
	 *             {@code COMMAND_NOT_ALLOWED} if the parent node is not an
	 *             interior node, or in non-atomic sessions if the underlying
	 *             plugin is read-only or does not support non-atomic writing
	 *             <li>{@code METADATA_MISMATCH} if the node could not be
	 *             created because of meta-data restrictions (see above) <li>
	 *             {@code TRANSACTION_ERROR} in an atomic session if the
	 *             underlying plugin is read-only or does not support atomic
	 *             writing <li>{@code DATA_STORE_FAILURE} if an error occurred
	 *             while accessing the data store <li>{@code COMMAND_FAILED} if
	 *             the URI is not within the current session's subtree, or if
	 ...
	 *             execute the underlying management operation, or, in case of
	 *             local sessions, if the caller does not have
	 *             {@code DmtPermission} for the parent node with the Add action
	 *             present
	 */
	 ...
	 *            the URI of the node to create
	 * @param type
	 *            the type URI of the interior node, can be {@code null} if no
	 ...
	 *             writing <li>{@code DATA_STORE_FAILURE} if an error occurred
	 *             while accessing the data store <li>{@code COMMAND_FAILED} if
	 *             the URI is not within the current session's subtree, if the
	 *             type string is invalid (see above), or if some unspecified
	 *             error is encountered while attempting to complete the command
	 *             </ul>
	 * @throws DmtIllegalStateException
	 *             if the session was opened using the {@code LOCK_TYPE_SHARED}
	 ...
	 * {@code DmtException} with error code {@code METADATA_MISMATCH}. Note that
	 ...
	 * specified value is {@code null}, the default value is taken. If the node
	 * does not have a default MIME type or value (if needed), this method will
	 * throw a {@code DmtException} with error code {@code METADATA_MISMATCH}.
	 ...
	 *            the URI of the node to create
	 * @param value
	 *            the value to be given to the new node, can be {@code null}
	 * @throws DmtException
	 *             with the following possible error codes:
	 *             <ul>
	 *             <li>
	 ...
	 * value or MIME type is {@code null}, their default values are taken. If
	 ...
	 *            the value to be given to the new node, can be {@code null}
	 * @param mimeType
	 *            the MIME type to be given to the new node, can be {@code null}
	 * @throws DmtException
	 *             with the following possible error codes:
	 *             <ul>
	 *             <li>
	 ...
	 *             writing <li>{@code DATA_STORE_FAILURE} if an error occurred
	 *             while accessing the data store <li>{@code COMMAND_FAILED} if
	 *             the URI is not within the current session's subtree, if
	 *             {@code mimeType} is not a proper MIME type string (see
	 ...
	 * {@code MetaNode.CMD_DELETE} access type, and if zero occurrences of the
	 ...
	 *             {@code Delete} operation for the associated principal <li>
	 *             {@code COMMAND_NOT_ALLOWED} if the target node is the root of
	 *             the session, or in non-atomic sessions if the underlying
	 *             plugin is read-only or does not support non-atomic writing
	 *             <li>{@code METADATA_MISMATCH} if the node could not be
	 *             deleted because of meta-data restrictions (see above) <li>
	 *             {@code TRANSACTION_ERROR} in an atomic session if the
	 *             underlying plugin is read-only or does not support atomic
	 *             writing <li>{@code DATA_STORE_FAILURE} if an error occurred
	 *             while accessing the data store <li>{@code COMMAND_FAILED} if
	 ...
	 * the {@code MetaNode.CMD_REPLACE} access type, and the name of the new
	 ...
	 *             {@code NODE_ALREADY_EXISTS} if there already exists a sibling
	 *             of {@code nodeUri} with the name {@code newName} <li>
	 *             {@code PERMISSION_DENIED} if the session is associated with a
	 *             principal and the ACL of the node does not allow the
	 ...
	 *             plugin is read-only or does not support non-atomic writing
	 *             <li>{@code METADATA_MISMATCH} if the node could not be
	 *             renamed because of meta-data restrictions (see above) <li>
	 *             {@code TRANSACTION_ERROR} in an atomic session if the
	 *             underlying plugin is read-only or does not support atomic
	 *             writing <li>{@code DATA_STORE_FAILURE} if an error occurred
	 *             while accessing the data store <li>{@code COMMAND_FAILED} if
	 ...
	 * be defined by the node's {@code MetaNode}. The method throws a
	 * {@code METADATA_MISMATCH} exception if the node does not have a default
	 * value.
	 * 
	 * @param nodeUri
	 *            the URI of the node
	 ...
	 *             writing
	 *             <li>{@code METADATA_MISMATCH} if the node is permanent or
	 *             cannot be modified according to the meta-data (does not have
	 *             the {@code MetaNode.CMD_REPLACE} access type), or if there is
	 ...
	 *             {@code DmtPermission} for the node with the Replace action
	 *             present
	 * @see #setNodeValue(String, DmtData)
	 */
	 ...
	 * must be {@code FORMAT_NODE}, while for leaf nodes this format must not be
	 * used.
	 * <p>
	 * If the specified value is {@code null}, the default value is taken. In
	 * this case, if the node does not have a default value, this method will
	 * throw a {@code DmtException} with error code {@code METADATA_MISMATCH}.
	 * Nodes of {@code null} format can be set by using
	 * {@link DmtData#NULL_VALUE} as second argument.
	 * <p>
	 ...
	 * structure of the subtree is not modified by the operation.
	 * 
	 * @param nodeUri
	 *            the URI of the node
	 * @param data
	 *            the data to be set, can be {@code null}
	 * @throws DmtException
	 *             with the following possible error codes:
	 *             <ul>
	 *             {@code INVALID_URI} if {@code nodeUri} is {@code null} or
	 ...
	 *             vice versa), or in non-atomic sessions if the underlying
	 *             plugin is read-only or does not support non-atomic writing
	 *             <li>{@code METADATA_MISMATCH} if the node is permanent or
	 *             cannot be modified according to the meta-data (does not have
	 *             the {@code MetaNode.CMD_REPLACE} access type), or if the
	 *             given value does not conform to the meta-data value
	 *             constraints <li>{@code FEATURE_NOT_SUPPORTED} if the
	 *             specified node is an interior node and does not support Java
	 *             object values <li>{@code TRANSACTION_ERROR} in an atomic
	 *             session if the underlying plugin is read-only or does not
	 *             support atomic writing <li>{@code DATA_STORE_FAILURE} if an
	 *             error occurred while accessing the data store <li>
	 *             {@code COMMAND_FAILED} if the URI is not within the current
	 *             session's subtree, or if some unspecified error is
	 *             encountered while attempting to complete the command
	 *             </ul>
	 * @throws DmtIllegalStateException
	 *             if the session was opened using the {@code LOCK_TYPE_SHARED}
	 ...
	 *            the URI of the node
	 * @param title
	 *            the title text of the node, can be {@code null}
	 * @throws DmtException
	 *             with the following possible error codes:
	 *             <ul>
	 *             <li>{@code INVALID_URI} if {@code nodeUri} is {@code null} or
	 ...
	 *             <li>{@code FEATURE_NOT_SUPPORTED} if the Title property is
	 *             not supported by the DmtAdmin implementation or the
	 *             underlying plugin
	 *             <li>{@code TRANSACTION_ERROR} in an atomic session if the
	 *             underlying plugin is read-only or does not support atomic
	 *             writing
	 *             <li>{@code DATA_STORE_FAILURE} if an error occurred while
	 *             accessing the data store
	 *             <li>{@code COMMAND_FAILED} if the title string is too long,
	 ...
	 * document.
	 * <p>
	 * For interior nodes, a {@code null} type string means that there is no DDF
	 ...
	 *            the URI of the node
	 * @param type
	 *            the type of the node, can be {@code null}
	 * @throws DmtException
	 *             with the following possible error codes:
	 *             <ul>
	 *             <li>
	 ...
	 *             principal and the ACL of the node does not allow the
	 *             {@code Replace} operation for the associated principal <li>
	 *             {@code COMMAND_NOT_ALLOWED} in non-atomic sessions if the
	 *             underlying plugin is read-only or does not support non-atomic
	 *             writing <li>{@code METADATA_MISMATCH} if the node is
	 *             permanent or cannot be modified according to the meta-data
	 *             (does not have the {@code MetaNode.CMD_REPLACE} access type),
	 *             and in case of leaf nodes, if {@code null} is given and there
	 *             is no default MIME type, or the given MIME type is not
	 *             allowed <li>{@code TRANSACTION_ERROR} in an atomic session if
	 *             the underlying plugin is read-only or does not support atomic
	 *             writing <li>{@code DATA_STORE_FAILURE} if an error occurred
	 *             while accessing the data store <li>{@code COMMAND_FAILED} if
	 *             the URI is not within the current session's subtree, if the
	 *             type string is invalid (see above), or if some unspecified
	 ...
	 *             with a principal and the ACL of the node does not allow the
	 *             {@code Get} operation for the associated principal
	 *             <li>{@code COMMAND_NOT_ALLOWED} if the specified node is not
	 *             an interior node
	 *             <li>{@code METADATA_MISMATCH} if node information cannot be
	 ...
	 * <p>
	 * The {@code MetaNode} object returned to the client is the combination of
	 ...
	 * are not defined, it may throw {@code DmtException} with the error code
	 * {@code NODE_NOT_FOUND}. To allow easier implementation of plugins that do
	 * not provide meta-data, it is allowed to return {@code null} for any node,
	 ...
	 *             is associated with a principal and the ACL of the node does
	 *             not allow the {@code Get} operation for the associated
	 *             principal <li>{@code DATA_STORE_FAILURE} if an error occurred
	 *             while accessing the data store <li>{@code COMMAND_FAILED} if
	 *             the URI is not within the current session's subtree, or if
	 *             some unspecified error is encountered while attempting to
	 *             complete the command
	 ...
	 *             supported by the DmtAdmin implementation or the underlying
	 *             plugin
	 *             <li>{@code DATA_STORE_FAILURE} if an error occurred while
	 *             accessing the data store
	 *             <li>{@code COMMAND_FAILED} if the URI is not within the
	 ...
	 *             retrieved according to the meta-data (it does not have
	 *             {@code MetaNode.CMD_GET} access type)
	 *             <li>{@code FEATURE_NOT_SUPPORTED} if the Timestamp property
	 ...
	 *             non-existing node
	 *             <li>{@code PERMISSION_DENIED} if the session is associated
	 *             with a principal and the ACL of the node (and the ACLs of all
	 *             its descendants in case of interior nodes) do not allow the
	 *             {@code Get} operation for the associated principal
	 ...
	 *             retrieved according to the meta-data (it does not have
	 *             {@code MetaNode.CMD_GET} access type)
	 *             <li>{@code FEATURE_NOT_SUPPORTED} if the Version property is
	 *             not supported by the DmtAdmin implementation or the
	 *             underlying plugin
	 *             <li>{@code DATA_STORE_FAILURE} if an error occurred while
	 *             accessing the data store
