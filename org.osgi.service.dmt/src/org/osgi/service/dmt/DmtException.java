...
public class DmtException extends Exception {
     ...
     * return this error code if the Dmt Admin implementation does not support
     ...
     * segment number limits are implementation depenedent, their minimum
     ...
     * specific error situations for this code, but it can generally be used
     * for any unexpected condition that causes the command to fail.
     * <p>
    ...
    /**
     * The rollback command was not completed successfully.  The tree might be
     * in an inconsistent state after this error.
     * <p>
     ...
     * </ul>
     * <p>
     * See the {@link DmtAdmin#mangle(String)} method for support on escaping
     * invalid characters in a URI.
     * <p>
     ...
     * This can happen if there is no Remote Alert Sender willing to
     * forward the alert to the given principal, or if no principal was 
     * given and the Dmt Admin did not find an appropriate default destination.
     * <p>
     * This error code does not correspond to any OMA DM response status code.
     * It should be translated to the code 500 &quot;Command Failed&quot; when 
     * transferring over OMA DM.
     ...
     * length of time while the Dmt Admin waits for the blocking session(s) to
    ...
    private static String getCodeText(int code) {
        ...
        case SESSION_CREATION_TIMEOUT: return "SESSION_CREATION_TIMEOUT";
        default:
            return "<unknown code>";
        }
    }
