...
package org.osgi.test.cases.syncbundlelistener.tb1;
...
*/
public class PermissionTester implements BundleActivator, SynchronousBundleListener
{
			...
			if (_linkRef != null) {
				...
				if (_link != null) {
					...
					try {
						...
						if (sm == null) {
							_link.log("Expected behaviour when registering sync listener without admin permission.","OK");
						} else {
							_link.log("Were able to add a synchrounous listener without admin permission","FAIL");
						}
					} catch (SecurityException e) {
						_link.log("Expected behaviour when registering sync listener without admin permission.","OK");
					} catch(Exception e2) {
						_link.log("Got unexected exception in permissionTester","FAIL");
						e2.printStackTrace(); 
					}
				} else
