...
package org.osgi.test.cases.syncbundlelistener.tb2;
		...
		try
		{
			...
			if (_linkRef!=null)
			{
				...
				if (_link!=null)
				{
					...
					{
						_link.log("Got exception when registering multiple bunldelisteners","FAIL");;
					}
				}	
				else
        ...
        try
        {
			...
			else
			{
				if (fe.getType()==BundleEvent.INSTALLED)
					_link.log("BundleListener caught it's own install event","FAIL");
				//It's ok for a bundle to catch it's own events except for the install event				
			}
		}
       catch(Exception e) 
		{ e.printStackTrace(); }
