...
package org.osgi.test.cases.syncbundlelistener.tbc;
	...
	BundleActivator
{
	...
	*/
	...
	/**
		control that a security exception is thrown if we try
		to add a synchronous bundle listener without adminPermission
	*/
	...
	void testPermissions()
	{
		try
		{
			Bundle permissionBundle=_context.installBundle(_tcHome + "tb1.jar");
	...
	*/
	synchronized void  testSyncBundleListener()
		throws Exception
	{
		try
		{		
	 ...
	 /**
	   Tests that synchronous bundles are called before normalBundleListener.
		We can't test this in testSyncBundleListener because different framework
		behavior generate different outputs.
	*/
	synchronized void  testListenerOrder()
		throws Exception
	{
		try
		{
		// Install another bundle with an asynchronous listener			
		Bundle aListenerBundle=_context.installBundle(_tcHome + "tb4.jar");
		...
		try
		{
			if (fe.getBundle()!=_context.getBundle())
			{
				...
				case BundleEvent.INSTALLED:
					log("Bundle is being installed.","waiting 3 seconds in SyncBundleListener.");
					wait(3000);
			   	 	_link.log("SyncBundleListener: INSTALLED has waited 3 seconds.");
					break;
				
				case BundleEvent.STARTED:
					log("Bundle is being started.","waiting 3 seconds in SyncBundleListener.");
					wait(3000);
					_link.log("SyncBundleListener: STARTED has waited 3 seconds.");
				    break;
				
				case BundleEvent.STOPPED:
					log("Bundle is being stopped.","waiting 3 seconds in SyncBundleListener.");
					wait(3000);
					_link.log("SyncBundleLiastener: STOPPED has waited 3 seconds.");
			   	 break;
				
				case BundleEvent.UNINSTALLED:
				    log("Bundle is being uninstalled.","waiting 3 seconds in SyncBundleListener.");
					wait(3000);
					_link.log("SyncBundleListener: UNINSTALL has waited 3 seconds");
			   		 break;
				
				case BundleEvent.UPDATED:
					log("Bundle is being updated.","waiting 3 seconds in SyncBundleListener.");
					wait(3000);
					_link.log("SyncBundleListener: UPDATED has waited 3 seconds.");
					break;									
				}
			}
			else
