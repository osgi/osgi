...
public final class TBundlePermission extends BasicPermission {
	 ...
	 */
	public TBundlePermission(String symbolicName, String actions) {
	 ...
	 */
	TBundlePermission(String symbolicName, int mask) {
		super(symbolicName);
		setTransients(mask);
	}
	...
	public boolean implies(Permission p) {
		if (!(p instanceof TBundlePermission)) {
			return false;
		}
		TBundlePermission requested = (TBundlePermission) p;
	...
	public PermissionCollection newPermissionCollection() {
		return new TBundlePermissionCollection();
	}
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof TBundlePermission)) {
			return false;
		}

		TBundlePermission bp = (TBundlePermission) obj;
...
final class TBundlePermissionCollection extends PermissionCollection {
	 ...
	 * 
	 */
	public TBundlePermissionCollection() {
	...
	public void add(final Permission permission) {
		if (!(permission instanceof TBundlePermission)) {
		...
		final TBundlePermission bp = (TBundlePermission) permission;
		final String name = bp.getName();
		synchronized (this) {
			Map pc = permissions;
			TBundlePermission existing = (TBundlePermission) pc.get(name);
			if (existing != null) {
				final int oldMask = existing.getActionsMask();
				final int newMask = bp.getActionsMask();
				if (oldMask != newMask) {
					pc.put(name, new TBundlePermission(name, oldMask
							| newMask));
	...
	public boolean implies(final Permission permission) {
		...
		TBundlePermission requested = (TBundlePermission) permission;
		String requestedName = requested.getName();
		final int desired = requested.getActionsMask();
		int effective = TBundlePermission.ACTION_NONE;
		TBundlePermission bp;

		synchronized (this) {
			Map pc = permissions;
			/* short circuit if the "*" Permission was added */
			if (all_allowed) {
				bp = (TBundlePermission) pc.get("*");
				if (bp != null) {
			...
			bp = (TBundlePermission) pc.get(requestedName);
			// strategy:
