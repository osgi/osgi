...
public final class TPackagePermission extends BasicPermission {
	 ...
	 */
	transient final TBundle					bundle;
	 ...
	 */
	public TPackagePermission(String name, String actions) {
	 ...
	 * @since 1.5
	 */
	public TPackagePermission(String name, TBundle exportingBundle, String actions) {
		super(name);
	 ...
	 */
	TPackagePermission(String name, int mask) {
		super(name);
	...
	public boolean implies(Permission p) {
		if (!(p instanceof TPackagePermission)) {
			return false;
		}
		TPackagePermission requested = (TPackagePermission) p;
		if (bundle != null) {
			return false;
		}
	 ...
	 *         object; <code>false</code> otherwise.
	 */
	boolean implies0(TPackagePermission requested, int effective) {
	...
	public PermissionCollection newPermissionCollection() {
		return new TPackagePermissionCollection();
	}
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof TPackagePermission)) {
			return false;
		}

		TPackagePermission pp = (TPackagePermission) obj;
...
final class TPackagePermissionCollection extends PermissionCollection {
	 ...
	 */
	public TPackagePermissionCollection() {
	...
	public void add(final Permission permission) {
		if (!(permission instanceof TPackagePermission)) {
		...
		final TPackagePermission pp = (TPackagePermission) permission;
		if (pp.bundle != null) {
		...
		synchronized (this) {
			...
			final TPackagePermission existing = (TPackagePermission) pc.get(name);
			if (existing != null) {
				final int oldMask = existing.action_mask;
				final int newMask = pp.action_mask;
				if (oldMask != newMask) {
					pc
							.put(name, new TPackagePermission(name, oldMask
									| newMask));
	...
	public boolean implies(final Permission permission) {
		...
		final TPackagePermission requested = (TPackagePermission) permission;
		/* if requested permission has a filter, then it is an invalid argument */
		if (requested.filter != null) {
			return false;
		}
		...
		final int desired = requested.action_mask;
		int effective = TPackagePermission.ACTION_NONE;

		Collection perms;
		synchronized (this) {
			Map pc = permissions;
			TPackagePermission pp;
			/* short circuit if the "*" Permission was added */
			if (all_allowed) {
				pp = (TPackagePermission) pc.get("*");
				if (pp != null) {
			 ...
			 */
			pp = (TPackagePermission) pc.get(requestedName);
			if (pp != null) {
		...
		for (Iterator iter = perms.iterator(); iter.hasNext();) {
			if (((TPackagePermission) iter.next())
