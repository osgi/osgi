...
public final class TAdminPermission extends BasicPermission {
	 ...
	 */
	public TAdminPermission(String filter, String actions) {
	 ...
	 * @since 1.3
	 */
	public TAdminPermission(TBundle bundle, String actions) {
	 ...
	 */
	private static String createName(TBundle bundle) {
		if (bundle == null) {
	 ...
	 */
	TAdminPermission(TFilter filter, int mask) {
	 ...
	 * @param mask action mask
	 */
	private void setTransients(TFilter filter, int mask) {
		this.filter = filter;
	...
	public boolean implies(Permission p) {
		if (!(p instanceof TAdminPermission)) {
			return false;
		}
		TAdminPermission requested = (TAdminPermission) p;
		if (bundle != null) {
			return false;
		}
	 ...
	 *         object; <code>false</code> otherwise.
	 */
	boolean implies0(TAdminPermission requested, int effective) {
	...
	public PermissionCollection newPermissionCollection() {
		return new TAdminPermissionCollection();
	}
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof TAdminPermission)) {
			return false;
		}

		TAdminPermission ap = (TAdminPermission) obj;
 ...
 */
final class TAdminPermissionCollection extends PermissionCollection {
	 ...
	 * 
	 */
	public TAdminPermissionCollection() {
	...
	public void add(Permission permission) {
		if (!(permission instanceof TAdminPermission)) {
			throw new IllegalArgumentException("invalid permission: "
					+ permission);
		}
		if (isReadOnly()) {
		...
		final TAdminPermission ap = (TAdminPermission) permission;
		if (ap.bundle != null) {
		...
		synchronized (this) {
			Map pc = permissions;
			TAdminPermission existing = (TAdminPermission) pc.get(name);
			if (existing != null) {
				int oldMask = existing.action_mask;
				int newMask = ap.action_mask;

				if (oldMask != newMask) {
					pc.put(name, new TAdminPermission(existing.filter, oldMask
							| newMask));
				}
			}
			else {
	...
	public boolean implies(Permission permission) {
		...
		TAdminPermission requested = (TAdminPermission) permission;
		// if requested permission has a filter, then it is an invalid argument
		if (requested.filter != null) {
			return false;
		}
		int effective = TAdminPermission.ACTION_NONE;
		Collection perms;
		synchronized (this) {
			Map pc = permissions;
			// short circuit if the "*" Permission was added
			if (all_allowed) {
				TAdminPermission ap = (TAdminPermission) pc.get("*");
				if (ap != null) {
		...
		for (Iterator iter = perms.iterator(); iter.hasNext();) {
			if (((TAdminPermission) iter.next()).implies0(requested, effective)) {
				return true;
			}
		}
		return false;
