 ...
 * @version $Revision$
 */
public class TFrameworkUtil {
	/**
	 * FrameworkUtil objects may not be constructed.
	 */
	private TFrameworkUtil() {
	 ...
	 * by {@link TBundleContext#createFilter(String)}.
	 * 
	 ...
	 * @see TFilter
	 */
	public static TFilter createFilter(String filter)
	 ...
	 * @since 1.5
	 */
	public static TBundle getBundle(final Class classFromBundle) {
		...
		if (cl instanceof TBundleReference) {
			return ((TBundleReference) cl).getBundle();
		}
		return null;
	}
	 ...
	 */
	private static class FilterImpl implements TFilter {
		/* filter operators */
		 ...
		 * used to match a {@link TServiceReference} or a Dictionary.
		 * 
		 * <p>
		 * If the filter cannot be parsed, an {@link TInvalidSyntaxException}
		 * will be thrown with a human readable message where the filter became
		 * unparsable.
		 * 
		 * @param filterString the filter string.
		 * @exception TInvalidSyntaxException If the filter parameter contains an
		...
		private static class Parser {
			...
			FilterImpl parse() throws TInvalidSyntaxException {
				FilterImpl filter;
				try {
					filter = parse_filter();
				}
				catch (ArrayIndexOutOfBoundsException e) {
					throw new TInvalidSyntaxException("Filter ended abruptly",
							filterstring);
				}

				if (pos != filterChars.length) {
					throw new TInvalidSyntaxException(
			...
			private FilterImpl parse_filter() throws TInvalidSyntaxException {
				FilterImpl filter;
				skipWhiteSpace();

				if (filterChars[pos] != '(') {
					throw new TInvalidSyntaxException("Missing '(': "
							+ filterstring.substring(pos), filterstring);
				}
			...
			private FilterImpl parse_filtercomp() throws TInvalidSyntaxException {
				skipWhiteSpace();
			...
			private FilterImpl parse_and() throws TInvalidSyntaxException {
				skipWhiteSpace();
			...
			private FilterImpl parse_or() throws TInvalidSyntaxException {
				skipWhiteSpace();
			...
			private FilterImpl parse_not() throws TInvalidSyntaxException {
				skipWhiteSpace();
			...
			private FilterImpl parse_item() throws TInvalidSyntaxException {
				...
				throw new TInvalidSyntaxException("Invalid operator: "
						+ filterstring.substring(pos), filterstring);
			}

			private String parse_attr() throws TInvalidSyntaxException {
				skipWhiteSpace();
				...
				if (length == 0) {
					throw new TInvalidSyntaxException("Missing attr: "
							+ filterstring.substring(pos), filterstring);
				}
			...
			private String parse_value() throws TInvalidSyntaxException {
				...
				parseloop: while (true) {
					...
					switch (c) {
						...
						case '(' : {
							throw new TInvalidSyntaxException("Invalid value: "
									+ filterstring.substring(pos), filterstring);
						}
				...
				if (sb.length() == 0) {
					throw new TInvalidSyntaxException("Missing value: "
							+ filterstring.substring(pos), filterstring);
				}
			...
			private Object parse_substring() throws TInvalidSyntaxException {
				StringBuffer sb = new StringBuffer(filterChars.length - pos);
	...
	private static class ServiceReferenceDictionary extends Dictionary {
		private final TServiceReference	reference;

		ServiceReferenceDictionary(TServiceReference reference) {
