...
public final class TServicePermission extends BasicPermission {
	 ...
	 */
	transient final TServiceReference		service;
	 ...
	 */
	public TServicePermission(String name, String actions) {
	 ...
	 * @since 1.5
	 */
	public TServicePermission(TServiceReference reference, String actions) {
		super(createName(reference));
		setTransients(null, parseActions(actions));
		this.service = reference;
		this.objectClass = (String[]) reference
				.getProperty(TConstants.OBJECTCLASS);
	 ...
	 */
	private static String createName(TServiceReference reference) {
		if (reference == null) {
			throw new IllegalArgumentException("reference must not be null");
		}
		StringBuffer sb = new StringBuffer("(service.id=");
		sb.append(reference.getProperty(TConstants.SERVICE_ID));
		sb.append(")");
		return sb.toString();
	}
	 ...
	 */
	TServicePermission(String name, int mask) {
		super(name);
	 ...
	 * @param mask action mask
	 */
	private void setTransients(TFilter f, int mask) {
	 ...
	 */
	private static TFilter parseFilter(String filterString) {
		...
		try {
			return TFrameworkUtil.createFilter(filterString);
		}
	...
	public boolean implies(Permission p) {
		if (!(p instanceof TServicePermission)) {
			return false;
		}
		TServicePermission requested = (TServicePermission) p;
		if (service != null) {
			return false;
		}
	 ...
	 *         object; <code>false</code> otherwise.
	 */
	boolean implies0(TServicePermission requested, int effective) {
	...
	public PermissionCollection newPermissionCollection() {
		return new TServicePermissionCollection();
	}
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof TServicePermission)) {
			return false;
		}

		TServicePermission sp = (TServicePermission) obj;
	...
	private Dictionary getProperties() {
		...
		if (service == null) {
			...
			if (filter == null) {
				result.put(TConstants.OBJECTCLASS, new String[] {getName()});
			}
			return properties = result;
		}
		final Map props = new HashMap(4);
		final TBundle bundle = service.getBundle();
		if (bundle != null) {
			AccessController.doPrivileged(new PrivilegedAction() {
				public Object run() {
					...
					TSignerProperty signer = new TSignerProperty(bundle);
	...
	private static class Properties extends Dictionary {
		...
		private final TServiceReference	service;

		Properties(Map properties, TServiceReference service) {
 ...
 */
final class TServicePermissionCollection extends PermissionCollection {
	 ...
	 */
	public TServicePermissionCollection() {
	...
	public void add(final Permission permission) {
		if (!(permission instanceof TServicePermission)) {
			throw new IllegalArgumentException("invalid permission: "
					+ permission);
		}
		if (isReadOnly()) {
		...
		final TServicePermission sp = (TServicePermission) permission;
		if (sp.service != null) {
		...
		synchronized (this) {
			...
			final TServicePermission existing = (TServicePermission) pc.get(name);
			...
			if (existing != null) {
				...
				if (oldMask != newMask) {
					pc
							.put(name, new TServicePermission(name, oldMask
							| newMask));
				}
			}
			else {
	...
	public boolean implies(final Permission permission) {
		...
		final TServicePermission requested = (TServicePermission) permission;
		/* if requested permission has a filter, then it is an invalid argument */
		if (requested.filter != null) {
			return false;
		}

		int effective = TServicePermission.ACTION_NONE;
		Collection perms;
		synchronized (this) {
			final int desired = requested.action_mask;
			/* short circuit if the "*" Permission was added */
			if (all_allowed) {
				TServicePermission sp = (TServicePermission) permissions.get("*");
				if (sp != null) {
		...
		for (Iterator iter = perms.iterator(); iter.hasNext();) {
			if (((TServicePermission) iter.next())
	...
	private int effective(String requestedName, final int desired,
		...
		TServicePermission sp = (TServicePermission) pc.get(requestedName);
		// strategy:
		...
		while ((last = requestedName.lastIndexOf(".", offset)) != -1) {
			...
			sp = (TServicePermission) pc.get(requestedName);
			if (sp != null) {
				effective |= sp.action_mask;
				if ((effective & desired) == desired) {
					return effective;
