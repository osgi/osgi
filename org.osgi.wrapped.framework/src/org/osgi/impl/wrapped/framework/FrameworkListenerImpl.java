...
public class FrameworkListenerImpl implements FrameworkListener {
	final TFrameworkListener	listener;
	FrameworkListenerImpl(TFrameworkListener listener) {
		this.listener = listener;
	}

	public void frameworkEvent(FrameworkEvent event) {
		listener.frameworkEvent(new TFrameworkEvent(event.getType(),
				new TBundleImpl(event.getBundle()), event.getThrowable()));
	}
