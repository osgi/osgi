...
public interface PushStream<T> extends Closeable {
	 ...
	 */
	PushStream<T> fork(int n, int delay, Executor e) throws Exception;

	/**
	 * Buffer the events in a queue using default values for the queue size
	 * and other behaviours. Buffered work will be processed asynchronously
	 * in the rest of the chain. Buffering also blocks the transmission of
	 * back pressure to previous elements in the chain, although back pressure 
	 * is honoured by the buffer. 
	 * 
	 * <p>
	 * Buffers are useful for "bursty" event sources which produce a 
	 * number of events close together, then none for some time. These
	 * bursts can sometimes overwhelm downstream event consumers. Buffering
	 * will not, however, protect downstream components from a source
	 * which produces events faster than they can be consumed. For
	 * fast sources {@link #filter(Predicate)} and {@link #coalesce(int, Function)}
	 ...
	 *            an executor to use for the background threads.
	 */
	PushStream<T> buffer() throws Exception;
	...
	<U extends BlockingQueue<PushEvent<? extends T>>> PushStream<T> buffer(int parallelism, Executor executor, 
			U queue, QueuePolicy<T, U> queuePolicy, PushbackPolicy<T,U> pushbackPolicy) throws Exception;
	 ...
	 * @throws Exception
	 */
	PushStream<? extends T> merge(PushEventSource<? extends T> source) throws Exception;
	 ...
	 * 
	 * <p>
	 * This method differs from other methods of AsyncStream in three significant
	 * ways:
	 * 
	 * <ul>
	 *   <li>The return value contains multiple streams.</li>
	 *   <li>This stream will only close when all of these child streams have closed. </li>
	 *   <li>Event delivery is made to all open children that accept the event.</li>
	 * </ul>
	 * 
	 * @param predicates
	...
	PushStream<T>[] split(Predicate<? super T>... predicates) throws Exception;
	 ...
	 * @return
	 */
	PushStream<T> sequential() throws Exception;
	 ...
	 * @throws Exception
	 */
	<R> PushStream<R> coalesce(Function<? super T, Optional<R>> f) throws Exception;

	/**
	 * Coalesces a number of events into a new type of event. A fixed number
	 * of input events are forwarded to a accumulator function. This function 
	 ...
	 * @throws Exception
	 */
	public <R> PushStream<R> coalesce(int count, Function<Collection<T>, R> f) throws Exception;
	 ...
	 * The accumulator function then returns new event data to be forwarded on.
	 * 
	 * @param count
	 * @param f
	 * @return
	 * @throws Exception
	 */
	public <R> PushStream<R> coalesce(IntSupplier count, Function<Collection<T>, R> f) throws Exception;
	 ...
	 * Note that:
	 * <ul>
	 *     <li>The collection forwarded to the accumulator function will be empty
	 *     if no events arrived during the time interval.</li>
	 *     <li>The accumulator function will be run and the forwarded event delivered
	 *     as a different task, (and therefore potentially on a different thread) 
	 *     from the one that delivered the event to this {@link PushStream}.</li>
	 ...
	 * @throws Exception
	 */
	<R> PushStream<R> window(long time, TimeUnit unit, Function<Collection<T>, R> f) throws Exception;
	 ...
	 *     if no events arrived during the time interval.</li>
	 *     <li>The accumulator function will be run and the forwarded event delivered
	 *     by a task given to the supplied executor.</li>
	 *     <li>Due to the buffering and asynchronous delivery required, this method
	 *     prevents the propagation of back-pressure to earlier stages</li>
	 * </ul>
	 * @param f
	 ...
	 * @throws Exception
	 */
	<R> PushStream<R> window(long time, TimeUnit unit, Executor executor, Function<Collection<T>, R> f) throws Exception;
	
	/**
	 * Buffers a number of events over a variable time interval and then forwards
	 * the events to an accumulator function. The length of time over which events
	 * are buffered is determined by the time function. A maximum number of events 
	 * can also be requested, if this number of events is reached then the accumulator
	 * will be called early. The accumulator function returns new event 
	 * data to be forwarded on. It is also given the length of time for which the 
	 * buffer accumulated data. This may be less than the requested interval if
	 * the buffer reached the maximum number of requested events early.
	 * 
	 * Note that:
	 * <ul>
	 *     <li>The collection forwarded to the accumulator function will be empty
	 ...
	 * @throws Exception
	 */
	<R> PushStream<R> window(LongSupplier time, IntSupplier maxEvents, BiFunction<Long, Collection<T>, R> f) throws Exception;
	 ...
	 * @throws Exception
	 */
	<R> PushStream<R> window(LongSupplier time, IntSupplier maxEvents, Executor executor, BiFunction<Long, Collection<T>, R> f) throws Exception;
	 ...
	 * @throws Exception
	 */
	Promise<Void> forEach(Consumer<? super T> action) throws Exception;
	 ...
	 * @param accumulator
	 * @param combiner
	 *            combines to U's into one U (e.g. how combine two lists)
	 * @return The promise
	 */
	 ...
	 * @return a promise
	 */
	Promise<Long> forEachEvent(PushEventConsumer<? super T> action) throws Exception;
