...
/**
 * @methodUnderTest org.osgi.service.application.ScheduledApplication#getArguments
...
public class GetArguments implements TestInterface {
	...
	/**
	 * @testID testGetArguments001
	 * @testDescription Asserts if the arguments is correctly returned when an
	...
	public void testGetArguments001() {
		...
		try {
			...
			Map megProp = tbc.getMeg2Properties();

			sa = tbc.getAppDescriptor2().schedule(megProp, "*", null, false);

			tbc
					.assertEquals(
							"Asserts if the arguments is correctly returned when an application is scheduled",
							megProp, sa.getArguments());
	...
	public void testGetArguments002() {
		...
		try {
			...
			tbc.assertNull(MessagesConstants.getMessage(
					...
					new String[] { "event filter" }), sa.getArguments());
