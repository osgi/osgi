...
/**
 * A simple server that responds to RSH clients.  It has a hard coded
 * shared secret.  It will listen to requests on port 1111.
 *
...
public class RshServer extends Thread {
    ...
    /**
     * Encode the plainText returning the cipher text.  Note that
     * the macVal will get updated with the calculated MAC.
     */
    byte []encodeStream(byte plainText[], byte rshSecret[], byte clientFG[], byte serverFG[], byte macVal[]) {
        byte m1[], m2[], ka[];
    ...
    /** The thread that accepts connections.  Before processing a
     * connection, it will start up another RshServer to processes the
     * next connection. */
    public void run() {
        ...
        try {
            ...
            byte cipherText[] = encodeStream(plainText, rshSecret, clientFG, serverFG, mac);
    ...
    int fromHex(char c) {
        if (c >= '0' && c <= '9')
            ...
            c -= 'A' - 0xa;
	else throw new RuntimeException(c + " is not a hex char");
	return c;
    }

    /** This decodes the URLencoded request.  It is here since URLDecoder
     * didn't appear until JDK 1.2 */
    String urlDecode(String string) {
        ...
        for (int i = 0; i < string.length(); i++) {
            ...
            if (c == '%') {
                sb
                    .append(
                        (char) ((fromHex(string.charAt(++i)) << 4)
                            + fromHex(string.charAt(++i))));
            } else {
                sb.append(c);
            }
        }
    ...
    Hashtable parseArgs(String string) {
        ...
        do {
            ...
            if (equalIndex != -1) {
                table.put(
                    urlDecode(pair.substring(0, equalIndex)),
    ...
    byte[] base64Decode(byte string[]) {
        ...
        for(int i = 0; i < string.length; i+= 4) {
            ...
            if (string[i+2] != '=') baos.write((byte)((t>>8)&0xff));
            if (string[i+3] != '=') baos.write((byte)((t)&0xff));
        }
