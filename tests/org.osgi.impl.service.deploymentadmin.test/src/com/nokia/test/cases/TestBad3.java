...
public class TestBad3 extends TestCaseClass {

    protected TestBad3(TestRunnerImpl tRunner) throws Exception {
        super(tRunner);
    }
    ...
    public void doTest(Db db, DeploymentAdmin da) throws Exception {
		...
		try {
			is = new FileInputStream(tRunner.getFile("bad_db_test_03.dp"));
			try {
    ...
    public PermissionInfo[] getNeededPermissions() {
        return new PermissionInfo[] {
                new PermissionInfo(FilePermission.class.getName(), 
                        tRunner.getFile("bad_db_test_03.dp").getAbsolutePath(), "read"),
                new PermissionInfo(DeploymentAdminPermission.class.getName(), 
                        "(name=bad_db_test_03)", "install, uninstall")
            };
    }
