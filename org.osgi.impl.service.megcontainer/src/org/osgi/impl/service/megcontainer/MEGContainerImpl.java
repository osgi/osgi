...
public class MEGContainerImpl implements MEGContainer, BundleListener,
	...
	public ApplicationDescriptor[] installApplication(InputStream inputStream)
		...
		throw new Exception(
				"Installing from stream is not allowed for MEG applications!");
	}
	...
	public ApplicationDescriptor[] uninstallApplication(
		...
		long bundleId = Long.parseLong((String) (appDescriptor
				.getProperties("").get("bundle_id")));
	...
	public ApplicationDescriptor[] upgradeApplication(
		...
		throw new Exception(
				"Upgrading from stream is not allowed for MEG applications!");
	}
	...
	public Application createApplication(ApplicationContext appContext,
		...
		long bundleID = Long.parseLong((String) (appDesc.getProperties("")
				.get("bundle_id")));
		...
		if (desc == null)
			throw new Exception(
					"Application doesn't installed onto the MEG container!");
		int i;
		...
		MEGApplicationContext megAppContext = new MEGApplicationContextImpl(bc,
		...
		MEGApplication app = (MEGApplication) constructor
				.newInstance(new Object[] {megAppContext});
		...
		if (bundleIDs.contains(bundleStr)) {
			switch (event.getType()) {
				...
				case BundleEvent.UNINSTALLED :
					...
					if (desc != null) {
						for (int i = 0; i != desc.serviceRegistrations.length; i++)
							uninstalledAppUIDs.remove(desc.applications[i]
									.getUniqueID());
						saveVector(uninstalledAppUIDs, "UninstalledAppUIDs");
					}
			...
			while (enum.hasMoreElements()) {
				...
				if (application.getTagName().equals("application")) {
					...
					while (childrenEnum.hasMoreElements()) {
						XMLElement propertyElement = (XMLElement) (childrenEnum
								.nextElement());
						if (propertyElement.getTagName().equals("singleton"))
							props.setProperty("singleton", propertyElement
									.getProperty("value"));
						if (propertyElement.getTagName().equals("version"))
							props.setProperty("version", propertyElement
									.getProperty("value"));
						if (propertyElement.getTagName().equals("autostart"))
							props.setProperty("autostart", propertyElement
									.getProperty("value"));
						if (propertyElement.getTagName().equals("category"))
							props.setProperty("category", propertyElement
									.getProperty("value"));
						if (propertyElement.getTagName().equals("class"))
							startClass = propertyElement.getProperty("value");
						if (propertyElement.getTagName().equals(
								"required_services")
								|| propertyElement.getTagName().equals(
										"required_physical_resource")) {
							String services = propertyElement
									.getProperty("value");
							int ndx = 0;
							while (ndx != -1) {
						...
						if (propertyElement.getTagName().equals(
								"required_logical_resource")) {
							String packages = propertyElement
									.getProperty("value");
							int ndx = 0;
							while (ndx != -1) {
						...
						if (propertyElement.getTagName().equals("subscribe")) {
							String topicString = propertyElement
									.getProperty("event");
							String actionString = propertyElement
									.getProperty("action");
							...
							if (actionString.equals("start"))
								...
								if (actionString.equals("stop"))
									action = EventSubscribe.STOP;
								else
									if (actionString.equals("suspend"))
										action = EventSubscribe.SUSPEND;
									else
										if (actionString.equals("resume"))
											action = EventSubscribe.RESUME;
										else
											if (actionString.equals("listener"))
						...
						if (propertyElement.getTagName().equals("locale")) {
							...
							Enumeration childNodes = propertyElement
									.enumerateChildren();
							while (childNodes.hasMoreElements()) {
								XMLElement childNode = (XMLElement) (childNodes
										.nextElement());
								if (childNode.getTagName().equals("name"))
									names.put(lang, childNode
											.getProperty("value"));
								if (childNode.getTagName().equals("icon")) {
									String sizeX = childNode
											.getProperty("sizex");
									String sizeY = childNode
											.getProperty("sizey");
