...
public class UpdateDispatcher extends Thread {
  ...
  public void run() {
    ...
    while (active) {
      ...
      if (((Boolean) update[2]).booleanValue()) {
        try {
          ((Producer) update[0]).consumersConnected((Wire[]) update[1]);
        } catch (Throwable t) {
          parent.bCast(WireAdminEvent.PRODUCER_EXCEPTION, (Wire)update[3], t);
        }
      } else {
        try {
          ((Consumer) update[0]).producersConnected((Wire[]) update[1]);
        } catch (Throwable t) {
          parent.bCast(WireAdminEvent.CONSUMER_EXCEPTION, (Wire)update[3], t);
        }
      }
    }
  }
  ...
  private void addUpdate(Object receiver, Wire source, Wire[] wires, boolean producer) {
    synchronized (updates) {
      updates.addElement(new Object[]{receiver, wires, new Boolean(producer), source});
    }
    if (!isAlive()) {
      start();
    }
