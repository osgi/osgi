...
public class WireImpl implements Wire, WireConstants {
   ...
   * <tt>Consumer</tt> service or <tt>null</tt> if
   ...
   * this methods returns the last value provided by the <tt>Producer</tt> service.
   *
        ...
        if (isCorrectType(result)) {
          ...
          if (result instanceof Envelope[]) {
            ...
            for (int i = 0; i < envs.length; ++i) {
              if (envs[i].getScope() != null && this.hasScope(envs[i].getScope())) {
                list.add(envs[i]);
              } else {
                changed = true;
              }
  ...
  public void update(Object value) {
    ...
    if (isCorrectType(value.getClass())) {
      if (filter != null) {
        ...
        Hashtable myProps = new Hashtable(value instanceof Number ? 5 : 3);
        myProps.put(WireConstants.WIREVALUE_CURRENT, value);
        myProps.put(WireConstants.WIREVALUE_PREVIOUS, lastValue);
        myProps.put(WireConstants.WIREVALUE_ELAPSED,
                    new Long(timeStamp == -1 ? 0 : System.currentTimeMillis() - timeStamp));
        ...
        if (value instanceof Number && lastValue instanceof Number) {
          ...
          Double relDelta = new Double((lastNumber - currentNumber) / currentNumber);
          myProps.put(WireConstants.WIREVALUE_DELTA_RELATIVE, relDelta);
          if (!filter.match(myProps)) {
            parent.getLogService().log(LogService.LOG_DEBUG, "filter match on update failed");
            return;
          }
        }
      }
  ...
  private boolean isCorrectType(Object object) {
    ...
    if (flavors == null) {
      // every consumer should specify flavors, the error has been logged, but should be corrected
      return true;
    }
...
// Envelope?
  ...
  public void delete() {
    if (getProducer()!=null) {
      ...
      parent.getUpdateDispatcher().addProducerUpdate(getProducer(), this, newWire);
    }
    if (getConsumer()!=null) {
      ...
      parent.getUpdateDispatcher().addConsumerUpdate(getConsumer(), this, null);
    }
    ...
    context = null;
    connected = false;
    parent.getLogService().log(LogService.LOG_DEBUG, "wire with PID " + getPID() + " deleted");
  }
  ...
  public void setProperties(Dictionary properties) {
    ...
    String filterString = (String) properties.get(WireConstants.WIREADMIN_FILTER);
    if (filterString != null) {
      try {
        filter = context.createFilter(filterString);
      } catch (InvalidSyntaxException ivsx) {
        filter = null;
        parent.getLogService().log(LogService.LOG_WARNING, "invalid filter in wire properties", ivsx);
      }
    } else {
      filter = null;
    }
    parent.getUpdateDispatcher().addProducerUpdate(getProducer(), this, parent.getProducerWires(producerPID));
    parent.getUpdateDispatcher().addConsumerUpdate(getConsumer(), this, parent.getConsumerWires(consumerPID));
  }
  ...
  private void checkConnection() {
    if (connected) {
      if (producerReference == null || consumerReference == null) {
        ...
        Producer producer = getProducer();
        if (producer != null) {
          parent.getUpdateDispatcher().addProducerUpdate(producer, this, parent.getProducerWires(producerPID));
        } else {
          parent.getUpdateDispatcher().addProducerUpdate(producer, this, null);
          Consumer consumer = getConsumer();
          if (consumer != null) {
            parent.getUpdateDispatcher().addConsumerUpdate(consumer, this, parent.getConsumerWires(consumerPID));
          }
        }
      }
    } else {
  ...
  private void setFlavors(ServiceReference consumerReference) {
    ...
    try {
      Class[] newFlavors = (Class[])
          consumerReference.getProperty(WireConstants.WIREADMIN_CONSUMER_FLAVORS);
      if (newFlavors != null) {
        flavors = newFlavors;
        return;
      }
    ...
    flavors = null;
    parent.getLogService().log(LogService.LOG_ERROR, "consumer registered w/o flavors");
  }
  ...
  public void setProducer(ServiceReference producer) {
    ...
    if (producerReference != null &&
        producerReference.getProperty(WireConstants.WIREADMIN_PRODUCER_FILTERS) != null) {
  ...
  void setScope() {
    ...
    Vector a = filterOnPermission((String[]) pr.getProperty(WIREADMIN_PRODUCER_SCOPE), WirePermission.PRODUCE, pr.getBundle());
    Vector b = filterOnPermission((String[]) cr.getProperty(WIREADMIN_CONSUMER_SCOPE), WirePermission.CONSUME, cr.getBundle());
    if (a == null || b == null)
      return;
