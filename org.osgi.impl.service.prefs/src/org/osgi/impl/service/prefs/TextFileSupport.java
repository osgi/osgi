...
class TextFileSupport {
	...
	try {
	    ...
	    br.close();
            ...
            System.err.println(
                "Couldn't read preferences from: " + preferencesFile);
	    e.printStackTrace();
        }
    }
	...
	throws BackingStoreException
    {
	    ...
	    exportPreferences(osw, prefs);
	        ...
	        "Couldn't write preferences to: " + preferencesFile +
		", " + e);
        }
    }
    ...
    /**
     * Export the specified preferences node and all
     ...
     *         backing store.
     */
    private static void exportPreferences(OutputStreamWriter osw, Preferences prefs)
	...
	try {
		...
		output(osw, keys[i]);
		osw.write('=');
		output(osw, prefs.get(keys[i],null)); // default value shouldn't get used
		osw.write('\n');
	    }
     ...
     *         results in an <tt>IOException</tt>.
     * @throws InvalidPreferencesFormatException if the input Properties
     *         file has an invalid key.
     */
    ...
    /**
     * Read a key, terminated by an unescaped '=' character.  Unescape
     * the result.
     */
    private static String readKey(int nextChar, Reader br) 
	throws IOException
    {
