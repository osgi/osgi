...
public abstract class AbstractPreferences implements Preferences {
     ...
     * has not been removed, and invokes {@link #putSpi(String,String)}.
     *
     ...
     * invocation returns <tt>null</tt> in which case this invocation
     * returns <tt>def</tt>.
     *
     ...
     *
     * <p>This implementation obtains this preference node's lock,
     * checks that the node has not been removed, and invokes
     * {@link #removeSpi(String)}.
     *
     ...
     *
     * <p>This implementation translates <tt>value</tt> to a string with
     * {@link Long#toString(int)} and invokes {@link #put(String,String)}
     * on the result.
     *
     * @param key key with which the string form of value is to be associated.
     * @param value value whose string form is to be associated with key.
     ...
     *         <tt>key</tt> in this preference node, or <tt>null</tt> if the
     *         associated value does not exist or cannot be interpreted as
     *         a boolean.
     * @throws IllegalStateException if this node (or an ancestor) has been
     *         removed with the {@link #removeNode()} method.
     ...
     * {@link Float#valueOf(String)}.  If the attempt succeeds, the return
     * value is returned by this method.  Otherwise, <tt>def</tt> is returned.
     *
     ...
     * {@link Double#valueOf(String)}.  If the attempt succeeds, the return
     * value is returned by this method.  Otherwise, <tt>def</tt> is returned.
     *
    ...
    /**
     * Implements the <tt>childrenNames</tt> method as per the specification in
     * {@link Preferences#childrenNames()}.
     *
     * <p>This implementation obtains this preference node's lock, checks that
     * the node has not been removed and invokes {@link #childrenNamesSpi()}.
     *
     ...
     * root is returned.  Otherwise, this breaks <tt>path</tt> into tokens and
     ...
     * #childSpi(String)} method is invoked.  When there are no more tokens,
     * the value returned by <tt>child</tt> is returned by this method.  If
     * during the traversal, two <tt>"/"</tt> tokens occur consecutively, or
     * the final token is <tt>"/"</tt> (rather than a name>, an appropriate
     ...
     * and calls a recursive helper method that traverses the subtree rooted
     * at this node.  The recursive call locks the node on which it was called
     * and calls itself on each of its children in turn.  The it removes the
     * node using {@link #removeSpi()} and marks the node as removed.
     ...
     * <tt>flush</tt> method is invoked on an ancestor of this node.
     */
     ...
     * It is guaranteed that <tt>child</tt> is non-null, non-empty
     * and does not contain the slash character ('/').  Also, it is guaranteed
     ...
     * guaranteed that this node has not been removed.  (The implementor
     * needn't check for any of these things.)
     *
    ...
    /**
     * Returns <tt>true</tt> if this node (or an ancestor) has been
     * removed with the {@link #removeNode()} method, otherwise false. 
     */
