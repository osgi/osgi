...
public class WireAPITestProducerImpl implements Producer {
	...
	public Object polled(Wire wire) {
		if ("42".equals(wire.getProperties().get("org.osgi.test.wireadmin.property"))) {
			if (throwsException) {
				wac.log("wire api test", "Producer.polled called for correct wire. Throwing exception ...");
				throw new RuntimeException("test exception");
			} else {
				wac.log("wire api test", "Producer.polled called for correct wire. Returning '42'");
			}
		} else {
			wac.log("wire api test", "Producer.polled called for incorrect wire " + wire + ". Crashing now!");
