 ...
 * 
 * $Log$
 * Revision 1.5  2005/07/26 09:57:36  pdobrev
 * revertedclasses to its version 1.1
 *
 * Revision 1.7  2003/12/16 15:06:17  vpanushev
 * 1. test_wireadmin_DeleteWire is changed to reproduce the case described in issue #245 from the issue tracker
 * 2. test_wireadmin_RestartWireadmin rewriten
 *
 * Revision 1.6  2003/11/14 07:18:29  vpanushev
...
public class WireAdminControl extends DefaultTestBundleControl {
	...
	static String[] methods =
		new String[] {
			"prepare",
			"test_wireadmin_CreateWire",
			"test_wireadmin_DeleteWire",
			...
			"test_wireadmin_multipleRegistration",
			"test_wireadmin_GetFlavors",
			"testPollUpdate",
			"testValueFiltering",
			"testEvents",
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	///////////////////////////////////////////wire admin test methods start here ///////////////////////////////////////////
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 * create and connect 
	 * check if the methods of the Wire interface return a correct results 
	 * check if the proper methods are called (producersConnected, consumersConnected)
	 */
	...
	public void test_wireadmin_CreateWire() throws Exception {
		...
		helper.registerConsumer("consumer.ConsumerImplA", new Class[] { String.class, Integer.class, Float.class });
		...
		if (compareWireProperties(wire.getProperties(), newProperties)) {
			log("Test wire properties after updateWire ", "Operation passes: OK.");
		} else {
	 ...
	 * 
	 * isValid (should be false after deletion)
	 * isConnected (should be false after deletion)
	 * Wire.getFlavors (should return null)
	 * WireAdmin.getWires (should return null when filter matches the deleted wire)
	 */
	...
	public void test_wireadmin_DeleteWire() throws Exception {
		...
		Wire wire1 = wa.createWire("deletedWireProducer", "deletedWireConsumer", props);
		Wire wire2 = wa.createWire("deletedWireProducer", "deletedWireConsumer", props);
		synchCounter = 0;
		log(subtest, "registering consumer and producer so the wires are connected when we delete one of them later");
		helper.registerProducer("deletedWireProducer", new Class[] { java.lang.String.class });
		helper.registerConsumer("deletedWireConsumer", new Class[] { java.lang.String.class });
		int timeoutCounter = 0;
		while (synchCounter < 4 && timeoutCounter++ < 100) Thread.sleep(100L);

		log(subtest, "Deleting wire. Must call consumersConnected and producersConnected");
		synchCounter = 0;
		wa.deleteWire(wire1);
		timeoutCounter = 0;
		while (synchCounter < 2 && timeoutCounter++ < 100) Thread.sleep(100L);

		Wire[] wires = (Wire[])returnedWires.get("deletedWireProducer");
		if ((wires != null) && (wires.length == 1)) {
			log(subtest, "{deletedWireProducer} Invokes consumersConnected(..) CORRECT");
		} else {
			log(subtest, "{deletedWireProducer} Invokes consumersConnected(..) NOT OK, Wrong Wire[]");
		}

		wires = (Wire[])returnedWires.get("deletedWireConsumer");
		if ((wires != null) && (wires.length == 1)) {
			log(subtest, "{deletedWireConsumer} Invokes producersConnected(..) CORRECT");
		} else {
			log(subtest, "{deletedWireConsumer} Invokes producersConnected(..) NOT OK, Wrong Wire[]");
		}
		...
		if (wire1.isValid() && wire2.isValid()) {
			log(subtest, "Test Wire.isValid after deleteWire falied! Wire.isValid() returns true ");
		}	else {
			log(subtest, "Test Wire.isValid after deleteWire OK.");
		}
		
		if (wire1.isConnected() && wire2.isConnected()) {
			log(subtest, "Test wire.isConnected after deleteWire failed! Wire.isConnected returns true");
		}	else {
		...
		wires = wa.getWires("(&(wireadmin.producer.pid=deletedWireProducer)(wireadmin.consumer.pid=deletedWireConsumer))");
		if (wires == null) {
	 ...
	 * 
	 * Create a wire with non-registered pids (wire should be valid but not connected)
	 * Create a wire with null pid (behaviour not specified - could be ignored, however
	 * an Exception is expected by the test)
	 * Create a wire with invalid properties (Exception should be thrown) 
	 * Create a wire with incompatible flavors (should be created but data should not be tranfered 
	 * over it)
	 */
	...
	public void test_wireadmin_IncorrectCreateWire() throws Exception {
		...
		if (wire.isValid() && !wire.isConnected()) {
			log("Test createWire with non-existing PIDs ", "Operation passed: OK.");
		} else {
		...
		synchCounter = 0;
		int timeoutCounter = 0;
		helper.registerConsumer("consumer.ConsumerImplB", new Class[] { Date.class });
		while ((synchCounter < 1) && (timeoutCounter++ < 100))
			Thread.sleep(50);
		...
		if (wire.isValid() && wire.isConnected() && notUpdated(wire)) {
			log("Test createWire with incompatible flavors", "Operation passed: OK.");
		} else {
	 ...
	 *
	 * Update with invalid properties (Exception should be thrown)
	 * Update a deleted wire (not defined in the specification - the test 
	 *	expecs an Exception to be thrown)
	 */
	...
	public void test_wireadmin_IncorrectUpdateWire() throws Exception {
		...
		try {
			...
			if (compareWireProperties(wire.getProperties(), properties)) {
				...
				log("Test updateWire with case insensitive properties' keys", "Operation passed: OK.");
			}
		} catch (Exception e) {
	...
	/**
	 * Test what happens with the wire when consumer/producers are registered/unregistered
	 * Wire should remain persistent after such operation
	 */
	...
	public void test_wireadmin_RegisterUnregisterConsumerProducer() throws Exception {
		...
		synchCounter = 0;
		int timeoutCounter = 0;
		helper.registerConsumer("consumer.ConsumerImplC", new Class[] { String.class, Integer.class, Float.class });
		while ((synchCounter < 1) && (timeoutCounter++ < 100))
			Thread.sleep(50);
		...
		synchCounter = 0;
		timeoutCounter = 0;
		helper.registerProducer("producer.ProducerImplC", new Class[] { String.class, Float.class, Boolean.class });
		while ((synchCounter < 1) && (timeoutCounter++ < 100))
			Thread.sleep(50);
		...
		try {
			...
			if (compareWireProperties(wire.getProperties(), oldProperties)) {
				log("Test wire properties after Producer's re- registration", "Operation passed: OK.");
			} else {
				log("Test wire properties after Producer's re- registration", "NOT OK.");
			}
		} catch (Exception e) {
			log("Test wire properties after Producer's re- registration", "NOT OK.");
			e.printStackTrace();
		...
		try {
			wire = wa.getWires("(&(wireadmin.producer.pid=producer.ProducerImplC)(wireadmin.consumer.pid=consumer.ConsumerImplC))")[0];
			if (compareWireProperties(wire.getProperties(), oldProperties)) {
				log("Test wire properties after Consumer's re- registration", "Operation passed: OK.");
			} else {
				log("Test wire properties after Consumer's re- registration", "NOT OK.");
			}
		} catch (Exception e) {
			log("Test wire properties after Consumer's re- registration", "NOT OK.");
			e.printStackTrace();
	...
	/**
	 * Test updating a wire, befor/after its consumer or producer was unregistered
	...
	public void test_wireadmin_UpdateAfterUnregister() throws Exception {
		...
		try {
			wire = wa.getWires("(&(wireadmin.producer.pid=producer.ProducerImplC)(wireadmin.consumer.pid=consumer.ConsumerImplC))")[0];
			if (compareWireProperties(wire.getProperties(), properties)) {
				log("Test updateWire with unregistered Consumer", "Operation passed: OK.");
			} else {
	...
	public void test_wireadmin_RestartWireadmin() throws Exception {
		...
		helper.registerProducer("producer1", new Class[] { java.lang.String.class });
		helper.registerConsumer("consumer1", new Class[] { java.lang.String.class });
		...
		helper.registerProducer("producer2", new Class[] { java.lang.String.class });
		helper.registerConsumer("consumer2", new Class[] { java.lang.String.class });
		timeoutCounter = 0;
		
		log(subtest, "starting wire admin bundle again");
		log(subtest, "Must call consumersConnected(..) for: producer1, producer2");
		...
		try {
			...
			if (wires != null) {
				...
				if (wire1 != null && wire1.isValid() && wire1.isConnected()) {
					log(subtest, "Test wire state after WireAdmin restart: first wire is valid and connected OK.");
				} else {
					log(subtest, "Test wire state after WireAdmin restart: first wire isn't valid or connected.");
				}
			} else {
				log(subtest, "Test wire state after WireAdmin restarting: Wire is null.");
			}
			...
			if (wires != null) {
				...
				if (wire2 != null && wire2.isValid() && wire2.isConnected()) {
					log(subtest, "Test wire state after WireAdmin restart: second wire is valid and connected OK.");
				} else {
					log(subtest, "Test wire state after WireAdmin restart: second wire isn't valid or connected.");
				}
			} else {
				log(subtest, "Test wire state after WireAdmin restarting: Wire is null.");
			}
	...
	public void test_wireadmin_GetFlavors() throws Exception {
		helper.registerConsumer("consumer.ConsumerImplF", new String[] { "AA", "BB" });
		Hashtable h = new Hashtable();
		h.put("org.osgi.test.wireadmin", "yes");
		Wire wire1 = wa.createWire("producer.ProducerImplC", "consumer.ConsumerImplF", h);
		if (wire1.getFlavors() == null) {
			log("Test wire getFlavors with incorrect consumer's flavors", "Operation passed: OK.");
		} else {
			log("Test wire getFlavors with incorrect consumer's flavors", "NOT OK.");
		}
	}
	...
	public void test_wireadmin_multipleRegistration() throws Exception {
		...
		if (compareWires_Flavors(correct, wa.getWires("(org.osgi.test.wireadmin=yes)"))) {
			log("Test Wireadmin Wires after Producer's unregistration", "Operation passed: OK.");
		} else {
			log("Test Wireadmin Wires after Producer's unregistration", "NOT OK.");
		}
		...
		if (compareWires_Flavors(correct, wa.getWires("(org.osgi.test.wireadmin=yes)"))) {
			log("Test Wireadmin Wires after Producer's registration", "Operation passed: OK.");
		} else {
		...
		compareConnected("producer.ProducerImplC", "");
		if (compareWires_Flavors(correct, wa.getWires("(org.osgi.test.wireadmin=yes)"))) {
			log("Test Wireadmin Wires after Consumer's unregistration", "Operation passed: OK.");
		} else {
	...
	public void testPollUpdate() throws Exception {
		...
		h.put("org.osgi.test.wireadmin", "yes");
		Wire localWire = wa.createWire("wireAPITest.producer.pid", "wireAPITest.consumer.pid", h);
		...
		p_h.put(WireConstants.WIREADMIN_PRODUCER_FLAVORS, new Class[] { Integer.class });
		p_h.put(Constants.SERVICE_PID, "wireAPITest.producer.pid");
		WireAPITestProducerImpl producer = new WireAPITestProducerImpl(this);
		ServiceRegistration reg_producer = context.registerService(Producer.class.getName(), producer, p_h);

		// register consumer
		log("wire api test", "register test consumer");
		Hashtable c_h = new Hashtable();
		c_h.put(WireConstants.WIREADMIN_CONSUMER_FLAVORS, new Class[] { Integer.class });
		c_h.put(Constants.SERVICE_PID, "wireAPITest.consumer.pid");
		WireAPITestConsumerImpl consumer = new WireAPITestConsumerImpl(this);
		ServiceRegistration reg_consumer = context.registerService(Consumer.class.getName(), consumer, c_h);
		...
		log("wire api test", "polling connected wire with producer throwing exception");
	...
	public void testValueFiltering() throws Exception {
		...
		Wire localWire = wa.createWire("filter.test.producer.pid", "filter.test.conusmer.pid", h);
		...
		ServiceRegistration reg_producer = context.registerService(Producer.class.getName(), fpi, p_h);
		...
		ServiceRegistration reg_consumer = context.registerService(Consumer.class.getName(), new FilteredConsumerImpl(this), c_h);

		// wait until filtered producer's method consumersConnected is called with the test wire
		int i = 0;
		...
		log(subtest, "Current value test - notifying producer. Will send integers [0..9]");
		...
		// previous value test
		// last received value was 5 so the consumer should receive 0 and nothing else
		filter = "(" + WireConstants.WIREVALUE_PREVIOUS + "=5)";
		h.put(WireConstants.WIREADMIN_FILTER, filter);
		wa.updateWire(localWire, h);
		log(subtest, "Previous value test - notifying producer. Will send integers [0..9]");
		log(subtest, "Filter is " + filter);
		...
		log(subtest, "Value delta absolute test - notifying producer. Will send integers [0, 2, 4, 6, 8]");
		log(subtest, "Filter is " + filter);
		...
		log(subtest, "Value delta relative test - notifying producer. Will send integers [0, 2, 4, 6, 8]");
		log(subtest, "Filter is " + filter);
		...
		// time elapsed test
		// 4 should be received since it is the first one that has waited so long
		filter = "(" + WireConstants.WIREVALUE_ELAPSED + ">=500)";
		h.put(WireConstants.WIREADMIN_FILTER, filter);
		wa.updateWire(localWire, h);

		Thread.sleep(500); // ensure the time is elapsed
		log(subtest, "Time elapsed test - notifying producer. Will send integers [0, 2, 4, 6, 8]");
		log(subtest, "Filter is " + filter);
		log(subtest, "Consumer should receive values [0, 4, 6, 8]");
		// make the producer wait before sending updates
		fpi.updateWire(2, true);
		counter = 0;
		while ((counter++ < 500) && (FilteredConsumerImpl.valuesReceived < 4)) { // timeout increased to 25 seconds
			Thread.sleep(50);
		}
		FilteredConsumerImpl.valuesReceived = 0;
		log(subtest, delimiter);
		...
		log(subtest, "update the producer properties - add WIREADMIN_PRODUCER_FILTERS property");
		...
		h.put(WireConstants.WIREADMIN_FILTER, filter);
		wa.updateWire(localWire, h);
		log(subtest, "Filtering disabled test - notifying producer. Will send integers [0, 3, 6, 9]");
		log(subtest, "Filter is " + filter);
	...
	public void testEvents() throws Exception {
		...
		TestWireAdminListener listener = new TestWireAdminListener(this, false); // should receive all events
		TestWireAdminListener dummy = new TestWireAdminListener(this, true); // shouldn't receive events

		//real listener
		Hashtable walProps = new Hashtable();
		walProps.put(WireConstants.WIREADMIN_EVENTS, mask);
		ServiceRegistration listenerReg = context.registerService(WireAdminListener.class.getName(), listener, walProps);

		//dummy listener
		Hashtable dummyProps = new Hashtable();
		dummyProps.put(WireConstants.WIREADMIN_EVENTS, new Integer(0));
		ServiceRegistration dummyReg = context.registerService(WireAdminListener.class.getName(), dummy, dummyProps);
		...
		w = wa.createWire("producer.event.test.pid", "consumer.event.test.pid", h);
		synchronized (synch) {
			if (!listener.called)
				synch.wait(5000);
		}
		...
		// receive only  PRODUCERS_EXCEPTION, CONSUMERS_EXCEPTION and WIRE_DELETED events from now
		mask = new Integer(WireAdminEvent.CONSUMER_EXCEPTION | WireAdminEvent.PRODUCER_EXCEPTION | WireAdminEvent.WIRE_DELETED);
		walProps.put(WireConstants.WIREADMIN_EVENTS, mask);
		listenerReg.setProperties(walProps);

		log(subtest, "cause exception from producersConnected: CONSUMER_EXCEPTION is expected");
		listener.called = false;
		...
		log(subtest, "cause exception from updated: CONSUMER_EXCEPTION is expected");
		listener.called = false;
		w.update("42");
		synchronized (synch) {
			if (!listener.called)
		...
		helper.unregisterEventConsumer();
		helper.unregisterEventProducer();
		log(subtest, "cause exception from consumersConnected: PRODUCER_EXCEPTION is expected");
		listener.called = false;
		helper.registerEventConsumer(false);
		...
		log(subtest, "cause exception from polled: PRODUCER_EXCEPTION is expected");
		listener.called = false;
		w.poll();
		synchronized (synch) {
			if (!listener.called)
		...
		log(subtest, "cause exception form producersConnected of a 'free' Consumer. CONSUMER_EXCEPTION is expected");
		listener.called = false;
		helper.registerEventConsumer(true);
		synchronized (synch) {
			if (!listener.called)
		...
		log(subtest, "cause exception form consumersConnected of a 'free' Producer. PRODUCER_EXCEPTION is expected");
		listener.called = false;
		helper.registerEventProducer(true);
		synchronized (synch) {
			if (!listener.called)
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	////////////////////////////////////////////wire admin test methods end here ////////////////////////////////////////////  
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/////////////////////////////////////////state manager test methods start here //////////////////////////////////////////  
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public void test_statemanagement_null_scopes() {
		...
		if (wire.getScope() == null) {
			log("Test Wire Scope with null scope and without WirePermission", "Operation passed: OK.");
		} else {
		...
		if (compareScopes(wire.getScope(), null)) {
			log("Test Wire Scope with null scope, after setting WirePermissions", "Operation passed: OK.");
		} else {
			log("Test Wire Scope with null scope, after setting WirePermissions", "NOT OK.");
		}
	...
	public void test_statemanagement_consumer_scope() {
		...
		wire = wa.createWire("producer.ProducerImplA", "consumer.ConsumerImplB", h);
		if (wire.getScope() == null) {
			log("Test Wire Scope with null producer scope without WirePermission", "Operation passed: OK.");
		} else {
			log("Test Wire Scope with null producer scope without WirePermission", "NOT OK.");
		}
		wa.deleteWire(wire);
		setWirePermissions(new String[] { "*" }, consumer, true);
		...
		wire = wa.createWire("producer.ProducerImplA", "consumer.ConsumerImplB", h);
		if (compareScopes(wire.getScope(), null)) {
			log("Test Wire Scope with null producer scope, after setting WirePermission", "Operation passed: OK.");
		} else {
			log("Test Wire Scope with null producer scope, after setting WirePermission", "NOT OK.");
		}
	...
	public void test_statemanagement_all_scope() {
		...
		if (compareScopes(wire.getScope(), new String[] { "A", "B", "C" })) {
			log("Test Wire Scope without WirePermission", "Operation passed: OK.");
		} else {
			log("Test Wire Scope without WirePermission", "NOT OK.");
		}
		...
		if (compareScopes(wire.getScope(), new String[] { "B" })) {
			log("Test Wire Scope, after setting WirePermission", "Operation passed: OK.");
		} else {
		...
		if (envelope.equals(wire.getLastValue())) {
			log("Test Wire update with envelope object", "Operation passed: OK.");
		} else {
	...
	public void test_statemanagement_all_asterisk() {
		...
		if (compareScopes(wire.getScope(), new String[] { "*" })) {
			log("Test Wire Scope with *(scope) and *(WirePermission)", "Operation passed: OK.");
		} else {
			log("Test Wire Scope with *(scope) and *(WirePermission)", "NOT OK.");
		}
	...
	public void test_statemanagement_missing_scope_permissions() {
		...
		wire.update(envelope);
		if (wire.getLastValue() == null) {
			log("Test Wire update with incompatible wire scope", "Operation passed: OK.");
		} else {
	...
	public void test_statemanagement_scope_intersection() {
		...
		wire.update(envelope);
		if (envelope.equals(wire.getLastValue())) {
			log("Test Wire update with correct envelope object", "Operation passed: OK.");
		} else {
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/////////////////////////////////////////state manager test methods end here //////////////////////////////////////////
	///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	*
	*/
	private void setWirePermissions(String[] permissions, Bundle bundle, boolean flag) {
		...
		for (int i = 0; i < permissions.length; i++) {
			perms[i] = new PermissionInfo("org.osgi.service.wireadmin.WirePermission", permissions[i], type);
		}
	...
	private void compareConnected(String producerPID, String consumerPID) {
		...
		if (!producerPID.equals("")) {
			wires = getConnected(getWiresForFilter("(wireadmin.producer.pid=" + producerPID + ")"));
			if (returnedWires.get(producerPID) == null) {
				log("{" + producerPID + "} Invokes consumersConnected(..) ", "NOT OK, Not invoked");
			} else {
				...
				if (compareWires_Flavors(wires, returned)) {
					log("{" + producerPID + "} Invokes consumersConnected(..) ", "CORRECT");
				} else {
					log("{" + producerPID + "} Invokes consumersConnected(..) ", "NOT OK, Wrong Wire[]");
				}
				returnedWires.remove(producerPID);
			}
		}
		if (!consumerPID.equals("")) {
			wires = getConnected(getWiresForFilter("(wireadmin.consumer.pid=" + consumerPID + ")"));
			if (returnedWires.get(consumerPID) == null) {
				log("{" + consumerPID + "} Invokes producersConnected(..) ", "NOT OK, Not invoked");
			} else {
				...
				if (compareWires_Flavors(wires, returned)) {
					log("{" + consumerPID + "} Invokes producersConnected(..) ", "CORRECT");
				} else {
					log("{" + consumerPID + "} Invokes producersConnected(..) ", "NOT OK, Wrong Wire[]");
				}
	...
	private PermissionAdmin getPermissionAdmin() {
		pRef = context.getServiceReference("org.osgi.service.permissionadmin.PermissionAdmin");
		if (pRef != null) {
			return (PermissionAdmin) context.getService(pRef);
		} else {
			throw new RuntimeException("PermissionAdmin service not present! Unable to proceed test");
		}
	}

	private WireAdmin getWireadmin() {
		context = getContext();
		waRef = context.getServiceReference("org.osgi.service.wireadmin.WireAdmin");
		if (waRef != null) {
			return (WireAdmin) context.getService(waRef);
		} else {
			throw new RuntimeException("Wire admin service not present! Unable to proceed test");
		}
	}

	////////// some helper methods for additonal debugging ... not used by default
	private void dumpWires(String filter) {
		synchronized (this.getClass()) { // to avoid messups when called from different threads
			try {
				Wire[] wiress = wa.getWires(filter);
				if (wiress == null) {
					System.out.println("No wires registered for filter " + filter);
					return;
				}
	...
	private void dumpProducers(String filter) {
		synchronized (this.getClass()) { // to avoid messups when called from different threads
			try {
				ServiceReference[] producers = context.getServiceReferences(Producer.class.getName(), filter);
				if (producers == null) {
					System.out.println("No producers registered for filter " + filter);
					return;
				}
				for (int i = 0; i < producers.length; i++) {
					String[] props = producers[i].getPropertyKeys();
					System.out.println("Producer: " + context.getService(producers[i]));
					context.ungetService(producers[i]);
					System.out.println("Producer props:");
					for (int j = 0; j < props.length; j++) {
						System.out.println("property: " + props[j]);
						System.out.println("value: " + producers[i].getProperty(props[j]));
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
	...
	private void dumpConsumers(String filter) {
		synchronized (this.getClass()) { // to avoid messups when called from different threads
			try {
				ServiceReference[] consumers = context.getServiceReferences(Consumer.class.getName(), filter);
				if (consumers == null) {
					System.out.println("No consumer registered for filter " + filter);
					return;
				}
				for (int i = 0; i < consumers.length; i++) {
					String[] props = consumers[i].getPropertyKeys();
					System.out.println("Consumer: " + context.getService(consumers[i]));
					context.ungetService(consumers[i]);
					System.out.println("Consumer props:");
					for (int j = 0; j < props.length; j++) {
						System.out.println("property: " + props[j]);
						System.out.println("value: " + consumers[i].getProperty(props[j]));
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
