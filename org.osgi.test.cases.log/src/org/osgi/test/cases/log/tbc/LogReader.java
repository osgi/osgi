...
public class LogReader implements LogListener {
    ...
    public LogReader(LogReaderService logReaderService,
                     LogService logService,
    ...
    public void stop() {
        ...
        synchronized (this)    // issue 149: MUST not be synchronized when we log the stopString
        {
            while (waitingToStop)
            {
                try {
    ...
    /**
     * Saves log entries that were created by the bundle
     * we are assigned to listen to.
     *
     * Log messages containing stopmessages are never logged.
     */
    public void logged(LogEntry entry) {
        try {
            // System.out.print(id + " Called - " + entry.getBundle().getBundleId());
            if(isListening()) {
                // System.out.println(" - logging");
                System.out.println("Message: " + entry.getMessage());
                if(bundle.equals(entry.getBundle())) {
                    System.out.println(bundle.getBundleId() + " == " + entry.getBundle().getBundleId());
    ...
    /**
     * Returns a vector of Strings containing all logentries collected
     * so far. If this method is called more than once, it will not
     * return the same entries more than once (it throws away
     * everything that has been read earlier).
     */
    public Vector getLog() {
        if(isListening()) {
            /* If someone tries to read the results before the log has
               been stopped, the the results are unpredictable, which
               should not be allowed in a test case */
            throw new RuntimeException(id + " - stop the logging before reading the results");
        }
    ...
    /**
     * Converts a logentry to a String. The timestamp is not included
     * since this makes the String uncomparable with an expected
     * outcome.
     */
    public String logEntryToString(LogEntry entry) {
        ...
        if(e != null) {
            // This was changed to support R3 (section 9.5) in which LogEntry.getException may not return the original exception
            // exceptionString = " " + e.getClass().getName();
            exceptionString = " " + e.toString();
        }

        return levelToString(entry.getLevel()) + " "
               + entry.getMessage()
               + exceptionString;
    }
