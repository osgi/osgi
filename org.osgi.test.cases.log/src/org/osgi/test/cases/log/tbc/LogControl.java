...
public class LogControl extends DefaultTestBundleControl {
    ...
    public void prepare() throws Exception {
        logServiceReference = getContext().getServiceReference(LogService.class.getName());
...
Object property = sr.getProperty(keys[i]);
...
System.out.println(keys[i] + "=" + property);
}
...
/*
installSecondBundle();
logService.log(LogService.LOG_INFO, "Hello14");
logReadSvc.addLogListener(testLogListener2);
removeSecondBundleLogListener();
...
/*
public void testAnotherBundle() {
LogReader reader = new LogReader(logReaderService, logService,
getContext().getBundle(), "reader");
reader.start();

ContextSharerInfo csi = installContextSharer();
System.out.println("csi == " + csi);
System.out.println(csi.getBundle() + ", " + csi.getServiceReference() + ", " + csi.getService());
logService.log(LogService.LOG_WARNING, csi.getService().getContext().getBundle().getLocation());
