...
public class TBC extends Thread
{
	...
	*/
	TBC(BundleContext context)
	{
		...
		this.linkRef = context.getServiceReference(TestCaseLink.class.getName());
	...
	*/
		...
		throws Exception
	{
		...
		// sr = context.getServiceReference("org.osgi.service.packageadmin.PackageAdmin");
		sr = context.getServiceReference(PackageAdmin.class.getName());
		PackageAdmin pa = (PackageAdmin)context.getService(sr);
		//Check that the packages doesn?t exist before and do not run if they do
				...
				|| (pa.getExportedPackage(TP3) != null))
		{
			...
			if (ep.length == 1) {
				log("What is TestBundle 1 exporting. : org.osgi.test.cases.packageadmin.tc2.tb1 : '"+ep[0].getName()+"'");
			}
			...
			for (int i = 0 ; i < tps133.length;i++) {
				if( tps133[i].getName().equals(TP1)){
					tp1present = true;
				} else if (tps133[i].getName().equals(TP3)) {
					tp3present =true;
				}
			}

			log("Does TestBundle 1 contain Testpackage 1 before update: true : '"+tp1present+"'");
			log("Does TestBundle 1 contain Testpackage 3 before update: false : '"+tp3present+"'");

            link.send("25");

			//uninstall tb1
			//install tb3 called the same as tb1 (tb1.jar) but exporting package ...testpackage3 (simulates updating of the bundle)
			tb5 = context.installBundle(tcHome + "tb5.jar");
			try {
				tb5.getResource("www/tb1.jar");
			} catch(IllegalStateException e) {
				log("Could not getResource on a nonstarted but installed bundle." + e);
			}
			tb5.start();
			...
			for (int i = 0 ; i < tps13.length;i++) {
				if( tps13[i].getName().equals(TP1)){
					tp1present = true;
				} else if (tps13[i].getName().equals(TP3)) {
					tp3present =true;
				}
			}
			log("Because of lazy update, now TestBundle 1 should contain TestPackage 1 and TestPackage 3.");
			log("Does TestBundle 1 contain Testpackage 1 after update: true : '"+tp1present+"'");
			log("Does TestBundle 1 contain Testpackage 3 after update: true : '"+tp3present+"'");

			//Call PackageAdmin.getExportedPackage(...testpackage1) and getExportedPackage(...testpackage3)
			//Is ...testpackage1 exported (yes, not null)
			//Is ...testpackage3 exported (yes, not null)
			log("Is TestPackage 1 exported: true : '"+(pa.getExportedPackage(TP1)!=null)+"'");
			log("Is TestPackage 3 exported: true : '"+(pa.getExportedPackage(TP3)!=null)+"'");


			//is tb4 active (yes)
			//is tb2 active (yes)
			log("Is testbundle 2 still active: true: '" + (tb2.getState() == Bundle.ACTIVE)+"'");
			log("Is testbundle 4 still active: true: '" + (tb4.getState() == Bundle.ACTIVE)+"'");
            link.send("50");
			...
			for (int i = 0 ; i < tps3.length;i++) {
				if( (tps3[i].getName()!=null)&&tps3[i].getName().equals(TP1)){
					tp1present = true;
				} else if ((tps3[i].getName()!=null)
						&&tps3[i].getName().equals(TP3)) {
					tp3present =true;
				}
			}

			log("Does TestBundle 1 contain Testpackage 1 after refresh: false : '"+tp1present+"'");
			log("Does TestBundle 1 contain Testpackage 3 after refresh: true : '"+tp3present+"'");
			//Call PackageAdmin.getExportedPackage(...testpackage3)
			//Is ...testpackage3 exported (yes, not null)
			log("Is TestPackage 1 exported after refresh: false : '"+(pa.getExportedPackage(TP1)!=null)+"'");
			log("Is TestPackage 3 exported after refresh: true : '"+(pa.getExportedPackage(TP3)!=null)+"'");

			//is tb4 active (yes)
			//is tb2 active (no)
			log("Is testbundle 2 still active: false: '" + (tb2.getState() == Bundle.ACTIVE)+"'");
			log("Is testbundle 4 still active: true: '" + (tb4.getState() == Bundle.ACTIVE)+"'");
