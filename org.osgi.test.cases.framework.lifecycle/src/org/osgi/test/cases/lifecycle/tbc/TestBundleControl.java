...
package org.osgi.test.cases.lifecycle.tbc;
...
import org.osgi.test.cases.lifecycle.servicereferencegetter.*;
...
The testPermissions test is currently removed because of unknown state. It uses
tb10 but this seems to be removed from the upper makefile.

@author Ericsson Radio Systems AB
 */
public class TestBundleControl extends Thread
implements LogProxy, BundleListener, ServiceListener
{
	...
	static String[] methods = new String[] {
		"testNormalInstallStartStopUninstall",
			"testStartStopWithException",
			"testUnavailableURL",
			"testBrokenStream",
			"testEvents",
			"testServiceRegistrations",
			"testNormalUpdate",
			"testRollbackUpdate",
			"testStoppedBundleContext",
			//  			"testHangInStop", // Not used, behaviour is not specified in OSGi 1.0.
			"testGetLocation",
			"testPermissions"
	};

	/**
	Constructor. Gets a reference to the TestCaseLink to communicate with
	the TestCase.
	 */
	...
	public void run()
	{
		...
		_sr = _context.registerService(TestBundleControl.class.getName(),
				this, null);
		
		srLogProxy=_context.registerService(LogProxy.class.getName(),this,null);	
		...
		try
		{
			...
			if ( targetException instanceof BundleException )
				targetException = ((BundleException)targetException).getNestedException();
	 ...
	 */
	void testNormalInstallStartStopUninstall()
		throws Exception
	{
		Bundle  tb;
		String  res;
		...
		if (tb.getState() == Bundle.UNINSTALLED)
			...
			res = "TestBundle 1 uninstalled but incorrect state " + tb.getState();
	 ...
	 */
	void testStartStopWithException()
		throws Exception
	{
		Bundle tb2a, tb2b;
	...
	public void serviceChanged(ServiceEvent se)
	{
		...
		switch(se.getType())
		{
		case ServiceEvent.REGISTERED:   event = "REGISTERED";   		break;
		case ServiceEvent.MODIFIED: 			event = "MODIFIED"; 			break;
		case ServiceEvent.UNREGISTERING:		event = "UNREGISTERING";		break;
		default:								event = "UNKNOWN EVENT";		break;
		}

		log("Testing ServiceEvents:", "Service " + service +
				" is " + event + ".");
		...
		throws Exception
	{
		...
		_context.addServiceListener(sl,
									"(objectClass=org.osgi.test.cases.lifecycle.tb5.EventTest)");
		...
		throws Exception
	{
		...
		clazzes = new String[]
			{
			TestBundleControl.class.getName(),
		...
		throws Exception
	{
		...
		if (tb.getState() == Bundle.ACTIVE)
			...
			res = "State is " +
				BundleState.stateName(tb.getState()) +
				", Error!";
		...
		throws Exception
	{
		...
		try
		{
			...
			ref = otherContext.getServiceReference(TestCaseLink.class.getName());
		...
		throws Exception
	{
		...
		// Use the same bundle as the "hang in stop" test, we won't start (or stop) it anyway.
	 ...
	 */
	synchronized void testPermissions()
		throws Exception
	{
		Bundle tb;
		Bundle tbPerm;
		...
		serviceRef = _context.getServiceReference(ServiceReferenceGetter.class.getName());
		if (serviceRef != null)
		{
			ServiceReferenceGetter serviceReferenceGetter = (ServiceReferenceGetter) _context.getService(serviceRef);
			serviceReferenceGetter.setServiceReference(_context.getServiceReference(PermissionAdmin.class.getName()));
		}
		...
		try {
			...
			else
			{
				if ( _eventQueue[_out] == event )
					...
					log( test,
						" invalid event " + event( _eventQueue[_out] ) + ", expected " + event( event ) );
	...
	String event( int event ) {
		switch( event ) {
		case 0: 												return "(NONE)";
		case BundleEvent.INSTALLED: 			return "INSTALLED(" + BundleEvent.INSTALLED + ")";
		case BundleEvent.STARTED:   			return "STARTED(" + BundleEvent.STARTED + ")";
		case BundleEvent.UPDATED:   			return "UPDATED(" + BundleEvent.UPDATED + ")";
		case BundleEvent.STOPPED:   			return "STOPPED(" + BundleEvent.STOPPED + ")";
		case BundleEvent.UNINSTALLED:   return "UNINSTALLED(" + BundleEvent.UNINSTALLED + ")";
	...
	void setPermissions()
	{
		//get the permission admin service
   	 	serviceRef = _context.getServiceReference(PermissionAdmin.class.getName());
		if (serviceRef != null)
			permissionAdmin = (PermissionAdmin) _context.getService(serviceRef);
		if (permissionAdmin != null)
		{
			//set the required permissions for the testcontrol and target bundle
			...
			permissionAdmin.setPermissions((_context.getBundle()).getLocation(),permInfoArrayLocation);
			permissionAdmin.setPermissions(_tcHome+"tb10.jar",permInfoArrayLocation);


			//set permissions for all bundle 5&8
			PermissionInfo[] permInfoArrayService ={
				new PermissionInfo(ServicePermission.class.getName(),"org.*","get,register"),
				new PermissionInfo(PackagePermission.class.getName(),"*","EXPORT,IMPORT")};
			permissionAdmin.setPermissions(_tcHome+"tb5.jar",permInfoArrayService);
			permissionAdmin.setPermissions(_tcHome+"tb8.jar",permInfoArrayService);

			//set permissions for tb10
			PermissionInfo[] permInfoArrayTB10 ={
				new PermissionInfo(ServicePermission.class.getName(),"org.osgi.test.*","get"),
				new PermissionInfo(ServicePermission.class.getName(),"org.osgi.framework.*","get,register"),
				new PermissionInfo(ServicePermission.class.getName(),"org.osgi.test.cases.lifecycle.servicereferencegetter.*","get,register"),
				new PermissionInfo(PackagePermission.class.getName(),"*","EXPORT,IMPORT")};
			permissionAdmin.setPermissions(_tcHome+"tb10.jar",permInfoArrayTB10);



			/*//set defaultpermissions for all bundles
	...
	void resetPermissions()
	{
		if (permissionAdmin != null)
		{
			...
			permissionAdmin.setPermissions((_context.getBundle()).getLocation(),null);
		}
	}
}
