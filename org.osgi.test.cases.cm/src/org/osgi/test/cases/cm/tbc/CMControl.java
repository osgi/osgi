...
public class CMControl extends DefaultTestBundleControl {
    ...
    //public static final String SERVICE_BUNDLE_LOCATION = "service.bundleLocation";
...
*/
    ...
    /**
     * Test that the methods throws IllegalStateException when
     * operating on a deleted Configuration
     */
    public void testDeletedConfiguration() throws Exception {
        ...
        MethodCall[] methods = {
                ...
                new MethodCall(Configuration.class, "setBundleLocation", String.class, "somelocation"),
                new MethodCall(Configuration.class, "update"),
                new MethodCall(Configuration.class, "update", Dictionary.class, new Hashtable())
            };
        ...
        for(int i = 0; i < methods.length; i++) {
            ...
            catch (AssertionFailedError e)
            {
                ...
                assertException(methods[i].getName(), IllegalStateException.class, e);
            }
        }
    }
    ...
    public void testGetConfiguration() throws Exception {
        ...
        /* Get a brand new configuration */
        conf = cm.getConfiguration(pid);
        checkConfiguration(conf, "A new Configuration object",
            pid, thisLocation);
        ...
        /* Change the location of the bundle and then get the
         Configuration again. The location should not have been touched. */
        String neverlandLocation = "http://neverneverland/";

        conf.setBundleLocation(neverlandLocation);

        conf = cm.getConfiguration(pid);
        assertEquals("Location Neverland", neverlandLocation, conf.getBundleLocation());

        /* Clear the location of the bundle and then get the
         Configuration again. The location should have been set again */

        //        conf.setBundleLocation(null);
        //        assertNull("Location null", conf.getBundleLocation());
        //        conf = cm.getConfiguration(pid);
        //        assertEquals("Location set again", getLocation(), conf.getBundleLocation());
    ...
    public void testGetConfigurationWithLocation() throws Exception {
        ...
        /* Get an existing configuration, but specify the location
    ...
    public void testUpdate() throws Exception {
        ...
        assertEquals("conf property 'somekey'", "somevalue", props.get("somekey"));
        ...
        assertEquals("conf2 property 'somekey'", "somevalue", props2.get("somekey"));
    ...
    public void testListConfigurations() throws Exception {
        ...
        Configuration[] configs = {
                cm.getConfiguration("pid1")
            };
        Configuration[] updatedConfigs = {
                cm.getConfiguration("pid2")
            };

        try {

            /* Update properties on some of configurations
             (to make them "active") */
    ...
    /**
     * Tests to register a ManagedService when a configuration is
     * existing for it.
     */
    public void testManagedServiceRegistration() throws Exception {
        ...
        /* Add the two properties added by the CM and then check for
         equality in the properties */
        props.put(Constants.SERVICE_PID, pid);
        // props.put(SERVICE_BUNDLE_LOCATION, "cm_TBC"); R3 does not include service.bundleLocation anymore!
    ...
    public void testManagedProperties() throws Exception {
        ...
        props.put("StringArray", new String[] {
                "string1", "string2"
            }

        );
        props.put("IntegerArray", new Integer[] {
                new Integer(1), new Integer(2)
            }

        );
        props.put("LongArray", new Long[] {
                new Long(1), new Long(2)
            }

        );
        props.put("FloatArray", new Float[] {
                new Float(1.1), new Float(2.2)
            }

        );
        props.put("DoubleArray", new Double[] {
                new Double(1.1), new Double(2.2)
            }

        );
        props.put("ByteArray", new Byte[] {
                new Byte((byte) -1), new Byte((byte) -2)
            }

        );
        props.put("ShortArray", new Short[] {
        ...
        //
        //);
        props.put("CharacterArray", new Character[] {
                new Character('N'), new Character('O')
            }

        );
        props.put("BooleanArray", new Boolean[] {
        ...
        props.put("VectorArray", new Vector[] {
                v1, new Vector()
            }
        ...
        /* Add the two properties added by the CM and then check for
         equality in the properties (including preserved case) */
        props.put(Constants.SERVICE_PID, pid);
        // props.put(SERVICE_BUNDLE_LOCATION, "cm_TBC"); R3 does not include service.bundleLocation anymore!
    ...
    private void commonTestCreateFactoryConfiguration(boolean withLocation, String location)
        throws Exception {
        ...
        for(int i = 0; i < NUMBER_OF_CONFIGS; i++) {
            ...
            assertEquals("Correct factory pid", factorypid, conf.getFactoryPid());
    ...
    public void testManagedServiceFactory() throws Exception {
        ...
        /* Register a factory */
        ManagedServiceFactoryImpl msf =
            new ManagedServiceFactoryImpl("msf", "testprop", semaphore);
    ...
    /**
     * Compares two Configurations for equality. Configuration.equals()
     * is not specified in the spec, so this is a helper method
     * that compares pids.
     * <p>
     * Two Configurations are considered equal if the got the same pid
     * or if both are null.
     */
    ...
    class Plugin implements ConfigurationPlugin {
        ...
        public void modifyConfiguration(ServiceReference ref,
            ...
            for(int i = 0; i < types.length; i++) {
                ...
                else if("org.osgi.service.cm.ManagedServiceFactory".equals(types[i])) {
                    props.put("plugin.factory." + index,
                        "added by plugin#" + index);
                    break;
                }
            }
        }
