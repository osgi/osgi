 ...
 * present, we basically implement our own here.  Note, this is only for a reference
 * implementation.  The two big things are missing.  First, user authentication or possibly
 * just a check that the requests are coming from a local interface needs to be done.
 * And second, we need to turn off after provisioning is "through" and turn back on when
 * needed.
 * 
 * @author breed@almaden.ibm.com
 */
public class SimpleWebInterface extends Thread {
    /** The ProvisioningService.  Since there should only be one ProvisioningService we can
     * use a static variable. */
    static ProvisioningService svc;
    /** The Log of events in the ProvisiongService. Since there should only be one ProvisioningService
     *  we can use a static variable */
    static ProvisioningLog log;
    /** The ServerSocket requests come in on.  Since there should only be one ProvisioningService
     *  we can use a static variable */
    static ServerSocket ss;

    /** Constructs a Simple Provisioning Web interface for the ProvisioningService.
    ...
    /** To avoid creating another class.  We use this constructor to launch another thread to accept
     * the next connection. 
     */
    ...
    /** Prints out a form to submit an initial provisioning URL.  It also lists the Provisioning Event log.
     */
	...
	ps.println("<input type=\"submit\" value=\"Retrieve\">");
        ps.println("</form>");
        ps.println("<form method=\"POST\" enctype=\"multipart/form-data\" action=\"/\">");
        ps.println("<b>Provisioning Upload </b><input type=\"file\" name=\"file\">");
    ...
    static byte redirectBytes[] =
        "HTTP/1.0 302 Redirect to form\r\nContent-Type: text/html\r\nLocation: /\r\n\r\nGo to <a href=\"/\">here</a>\r\n"
            .getBytes();
	...
	public int read() throws IOException {
		...
		do {
		    ...
		    switch(c) {
		    case '\r': break;
		    case '\n': if (lastNL) break out; lastNL = true; break;
		    default: lastNL = false;
		    }
		} while(c != -1);
		if (c == -1) { atEof = true; return -1; }
		// All ready to read
	...
	final String contentTypeTag = "\ncontent-type: multipart/form-data";
		...
		/* Now that everything is parsed out, kick off the provisioning cycle. */
		...
		} catch(Exception e) { log.log("Error processing upload"); }
	    }
        } finally {
    ...
    void shutdown() {
        try {
            ss.close();
            ss = null;
            /* It seems we can't return too quickly because the socket may not be completely closed.. */
