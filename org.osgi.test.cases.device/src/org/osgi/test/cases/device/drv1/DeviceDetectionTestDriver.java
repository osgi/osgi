...
/**
 * A common driver implementation. It is used in the device
 * detection test and will match only to some of the devices - 
 * those that are intended to have drivers.
 *
 * @author ProSyst
 * @version 1.0
 */
...
public class DeviceDetectionTestDriver implements Driver, BundleActivator {
  ...
  /* !!!FIX!!! The device service this driver is attaching to 
   * 					 The driver must hold this  until it is uninstalled 
   *           (or the device is uninstalled) else the device will be
   *           considered idle and will be included in any searches. 
   */
  private Device device = null;
  private ServiceReference deviceRef = null;
  
  /**
   * The start method of the activator of this Driver bundle. 
   * As required registers the Driver service synchronously with
   * the bundle start.
   */
  public void start(BundleContext bc) {
    ...
    driverRegistration = bc.registerService("org.osgi.service.device.Driver", this, h);
  }
   ...
   * @return 255 if the Driver is intended to have a driver - thouse are drivers
   *   			 with IDs "basicDevice", "basicDevice_noDevice", "basicDevice_noCategory" 
   */
  public int match(ServiceReference reference) {
    ...
    if ("basicDevice".equals(driverID) || 
        "basicDevice_noDevice".equals(driverID) || "basicDevice_noCategory".equals(driverID)) {
      return 255;
    } else if ("basicDevice_noDriver".equals(driverID) || "basicDevice_noDevice_noDriver".equals(driverID)) {
   ...
   * TestBundleControl.MESSAGE_ERROR to indicate that there is a wrong attachment.
   *
   * @param reference service reference to the registred device.
   * @returns null
   */
