...
/**
 * Used in the driver loadin test. Simulates some crashes that may
 * happen in the loadDriver method
 *
 * @author ProSyst
 * @version 1.0
 */
public class DriverLoadingLocator2 implements DriverLocator {
   ...
   * @returns null if id is equal to "returnNull", an InputStream that contains
   *          invalid data if id is equal to "invalidStream"
   * @throws RuntimeException if id equals to "exceptionInLoading" or if unkown id is passed
   */
  public InputStream loadDriver(String id) {
    if ("exceptionInLoading".equals(id)) {
      throw new RuntimeException("[load driver] testing the device manager exception handling");
    } else if ("returnNull".equals(id)) { 
      return null;
    } else if ("invalidStream".equals(id)) {
      ...
      throw new RuntimeException("[load driver locator 2] Invalid ID passed!!!");
    }
  }
}