...
/**
 * The locator used in the device detection test. For all IDs it
 * retunrs an InputStream to the basic driver (in fact this is the
 * only driver bundle in the whole test case)
 */
public class BasicTestLocator implements DriverLocator {
   ...
   * @retunrs a single element String array containing the property
   *          with key "deviceID" from the props Dictionary
   */
   ...
   * This implementation returns always InputStream to the 
   * same bundle.
   * 
  ...
  public InputStream loadDriver(String id) {
    ...
    try {
      return (InputStream)AccessController.doPrivileged(new PrivilegedExceptionAction(){
      	  ...
      	  {
            URL url = new URL(TestBundleControl.tcHome + "drv1.jar");
