...
/**
 * This locator is registered to check that it will be called
 * when a driver is unregistered.
 * 
...
public class EmptyLocator implements DriverLocator {
  ...
  /**
   * Will just log if drivers are searched for device with ID "standalone driver test device"
   * Searching drivers for other devices will fail the test
   * 
  ...
  public String[] findDrivers(Dictionary props) {
    if ("standalone driver test device".equals(props.get("deviceID"))) {
      ...
      log("find drivers invoked for: " + props.get("deviceID") + " - Error!");
    }
    return null;
  }
