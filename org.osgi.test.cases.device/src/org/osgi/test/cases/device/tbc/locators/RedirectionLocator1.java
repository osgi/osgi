...
/**
 * The locator that will find the redirecting driver and later will 
 * find the driver pointed from the redirecting dirver.
 *
 * @author ProSyst
 * @version 1.0
 */
public class RedirectionLocator1 implements DriverLocator {
   ...
   * @retunrs a one element String array that contains the id of
   *          the redirecting driver
   */
   ...
   *         to the passed id or null if unkown id passed
   */
  public InputStream loadDriver(final String id) throws IOException {
      ...
      return (InputStream)AccessController.doPrivileged(new PrivilegedExceptionAction(){
	        ...
	        if ("redirecting_driver".equals(id)) {
	          try {
	            URL url = new URL(TestBundleControl.tcHome + "drv6.jar");
	        ...
	        } else if ("Driver_Winner".equals(id)) {
	        try {
