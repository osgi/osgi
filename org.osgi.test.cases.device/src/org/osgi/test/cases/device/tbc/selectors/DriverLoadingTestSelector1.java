...
/**
 * A selector used in the driver loading test. It will select the 
 * winner form three drivers.
 *
 * @author ProSyst
 * @version 1.0
 */ 
public class DriverLoadingTestSelector1 implements DriverSelector {
  ...
  /**
   * Logs the drivers from the passed matches list and returns the 
   * index of the winner driver.
   * 
  ...
  public int select(ServiceReference reference, Match[] matches) {
    ...
    for (int i = 0; i < matches.length; i++) {
      if ("Driver_Winner".equals((String)matches[i].getDriver().getProperty("DRIVER_ID"))) {
        log("winner selected");
        return i;
      }
    }
  ...
  private void logDriverList(Match[] matches) {
    ...
    while (moved) {
      ...
      for (int i = 0; i < matches.length - 1; i++) {
        String driver1 = (String)matches[i].getDriver().getProperty("DRIVER_ID");
        String driver2 = (String)matches[i+1].getDriver().getProperty("DRIVER_ID");
