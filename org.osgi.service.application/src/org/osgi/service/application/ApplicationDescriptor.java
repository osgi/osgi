...
public abstract class ApplicationDescriptor {
	 ...
	 * 
	 * @param applicationId The identifier of the application. Its value is also
	 *        available as the {@code service.pid} service property of this
	 *        {@code ApplicationDescriptor} service. This parameter must not be
	 *        {@code null}.
	 * @throws NullPointerException if the specified {@code applicationId} is
	 *         null.
	 */
	 ...
	 * {@link ApplicationAdminPermission#implies(java.security.Permission)}
	 * method to match target {@code ApplicationDescriptor} and filter.
	 * 
	 * @param pattern a pattern for a chain of Distinguished Names. It must not
	 *        be null.
	 * @return {@code true} if the specified pattern matches at least one of the
	 ...
	 * well. The returned {@code Map} will include the service properties of
	 * this {@code ApplicationDescriptor} as well.
	 * <p>
	 ...
	 * 
	 * @return copy of the service properties of this application descriptor
	 *         service, according to the specified locale. If locale is null
	 *         then the default locale's properties will be returned. (Since
	 *         service properties are always exist it cannot return null.)
	 * 
		...
		if (containerLocked != null && containerLocked.booleanValue() != isLocked) {
			try {
				if (isLocked) /* if the container's information is not correct */
					lockSpecific(); /*
									 * about the locking state (after loading
									 * the lock states)
									 */
				else
					unlockSpecific();
	 ...
	 * {@code locale} argument is not the empty String. The value {@code null}
	 ...
	 * properties as well (e.g. service.id, service.vendor etc.) and specialized
	 * application descriptors may offer further service properties. The
	 * returned {@code Map} contains a snapshot of the properties. It will not
	 * reflect further changes in the property values nor will the update of the
	 * Map change the corresponding service property.
	 * 
	 * @param locale the locale to be used for localizing the properties. If
	 *        {@code null} the default locale should be used. If it is the empty
	 *        String ({@code ""}) then raw (non-localized) values should be
	 *        returned.
	 * 
	 ...
	 *        may be null
	 * 
	 * @return the registered application model specific application handle for
	 *         the newly created and started instance.
	 * 
	 * @throws IllegalStateException if the application descriptor is
	 *         unregistered
	...
	public final ScheduledApplication schedule(String scheduleId, Map arguments, String topic, String eventFilter, boolean recurring) throws InvalidSyntaxException, ApplicationException {
		isLaunchableSpecific(); // checks if the ApplicationDescriptor was
		...
		static {
			AccessController.doPrivileged(new PrivilegedAction() {
				public Object run() {
					...
					try {
						setApplicationDescriptor = implementation.getMethod("setApplicationDescriptor", new Class[] {ApplicationDescriptor.class, String.class});
						isLocked = implementation.getMethod("isLocked", new Class[] {});
						lock = implementation.getMethod("lock", new Class[] {});
						unlock = implementation.getMethod("unlock", new Class[] {});
						schedule = implementation.getMethod("schedule", new Class[] {String.class, Map.class, String.class, String.class, boolean.class});
						launch = implementation.getMethod("launch", new Class[] {Map.class});
