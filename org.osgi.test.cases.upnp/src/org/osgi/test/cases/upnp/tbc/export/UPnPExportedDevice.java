...
public class UPnPExportedDevice implements UPnPDevice {
  ...
  public UPnPExportedDevice (TestService[] service, BundleContext bc) {
    ...
    props.put(UPnPDevice.TYPE, "urn:schemas-prosyst-com:device:UPnPTesterType:1");
  ...
  public static UPnPExportedDevice newUPnPTestervice() {
    ...
    TestStateVariable[] vars = new TestStateVariable[] {testingSV, testMsg, outNumber, outInt, outChar, outFloat,
    ...
    TestAction msgAct = new TestAction("testMsg", null, new String[]{TESTMSG_SV}, null, nameVar);
    nameVar.put(testingSV.getName(), testingSV);
    nameVar.put(levski.getName(), levski);
    TestAction printAct = new TestAction("testPrint", null, new String[]{TESTMSG_SV}, new String[]{"blabla"}, nameVar);
    nameVar.put(outNumber.getName(), outNumber);
    TestAction numAct = new TestAction("testNum", null, new String[]{UPnPConstants.N_OUT_NUMBER}, null, nameVar);
    nameVar.put(outInt.getName(), outInt);
    TestAction intAct = new TestAction("testInt", null, new String[]{UPnPConstants.N_OUT_INT}, null, nameVar);
    nameVar.put(outChar.getName(), outChar);
    TestAction charAct = new TestAction("testChar", null, new String[]{UPnPConstants.N_OUT_CHAR}, null, nameVar);
    nameVar.put(outFloat.getName(), outFloat);
    TestAction floatAct = new TestAction("testFloat", null, new String[]{UPnPConstants.N_OUT_FLOAT}, null, nameVar);
    ...
    TestAction testALL = new TestAction("testALL", null, new String[]{UPnPConstants.N_IN_STRING, UPnPConstants.N_IN_BOOLEAN, UPnPConstants.N_IN_NUMBER, UPnPConstants.N_IN_INT,
                                                                      UPnPConstants.N_IN_CHAR, UPnPConstants.N_IN_FLOAT}, new String[]{TESTMSG_SV,
                                                                      TESTING_SV, UPnPConstants.N_OUT_NUMBER, UPnPConstants.N_OUT_INT,UPnPConstants.N_OUT_CHAR,
                                                                      UPnPConstants.N_OUT_FLOAT}, nameVar);

    TestAction[] actions = new TestAction[]{testALL, printAct, msgAct, numAct, intAct, charAct, floatAct};
    // init printer service
    TestService[] services = new TestService[] {
      new TestService(actions, vars, bc) {
        public String getId() {
          return SERVICE_ID;
        }
