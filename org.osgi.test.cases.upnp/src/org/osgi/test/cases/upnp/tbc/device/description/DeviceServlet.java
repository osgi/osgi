...
//    logger.log("{DeviceServlet} - Incomming GET request");
    ...
    if ( (uri.equals(UPnPConstants.SR_DESC)) && (xml != null) ) {
      if ( xml.equals("root") ) {
        ...
        InputStream in = DeviceServlet.class.getResourceAsStream("/org/osgi/test/cases/upnp/tbc/resources/" + xml);
    ...
    } else if ( (uri.equals(UPnPConstants.SR_IM)) && (icon != null) ) {
      InputStream in = DeviceServlet.class.getResourceAsStream("/org/osgi/test/cases/upnp/tbc/resources/images/" + icon);
      byte[] bytes = new byte[1024];
...
////      for (xx = en.nextElement(); en.hasMoreElements(); xx = en.nextElement()) {
////        //#warning DUMP
...
////    System.out.println(uri);
    try {
      if (uri.equals(UPnPConstants.SR_CON)) {
        ...
        String ns = man.substring(man.indexOf(UPnPConstants.V_NS) + 3).trim();
        String soap = req.getHeader(ns + "-" + UPnPConstants.N_SOAPACTION);
        String body = getBody(req);
  ...
  private void genError(HttpServletResponse httpservletresponse, int i, String s) throws ServletException, IOException {
  ...
  private Dictionary getActionArguments(String xml, String actionName) {
    ...
    if (env_ns.length() == 0) {
      logger.log("Unable to find xmlns definition for <Envelope> tag in action XML");
      return null;
    }
    String body = extractStringT(xml, "<" + env_ns + ":Body>",  "</" + env_ns + ":Body>");
    if (body.length() == 0) {
    ...
    if (act_ns.length() == 0) {
      logger.log("Unable to find the XML NS for <" + actionName + "> or the tag it self in action XML");
      return null;
    }


    String act_xmlns = extractStringT(body, actionName, ">");
    String arguments = extractStringT(body, ("<" + act_ns + ":" + actionName + act_xmlns + ">"), ("</" + act_ns + ":" + actionName + ">"));

    if (arguments.length() == 0) {
      logger.log("There are no argumets in the invoke XML, although that is a possible scenario the testcase has no actions with no arguments");
      return null;
    }
...
////      }
////    }
////    if (ll == null || !ll.trim().substring(0, ll.indexOf(" ")).equals(UPnPConstants.ENV_ST)) {
////      return null;
////    }
////
////    ll = br.readLine();
...
////  }
////
////  private Dictionary getActionArguments (String body, String serv, String act) throws IOException {
////    Hashtable hash = new Hashtable();
////    BufferedReader br = new BufferedReader (new StringReader( body ));
...
////      }
////
////      String var = ll.substring(ll.indexOf(UPnPConstants.LB) + 1, ll.indexOf(UPnPConstants.RB));
////      if(!ll.startsWith(UPnPConstants.LB + var + UPnPConstants.RB) || !ll.endsWith(UPnPConstants.LCB + var + UPnPConstants.RB)) {
////        break;
////      }
////      String val = ll.substring(("<" + var + ">").length(), ll.indexOf("</" + var + ">"));
////      hash.put(var, val);
////    }
////
////    if( flag ) {
...
////  }
  ...
  private void replyControl (String dsi, String soap, String body, HttpServletResponse res, boolean mposted) throws ServletException, IOException {
    int in = 0;
    ...
    if (args == null) {
      logger.log("Error while parsing invoke XML for action ".concat(actionName));
  ...
  public synchronized void doSubscribe(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
    ...
    try {
      ...
      long time = 1800;
      try {
        time = Long.parseLong(req.getHeader(UPnPConstants.N_TIMEOUT).substring(UPnPConstants.V_SEC.length()));
      ...
      if (sid != null) {
        ...
        if (dsi.indexOf("s") > 0) {
          ...
          Thread th = new Thread(new EventSender(sid, url, logger), "EVENT SENDER #0");
//          th.setName("EVENT SENDER #0");
          th.start();
          return;
        }
        else if (dsi.indexOf("r") > 0) {
          time = 2;
        }
      ...
      if ( eventing ) {
        Thread th = new Thread(new EventSender(sid, url, logger), "EVENT SENDER");
//        th.setName("EVENT SENDER");
  ...
  public synchronized void doUnsubscribe(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
//    logger.log("{DeviceServlet} - Incomming UNSUBSCRIBE request");
  ...
  public void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
    ...
    if(method.equals(UPnPConstants.SUBSCRIBE)) {
      doSubscribe(req, res);
    } else if(method.equals(UPnPConstants.UNSUBSCRIBE)) {
      doUnsubscribe(req, res);
    } else if(method.equals(UPnPConstants.MPOST)) {
      doMPost(req, res);
    } else {
