...
public class TestStarter {
  ...
  public TestStarter (HttpService http, DefaultTestBundleControl logger) throws Exception {
    this.http = http;
    this.logger = logger;
    ...
    http.registerServlet(UPnPConstants.SR_DESC, new DeviceServlet(logger), null, dscontext);
    http.registerServlet(UPnPConstants.SR_IM, new DeviceServlet(logger), null, dscontext);
    http.registerServlet(UPnPConstants.SR_CON, new DeviceServlet(logger), null, dscontext);
    http.registerServlet(UPnPConstants.SR_EV, new DeviceServlet(logger), null, dscontext);
    http.registerServlet(UPnPConstants.SR_PRES, new DeviceServlet(logger), null, dscontext);
