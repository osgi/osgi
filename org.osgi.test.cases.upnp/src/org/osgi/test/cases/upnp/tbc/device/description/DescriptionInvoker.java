...
public class DescriptionInvoker {
  ...
  public void response (String serviceType, String actionName, Dictionary args, boolean mposted, HttpServletResponse res) throws ServletException, IOException {
    hash.clear();
    ...
    else if (actionName.equals(UPnPConstants.ACT_MPOS)) {
      if (! mposted ) {
    ...
    else if (actionName.equals(UPnPConstants.ACT_PIS)) {
      postInSucc (serviceType, actionName, args, res);
    }
    else if (actionName.equals(UPnPConstants.ACT_MPIS)) {
      if (! mposted ) {
        sendError (res, 405, UPnPConstants.ERR_MNA);
        return;
      }
    ...
    else if (actionName.equals(UPnPConstants.ACT_PF)) {
      postFail (serviceType, actionName, args, res);
    }
    else if (actionName.equals(UPnPConstants.ACT_MPF)) {
      if (!mposted) {
        sendError (res, 405, UPnPConstants.ERR_MNA);
        return;
      }
    ...
    else if (actionName.equals(UPnPConstants.ACT_PB)) {
      postBlock (serviceType, actionName, args, res);
    }
    else if (actionName.equals(UPnPConstants.ACT_MPB)) {
      if (! mposted ) {
        sendError (res, 405, UPnPConstants.ERR_MNA);
        return;
      }
    ...
    else if (actionName.equals(UPnPConstants.ACT_QSV)) {
      postQuery(serviceType, actionName, args, res);
    }
  }

  private void postOutSucc(String serv, String act, Dictionary params, HttpServletResponse res) throws ServletException, IOException {
    ...
    try {
      hash.put(UPnPConstants.N_OUT_HEX, Hex.encode(UPnPConstants.V_OUT_HEX.getBytes()));
    } catch(Exception exc) {
  ...
  private void postInSucc(String serv, String act, Dictionary params, HttpServletResponse res) throws ServletException, IOException {
    ...
    if(!cur.equals(UPnPConstants.V_IN_INT)) {
      hash.put(UPnPConstants.N_OUT_OUT, UPnPConstants.V_OUT_OUT_FAILED + " " + UPnPConstants.N_IN_INT);
    }

    cur = (String) params.get(UPnPConstants.N_IN_UI4);
    if(!cur.equals(UPnPConstants.V_IN_UI4)) {
      hash.put(UPnPConstants.N_OUT_OUT, UPnPConstants.V_OUT_OUT_FAILED + " " + UPnPConstants.N_IN_UI4);
    }

    cur = (String) params.get(UPnPConstants.N_IN_NUMBER);
    if(!cur.equals(UPnPConstants.V_IN_NUMBER)) {
      hash.put(UPnPConstants.N_OUT_OUT, UPnPConstants.V_OUT_OUT_FAILED + " " + UPnPConstants.N_IN_NUMBER);
    }

    cur = (String) params.get(UPnPConstants.N_IN_FLOAT);
    if(!cur.equals(UPnPConstants.V_IN_FLOAT)) {
      hash.put(UPnPConstants.N_OUT_OUT, UPnPConstants.V_OUT_OUT_FAILED + " " + UPnPConstants.N_IN_FLOAT);
    }

    cur = (String) params.get(UPnPConstants.N_IN_CHAR);
    if(!cur.equals(UPnPConstants.V_IN_CHAR)) {
      hash.put(UPnPConstants.N_OUT_OUT, UPnPConstants.V_OUT_OUT_FAILED + " " + UPnPConstants.N_IN_CHAR);
    }

    cur = (String) params.get(UPnPConstants.N_IN_STRING);
    if(!cur.equals(UPnPConstants.V_IN_STRING)) {
      hash.put(UPnPConstants.N_OUT_OUT, UPnPConstants.V_OUT_OUT_FAILED + " " + UPnPConstants.N_IN_STRING);
    }

    cur = (String) params.get(UPnPConstants.N_IN_BOOLEAN);
    if(!cur.equals(UPnPConstants.V_IN_BOOLEAN)) {
      hash.put(UPnPConstants.N_OUT_OUT, UPnPConstants.V_OUT_OUT_FAILED + " " + UPnPConstants.N_IN_BOOLEAN);
    }
    ...
    try {
      ...
      if(!str.equals(UPnPConstants.V_IN_HEX)) {
        hash.put(UPnPConstants.N_OUT_OUT, UPnPConstants.V_OUT_OUT_FAILED + " " + UPnPConstants.N_IN_HEX);
      }
    } catch(Exception exc) {
      hash.put(UPnPConstants.N_OUT_OUT, UPnPConstants.V_OUT_OUT_FAILED + " " + UPnPConstants.N_IN_HEX);
    }
  ...
  private void postBlock(String serv, String act, Dictionary params, HttpServletResponse res) throws ServletException, IOException {
    try {
      Thread.sleep(40000L);
  ...
  private void postQuery(String serv, String act, Dictionary params, HttpServletResponse res) throws ServletException, IOException {
  }

  private void postFail(String serv, String act, Dictionary params, HttpServletResponse res) throws ServletException, IOException {
    sendError(res, 500, UPnPConstants.ERR_ISE);
  }


  public void sendOK(String serv, String act, Dictionary params, HttpServletResponse res) throws ServletException, IOException {
    ...
    res.setHeader(UPnPConstants.N_CL, String.valueOf(send.getBytes().length));
...
//    res.flushBuffer();
  ...
  public void sendError(HttpServletResponse res, int errCode, String errDesc) throws ServletException, IOException {
    StringBuffer sb = new StringBuffer();
    sb.append(UPnPConstants.ENV_ST);
    sb.append(UPnPConstants.CRLF);
    sb.append(UPnPConstants.ENV_XMLNS);
...
//    res.setHeader(UPnPConstants.ERROR, "");
    ...
    res.setHeader(UPnPConstants.N_CL, String.valueOf(cont.getBytes().length));
    res.setHeader(UPnPConstants.N_CT, "text/xml; charset=\"utf-8\"");
    res.setHeader(UPnPConstants.N_EXT, "");
    res.setHeader(UPnPConstants.N_SERVER, UPnPConstants.V_SERVER);
