...
public class UPnPTester implements UPnPEventListener {
  ...
  public UPnPTester (DefaultTestBundleControl control, int state, UPnPDevice dev, BundleContext bc) {
    ...
    try {
     ...
     control.log ("UPnPDevice MANUFACTURER: " + test);
      ...
      if(!test.equals(UPnPConstants.V_MANU_URL)) {
        control.log("SERVER: " + udn + UPnPConstants.DD + " Manufacturer URL not matched");
      }
      ...
      if(!test.equals(UPnPConstants.V_MOD_DESC)) {
        control.log("SERVER: " + udn + UPnPConstants.DD + " Model Description not matched");
      }

      test = (String) dict.get(UPnPDevice.MODEL_NAME);
      control.log ("UPnPDevice MODEL_NAME: " + test);
      if(!test.equals(UPnPConstants.V_MOD_NAME)) {
        control.log("SERVER: " + udn + UPnPConstants.DD + " Model Name not matched");
      }
        ...
        for (int i = 0; i < icos.length; i++) {
          ...
          if(icos[i].getHeight() == 16) {
            if(icos[i].getWidth() != 16) {
              control.log ("SERVER: " + udn + UPnPConstants.DD + " Not property width for icon: " + i);
            }
            fis = UPnPTester.class.getResourceAsStream("/org/osgi/test/cases/upnp/tbc/resources/images/small.gif");
          } else if(icos[i].getHeight() == 32){
            if(icos[i].getWidth() != 32) {
              control.log ("SERVER: " + udn + UPnPConstants.DD + " Not property width for icon: " + i);
            }
            fis = UPnPTester.class.getResourceAsStream("/org/osgi/test/cases/upnp/tbc/resources/images/big.gif");
          } else {
            control.log ("SERVER: " + udn + UPnPConstants.DD + " Not property height for icon: " + i);
          }
          if(icos[i].getDepth() != 8) {
            control.log ("SERVER: " + udn + UPnPConstants.DD + " Not property width for icon: " + i);
          }
          if(!icos[i].getMimeType().equals(UPnPConstants.V_MT)) {
            control.log ("SERVER: " + udn + UPnPConstants.DD + " Not property mime type for icon: " + i);
          }
          if(fis != null) {
            try {
              ...
              if(!checkArrays(bfis, bricon)) {
                control.log ("SERVER: " + udn + UPnPConstants.DD + " Icons are not the same: " + i);
              } else {
                control.log ("Compared icons are the same.Icon test is OK");
              }
              ricon.close();
              fis.close();
            } catch(Exception exc) {
              control.log ("SERVER: " + udn + UPnPConstants.DD + " Exception :" + exc + " while icon: " + i);
            }
          } else {
            control.log ("SERVER: " + udn + UPnPConstants.DD + " Unable to get file for icon: " + i);
          }
        }
      } else {
        control.log ("SERVER: " + udn + UPnPConstants.DD + " Wrong Number of Icons");
      }
    } catch (Exception er) {
      er.printStackTrace();
    }
  ...
  private void createControlTest() {
    ...
    try {
      act.invoke(hash);
      control.log ("SERVER: Exception not thrown while invoking: " + UPnPConstants.ACT_PF);
    } catch(Exception exc) {
    ...
    try {
      act.invoke(hash);
      control.log ("SERVER: Exception not thrown while invoking: " + UPnPConstants.ACT_MPF);
    } catch(Exception exc) {
      exc.printStackTrace();
      control.log ("Exception is thrown OK");
    }
    ...
    try {
      act.invoke(hash);
      control.log ("SERVER: Exception not thrown while invoking: " + UPnPConstants.ACT_PB);
    }
    catch(Exception exc) {
      control.log("Exception is thrown OK");
    }
    ...
    try {
      act.invoke(hash);
      control.log ("SERVER: Exception not thrown while invoking: " + UPnPConstants.ACT_MPB);
    }
    catch(Exception exc) {
      control.log("Exception is thrown OK");
    }
    ...
    try {
      ...
      if(!checkRet(dict)) {
        control.log ("SERVER: " + UPnPConstants.ACT_POS + " Invoking Failed");
      }
    } catch(Exception exc) {
    ...
    try {
      ...
      if(!checkRet(dict)) {
        control.log ("SERVER: " + UPnPConstants.ACT_MPOS + " Invoking Failed");
      }
    } catch(Exception exc) {
      control.log("Unexpected Exception");
      exc.printStackTrace();
    ...
    control.log ("Invoking an action Post In Success with invalid arguments");
    try {
      Dictionary dict = act.invoke(hash);
      control.log("SERVER: " + UPnPConstants.ACT_PIS + " Invoke was successful: The UPnP Driver doesn't check action arguments for validity");
    }
    catch(Exception exc) {
      control.log ("Exception is thrown OK");
    }
    ...
    hash.put(UPnPConstants.N_IN_FLOAT, fl);
    hash.put(UPnPConstants.N_IN_NUMBER, duble);
    hash.put(UPnPConstants.N_IN_CHAR, new Character(UPnPConstants.V_IN_CHAR.charAt(0)));
    hash.put(UPnPConstants.N_IN_STRING, UPnPConstants.V_IN_STRING);
    hash.put(UPnPConstants.N_IN_BOOLEAN, new Boolean(UPnPConstants.V_IN_BOOLEAN));
    hash.put(UPnPConstants.N_IN_HEX, UPnPConstants.V_IN_HEX.getBytes());
    ...
    try {
      ...
      if (dict != null) {
        ...
        if (!res.equals(UPnPConstants.V_OUT_OUT_OK)) {
          control.log ("SERVER: " + UPnPConstants.ACT_PIS + " Invoke Failed: " + res);
        }
      }
    } catch(Exception exc) {
      control.log ("SERVER: " + UPnPConstants.ACT_PIS + " Invoke Failed: Threre was an exception");
    }
    ...
    try {
      ...
      if (dict != null) {
        String res = (String) dict.get(UPnPConstants.N_OUT_OUT);
        if(!res.equals(UPnPConstants.V_OUT_OUT_OK)) {
          control.log ("SERVER: " + UPnPConstants.ACT_MPIS + " Invoking Failed: " + res);
        }
      }
    } catch(Exception exc) {
      control.log("Unexpected Exception");
  ...
  private boolean checkRet(Dictionary dict) throws Exception {
    ...
    if (dict != null) {
      ...
      if(st == null || !st.equals(UPnPConstants.V_OUT_HEX)) {
        control.log ("SERVER: " + UPnPConstants.N_OUT_HEX + " value is not ok: " + st);
        state = false;
      }

      Integer inint = (Integer) dict.get(UPnPConstants.N_OUT_INT);
      if(inint == null || (inint.intValue() != Integer.parseInt(UPnPConstants.V_OUT_INT))) {
        control.log ("SERVER: " + UPnPConstants.N_OUT_INT + " value is not ok: " + inint);
        state = false;
      }

      Long lo = (Long) dict.get(UPnPConstants.N_OUT_UI4);
      if(lo == null || (lo.longValue() != Long.parseLong(UPnPConstants.V_OUT_UI4))) {
        control.log ("SERVER: " + UPnPConstants.N_OUT_UI4 + " value is not ok: " + lo);
        state = false;
      }

      Float fl = (Float) dict.get(UPnPConstants.N_OUT_FLOAT);
      if(fl == null || (fl.floatValue() != Float.parseFloat(UPnPConstants.V_OUT_FLOAT))) {
        control.log ("SERVER: " + UPnPConstants.N_OUT_FLOAT + " value is not ok: " + fl);
        state = false;
      }

      Double dl = (Double) dict.get(UPnPConstants.N_OUT_NUMBER);
      if(dl == null || (dl.doubleValue() != Double.parseDouble(UPnPConstants.V_OUT_NUMBER))) {
        control.log ("SERVER: " + UPnPConstants.N_OUT_NUMBER + " value is not ok: " + dl);
        state = false;
      }

      Character ch = (Character) dict.get(UPnPConstants.N_OUT_CHAR);
      if(ch == null || (ch.charValue() != UPnPConstants.V_OUT_CHAR.charAt(0))) {
        control.log ("SERVER: " + UPnPConstants.N_OUT_CHAR + " value is not ok: " + ch);
        state = false;
      }
      ...
      if(bool == null || (bool.booleanValue() != (new Boolean(UPnPConstants.V_OUT_BOOLEAN)).booleanValue())) {
        control.log ("SERVER: " + UPnPConstants.N_OUT_BOOLEAN + " value is not ok: " + bool);
        state = false;
      }
    }
    return state;
  ...
  private void createEventTest() {
    ...
    control.log ("Device gets service urn:prosyst-com:serviceId:EventService");
    es = dev.getService(UPnPConstants.SEV_ID);
    currentServ = es.getId();
    try {
      filter = bc.createFilter("(&(" + UPnPDevice.ID + "=" + udn + ")(" + UPnPService.ID + "=" + UPnPConstants.SEV_ID +"))");
...
//    sr.unregister();
    if(!checkEvents()) {
      control.log ("SERVER: " + udn + UPnPConstants.DD + " Main events are not ok");
    } else {
  ...
  private boolean checkEvents() {
    ...
    try {
      ...
      control.log ("SERVER: " + udn + UPnPConstants.DD + " Exception while checking events: " + exc);
      return false;
    }
    return state;
  }
    ...
    switch(si) {
      case 9: {
        ...
        if(str1 == null || !str1.equals(UPnPConstants.V_OUT)) {
          control.log ("SERVER: " + UPnPConstants.N_OUT + " " + si + " value is not ok: " + str1);
          state = false;
        }
      }
      case 8: {
        ...
        if(st == null || !st.equals(UPnPConstants.V_HEX)) {
          control.log ("SERVER: " + UPnPConstants.N_HEX + " " + si + " value is not ok: " + st);
          state = false;
        }
      }
      case 7: {
        Boolean bool = (Boolean) dict.get(UPnPConstants.N_BOOLEAN);
        if(bool == null || (bool.booleanValue() != (new Boolean(UPnPConstants.V_BOOLEAN)).booleanValue())) {
          control.log ("SERVER: " + UPnPConstants.N_BOOLEAN + " " + si + " value is not ok: " + bool);
          state = false;
        }
      }
      case 6: {
        ...
        if(str == null || !str.equals(UPnPConstants.V_STRING)) {
          control.log ("SERVER: " + UPnPConstants.N_STRING + " " + si + " value is not ok: " + str);
          state = false;
        }
      }
      case 5: {
        Character ch = (Character) dict.get(UPnPConstants.N_CHAR);
        if(ch == null || (ch.charValue() != UPnPConstants.V_CHAR.charAt(0))) {
          control.log ("SERVER: " + UPnPConstants.N_CHAR + " " + si + " value is not ok: " + ch);
          state = false;
        }
      }
      case 4: {
        Float fl = (Float) dict.get(UPnPConstants.N_FLOAT);
        if(fl == null || (fl.floatValue() != Float.parseFloat(UPnPConstants.V_FLOAT))) {
          control.log ("SERVER: " + UPnPConstants.N_FLOAT + " " + si + " value is not ok: " + fl);
          state = false;
        }
      }
      case 3: {
        Double dl = (Double) dict.get(UPnPConstants.N_NUMBER);
        if(dl == null || (dl.doubleValue() != Double.parseDouble(UPnPConstants.V_NUMBER))) {
          control.log ("SERVER: " + UPnPConstants.N_NUMBER + " " + si + " value is not ok: " + dl);
          state = false;
        }
      }
      case 2: {
        Long lo = (Long) dict.get(UPnPConstants.N_UI4);
        if(lo == null || (lo.longValue() != Long.parseLong(UPnPConstants.V_UI4))) {
          control.log ("SERVER: " + UPnPConstants.N_UI4 + " " + si + " value is not ok: " + lo);
          state = false;
        }
      }
      case 1: {
        Integer in = (Integer) dict.get(UPnPConstants.N_INT);
        if(in == null || (in.intValue() != Integer.parseInt(UPnPConstants.V_INT))) {
          control.log ("SERVER: " + UPnPConstants.N_INT + " " + si + " value is not ok: " + in);
          state = false;
        }
      }
    }
  ...
  private void createExportTest () {
    ...
    try {
          ...
          for (int j = 0; j < elem.size(); j++) {
            ...
            if (tagDev.getName().equalsIgnoreCase("serviceList")) {
              ...
              for (int k = 0; k < servs.size(); k ++) {
                ...
                if (tagServ.getName().equalsIgnoreCase("service")) {
                  Vector sss = tagServ.getContent();
                  for (int p = 0; p < sss.size(); p ++) {
                    XMLTag tagSSS = (XMLTag) sss.elementAt(p);
                    Vector sps = tagSSS.getContent();
                    if (tagSSS.hasOnlyText() && sps.size() > 0) {
                      servsHash.put(tagSSS.getName(), sps.elementAt(0));
                    }
                  }
                }
              }
      ...
      String[] values = new String[]{UPnPConstants.V_OUT_STRING, UPnPConstants.V_OUT_BOOLEAN, UPnPConstants.V_OUT_NUMBER,
                                     UPnPConstants.V_OUT_INT, UPnPConstants.V_OUT_CHAR, UPnPConstants.V_OUT_FLOAT};
      control.log ("Creating Post message and send it");
      byte[] post = cp.createPOST(CONTROL_URL, host, port, SERVICE_TYPE, action, args, values);
  ...
  private void eventExportTest () {
    try {
      ...
      byte[] subscr = cp.createSUBSCRIBE(EVENT_SUB_URL, evHost, evPort, callback, 6);
...
//    System.out.println("SCPD: " + str);
    ...
    XMLTag rootTag = parser.getRootXMLTag();
    if (!rootTag.hasOnlyTags()) {
      throw new Exception ("SCPD XML:Root tag has something else except tags");
    }
    Vector content = rootTag.getContent();
    int len = content.size();
    for (int i = 0; i < len; i ++) {
          ...
          if (act.getName().equalsIgnoreCase("action")) {
            ...
            for (int k = 0; k < names.size(); k++) {
              ...
              if(name.getName().equalsIgnoreCase("name")) {
                ...
                hash.put(name.getContent().elementAt(0).toString(), argsNames);
              } else if (name.getName().equalsIgnoreCase("argumentList")) {
                ...
                for (int p = 0; p < args.size(); p++) {
                  ...
                  if (arg.getName().equalsIgnoreCase("argument")) {
                    ...
                    for (int y = 0; y < argsName.size(); y++) {
                      XMLTag xxx = (XMLTag) argsName.elementAt(y);
                      if (xxx.hasOnlyText() && xxx.getName().equalsIgnoreCase("name")) {
                        argname = xxx.getContent().elementAt(0).toString();
                      } else if(xxx.hasOnlyText() && xxx.getName().equalsIgnoreCase("direction")) {
                        String dir = xxx.getContent().elementAt(0).toString();
                        if(dir.equals("in")) {
                          argsNames.addElement(argname);
                          argname = null;
                        }
                      }
                    }
  ...
  private boolean testProps(Dictionary dic) {
    ...
    if(!test.equals(UPnPConstants.E_FN)) {
      control.log ("EXPORTED: " + udn + UPnPConstants.DD + " FRINDLY_NAME not matched");
      state = false;
    }
    ...
    if(!test.equals(UPnPConstants.E_MANU)) {
      control.log ("EXPORTED: " + udn + UPnPConstants.DD + " MANUFACTURER not matched");
      state = false;
    }
    ...
    if(!test.equals(UPnPConstants.E_MANU_URL)) {
      control.log ("EXPORTED: " + udn + UPnPConstants.DD + " MANUFACTURER_URL not matched");
      state = false;
    }
    ...
    if(!test.equals(UPnPConstants.E_MOD_NAME)) {
      control.log ("EXPORTED: " + udn + UPnPConstants.DD + " MODEL_NAME not matched");
      state = false;
    }
    ...
    if(!test.equals(UPnPConstants.E_MOD_NUMB)) {
      control.log ("EXPORTED: " + udn + UPnPConstants.DD + " MODEL_NUMBER not matched");
      state = false;
    }
    ...
    if(!test.equals(UPnPConstants.E_MOD_URL)) {
      control.log ("EXPORTED: " + udn + UPnPConstants.DD + " MODEL_URL not matched");
      state = false;
    }
    ...
    if(!test.equals(UPnPConstants.E_SN)) {
      control.log ("EXPORTED: " + udn + UPnPConstants.DD + " SERIAL_NUMBER not matched");
      state = false;
    }
    ...
    if(!test.equals(UPnPConstants.E_UPC)) {
      control.log ("EXPORTED: " + udn + UPnPConstants.DD + " UPC not matched");
      state = false;
    }
  ...
  public void notifyUPnPEvent(String did, String sid, Dictionary vals) {
    if (!did.equals(udn) || !sid.equals(currentServ)) {
      control.log ("SERVER: Received Event for UDN: " + did + " & SID: " + sid + ".\r\nExpected UDN: " + udn + " SID: " + currentServ);
      return;
    }
