...
public class UPnPControl extends DefaultTestBundleControl {
  ...
  public void prepare () throws Exception {
    ...
    httpRef = getContext().getServiceReference("org.osgi.service.http.HttpService");
    http = (HttpService) getContext().getService(httpRef);
    UPnPConstants.init();
    //UPnPConstants.HTTP_PORT = ((Integer) httpRef.getProperty("openPort")).intValue();
    UPnPConstants.HTTP_PORT = Integer.parseInt( System.getProperty("org.osgi.service.http.port", "80" ) );
	  log ("Register Service Listener to listen for service changes");
    ServicesListener listener = new ServicesListener(getContext());
    getContext().addServiceListener(listener, "(objectClass=org.osgi.service.upnp.UPnPDevice)");
...
//    Thread.sleep(2000);
  ...
  public void testDiscovery () {
	try {
	  UPnPTester device = new UPnPTester (this, UPnPTester.DISCOVERY, ServicesListener.getUPnPDevice(), getContext());
	} catch(Exception er) {
	  er.printStackTrace();
	}
  }
  ...
  public void testControl () {
	try {
	  UPnPTester device = new UPnPTester (this, UPnPTester.CONTROL, ServicesListener.getUPnPDevice(), getContext());
	} catch (Exception er) {
	  er.printStackTrace();
	}
  }
  ...
  public void testEvent () {
	try {
	  UPnPTester device = new UPnPTester (this, UPnPTester.EVENTING, ServicesListener.getUPnPDevice(), getContext());
	} catch (Exception er) {
	  er.printStackTrace();
	}
  }
  ...
  public void testExport () {
	...
	srf = getContext().registerService(UPnPDevice.class.getName(), ex_device, ex_device.getDescriptions(null));
	try {
	  Thread.sleep(4000);
	} catch (Exception er) {
	}
	UPnPTester device = new UPnPTester (this, UPnPTester.EXPORT, ServicesListener.getUPnPDevice(), getContext());
  }
