...
public abstract class TestService implements UPnPService, ServiceListener {
  ...
  public TestService (UPnPAction[] actions, UPnPStateVariable[] variables, BundleContext bc) {
    this.bc = bc;
    acts = new Hashtable();
    vars = new Hashtable();
    
    if (variables == null || variables.length < 1) {
      throw new IllegalArgumentException ("UPnPService must have at least one state variable");
    }
      ...
      if (listeners != null) {
        synchronized(listeners) {
          for (int i = 0; i < listeners.size(); i++) {
            UPnPEventListener tmp = (UPnPEventListener)listeners.elementAt(i);
  ...
  public final void registerListener () {
    ...
    try {
      ...
      ServiceReference[] refs = bc.getServiceReferences(UPnPEventListener.class.getName(), filter.toString());
...
//        synchronized (listeners) {
          for (int i = 0; i < refs.length; i++) {
            ...
            if (f != null && f.match(serviceProps)) {
              UPnPEventListener tmp = (UPnPEventListener)bc.getService(refs[i]);
  ...
  public void serviceChanged(ServiceEvent se) {
    if (se.getType() == ServiceEvent.REGISTERED) {
      synchronized(listeners) {
        ...
        if (f != null && f.match(serviceProps)) {
          UPnPEventListener tmp = (UPnPEventListener)bc.getService(ref);
          listeners.addElement(tmp);
          refLis.put(ref, tmp);
        }
      }
      ...
      synchronized(listeners) {
        ServiceReference ref = se.getServiceReference();
        UPnPEventListener tmp = (UPnPEventListener)refLis.remove(ref);
