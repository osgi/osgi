...
//    buff.append(UPnPConstants.V_ST_ALL);
    ...
    DatagramPacket search = new DatagramPacket(bytes, bytes.length, address, UPnPConstants.UPnPMCPort);
    
    return search;
  }
  
  public byte[] createPOST (String pathCntrl, String host, int port, String servType, String actName, String[] argName, String[] argValue) throws Exception {
    ...
    buf.append(actName + " xmlns:u=" + "\"" + servType + "\"" + UPnPConstants.RB);
    buf.append(UPnPConstants.CRLF);
    if (argName.length != argValue.length) {
      throw new Exception("The number of argument names have to be equal to number of argument values");
    }
...
//    System.out.println("POST: " + buf.toString());
  ...
  public byte[] createMPOST (String pathCntrl, String host, String port, String service, String actName, String[] argName, String[] argValue) throws Exception {
    StringBuffer buf = new StringBuffer();
    ...
    buf.append(UPnPConstants.CRLF);
    buf.append(UPnPConstants.ULB);
    buf.append(actName + " xmlns:u=" + "\"" + service + "\"" + UPnPConstants.RB);
    buf.append(UPnPConstants.CRLF);
    if (argName.length != argValue.length) {
      throw new Exception("The number of argument names have to be equal to number of argument values");
    }
  ...
  public byte[] createSUBSCRIBE (String publisher, String host, int port, String delivery, int dur) {
    StringBuffer buf = new StringBuffer();
