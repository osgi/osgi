...
public class ControlServer implements Runnable {
  ...
  public ControlServer (String host, int port, DefaultTestBundleControl control) throws Exception {
  ...
  private void checkAns (String ans) throws Exception {
    ...
    if (ll.equals("HTTP/1.1 200 OK")) {
      ...
      if (!rootTag.hasOnlyTags()) {
        throw new Exception ("CONTROL SERVER: Root tag has something else except tags");
      }
        ...
        for (int j = 0; j < elems.size(); j ++) {
          ...
          for (int k = 0; k < params.size(); k++) {
            ...
            if (param.hasOnlyText()) {
              hash.put (param.getName(), param.getContent().elementAt(0).toString());
            }
          }
        }
      }
    ...
    } else if (ll.equals("HTTP/1.1 500 Internal Server Error")) {
  ...
  private boolean controlTest(Dictionary val) {
    ...
    if (!str.equals(UPnPConstants.V_OUT_STRING)) {
      control.log ("SERVER: " + UPnPConstants.N_IN_STRING + " value is not ok: " + str);
      test = false;
    }
    ...
    if (!str.equals(UPnPConstants.V_OUT_BOOLEAN)) {
      control.log ("SERVER: " + UPnPConstants.N_IN_BOOLEAN + " value is not ok: " + str);
      test = false;
    }
    ...
    if (!str.equals(UPnPConstants.V_OUT_NUMBER)) {
      control.log ("SERVER: " + UPnPConstants.N_IN_NUMBER + " value is not ok: " + str);
      test = false;
    }
    ...
    if (!str.equals(UPnPConstants.V_OUT_INT)) {
      control.log ("SERVER: " + UPnPConstants.N_IN_INT + " value is not ok: " + str);
      test = false;
    }
    ...
    if (!str.equals(UPnPConstants.V_OUT_CHAR)) {
      control.log ("SERVER: " + UPnPConstants.N_IN_CHAR + " value is not ok: " + str);
      test = false;
    }
    ...
    if (!str.equals(UPnPConstants.V_OUT_FLOAT)) {
      control.log ("SERVER: " + UPnPConstants.N_IN_FLOAT + " value is not ok: " + str);
      test = false;
    }
