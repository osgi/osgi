...
class LStack extends Vector {
	Object peek() { return elementAt(0); }
	synchronized Object pop() { Object o = elementAt(0); removeElementAt(0);  return o; }
...
//      System.out.println("Cycle Stack[" + position + "]: " + stack);
      ...
      if(next == '<') {
        ...
        if(xml.charAt(position) == '/') {
          ...
          if(name == null) {
            throw new XMLException("End of xml reached, but tag not closed");
          }

          char nch = xml.charAt(position);
          if(nch == ' ' || nch == '/') {
            throw new XMLException("Closing tag can't has attributes and be selfclosed.Tag Name: " + name);
          }

          if(stack.isEmpty() || !name.equals((String) stack.peek())) {
            throw new XMLException("Closed not last opened tag.Tag Name: " + name);
          }

          tags.pop();
          stack.pop();
        } else if(xml.charAt(position) == '!') {
          //dtd
          position++;
          _dtd.append("<!");
          ...
          if(nch == ' ') {
            ...
            if(hash == null) {
              throw new XMLException("Error parsing attributes.Tag Name: " + name);
            }
            tag.setHash(hash);
          }
        ...
        while(position < xml.length()) {
            ...
            if((b == '"') && (xml.charAt(position - 1) != '\\')) {
              tag.appendChar(b);
              break;
            } else if((b == '"') && (xml.charAt(position - 1) == '\\') && (xml.charAt(position - 2) == '\\')) {
              tag.appendChar(b);
              break;
            }
            if(b == '\\' && xml.charAt(position + 1) == '\\') {
              tag.appendChar(b);
              position++;
            } else if(b == '\\' && xml.charAt(position + 1) == '"') {
              position++;
    ...
    if(!stack.isEmpty()) {
      throw new XMLException("Some opened tags not closed.Name of last: " + (String) stack.peek());
    }
      ...
      if(ch == ' ') {
        ...
        if(change) {
          ...
          while(position < sb.length()) {
            ...
            if((b == '"') && (sb.charAt(position - 1) != '\\')) {
              break;
            } else if((b == '"') && (sb.charAt(position - 1) == '\\') && (sb.charAt(position - 2) == '\\')) {
              break;
            }
            if(b == '\\' && sb.charAt(position + 1) == '\\') {
              valueAtt.append(b);
              position++;
            } else if(b == '\\' && sb.charAt(position + 1) == '"') {
              position++;
