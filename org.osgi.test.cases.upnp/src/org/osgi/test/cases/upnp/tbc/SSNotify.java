...
public class SSNotify extends Thread {
  ...
  public SSNotify (int port, DefaultTestBundleControl control) throws Exception {
    super("SSNotify");
  ...
  public static byte[] getDocBytes(InputStream in, long length, int timeout) throws IOException {
  ...
  private Hashtable readHeaders(InputStream is, int maxTime) throws IOException {
    String tmp = null;
...
////    System.out.println("line = " + new String(bos.toByteArray()));
  ...
  private Dictionary parseNotify (String ans) throws Exception {
    ...
    for (int i = 0; i < content.size(); i ++) {
      ...
      for (int j = 0; j < elems.size(); j ++) {
        ...
        if (elem.hasOnlyText()) {
          events.put(elem.getName(), elem.getContent().elementAt(0).toString());
        }
      }
    }
  ...
  private boolean checkEvents (Dictionary evs) {
    ...
    if (! evs.get(UPnPConstants.N_IN_BOOLEAN).equals(UPnPConstants.E_BOOLEAN)) {
      test = false;
    }
