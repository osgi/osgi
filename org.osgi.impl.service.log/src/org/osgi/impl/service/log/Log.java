                            ...
                            FrameworkListener
{
    ...
    /**
     * Default constructor. This is constructed by the framework
     * prior to calling the BundleActivator.start method.
     *
     */
    public Log()
    {
     ...
     * ServiceFactories to construct unique service object for each
     * bundle.
     *
     */
    protected void register()
    {
        ...
        logservice = context.registerService(org.osgi.service.log.LogService.class.getName(),
                                             new ServiceFactory() {
                                                 /**
        ...
        logreaderservice = context.registerService(org.osgi.service.log.LogReaderService.class.getName(),
                                                   new ServiceFactory() {
                                                       /**
                                                        * ServiceFactory.getService method.
                                                        */
                                                       public Object getService(Bundle bundle, ServiceRegistration registration)
                                                       {
                                                           LogReaderServiceImpl service = new LogReaderServiceImpl(Log.this, bundle);
    ...
    protected void log(int level, String message,
                       Bundle bundle, ServiceReference reference,
                       Throwable exception)

    {
        if (context != null)    /* in case someone still calls us after we've stopped */
        {
            LogEntry logentry = new LogEntryImpl(level, message, bundle, reference, exception);
    ...
    {
        for (int size = logentries.size(); size >= logSize; size = logentries.size())
        {
    ...
    public void frameworkEvent(FrameworkEvent event)
    {
        ...
        Throwable t = (eventType == FrameworkEvent.ERROR) ? event.getThrowable() : null;

        log(logType, getFrameworkEventTypeName(eventType), event.getBundle(), null, t);
    }
