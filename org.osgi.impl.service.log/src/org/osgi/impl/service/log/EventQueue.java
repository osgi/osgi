...
/**
 * The class manages the event queue to asynchronously deliver
 * LogEntrys to LogListeners.
 *
...
public class EventQueue extends Thread
{
    ...
    */
    public void run()
    {
        while (running)
        {
            try
            {
                ...
                deliverEvent((LogReaderServiceImpl)item[0], (Vector)item[1], (LogEntry)item[2]);
            }
            catch (Throwable t)
            {
            }
     ...
     *
     */
    protected synchronized void publishLogEntry(Vector logreaders, LogEntry logentry)
    {
        Enumeration enum = logreaders.elements();

        while (enum.hasMoreElements())
        {
            LogReaderServiceImpl logreader = (LogReaderServiceImpl)enum.nextElement();
    ...
    /**
     * This method waits until an item is in the queue and then removes
     ...
     */
    protected void deliverEvent(LogReaderServiceImpl logreader, Vector listeners, LogEntry logentry)
    {
