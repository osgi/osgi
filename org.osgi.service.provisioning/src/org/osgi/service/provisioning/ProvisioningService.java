 ...
 * <p>
 * The provisioning information is collected in a <tt>Dictionary</tt> object,
 ...
 * information. The key of the dictionary is a <tt>String</tt> object and the
 * value is a <tt>String</tt> or <tt>byte[]</tt> object. The single
 * exception is the PROVISIONING_UPDATE_COUNT value which is an Integer. The
 * <tt>provisioning</tt> prefix is reserved for keys defined by OSGi, other
 ...
 * register and get the Provisioning Service. The <tt>ServicePermission</tt>
 * is used to limit the bundles that can gain access to the Provisioning
 * Service. There is no check of <tt>Permission</tt> objects to read or modify
...
public interface ProvisioningService {
	 ...
	 * Service Platform. The value must be of type <tt>String</tt>.
	 */
	 ...
	 * provision data provider. The value must be of type <tt>String</tt>.
	 */
	 ...
	 * <tt>Integer</tt>. This key/value pair is also reflected in the
	...
	/**
	 * The key to the provisioning information that contains the location of the
	 * bundle to start with <tt>AllPermission</tt>. The bundle must have be
	...
	/**
	 * MIME type to be stored in the extra field of a <tt>ZipEntry</tt> object
	 * for String data.
	 */
	 ...
	 * <tt>UnsupportedOperationException</tt> to be thrown. Changes must be
	 * done using the <tt>setInformation</tt> and <tt>addInformation</tt>
	 ...
	 * contained in <tt>info</tt>. Any key/value pairs not in <tt>info</tt>
	 * will be removed from the Provisioning Information dictionary. This method
	 * causes the <tt>PROVISIONING_UPDATE_COUNT</tt> to be incremented.
	 * 
	...
	/**
	 * Adds the key/value pairs contained in <tt>info</tt> to the Provisioning
	...
	/**
	 * Processes the <tt>ZipInputStream</tt> and extracts information to add
	 * to the Provisioning Information dictionary, as well as, install/update
	 * and start bundles. This method causes the
	 * <tt>PROVISIONING_UPDATE_COUNT</tt> to be incremented.
	 * 
	 * @param zis the <tt>ZipInputStream</tt> that will be used to add
	 ...
	 *        an <tt>Extra</tt> field that corresponds to one of the four
	 *        defined MIME types (<tt>MIME_STRING</tt>,
	 *        <tt>MIME_BYTE_ARRAY</tt>,<tt>MIME_BUNDLE</tt>, and
	 *        <tt>MIME_BUNDLE_URL</tt>) in will be silently ignored.
