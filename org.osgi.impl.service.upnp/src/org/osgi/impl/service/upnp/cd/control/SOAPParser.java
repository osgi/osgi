...
public class SOAPParser {
	...
	// This method takes the query requestSOAP message and then parse it and returns the
	...
	public String queryReqParse(String xml) {
		...
		if (i != -1 && j != -1 && i != j) {
			String var = xml.substring(i + SOAPConstants.varName.length() + 1, j - 2);
			return var;
		}
		return null;
	}
	...
	public synchronized ParsedRequest controlReqParse(String xml) {
		...
		if (i != -1 && j != -1 && i != j) {
			...
			int startPos =xml.indexOf(SOAPConstants.saUrn) + SOAPConstants.saUrn.length();
			int endPos = xml.indexOf("\"",xml.indexOf(SOAPConstants.saUrn) + SOAPConstants.saUrn.length());
			serviceType = xml.substring(startPos, endPos);
			xml = (xml.substring(endPos + 2, xml.lastIndexOf("</u:" + actionName))).trim();
			...
			while (true) {
				...
				String argName = xml.substring(startArgIndex+1, xml.indexOf(">"));				
				int endArgIndex = xml.indexOf("</" + argName + ">");
				String argValue = xml.substring(startArgIndex + argName.length() + 2, endArgIndex);				
