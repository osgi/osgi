...
public class SSDPParser implements SSDPConstants , Runnable{
	...
	// This method sets data receives through multicast channel or unicast channel.
	...
	void parseData(){		
			...
			try{
				...
				String host = getHeaderValue("HOST:", tokensArray);
					if(host.equals(HOST_IP+":"+HOST_PORT)){
						...
						DatagramPacket packet = new DatagramPacket(errstring, errstring.length, address, port);
						for(int l = 0; l <= 1; l ++){
							datasoc.send(packet);						
						}
					}
		...
		}else if((data.indexOf("412") != -1) || (data.indexOf("ERROR") != -1)){
			System.out.println("error" + data);
		}
	}
	
	// This method validates the headers of M-SEARCH request. And send the M-SEARC response.
	void parseMsearchRequest(String[] tokensArray){
		...
		try{
			if((man != null) && (man.equals(DISCOVER)) && (maxWait != null) && (st != null)){
				...
				}else if(st.indexOf(":device:") != -1) {
					sendReplyForDeviceType(st, maxWait);
				}else if(st.indexOf(":service:") != -1) {
	...
	// This method reads all devices details and calls ssdpAll method by passing device ,embedded 
 	// device or service information.It is for M-SEARCH request with ssdp:all. 
	void sendReplyForSsdpAll(String maxWait){
		for(Enumeration enum = ssdpcomp.allDeviceDetails.elements(); enum.hasMoreElements();){ 
	...
	// This method make M-SEARCH responses for device ,embedded device or service.It is for 
 	// M-SEARCH request with ssdp:all.
	void ssdpAll(DeviceDetails devDet, String maxWait, String[] response, String type){
		...
		if(response.length == 3){
			...
			response[0]	= ssdpcomp.makeMsearchResponse(dt.toString(), ROOTDEVICE, usn, location);
			response[1]	= ssdpcomp.makeMsearchResponse(dt.toString(), uuid, uuid, location);
			usn 		= new String(uuid + "::" + devDet.getDevType());
			response[2]	= ssdpcomp.makeMsearchResponse(dt.toString(), devDet.getDevType(), usn, location);
		}else if(response.length == 2){
			response[0]	= ssdpcomp.makeMsearchResponse(dt.toString(), uuid, uuid, location);
			usn 		= new String(uuid + "::" + devDet.getDevType());
			response[1]	= ssdpcomp.makeMsearchResponse(dt.toString(), devDet.getDevType(), usn, location);
		}else if(response.length == 1){
			usn 		= new String(uuid + "::" + type);
			response[0]	= ssdpcomp.makeMsearchResponse(dt.toString(), type, usn, location);
		}
	...
	void sendReplyForRoot(String maxWait){
		...
		for(Enumeration enum = ssdpcomp.allDeviceDetails.elements(); enum.hasMoreElements();){ 
			...
			if(devDet.isRoot()){
				...
				response[0]	= ssdpcomp.makeMsearchResponse(dt.toString(), ROOTDEVICE, usn, devDet.getLocation());
				sendResponse(response, maxWait);
			}
		}
	}
	...
	void sendReplyForDevice(String uuid, String maxWait){
		...
		if(ssdpcomp.allDeviceDetails.get(uuid) != null){
			DeviceDetails devDet	= (DeviceDetails)ssdpcomp.allDeviceDetails.get(uuid);
			response[0]		= ssdpcomp.makeMsearchResponse(dt.toString(), uuid, uuid, devDet.getLocation());
			sendResponse(response, maxWait);
		}else{
				response[0] = new String("ERROR: DEVICE WITH THIS ID  "+ uuid +"  NOT FOUND");
			sendResponse(response,  maxWait);
		}
	}


	// This method make M-SEARCH responses for perticuler device type and version.			
	void sendReplyForDeviceType(String st, String maxWait){
		...
		for(Enumeration enum = ssdpcomp.allDeviceDetails.elements(); enum.hasMoreElements();){ 
			...
			if(devDet.isDevAvailable(st)){
				...
				response[0]	= ssdpcomp.makeMsearchResponse(dt.toString(), st, usn, devDet.getLocation());
				sendResponse(response, maxWait);				
			}else{
				response[0] = new String("ERROR: DEVICE WITH THIS TYPE AND VERSION "+ st +"  NOT FOUND");
				sendResponse(response,  maxWait);
			}
		}	
	}
	...
	// This method make M-SEARCH responses for perticuler service type and version.
	void sendReplyForServiceType(String st, String maxWait){
		...
		for(Enumeration enum = ssdpcomp.allDeviceDetails.elements(); enum.hasMoreElements();){ 
			...
			if(devDet.isServiceAvailable(st)){
				...
				response[0] = new String("ERROR: SERVICE WITH THIS TYPE AND VERSION "+ st +"  NOT FOUND");
				sendResponse(response,  maxWait);
			}
		}
	}
	...
	// This method provides waiting a random time between random number.and maxwait.
