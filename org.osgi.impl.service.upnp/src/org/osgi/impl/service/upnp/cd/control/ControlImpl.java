...
//import org.osgi.impl.service.upnp.cp.ssdp.*;
...
public class ControlImpl {
	...
	// This method will be called by the HttpServer to send the control/query request,
	// which is coming from the CP side.
	public synchronized void sendHttpRequest(Dictionary headers, String xml ,DataOutputStream dos) {
		...
		try {
			...
			if (!man) {
				...
				if (!manH.substring(0, indexSemi -1).equals(SOAPConstants.httpEnv)) {
					invalidReq = true;
					writedata(SOAPConstants.http + SOAPConstants.ERROR_412, dos);
					return;
				}
		    ...
		    if (soapAction.equals("\"urn:schemas-upnp-org:control-1-0#QueryStateVariable\"")) {
				return;
			} else {
				...
				if (upnpaction == null) {
					...
					new InvokeDeviceCallback(upnpaction,params,maker ,dos,req).start();
				}				
			}
		}catch(Exception e) {
	...
	public class InvokeDeviceCallback extends Thread {
		...
		public InvokeDeviceCallback(UPnPAction action , Dictionary params,SOAPMaker maker,
		DataOutputStream dos, ParsedRequest request) {
			this. action = action;
		...
		public void run() {
			try {				
				...
				Dictionary ConvertedParams = request.getParams(action, outParams);			
				String result = maker.createControlResponseOK(action.getName(), request.getServiceType(), ConvertedParams);				
