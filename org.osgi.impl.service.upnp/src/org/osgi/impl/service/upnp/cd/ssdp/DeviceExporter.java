...
public class DeviceExporter implements Runnable, SSDPConstants{
	...
	DeviceExporter(SSDPComponent comp, UPnPDevice device, UPnPExporter exporter, BundleContext bc){
		ssdpcomp	= comp;
		this.device	= device;
	...
	public void run(){
		if(device != null){
			if(makeDescriptions()){
				...
				System.out.println("Exported device does not have required values");
			}
		}
		multicastsock	= null;
		ssdpinet 	= null;		
	...
	boolean makeDescriptions(){
		...
		String des1 =	"<?xml version=\"1.0\"?>\r\n" + 
				...
				"  <minor>0</minor>\r\n"+
				"</specVersion>\r\n"+
				"<URLBase>"+ "http://" +ssdpcomp.baseURL+"</URLBase>\r\n";
	...
	String makeEmbDescriptions(String[] childUDN1, String location, String server){
		try{
		...
		for(int i = 0; i < childUDN1.length; i++){
			try{	
				ServiceReference[] sref = bc.getServiceReferences("org.osgi.service.upnp.UPnPDevice", 
							"("+UPnPDevice.UDN+"="+childUDN1[i]+")");
				if(sref != null){
					UPnPDevice embDevice1	= (UPnPDevice)bc.getService(sref[0]);
					String tdes = makeDeviceDescription(embDevice1, "emb", location, server);
					if(tdes != null){
						des3 = des3 + tdes;
						Dictionary embprops1	= embDevice1.getDescriptions("en");
						String[] childUDN2 = (String[])embprops1.get("UPnP.device.childrenUDN");
						if(childUDN2 != null){
							String embDes = makeEmbDescriptions(childUDN2, location, server);
							if(embDes != null){
								des3 = des3 + embDes;
							}
						}
						des3 =	des3+ "<presentationURL>"+
							(String)embprops1.get("UPnP.presentationURL")+
							"</presentationURL>\r\n"+
							"</device>\r\n";
					}
				}
	...
	void addDeviceForSsdp(){		
		for(Enumeration enum = notifyDevices.elements(); enum.hasMoreElements();){ 
			DeviceDetails devDet	= (DeviceDetails)enum.nextElement();
			sendDeviceForNotify(devDet);
			
		}
		System.out.println("Sending notify messages is finished for intiall devices");
		
	}

	// This method makes perticuler device desription.
	String makeDeviceDescription(UPnPDevice dev, String type, String location, String server){
		...
		if(props.get("UPnP.device.type") != null){
			des =	des + "<deviceType>"+(String)props.get("UPnP.device.type")+"</deviceType>\r\n";
		...
		if(props.get("UPnP.device.friendlyName") != null){
			des =	des + "<friendlyName>"+
				(String)props.get("UPnP.device.friendlyName")+"</friendlyName>\r\n";
		}else{
			return null;
		}
		if(props.get("UPnP.device.manufacturer") != null){
			des =	des + "<manufacturer>"+
				(String)props.get("UPnP.device.manufacturer")+"</manufacturer>\r\n";
		}else{
			return null;
		}
		if(props.get("UPnP.device.manufacturerURL") != null){
			des =	des + "<manufacturerURL>"+
				(String)props.get("UPnP.device.manufacturerURL")+"</manufacturerURL>\r\n";
		}
		
		if(props.get("UPnP.device.modelDescription") != null){
			des =	des + "<modelDescription>"+
				(String)props.get("UPnP.device.modelDescription")+"</modelDescription>\r\n";
		}
		if(props.get("UPnP.device.modelName") != null){
			des =	des + "<modelName>"+
				(String)props.get("UPnP.device.modelName")+"</modelName>\r\n";
		}else{
			return null;
		}
		if(props.get("UPnP.device.modelNumber") != null){
			des =	des + "<modelNumber>"+
				(String)props.get("UPnP.device.modelNumber")+"</modelNumber>\r\n";
		}
		if(props.get("UPnP.device.modelURL") != null){
			des =	des + "<modelURL>"+
				(String)props.get("UPnP.device.modelURL")+"</modelURL>\r\n";
		}
		if(props.get("UPnP.device.serialNumber") != null){
			des =	des + "<serialNumber>"+
				(String)props.get("UPnP.device.serialNumber")+"</serialNumber>\r\n";
		}
		if(udn != null){
			des = des + "<UDN>"+ udn +"</UDN>\r\n";
		}else{
		...
		if(props.get("UPnP.device.UPC") != null){
			des = des + "<UPC>"+(String)props.get("UPnP.device.UPC")+"</UPC>\r\n";
		}
	...
	String makeServicesDescription(UPnPService[] services, String udn, DeviceDetails devDet){		
		...
		for(int i = 0; i < services.length; i++){
			...
			if((type != null) && (id != null)){
				servDes	=	servDes + "<service>\r\n"+
					...
					"<SCPDURL>"+"/samsungcd/"+ scpd + "</SCPDURL>\r\n"+
					"<controlURL>"	+ "http://" + ssdpcomp.genaURL + 
					"/" +scpd + "</controlURL>\r\n"+
				...
				if(SCPD != null){
					...
					exporter.addEventEntry("http://" + ssdpcomp.genaURL +"/" +scpd, services[i]);
					exporter.addControlEntry("http://" + ssdpcomp.genaURL +"/" +scpd, services[i]);
				}
			}
		}
	...
	String makeActionsDescription(UPnPService service, UPnPAction[] actions){
		...
		for(int i = 0; i < actions.length; i++){
			...
			actionDes		= 	actionDes + 
								"<action>\r\n" + 
								"<name>" + actions[i].getName() + "</name>\r\n" ;
 
			if (inargs != null || outargs != null ) {		
				actionDes = actionDes + "<argumentList>\r\n";
 				if (inargs != null) {					
					for(int in = 0; in < inargs.length; in++){
						UPnPStateVariable var = actions[i].getStateVariable(inargs[in]);
						if(var != null){
							actionDes = 	actionDes + 
									"<argument>\r\n" +
									"	<name>"+ inargs[in] + "</name>\r\n"+
									"	<direction>in</direction>\r\n"+
									"	<relatedStateVariable>" + var.getName() + "</relatedStateVariable>\r\n"+
									"</argument>\r\n";
							//variables.add(var);
						}
					}
				...
				if (outargs != null) {
					for(int ou = 0; ou < outargs.length; ou++){
						UPnPStateVariable var = actions[i].getStateVariable(outargs[ou]);
						if(var != null){
							actionDes = 	actionDes + 
										"<argument>\r\n" +
	...
	String makeStatevarDescription(Vector stVars){
		...
		for(Enumeration enum = stVars.elements(); enum.hasMoreElements();){
			...
			varsDes	=	varsDes + 
							...
							"	<defaultValue>" + var.getDefaultValue() + "</defaultValue>\r\n";
			...
			if(allowedVals != null){
				...
				for(int i = 0; i < allowedVals.length; i++){
					varsDes	= varsDes + 
							"		<allowedValue>" + allowedVals[i] + "</allowedValue>\r\n";
				...
				if((max != null) && (min != null)){
					...
					if(type.equals("java.lang.Integer")){
						varsDes = 	varsDes + 
								"		<minimum>" + min.intValue() + "</minimum>\r\n"+
								"		<maximum>" + max.intValue() +"</maximum>\r\n"+
								"		<step>" + step.intValue() + "</step>\r\n"; 
					}else if(type.equals("java.lang.Short")){
						varsDes = 	varsDes + 
								"		<minimum>" + min.shortValue() + "</minimum>\r\n"+
								"		<maximum>" + max.shortValue() +"</maximum>\r\n"+
								"		<step>" + step.shortValue() + "</step>\r\n"; 
					}else if(type.equals("java.lang.Long")){
						varsDes = 	varsDes + 
								"		<minimum>" + min.longValue() + "</minimum>\r\n"+
								"		<maximum>" + max.longValue() +"</maximum>\r\n"+
								"		<step>" + step.longValue() + "</step>\r\n"; 
					}else if(type.equals("java.lang.Byte")){
						varsDes = 	varsDes + 
								"		<minimum>" + min.byteValue() + "</minimum>\r\n"+
								"		<maximum>" + max.byteValue() +"</maximum>\r\n"+
								"		<step>" + step.byteValue() + "</step>\r\n"; 
					}else if(type.equals("java.lang.Float")){
						varsDes = 	varsDes + 
								"		<minimum>" + min.floatValue() + "</minimum>\r\n"+
								"		<maximum>" + max.floatValue() +"</maximum>\r\n"+
								"		<step>" + step.floatValue() + "</step>\r\n"; 
					}else if(type.equals("java.lang.Double")){
						varsDes = 	varsDes + 
								"		<minimum>" + min.doubleValue() + "</minimum>\r\n"+
								"		<maximum>" + max.doubleValue() +"</maximum>\r\n"+
								"		<step>" + step.doubleValue() + "</step>\r\n"; 
					}
	...
	String makeIconsDescription(UPnPIcon[] icons, String udn){
		...
		for(int i = 0; i < icons.length; i++){
			...
			if((width != -1) && (height != -1) && (depth != -1) && (mime != null) && (in != null)){
	...
	public void sendDeviceForNotify(DeviceDetails devDet){
		...
		if(devDet.isRoot()){
			sendNotify("root",uuid, devDet.getDevType(), null, devDet.getLocation(), devDet.getServer());
		}else{
			sendNotify("embdev",uuid, devDet.getDevType(), null, devDet.getLocation(), devDet.getServer());
		}
	...
	// This method sends notify messages according input parameters. It sets the expiraion time
	...
	String makeNotifyMessage(String nt, String usn, String location, String server){		
		notifyMessage = new String(
							NOTIFY + "\r\n" + 
							HOST +"\r\n" + 
							CACHE + " max-age = " + ssdpcomp.cacheControl + "\r\n" + 
							LOC + " " + location +"\r\n"+
							NT + " " + nt + "\r\n"+ 
							NTS + " " + ALIVE + "\r\n" +
							SERVER + " " + server + "\r\n" + 
