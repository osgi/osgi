...
class SendEvents extends Thread {
	...
	// Run method of the thread. converts the statevariables to the respective xml format and
 	// if mulitiple callbacks are mentioned, sends the output to all the callbacks.
	public void run() {
    ...
    // messages using tcp/ip socket.
   	boolean notifySubscribers (String callback, Subscription subscription ,String xml) {
		...
		try {
			...
			dos = new DataOutputStream(new BufferedOutputStream(eventSocket.getOutputStream(),1024));
	...
	void formEventMessage(Subscription subscription , int content_length) {
		...
		if(path.trim().length()>0) {
			message = GenaConstants.GENA_NOTIFY + " "+ path + " " + GenaConstants.GENA_SERVER_VERSION+"\r\n";
		}else {
			message = GenaConstants.GENA_NOTIFY + " "+ callback + " " + GenaConstants.GENA_SERVER_VERSION+"\r\n";				
		}
		...
		message = 
			...
			GenaConstants.GENA_SID + ": " + subscription.getSubscriptionId() +"\r\n" +
			GenaConstants.GENA_SEQ + ": "+ subscription.getEventkey() + "\r\n\r\n";	
		
	}

	// This method creates a socket and returns it. Based on the given path, extracts the host
    // and the port for  creating the socket.If any error occurs while creating the socket, 
	...
	// This method is used to convert the given statevariables to an appropriate xml format
