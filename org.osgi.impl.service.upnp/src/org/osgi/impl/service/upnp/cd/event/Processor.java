...
public final class Processor implements Runnable {
	...
	Processor(GenaServer genaServer, Socket client) {
  	...
  	// Run method of the processor thread.  parses the given request and sets the appropriate
	// headers . This object will be passed to the servlet request object to fullfill servlet
	// httprequest functionality. Creates a http response object based on the given request
	// and processes the request and sends the output back to the client.
	public void run() {
		BufferedInputStream ins = null;
    	try {    	
    		dos = new DataOutputStream(new BufferedOutputStream(client.getOutputStream(),1024));    	
	...
	// This function is used for setting the outbuffer with the proper error message
	private void errorOutput(int i, String message) {
		try {
			...
			String errorResponse = 
				...
				System.getProperty("os.version")+ " UPnP/1.0 SamsungUPnP/1.0\r\n\r\n";			
	...
	public void processRequest() {
		...
		if(reqMethod.equals("subscribe")) {		
			if(reqParser.headers.get("nt").equals("upnp:event") && reqParser.headers.get("callback") != "") {
				insertHeaderValue();	
			}
			else {
			...
			if(reqParser.headers.get("sid") != null){			
				...
				SubscriptionCheck sc = new SubscriptionCheck(dos, reqParser.headers,genaServer.context);
				sc.subscribe();
				sc = null;
				return;
			}
	...
	void processSoap(){		
		reqParser.headers.put(reqParser.reqMethod.toLowerCase() , reqParser.reqUriPath);
		genaServer.cti.sendHttpRequest(reqParser.headers,reqParser.contentBody ,dos);
	}
}
