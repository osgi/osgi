...
public class EventServiceImpl implements EventService {
    ...
    public EventServiceImpl(UPnPController contr,String IP) {
   	...
   	public void subscribe(String publisherpath, String hostString, 
		...
		if(publisherpath == null || publisherpath.trim().length()==0) {
   		...
   		Thread t = new Thread(new SendSubscription(publisherpath, hostString, timeout,listener));
   		t.start();
   	}

  
   // Method used for unsubscribing a subscription. If a valid subscription id is passed, 
   // unsubscribes from the evening layer so that no more notifications will be send to that
   ...
   // For renewing a subscription , this method is used. Accepts two arguments, subscription
   // id and timeout period. 
    ...
    // Http stack receives it and passes the data to this method. This method inturn notifies
	...
	// This class notifies all the client with the UPnPEvent object by calling serviceStateChanged method.
	public class NotifyListeners extends Thread { 		
		...
		public NotifyListeners(UPnPListener listener,UPnPEvent upnpEvent) {
   		...
   		// This method calls the serviceStateChanged from all the subscribed listeners.
		public void run() {
