...
public class Document {	
	...
	// This constructor creates the document object based on the given arguments.
	public Document(String url, boolean file, BundleContext bc)   {
	    ...
	    if(file == true) {
            ...
            fullPath = fullPath.substring(fullPath.indexOf(File.separator),fullPath.length());
	        ...
	        try { 
		        ...
		        fis.close();
		    } catch(Exception eI){System.out.println(eI.getMessage());}        
	    }
	    else { xmlFile = url; }
	...
	// This method checks for the schema,stylesheet, comment , prolog or a dtd declarations at the top of the xml document.
    void checkSchemaOrDtd(String cont) {
		...
		else if(cont.substring(xmlPtr,xmlPtr+3).equals("!--")) {
			chec = true;
		}
		else if(cont.substring(xmlPtr,xmlPtr+3).equals("xml")) {
			chec = true;
		}
		else if(cont.substring(xmlPtr,xmlPtr+15).equals("?xml-stylesheet")) {
 			chec = true;
		}
		else if(cont.substring(xmlPtr,xmlPtr+14).equals("xsl:stylesheet")) {
			 chec = true;
 		}
	...
	void doParse(String xmlFile) throws Exception  {
    	try {
			...
			if(tmpVal.equals("<?xml")) {
 			...
 			move(xmlFile);
			xmlPtr++;
			checkSchemaOrDtd(xmlFile);  // Checks for all the schema and dtd declarations.
			value = xmlFile.charAt(xmlPtr);
			while(value != '>') {  // Fetches the root element name and root element attributes.
				if(rflag == true) {
			...
			while(value != '<') {  // eliminates white spaces after the root element.
				value = xmlFile.charAt(xmlPtr);
				xmlPtr++;
			}
			xmlPtr--;
			try {
 			...
 			makeDeviceOrService(); // Identifies whether xml file is of type device or service.
		}catch(Exception e) {System.out.println(e.getMessage());}
  	}

	// This method loads the xml file to a device object or a service object based on the given xml document.
	...
	// This method creates the Device object and loads all the details from the xml file.
 	void distributeDevice() throws Exception  {
		...
		try {
			...
			for(int ii=0;ii<v.size();ii++){
				...
				else if(ename.equals("friendlyName")){
                    rootDevice.setFriendlyName(eval);
				}
				else if(ename.equals("manufacturer")){
					rootDevice.setManufacturer(eval);
				}
				else if(ename.equals("manufacturerURL")){
					rootDevice.setManufacturerURL(eval);
				}
				else if(ename.equals("modelDescription")){
					rootDevice.setModelDescription(eval);
				}
				else if(ename.equals("modelName")){
					rootDevice.setModelName(eval);
				}
				else if(ename.equals("modelNumber")){
					rootDevice.setModelNumber(eval);
				}
				else if(ename.equals("modelURL")){
					rootDevice.setModelURL(eval);
				}
				else if(ename.equals("serialNumber")){
					rootDevice.setSerialNumber(eval);
				}
				else if(ename.equals("UDN")){
					rootDevice.setUDN(eval);
				}
				else if(ename.equals("UPC")){
					rootDevice.setUPC(eval);
				}
				else if(ename.equals("iconList")){
 					Vector v1 = getElementsByTagName("iconList");
					Element elment = (Element) v1.elementAt(0);
					v1  = elment.getAllElements();
					Icon iarr[] = new Icon[v1.size()];
					for(int iconctr=0;iconctr<iarr.length;iconctr++){
						iarr[iconctr] = new Icon();
					}
					//sets the values for all the icons.
					for(int iconctr=0;iconctr<v1.size();iconctr++){
						elment = (Element)v1.elementAt(iconctr);
						Vector vvec = elment.getAllElements();
						for(int j=0;j<vvec.size();j++){
							Element elmt = (Element) vvec.elementAt(j);
							String enam = elmt.getName();
							String eva  = elmt.getValue().trim();
							if(enam.equals("mimetype")){
								iarr[iconctr].setMimeType(eva);
							}
							else if(enam.equals("width")) {
								iarr[iconctr].setWidth(Integer.parseInt(eva));
							}
							else if(enam.equals("height")) {
								iarr[iconctr].setHeight(Integer.parseInt(eva));
							}
							else if(enam.equals("depth")){
								iarr[iconctr].setDepth(Integer.parseInt(eva));
							}
							else if(enam.equals("url")){
								iarr[iconctr].setURL(eva);
							}
						}
					}
					rootDevice.setIcons(iarr);
 				}
 				//sets the values for all the services.
				else if(ename.equals("serviceList")){
					Vector v1 = getElementsByTagName("serviceList");
					Element elment = (Element) v1.elementAt(0);
					v1  = elment.getAllElements();
					ServiceInfo sarr[] = new ServiceInfo[v1.size()];
					for(int servctr=0;servctr<sarr.length;servctr++){
						sarr[servctr] = new ServiceInfo();
					}
					for(int servctr=0;servctr<v1.size();servctr++){
						elment = (Element)v1.elementAt(servctr);
						Vector allservs = elment.getAllElements();
						for(int sctr=0;sctr<allservs.size();sctr++){
							Element elmt = (Element) allservs.elementAt(sctr);
							String enam = elmt.getName();
							String eva  = elmt.getValue();
							if(enam.equals("serviceType")) {
								sarr[servctr].setServiceType(eva);
							}
							if(enam.equals("serviceId")){
								sarr[servctr].setServiceID(eva);
							}
							if(enam.equals("SCPDURL")){
								sarr[servctr].setSCPDURL(eva);
							}
							if(enam.equals("controlURL")){
								sarr[servctr].setControlURL(eva);
							}
							if(enam.equals("eventSubURL")){
								sarr[servctr].setEventSubURL(eva);
							}
						}
					}
					rootDevice.setServices(sarr,rootDevice);
				}
				// sets the values for the embedded devices.
				else if(ename.equals("deviceList")){
					Vector dlist = getElementsByTagName("deviceList");
					if(dlist.size()>0){
						Element el1 = (Element)dlist.elementAt(0);
						Vector embdevices = el1.getAllElements();
						RootDevice eDevices[] = new RootDevice[embdevices.size()];
						for(int edevctr=0;edevctr<embdevices.size();edevctr++){
							eDevices[edevctr] = new RootDevice(rootDevice,bcc1);
							eDevices[edevctr].setMajor(rootDevice.getMajor());
							eDevices[edevctr].setMinor(rootDevice.getMinor());
							eDevices[edevctr].setURLBase(rootDevice.getURLBase());
							Element el2 = (Element) embdevices.elementAt(edevctr);
							addEmbeddedDevice(el2,eDevices[edevctr]);
						}
						rootDevice.setEmbeddedDevices(eDevices);
					}
				}
				else if(ename.equals("presentationURL")){
					rootDevice.setPresentationURL(eval);
				}
			}
		}catch(Exception e){System.out.println(e.getMessage());}
    }
   	...
   	void addEmbeddedDevice(Element el, RootDevice dev) throws Exception {
		...
		try {
			...
			for(int ii=0;ii<v.size();ii++){
				...
				//sets the values for the icons.
				else if(ename.equals("iconList")){
 					Vector v1 = getElementsByTagName("iconList");
					Element elment = (Element) v1.elementAt(ctr);
					v1  = elment.getAllElements();
					Icon iarr[] = new Icon[v1.size()];
					for(int ictr=0;ictr<iarr.length;ictr++){
						iarr[ictr] = new Icon();
					}
					for(int ictr=0;ictr<v1.size();ictr++){
						elment = (Element)v1.elementAt(ictr);
						Vector allIcons = elment.getAllElements();
						for(int iconctr=0;iconctr<allIcons.size();iconctr++){
							Element elmt = (Element) allIcons.elementAt(iconctr);
							String enam = elmt.getName();
							String eva  = elmt.getValue().trim();
 							if(enam.equals("mimetype")){
								iarr[ictr].setMimeType(eva);
							}
							else if(enam.equals("width")) {
								iarr[ictr].setWidth(Integer.parseInt(eva));
							}
							else if(enam.equals("height")) {
								iarr[ictr].setHeight(Integer.parseInt(eva));
							}
							else if(enam.equals("depth")){
								iarr[ictr].setDepth(Integer.parseInt(eva));
							}
							else if(enam.equals("url")){
								iarr[ictr].setURL(eva);
							}
						}
					}
					dev.setIcons(iarr);
 				}
 				//sets the values for the services of the embedded device.
				else if(ename.equals("serviceList")){
					Vector v1 = getElementsByTagName("serviceList");
					Element elment = (Element) v1.elementAt(ctr+1);
					v1  = elment.getAllElements();
					ServiceInfo sarr[] = new ServiceInfo[v1.size()];
					for(int sctr=0;sctr<sarr.length;sctr++){
						sarr[sctr] = new ServiceInfo();
					}
					for(int sctr=0;sctr<v1.size();sctr++){
						elment = (Element)v1.elementAt(sctr);
						Vector vvec = elment.getAllElements();
						for(int servicctr=0;servicctr<vvec.size();servicctr++){
							Element elmt = (Element) vvec.elementAt(servicctr);
							String enam = elmt.getName();
							String eva  = elmt.getValue();
							if(enam.equals("serviceType")) {
								sarr[sctr].setServiceType(eva);
							}
							if(enam.equals("serviceId")){
								sarr[sctr].setServiceID(eva);
							}
							if(enam.equals("SCPDURL")){
								sarr[sctr].setSCPDURL(eva);
							}
							if(enam.equals("controlURL")){
								sarr[sctr].setControlURL(eva);
							}
							if(enam.equals("eventSubURL")){
								sarr[sctr].setEventSubURL(eva);
							}
						}
					}
					dev.setServices(sarr,rootDevice);
				}
				// checks for embedded device of a device.
				else if(ename.equals("deviceList")){
 					Vector v1 = getElementsByTagName("deviceList");
					Element ee1 =null;
  					ee1 = (Element)v1.elementAt(++ctr);
					Vector v2 = ee1.getAllElements(); 
					RootDevice eDev[] = new RootDevice[v2.size()];
					if(v2.size()==0){
						eDev = null;
					}
					
					for(int edctr=0;edctr<v2.size();edctr++){ 
						Element ee2 = (Element) v2.elementAt(edctr);
 						eDev[edctr] = new RootDevice(rootDevice,bcc1);
						addEmbeddedDevice(ee2, eDev[edctr]);
					}
					dev.setEmbeddedDevices(eDev);
				}
				else if(ename.equals("presentationURL")){
					dev.setPresentationURL(eval);
				}
			}
		}catch(Exception e){ 			
	...
	// This method creates the ServiceDescription object and loads all the details from the xml file.
 	void distributeService() throws Exception {
		...
		for(int j=0;j<v3.size();j++){
			...
			for(int k=0;k<v4.size();k++){
				...
				if(el4.getName().equals("name")){					
				...
				else if(el4.getName().equals("argumentList")){
					...
					for(int argument=0;argument<v5.size();argument++){
						...
						for(int argList=0;argList<v6.size();argList++){
							...
							else if(ena.equals("direction")){
								args[argument].setDirection(eva);
							}
							else if(ena.equals("retval")){
								args[argument].setReturnValue(eva);
							}
							else if(ena.equals("relatedStateVariable")){
								args[argument].setRelatedStateVariable(eva);
							}							
						}	
					}
		...
		for(int j=0;j<v2.size();j++){
			...
			for(int se=0;se<v4.size();se++){
				...
				else if(ena.equals("dataType")){
					sv[j].setDataType(eva);
				}
				else if(ena.equals("defaultValue")){
					sv[j].setDefaultValue(eva);
				}
				else if(ena.equals("allowedValueList")){
				...
				else if(ena.equals("allowedValueRange")){
					Vector v5 = el3.getAllElements();
					/*for(int k1=0;k1<v5.size();k1++){
						Element el4 = (Element) v5.elementAt(k1);
						String allowedrange = el4.getName();
						if(allowedrange.equals("maximum")){
						...
						else if(allowedrange.equals("minimum")){
                            ...
                            if(minval.indexOf('.')!=-1){
                               ...
                               nor = new Double(minval);
                            }
							sv[j].setMinimum(nor);
						}
						else if(allowedrange.equals("step")){
							String stepval = el4.getValue().trim();
                            Number nor ;
                            if(stepval.indexOf('.')!=-1){
                            	nor = new Long(stepval);
                            }
                            else {
                            	nor = new Double(stepval);
                            }
							sv[j].setStep(nor);							
						}
					}*/
	...
	void attachAttribs(String aval, Vector attributes){
		...
		Attribute attr = new Attribute(aval.substring(0,ptr),aval.substring(ptr+2,aval.length()));
		attributes.addElement(attr);
	}

	// This method creates a tree of all the child elements based on the given xml document.
				...
				if(strVal.charAt(strVal.length()-1)=='/') {
					...
					if(strVal.length() >3) {
						if(strVal.substring(0,3).equals(">>>")) {
 							prevE.setValue(obj[0].substring(3,obj[0].length()));
							disFlag = false;
						}
					}
					if(disFlag) {
	...
	// This method eliminates the white space, tab chars, new line and other escape sequences.
		...
		// identifies if there is a comment.
 		if(commt) {
			while(cmt) {
				...
				value = cont.charAt(xmlPtr);
				if(value == '-') {
					if(cont.charAt(xmlPtr+1)=='-' && cont.charAt(xmlPtr+2)=='>'){
						xmlPtr+=3;
						cmt = false;
						break;
					}
	...
	String elemen = "";
     while(value != '<') {
	  	value = cont.charAt(xmlPtr);
  		if(value =='<' && cont.charAt(xmlPtr+1) =='!' && cont.charAt(xmlPtr+2) == '-' &&
	...
	// This method returns all the Elements of the xml file which matches the given tag name
   public Vector getElementsByTagName(String tag)   {
	...
	// This method checks for the matching tag name in the loop. And which ever tag matches it adds
   ...
   public ServiceInfo getServiceInfo()   {
	...
	// This method returns the type of the xml document, whether it is of type device or service.
