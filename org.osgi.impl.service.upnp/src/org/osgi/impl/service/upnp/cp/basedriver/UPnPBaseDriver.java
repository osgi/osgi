...
public class UPnPBaseDriver implements UPnPDeviceListener{
	...
	// This constructor creates the UPnPBaseDriver object based on the controller and the BundleContext object.
	...
	// This method starts the base driver. And registers with controller for getting notifications.
	public void start(){
	...
	// This method stops the base driver. And unregisters with controller for getting notifications.
	public void stop(){
	...
	synchronized public void addDevice(String uuid, RootDevice deviceinfo){
		...
		if(devinfo != null){
			...
			try{
				...
				ServiceRegistration sr = bc.registerService(names, upnpdevice, props);				
	...
	// This method is called whenever a embeded device is found from the CD.
   	// regEmbedded method gets the properties of the new embeded device and then registers
   	// the service in the osgi framework.
	public void regEmbedded(RootDevice[] sembdevices){
		...
		for(int i = 0; i < sembdevices.length; i++){
			...
			if(devices.get(euuid) == null){
				...
				try{
					upnpdevice = new UPnPDeviceImpl(this, sembdevices[i], props, bc);
					System.out.println("REGISTERING UPnP DEVICE");
					ServiceRegistration sr = bc.registerService("org.osgi.service.upnp.UPnPDevice", upnpdevice, props);					
	...
	Properties getDeviceProps(Properties props, RootDevice devinfo){
		...
		if(devinfo.getManufacturerURL() != null){
			props.put(UPnPDevice.MANUFACTURER_URL,devinfo.getManufacturerURL());
		}
		if(devinfo.getModelDescription() != null){
			props.put(UPnPDevice.MODEL_DESCRIPTION,devinfo.getModelDescription());
		}
		...
		if(devinfo.getPresentationURL() != null){
			props.put(UPnPDevice.PRESENTATION_URL,devinfo.getPresentationURL());
		}
		return props;
	}
	...
	synchronized public void removeDevice(String uuid){
		try{
			if(servicerefs.get(uuid) != null){				
				ServiceRegistration sreg = (ServiceRegistration)servicerefs.get(uuid);
				if(sreg != null){
					UPnPDevice rootdev = (UPnPDevice)devices.get(uuid);
					if(rootdev != null){
						Dictionary props	= rootdev.getDescriptions("en");
						String[] childUDN	= (String[])props.get(UPnPDevice.CHILDREN_UDN);
						if(childUDN != null){
