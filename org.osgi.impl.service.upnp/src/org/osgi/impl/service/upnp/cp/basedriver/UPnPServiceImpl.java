...
public class UPnPServiceImpl implements SamsungUPnPService, ServiceTrackerCustomizer, UPnPListener{
    ...
    // This constructor creates the UPnPServiceImpl object based on the given details.
	UPnPServiceImpl(ServiceInfo serviceinfo, UPnPBaseDriver	basedriver, String devid, String devtype, BundleContext bc){
		...
		if(serviceinfo != null){
			...
			if(actionss != null){
				...
				for(int i = 0; i < actionss.length; i++){
					UPnPActionImpl upnpaction = new UPnPActionImpl(actionss[i], basedriver, this);
			...
			if(vars != null){
				...
				for(int j = 0; j < vars.length; j++){
					UPnPStateVariableImpl upnpvar = new UPnPStateVariableImpl(vars[j]);
					variables[j] = upnpvar;
				}
			}		
			stk = new ServiceTracker(bc, UPnPEventListener.class.getName(), this);
			stk.open();
		}
	}
    ...
    // This method called by service tracker when ever it tracks the log Service added.It subscribes
   ...
   // This method called by service tracker when ever it tracks log Service removed
 ...
 // This method is called whenever the service state is been changed.
	...
	public void subscribe(){		
		if(basedriver.eventservice != null){
			try{
				basedriver.eventservice.subscribe(eventUrl, urlbase, this, "infinite");
			}catch(Exception e){System.out.println(e.getMessage());}
		}
	}
