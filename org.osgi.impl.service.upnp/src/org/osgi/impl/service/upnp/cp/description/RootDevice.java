...
public class RootDevice {
	...
	// This method returns all the services which are matching the eventSubURL of the services.
	public ServiceInfo getServiceBasedOnURL(String urlValue){
		...
		for(int servCtr=0;servCtr<serv.length;servCtr++){
			if(serv[servCtr].getEventSubURL().trim().equalsIgnoreCase(urlValue)){
				return serv[servCtr];
			}
		}
		return null;
	...
	// This method sets the URLBase for the device object from the device xml file.
	public void setURLBase(String base) {
		urlbase = base;
	}
	
	// This method sets the major version for the device object from the device xml file.
	public void setMajor(int ma) {		
		major = ma;
	}
	
	// This method sets the minor version for the device object from the device xml file.
