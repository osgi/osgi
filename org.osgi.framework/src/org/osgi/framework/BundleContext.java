 ...
 * 
 * <p>
 * <tt>BundleContext</tt> methods allow a bundle to:
 * <ul>
 * <li>Subscribe to events published by the Framework.
 * <li>Register service objects with the Framework service registry.
 * <li>Retrieve <tt>ServiceReferences</tt> from the Framework service
 * registry.
 ...
 * <li>Create <tt>File</tt> objects for files in a persistent storage area
 * provided for the bundle by the Framework.
 * </ul>
 * 
 * <p>
 * A <tt>BundleContext</tt> object will be created and provided to this bundle
 * when it is started using the {@link BundleActivator#start}method. The same
 * <tt>BundleContext</tt> object will be passed to this bundle when it is
 * stopped using the {@link BundleActivator#stop}method. <tt>BundleContext</tt>
 * is generally for the private use of this bundle and is not meant to be shared
 * with other bundles in the OSGi environment. <tt>BundleContext</tt> is used
 * when resolving <tt>ServiceListener</tt> and <tt>EventListener</tt>
 * objects.
 * 
 * <p>
 * The <tt>BundleContext</tt> object is only valid during an execution
 * instance of this bundle; that is, during the period from when this bundle is
 * called by <tt>BundleActivator.start</tt> until after this bundle is called
 * and returns from <tt>BundleActivator.stop</tt> (or if
 * <tt>BundleActivator.start</tt> terminates with an exception). If the
 * <tt>BundleContext</tt> object is used subsequently, an
 * <tt>IllegalStateException</tt> must be thrown. When this bundle is
 * restarted, a new <tt>BundleContext</tt> object must be created.
 * 
 * <p>
 * The Framework is the only entity that can create <tt>BundleContext</tt>
...
public abstract interface BundleContext {
	 ...
	 * method returns <tt>null</tt> if the property is not found.
	 * 
	 * <p>
	 ...
	 *            appropriate <tt>PropertyPermission</tt> to read the
	 ...
	 * obtained from <tt>location</tt> as interpreted by the Framework in an
	 ...
	 * installed, the <tt>Bundle</tt> object for that bundle is returned.
	 * 
	 * <li>The bundle's content is read from the location string. If this
	 * fails, a {@link BundleException}is thrown.
	 * 
	 * <li>The bundle's <tt>Bundle-NativeCode</tt> dependencies are resolved.
	 * If this fails, a <tt>BundleException</tt> is thrown.
	 * 
	 * <li>The bundle's associated resources are allocated. The associated
	 * resources minimally consist of a unique identifier, and a persistent
	 * storage area if the platform has file system support. If this step fails,
	 * a <tt>BundleException</tt> is thrown.
	 * 
	 ...
	 * <tt>BundleContext.installBundle(String location)</tt>, except that the
	 * bundle's content will be read from the <tt>InputStream</tt> object. The
	 * location identifier string specified will be used as the identity of the
	 * bundle.
	 * 
	 * <p>
	 * This method must always close the <tt>InputStream</tt> object, even if
	 * an exception is thrown.
	 * 
	 * @param location The location identifier of the bundle to install.
	 * @param input The <tt>InputStream</tt> object from which this bundle
	 *        will be read.
	 * @return The <tt>Bundle</tt> object of the installed bundle.
	 ...
	 * @return A <tt>Bundle</tt> object, or <tt>null</tt> if the identifier
	 ...
	 * 
	 * @return An array of <tt>Bundle</tt> objects; one object per installed
	 *         bundle.
	 */
	 ...
	 * <tt>filter</tt> to this context bundle's list of listeners.
	 * <p>
	 * See {@link #getBundle()}for a definition of context bundle, and
	 * {@link Filter}for a description of the filter syntax.
	 * <tt>ServiceListener</tt> objects are notified when a service has a
	 * lifecycle state change.
	 * 
	 * <p>
	 * If this context bundle's list of listeners already contains a listener
	 * <tt>l</tt> such that <tt>(l==listener)</tt>, this method replaces
	 * that listener's filter (which may be <tt>null</tt>) with the specified
	 * one (which may be <tt>null</tt>).
	 * 
	 * <p>
	 * The listener is called if the filter criteria is met. To filter based
	 * upon the class of the service, the filter should reference the
	 * {@link Constants#OBJECTCLASS}property. If <tt>filter</tt> is
	 * <tt>null</tt>, all services are considered to match the filter.
	 * 
	 * <p>
	 * When using a <tt>filter</tt>, it is possible that the
	 * <tt>ServiceEvent</tt> s for the complete life cycle of a service will
	 * not be delivered to the listener. For example, if the <tt>filter</tt>
	 * only matches when the property <tt>x</tt> has the value <tt>1</tt>,
	 * the listener will not be called if the service is registered with the
	 * property <tt>x</tt> not set to the value <tt>1</tt>. Subsequently,
	 * when the service is modified setting property <tt>x</tt> to the value
	 * <tt>1</tt>, the filter will match and the listener will be called with
	 * a <tt>ServiceEvent</tt> of type <tt>MODIFIED</tt>. Thus, the
	 * listener will not be called with a <tt>ServiceEvent</tt> of type
	 * <tt>REGISTERED</tt>.
	 * 
	 * <p>
	 * If the Java Runtime Environment supports permissions, the
	 * <tt>ServiceListener</tt> object will be notified of a service event
	 * only if the bundle that is registering it has the
	 * <tt>ServicePermission</tt> to get the service using at least one of the
	 * named classes the service was registered under.
	 * 
	 * @param listener The <tt>ServiceListener</tt> object to be added.
	 * @param filter The filter criteria.
	 * 
	 * @exception InvalidSyntaxException If <tt>filter</tt> contains an
	...
	/**
	 * Adds the specified <tt>ServiceListener</tt> object to this context
	 * bundle's list of listeners.
	 * 
	 * <p>
	 * This method is the same as calling
	 * <tt>BundleContext.addServiceListener(ServiceListener listener,
	 * String filter)</tt>
	 * with <tt>filter</tt> set to <tt>null</tt>.
	 * 
	 * @param listener The <tt>ServiceListener</tt> object to be added.
	 * @exception java.lang.IllegalStateException If this context bundle has
	 *            stopped.
	...
	/**
	 * Removes the specified <tt>ServiceListener</tt> object from this context
	 * bundle's list of listeners. See {@link #getBundle()}for a definition of
	 * context bundle.
	 * 
	 * <p>
	 * If <tt>listener</tt> is not contained in this context bundle's list of
	...
	/**
	 * Adds the specified <tt>BundleListener</tt> object to this context
	 ...
	 * <p>
	 * If this context bundle's list of listeners already contains a listener
	 * <tt>l</tt> such that <tt>(l==listener)</tt>, this method does
	 * nothing.
	 * 
	...
	/**
	 * Removes the specified <tt>BundleListener</tt> object from this context
	 * bundle's list of listeners. See {@link #getBundle()}for a definition of
	 * context bundle.
	 * 
	 * <p>
	...
	/**
	 * Adds the specified <tt>FrameworkListener</tt> object to this context
	 * bundle's list of listeners if not already present. See
	...
	/**
	 * Removes the specified <tt>FrameworkListener</tt> object from this
	 ...
	 * <tt>ServiceRegistration</tt> object is returned. The
	 * <tt>ServiceRegistration</tt> object is for the private use of the
	 ...
	 * <ol>
	 * <li>If <tt>service</tt> is not a <tt>ServiceFactory</tt>, an
	 * <tt>IllegalArgumentException</tt> is thrown if <tt>service</tt> is
	 * not an <tt>instanceof</tt> all the classes named.
	 * <li>The Framework adds these service properties to the specified
	 * <tt>Dictionary</tt> (which may be <tt>null</tt>): a property named
	 ...
	 * synchronously sent.
	 * <li>A <tt>ServiceRegistration</tt> object for this registration is
	 * returned.
	 * </ol>
	 * 
	 ...
	 *        properties object must all be <tt>String</tt> objects. See
	 *        {@link Constants}for a list of standard service property keys.
	 *        Changes should not be made to this object after calling this
	 *        method. To update the service's properties the
	 *        {@link ServiceRegistration#setProperties}method must be called.
	 *        <tt>properties</tt> may be <tt>null</tt> if the service has no
	 *        properties.
	 * 
	 ...
	 *            <li><tt>service</tt> is not a <tt>ServiceFactory</tt>
	 *            object and is not an instance of all the named classes in
	 *            <tt>clazzes</tt>.
	 *            <li><tt>properties</tt> contains case variants of the same
	 *            key name.
	 *            </ul>
	 * 
	 * @exception java.lang.SecurityException If the caller does not have the
	 *            <tt>ServicePermission</tt> to register the service for all
	 ...
	 * <tt>service</tt> will only be registered under a single class name.
	 ...
	 * <li>If <tt>clazz</tt> is not <tt>null</tt>, the set is further
	 ...
	 * 
	 * <p>
	 * This <tt>ServiceReference</tt> object is valid at the time of the call
	 ...
	 * <tt>null</tt> filter string. It is provided as a convenience for when
	 ...
	 * @return A <tt>ServiceReference</tt> object, or <tt>null</tt> if no
	 ...
	 * 
	 * <p>
	 * This method will always return <tt>null</tt> when the service
	 * associated with this <tt>reference</tt> has been unregistered.
	 * 
	 * <p>
	 ...
	 * <tt>ServiceFactory</tt> interface, the
	 ...
	 * If the service object returned by the <tt>ServiceFactory</tt> object is
	 * not an <tt>instanceof</tt> all the classes named when the service was
	 * registered or the <tt>ServiceFactory</tt> object throws an exception,
	 * <tt>null</tt> is returned and a Framework event of type
	 ...
	 *         <tt>reference</tt>, or <tt>null</tt> if the service is not
	 *         registered or does not implement the classes under which it was
	 *         registered in the case of a Service Factory.
	 * @exception java.lang.SecurityException If the caller does not have the
	 *            <tt>ServicePermission</tt> to get the service using at least
	 ...
	 * <tt>ServiceReference</tt> object. If the context bundle's use count for
	 * the service is zero, this method returns <tt>false</tt>. Otherwise,
	 ...
	 * service has been unregistered, <tt>false</tt> is returned.
	 * <li>The context bundle's use count for this service is decremented by
	 * one.
	 * <li>If the context bundle's use count for the service is currently zero
	 * and the service was registered with a <tt>ServiceFactory</tt> object,
	...
	/**
	 * Creates a <tt>File</tt> object for a file in the persistent storage
	 ...
	 * 
	 * <p>
	 * A <tt>File</tt> object for the base directory of the persistent storage
	 * area provided for the context bundle by the Framework can be obtained by
	 * calling this method with an empty string (" ") as <tt>filename</tt>.
	 * See {@link #getBundle()}for a definition of context bundle.
	 * 
	 * <p>
	 * If the Java Runtime Environment supports permissions, the Framework will
	 * ensure that the bundle has the <tt>java.io.FilePermission</tt> with
	 * actions <tt>read</tt>,<tt>write</tt>,<tt>delete</tt> for all
	 ...
	 *         <tt>null</tt> if the platform does not have file system
	 *         support.
	 * @exception java.lang.IllegalStateException If the context bundle has
	 *            stopped.
	 */
	...
	/**
	 * Creates a <tt>Filter</tt> object. This <tt>Filter</tt> object may be
	 * used to match a <tt>ServiceReference</tt> object or a
	 * <tt>Dictionary</tt> object. See {@link Filter}for a description of the
