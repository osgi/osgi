...
/**
 * @methodUnderTest org.osgi.util.gsm.IMEICondition#isSatisfied
...
public class IsSatisfied {
	 ...
	 *                  condition is created with a IMEI that is different from
	 *                  phone's IMEI
	 */
					...
					.assertTrue(
							MessagesConstants
									.getMessage(
											...
											new String[] { "IsSatisfied method returns false when a condition is created with a IMEI that is different from phone's IMEI" }),
							!cond.isSatisfied());
	 ...
	 *                  with different IMEI code are matched
	 * 
	 */
	public void testIsSatisfied002() {
        tbc.log("#testIsSatisfied002");
        try {
			Condition cond1 = IMEICondition.getInstance(
					PolicyTestControl.TEST_BUNDLE,
					PolicyTestControl.IMEI_VALID_CODE);

			Condition cond2 = IMEICondition.getInstance(
					PolicyTestControl.TEST_BUNDLE,
			...
			tbc
					.assertTrue(
							MessagesConstants
									.getMessage(
											MessagesConstants.ASSERT_TRUE,
											new String[] { "IsSatisfied method returns false when conditions with different IMEI code is matched" }),
	...
	public void testIsSatisfied004() {
        ...
        try {
			...
			Condition cond3 = IMEICondition.getInstance(
					PolicyTestControl.TEST_BUNDLE,
	 ...
	 *                  with same IMEI code are matched
	 */
			...
			tbc
					.assertTrue(
							MessagesConstants
									.getMessage(
											MessagesConstants.ASSERT_TRUE,
											new String[] { "IsSatisfied method returns true when a condition is created with a IMEI that is equals to phone's IMEI and all condition match the device" }),
							cond2.isSatisfied(new Condition[] { cond1 }, null));
     ...
     *                  is created with a IMEI that is equals to phone's IMEI
     */
            ...
            tbc
                    .assertTrue(
                            MessagesConstants
                                    .getMessage(
                                            MessagesConstants.ASSERT_TRUE,
                                            new String[] { "IsSatisfied method returns true when a condition is created with a IMEI equals to phone's IMEI" }),
                            cond.isSatisfied());
