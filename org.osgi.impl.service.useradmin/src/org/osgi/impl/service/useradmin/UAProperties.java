...
/**
 * A dictionary with permissions checks for setting, changing, and
 * deleting values.  
 * <p> 
 * Role properties are instances of this class. Changes to these objects
 * are propagated to the {@link UserAdmin} service and made
 * persistent. Only keys of the type <code>String</code> are
 * supported, and only values of type <code>String</code> or of type
 * <code>byte[]</code> are supported. If an attempt is made to store a
 * value of a type other than what is supported, an exception of type
...
public class UAProperties extends Dictionary {
  ...
  /**
   * The actual property map. Mapped from lowercased keys
   * for efficient lookup. The real keys are stored in keymap.
   */
  ...
  public Object put(Object key, Object value) {
	...
	if (value instanceof String || value instanceof byte[]) {
	  ...
	  throw new IllegalArgumentException("The value must be of type String or byte[],  got " +
					     value.getClass());
  ...
  /**
   * Loads values from the specified Properties object, with keys
   * prefixed by the specified name, into this dictionary.
   */
  public void load(String name, Properties p) {
    String key;
    for (int i = 0; 
	 (key = p.getProperty(name + i + "n")) != null; 
	 i++) {
