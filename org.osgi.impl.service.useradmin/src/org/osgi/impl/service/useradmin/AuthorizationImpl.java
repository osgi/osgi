...
public class AuthorizationImpl implements Authorization {
  ...
  /**
   * The UserAdmin version number for which this instance's caches 
   * are valid.
   */
  protected long ua_version;
  
  /**
   * Cache holding the roles checked for implication sofar. Must be
   * recomputed if the UserAdmin database is changed.
   */
  private Hashtable /* Role */ cache = new Hashtable();

  /**
   * The working set of roles, i.e., the roles that we currently
   * are working on. Used for loop detection.
   */
  private Vector /* Role */ working = new Vector();

  /**
   * The UserAdmin will set the first element of this array to false
   * when it is going away. All references to the UserAdmin from 
   * Authorization objects must be guarded by a check that the
   * UserAdmin is still alive.
   */
  ...
  /**
   * Gets the roles implied by this Authorization context.
   * Implementation of 
