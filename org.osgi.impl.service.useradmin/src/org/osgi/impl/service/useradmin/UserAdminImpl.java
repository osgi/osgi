...
public class UserAdminImpl implements UserAdmin {
  /**
   * The role database. It is represented as a mapping from role names
   * to roles.
   */
  ...
  /**
   * Flag telling if this UserAdmin is active or not. Used by 
   * Authorization objects to check if the UserAdmin that created
   * them is still active. The state must be a mutable object,
   * hence the placement as an array element.
   */
  ...
  public UserAdminImpl(Activator activator) {
    ...
    log = activator.log;
    listeners = new ServiceTracker(activator.bc, 
  ...
  /**
   * Creates a role. 
   * Implementation of {@link org.osgi.service.useradmin.UserAdmin#createRole}.
   *
  ...
  /**
   * Removes a role. 
   * Implementation of {@link org.osgi.service.useradmin.UserAdmin#removeRole}.
   */
  ...
  /**
   * Gets a named role. 
   * Implementation of {@link org.osgi.service.useradmin.UserAdmin#getRole}.
   */
  ...
  public User getUser(String key, String value) {
	...
	if (pval != null && pval instanceof String && value.equals(pval)) {
  ...
  /**
   * Gets the role matching the specified filter. 
   * Implementation of {@link org.osgi.service.useradmin.UserAdmin#getRoles}.
   */
  ...
  /**
   * Gets an authorization context for the specified user.
   * Implementation of {@link org.osgi.service.useradmin.UserAdmin#getAuthorization}.
   */
  ...
  protected void notifyListeners(int type, Role role) {
    UserAdminEvent event = new UserAdminEvent(activator.uasr.getReference(),
					      type, role);
  ...
  protected void save() {
    ...
    int counter = 0;
    for (Enumeration en = rolemap.elements(); 
	 en.hasMoreElements();
	 counter++) {
  ...
  void load() {
    ...
    String name;
    for (counter = 0; 
	 (name = p.getProperty(counter + "n")) != null;
	 counter++) {
	...
	String member;
	for (int i = 0; 
	     (member = p.getProperty(counter + "bm" + i)) != null; 
	     i++) {
	  group.basic_members.addElement(member);
	}
	for (int i = 0; 
	     (member = p.getProperty(counter + "rm" + i)) != null; 
	     i++) {
