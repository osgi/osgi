...
public class GroupImpl extends UserImpl implements Group {
  ...
  /**
   * Adds a basic member to this Group.
   * Implementation of {@link org.osgi.service.useradmin.Group#addMember}.
   */
  ...
  /**
   * Adds a required member to this Group.
   * Implementation of {@link org.osgi.service.useradmin.Group#addRequiredMember}.
   */
   ...
   * Implementation of {@link org.osgi.service.useradmin.Group#removeMember}.
   */
  public boolean removeMember(Role role) {
    ua.checkPermission(ua.adminPermission);
    if (role == null ||
	!(role instanceof RoleImpl) || 
	((RoleImpl)role).ua != ua)
      throw new IllegalArgumentException("Bad role");
    String name = role.getName();
    synchronized (this) {
      boolean removed = 
        basic_members.remove(name) || required_members.remove(name);      
      if (removed)
        ua.save();
      return removed;
    }
  ...
  /**
   * Gets the basic members of Group.
   * Implementation of {@link org.osgi.service.useradmin.Group#getMembers}.
   */
  ...
  /**
   * Gets the required members members of Group.
   * Implementation of {@link org.osgi.service.useradmin.Group#getRequiredMembers}.
   */
  ...
  protected boolean impliedBy(AuthorizationImpl auth) {
    ...
    synchronized (this) {
      ...
      for (Enumeration en = required_members.elements(); en.hasMoreElements();) {
	RoleImpl role = (RoleImpl)ua.getRole((String)en.nextElement());
	if (!role.impliedBy(auth)) {
	  return auth.cacheHaveRole(this, false);
	}
      }
      // Next check that at least one basic role is implied.
      for (Enumeration en = basic_members.elements(); en.hasMoreElements();) {
	RoleImpl role = (RoleImpl)ua.getRole((String)en.nextElement());
	if (role.impliedBy(auth)) {
