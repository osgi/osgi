...
public class BundleStateValidator extends Assert implements TestValidator, BundleAware {
    ...
    /**
     * Create a state validator for the given state.
     *
     * @param state  The expected bundle state.
     */
    BundleStateValidator(int state) {
        this.state = state;
    }
    ...
    public void validate(BundleContext testContext) throws Exception {
        assertEquals("Target bundle is in incorrect state", state, bundle.getState());
    }
}
