...
/**
 * A class to hook ServiceManager registration events in to
 * the different test phases.  This is used for ServiceManager
 * instances that are managed on the test controller side.
 */
public class ServiceManagerRegister implements TestCleanup, TestEventListener, TestInitializer {
    ...
    public ServiceManagerRegister(ServiceManager manager) {
        this(manager, (String[])null);
    }

    public ServiceManagerRegister(ServiceManager manager, String service) {
        this(manager, new String[] { service });
    }

    public ServiceManagerRegister(ServiceManager manager, String[] services) {
    ...
    /**
     * Register the defined services when triggered by a phase
     * stage or event.
     */
    public void register() {
        if (services == null) {
            ...
            for (int i = 0; i < services.length; i++) {
                manager.registerService(services[i]);
            }
        }
    }
