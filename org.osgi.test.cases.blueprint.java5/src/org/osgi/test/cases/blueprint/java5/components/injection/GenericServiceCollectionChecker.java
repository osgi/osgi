 ...
 */
public class GenericServiceCollectionChecker extends DependencyDriver {
    // the injected set of references.
    protected List<TestServiceOne> injectedList;
    // the injected set of references.
    protected Set<TestServiceOne> injectedSet;
    // an injected collection item
    protected Collection<TestServiceOne> injectedCollection;

    // our bundle context (used for requesting service instances)
    protected BundleContext bundleContext;

    public GenericServiceCollectionChecker(String componentId) {
        super(componentId);
    }
     ...
     * @param l
     */
    public void setGenericList(List<TestServiceOne> l) {
        injectedList = l;
    }
     ...
     * @param l
     */
    public void setGenericSet(Set<TestServiceOne> l) {
        injectedSet = l;
    }
     ...
     * @param l
     */
    public void setGenericCollection(Collection<TestServiceOne> l) {
    ...
    protected void validateList(ManagedService[] expected) {
        ...
        for (int i = 0; i < injectedList.size(); i++) {
            TestServiceOne service = injectedList.get(i);
     ...
     */
    protected void validateCollection(ManagedService[] expected, Collection<TestServiceOne> source) {
        ...
        int i = 0;
        Iterator<TestServiceOne> iterator = source.iterator();
        while (iterator.hasNext()) {
            // this can only be a ServiceReference
            TestServiceOne service = iterator.next();
            if (locateMatchingService(service, expected) == null) {
     ...
     */
    protected ManagedService locateMatchingService(TestServiceOne service, ManagedService[] expected) {
