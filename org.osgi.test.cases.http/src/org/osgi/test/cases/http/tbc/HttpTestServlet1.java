...
{
	private static final String pagePart1 =
		"<html><head><title>OSGi - HTTP Service test case</title></head><body><h1>";
	...
	static
	{
		...
		testCaseMap.put ("15", "15 Registration of servlet using default HTTP context</h1>");
		testCaseMap.put ("50", "50 Register servlet with empty doGet() method</h1>");
		testCaseMap.put ("51", "51 Verification of statements under handleSecurity()</h1>");
		...
		testCaseMap.put ("56", "56 Access of servlet with long parameter values</h1>");
		testCaseMap.put ("57", "57 Access URL with special parameter values</h1>");
		testCaseMap.put ("58a", "58a Access URL using getParameter with different values for the same parameter</h1>");
		testCaseMap.put ("58b", "58b Access URL using getParameterValues with different values for the same parameter</h1>");
		testCaseMap.put ("59", "59 Verify most common MIME types</h1>");
		testCaseMap.put ("60", "60 Verify resource name construction</h1>");
		testCaseMap.put ("61", "61 Verification of authentication under handleSecurity()</h1>");
	...
	ServletException
	{
		...
		else if ("8".equals (testCase))
		{
			...
			out.println (pagePart1 +
					testCaseMap.get (testCase) +
					"<p>Lowercase Servlet</p>" +
					pagePart2);
		}
		else if ("50".equals (testCase))
		{
		}
		else if ("52".equals (testCase))
		{
			response.setContentType ("text/html");
			PrintWriter out = response.getWriter ();
		...
		else if ("59".equals (testCase))
		{
			...
			if (type.compareTo ("plain") == 0)
			{
				...
				out.println ("OSGi - HTTP Service test case");
				out.println ("59 Verify most common MIME types");
			}
			else if (type.compareTo ("html") == 0)
			{
				...
				out.println (pagePart1 + testCaseMap.get (testCase) + pagePart2);
			}
				...
				InputStream in = (InputStream) AccessController.doPrivileged(new PrivilegedAction()
					{
						public Object run()
						{
							return getClass().getResourceAsStream("www/car.jpg");
						}
					});
		...
		else if ("61".equals (testCase))
		{
			response.setContentType ("text/html");
			PrintWriter out = response.getWriter ();
			
			//retrieving remote user and authentication that is set
			//by the default authentication mechanism
			
			out.println (pagePart1 +
					testCaseMap.get (testCase) +
					"<p>Remote User is " + request.getRemoteUser()+
					", Authentication Type is " + request.getAuthType() +
					"</p>" +
					pagePart2);
		}
		else if ("62".equals (testCase))
		{
			response.setContentType ("text/html");
			PrintWriter out = response.getWriter ();
			...
			String value = "val1";
			SC.setAttribute(paramName, value);
			
			out.println("<p>Writing parameter: " + paramName +
					" with value: " + value +
					"</p>" +
					pagePart2);
		}
