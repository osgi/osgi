...
/**
   Bundle used to install a bunch of services later interrogated with
   various filters.
...
public class InstallServices implements BundleActivator
{
	...
	*/
	public void start(BundleContext bc)
	{
		...
		_sra1 = bc.registerService(TestService.class.getName(), _sa1, _sa1Props);
		org.osgi.test.cases.filter.tbc.FilterControl.serviceA = _sra1;
		org.osgi.test.cases.filter.tbc.FilterControl.serviceAProperties = _sa1Props;
		...
		_srb1 = bc.registerService(TestService.class.getName(), _sb1, _sb1Props);

		_sb2 = new ServiceB();
		_sb2Props = new Properties();
		_sb2Props.put("name", "ServiceB");
		_sb2Props.put("version", new Float(2.0));
		_sb2Props.put("compatible", new Float[] { new Float(1.0), new Float(2.0)});
		...
		_srb2 = bc.registerService(TestService.class.getName(), _sb2, _sb2Props);
		...
		_src1 = bc.registerService(TestService.class.getName(), _sc1, _sc1Props);
		...
		_src2 = bc.registerService(TestService.class.getName(), _sc2, _sc2Props);
		...
		_src3 = bc.registerService(TestService.class.getName(), _sc3, _sc3Props);
	}
