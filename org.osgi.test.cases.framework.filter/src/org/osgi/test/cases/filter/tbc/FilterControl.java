...
package org.osgi.test.cases.filter.tbc;
...
public class FilterControl extends DefaultTestBundleControl {
	...
	static String[] methods = new String[] {
		"prepare",
		"test_framework_filter_m1",
		"test_framework_filter_m2",
		"test_framework_filter_m3",
		"test_framework_filter_m4",
		"test_framework_filter_m5",
		"test_framework_filter_m6",
		"test_framework_filter_m9",
		"test_framework_filter_m10",
    ...
    public void testFilter(String filter, String logtext) throws Exception {
        ...
        refs = getContext().getServiceReferences(
            "org.osgi.test.cases.filter.tb1.TestService", filter);
	...
	*/
	public void test_framework_filter_m2() throws Exception {
		...
		try
		{
			refs = getContext().getServiceReferences("org.osgi.test.cases.filter.tb1.TestService",
												 "(!(!(!(!(!(!(!(!(!(!(!(!(!(!(!(!(!(!(!(!(!(!(!(!(name=ServiceA))))))))))))))))))))))))");
	...
	*/
	...
	/**
	   Tests the approximate (~=) operation. Case and white space differences should
	   match.
	*/
	public void test_framework_filter_s1() throws Exception {
        testFilter("(name~=servicea)", "Testing approximate equality (case)");
        testFilter("(description~=Service A)", "Testing approximate equality (white space)");
	}


	/**
	   Help function used to sort and log an array of service
	   references.
	*/
		...
		throws IOException
	{
		...
		if (refs != null)
		{
			...
			// The order of the services isn't specified, so we'd better sort them.
