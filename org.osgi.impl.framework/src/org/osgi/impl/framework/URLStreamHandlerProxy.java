...
/**
 * Proxies URLStreamHandler by tracking services for a specific protocol.
 * There will be a proxy for each protocol.
 */
public class URLStreamHandlerProxy extends URLStreamHandler {
    ...
    /** Simply forward the request to the handler or 
     *  throw MalformedURLException if handler has unregistered. */
    protected URLConnection openConnection(URL u) throws java.io.IOException {
	URLStreamHandlerService handler = getHandler();
	if (handler == null) throw new MalformedURLException(protocol +
							     " handler gone");
	return handler.openConnection(u);
    }

    /** Simply forward the request to the handler or 
     *  throw IllegalStateException if handler has unregistered. */
    protected void parseURL(URL u, String spec, int start, int limit) {
	URLStreamHandlerService handler = getHandler();
	if (handler == null) throw new IllegalStateException(protocol +
							     " handler gone");
	handler.parseURL(setter, u, spec, start, limit);
    }

    /** We just pass this up to URLConnection.  URLStreamHandlerServices will
     *  use this method to call setURL since only the URLStreamHandler for
     *  the protocol (which is this proxy) will be able to call setURL. */
...
class MyURLStreamHandlerSetter implements URLStreamHandlerSetter
{
    ...
    public void setURL(URL u, String proto, String host, int port,
