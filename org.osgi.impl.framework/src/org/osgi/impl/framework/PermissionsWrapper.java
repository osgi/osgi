...
class PermissionsWrapper extends PermissionCollection {
	...
	/**
	 * Create the permissionCollection assigned to the bundle with the specified
	 * id. The collection contains the configured permissions for the bundle
	 * location plus implict granted permissions (FilePermission for the data
	 * area and java runtime permissions).
	 * 
	...
	private PermissionCollection makePermissionCollection(BundleImpl bundle) {
		...
		if (pa.runtimePermissions != null) {
			for (Enumeration e = pa.runtimePermissions.elements(); e
					.hasMoreElements();) {
	...
	/**
	 * Build a permissionCollection based on a set PermissionInfo objects. All
	 * the permissions that are available in the CLASSPATH are constructed and
	 * all the bundle based permissions are constructed as
		...
		for (int i = pi.length - 1; i >= 0; i--) {
			...
			try {
				...
				if (pc == FilePermission.class) {
					...
					Constructor c = pc.getConstructor(new Class[] {
