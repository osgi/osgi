 ...
 * An exported package.
 * 
 * Instances implementing this interface are created by the {@link PackageAdmin}
 * service.
 * <p>
 * Note that the information about an exported package provided by this class is
 * valid only until the next time <tt>PackageAdmin.refreshPackages()</tt> is
 * called. If an ExportedPackage becomes stale (that is, the package it
 * references has been updated or removed as a result of calling
 * PackageAdmin.refreshPackages()), its getName() and getSpecificationVersion()
 * continue to return their old values, isRemovalPending() returns true, and
 * getExportingBundle() and getImportingBundles() return null.
 * 
...
public class ExportedPackageImpl implements ExportedPackage {
	 ...
	 *         <tt>ExportedPackage</tt>, or null if this
	 *         <tt>ExportedPackage</tt> has become stale.
	 */
