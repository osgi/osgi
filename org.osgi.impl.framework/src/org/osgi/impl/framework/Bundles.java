...
/**
 * Here we handle all the bundles that are installed in framework. Also handles
 * load and save of bundle states to file, so that we can restart the platform.
 * 
...
public class Bundles {
	...
	/**
	 * Minimum version of data format saved data required for this class to be
	 * able to read data.
	 */
	...
	private String				version								= Integer
																			.toString(bundleInfoVersion);
	/**
	 * Comment of stored or loaded bundle property data.
	 */
	private String				comment								= "Bundle info";
	/**
	 * Table of all installed bundles in this framework. Key is bundle location.
	 */
	...
	/**
	 * The initial bundle start level. Contained in this class because we want
	 * to use bundle_info for persistent storage.
	 */
	...
	BundleImpl install(final String location, final InputStream in)
		...
		synchronized (this) {
			...
			try {
				b = (BundleImpl) AccessController
						.doPrivileged(new PrivilegedExceptionAction() {
							public Object run() throws Exception {
								BundleImpl res = null;
								InputStream bin;
								if (in == null) {
									bin = (new URL(location)).openConnection()
											.getInputStream();
								}
								else {
									bin = in;
								...
								try {
									res = new BundleImpl(bundlesDir, framework,
											location, bin);
								}
								finally {
									bin.close();
	 ...
	 * @return BundleImpl representing bundle or null if bundle was not found.
	 */
	...
	/**
	 * Try to load any saved framework state. This is done by installing all
	 * saved bundle from the local archive copy. And restoring the saved state
	 * for each bundle. This is only intended to be executed during start of
	 * framework.
	 * 
	 * @return True if we found a saved state.
	 * @exception Exception If we failed to read data or if data is corrupted.
	 */
	...
	synchronized void save() {
		try {
			...
			framework.listeners.frameworkEvent(new FrameworkEvent(
					FrameworkEvent.ERROR, framework.systemBundle, e
							.getException()));
		}
	}
	...
	private void save(OutputStream props) throws IOException {
		...
		bi_props.put("bundleinfo.initialbundlestartlevel", String
				.valueOf(initialBundleStartLevel));
	...
	private void load(InputStream props) throws Exception {
		...
		try {
			initialBundleStartLevel = Integer.parseInt(bi_props.getProperty(
					"bundleinfo.initialbundlestartlevel", Integer
							.toString(DEFAULT_BUNDLE_START_LEVEL)));
		...
		while (!bi_props.isEmpty()) {
			...
			// Only called at contruction of Framework, no bundle install event
			// needed.
		}
	}
}
