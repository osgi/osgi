...
/**
 * This is a simple main that can start framework and does
 * some simple operations install, start, stop, uninstall
 * and update.
 *
...
public class Main
{
    ...
    static FileTree dir = new FileTree(System.getProperty("org.osgi.framework.dir", "fwdir"));
    
    /**
     * Example of a main method for starting the OSGi
     * reference framework.
     */
	...
	System.out.println("This is a reference implementation of the OSGi Framework.");
	System.out.println("It is written to be correct and easy to understand and is");
	System.out.println("intended only to be used for test and reference purposes.");
	System.out.println("Copyright (c) 1999 - 2002 Gatespace AB. All Rights Reserved.");
	
	String base = System.getProperty("org.osgi.framework.install.base", "file:");
	...
	for (; i < args.length; i++) {
		...
		if ("-exit".equals(args[i])) {
			...
			if (bundle.indexOf(":") != -1) {
			    ...
			    id = framework.installBundle(base + bundle, null);
			}
			System.out.println("Installed: " + framework.getBundleLocation(id) + " (id#" + id + ")");
			i++;
		    } else {
			...
			System.out.println("Installed and started: " + framework.getBundleLocation(id) + " (id#" + id + ")");
			i++;
		    } else {
			error("No URL for install command");
		    }
		    ...
		    if (i+1 < args.length) { 
			long id = Long.parseLong(args[i+1]);
			framework.startBundle(id);
			System.out.println("Started: " + framework.getBundleLocation(id) + " (id#" + id + ")");
			i++;
		    } else {
			...
			System.out.println("Stopped: " + framework.getBundleLocation(id) + " (id#" + id + ")");
			i++;
		    } else {
			...
			System.out.println("Uninstalled: " + loc + " (id#" + id + ")");
			i++;
		    } else {
			...
			System.out.println("Updated: " + framework.getBundleLocation(id) + " (id#" + id + ")");
    ...
    {
	System.out.println("Usage:  java [properties]  org.osgi.impl.framework.Main [-init] [options]");
	...
	System.out.println("  -launch [SL]  Launch framework at start level SL (Default: 1)");
	...
	System.out.println("    List of packages exported from system classloader,");
	System.out.println("    all none java.* and org.osgi.framework in classpath");
