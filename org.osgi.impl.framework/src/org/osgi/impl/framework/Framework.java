...
/**
 * This class contains references to all common data structures inside
 * framework.
 * 
...
public class Framework {
	...
	public Framework(File dir) throws Exception {
		...
		if (checkPermissions) {
			...
			Policy.setPolicy(new FrameworkPolicy(permissions, Policy
					.getPolicy()));
			String[] classes = new String[] {PermissionAdmin.class.getName()};
			services.register(systemBundle, classes, permissions, null);
		}
		String[] classes = new String[] {PackageAdmin.class.getName()};
		services.register(systemBundle, classes, new PackageAdminImpl(this),
				null);
		bundles.load();
	}

	/**
	 * Start this Framework. This method starts all the bundles that were
	 * started at the time of the last shutdown.
	 * 
	 * <p>
	 * If the Framework is already started, this method does nothing. If the
	 * Framework is not started, this method will:
	 * <ol>
	 ...
	 * {@link Bundle#start}method. Reports any exceptions that occur during
	 * startup using <code>FrameworkErrorEvents</code>.</li>
	 ...
	 * 
	 * <p>
	 * If this Framework is not launched, it can still install, uninstall, start
	 * and stop bundles. (It does these tasks without broadcasting events,
	 * however.) Using Framework without launching it allows for off-line
	 * debugging of the Framework.
	 * </p>
	 *  
	 */
	...
	/**
	 * Start this Framework at an initial start level. For further details, see
	 * launch().
	 * 
	...
	/**
	 * Stop this Framework, suspending all started contexts. This method
	 * suspends all started contexts so that they can be automatically restarted
	 * when this Framework is next launched.
	 * 
	 * <p>
	 * If the framework is not started, this method does nothing. If the
	 * framework is started, this method will:
	 * <ol>
	 * <li>Set the state of the Framework to <i>inactive </i>.</li>
	 * <li>Suspended all started bundles as described in the
	 * {@link Bundle#stop}method except that the persistent state of the bundle
	 * will continue to be started. Reports any exceptions that occur during
	 * stopping using <code>FrameworkErrorEvents</code>.</li>
		...
		if (active) {
			...
			for (Iterator i = slist.iterator(); i.hasNext();) {
				...
				try {
					...
					listeners.frameworkEvent(new FrameworkEvent(
							FrameworkEvent.ERROR, b, be));
				}
			}
			shuttingdown = false;
			bundles.save();
	 ...
	 * @return A location as a string, or <code>null</code> if the identifier
	 *         doesn't match any installed bundle.
	 */
			...
			if (Constants.FRAMEWORK_VENDOR.equals(key)) {
				...
				if (Constants.FRAMEWORK_LANGUAGE.equals(key)) {
					...
					if (Constants.FRAMEWORK_OS_NAME.equals(key)) {
						// The name of the operating system of the hosting
						// computer.
						String osName = System.getProperty("os.name");
						if (osAliases.containsKey(osName.toLowerCase())) {
							osName = (String) osAliases.get(osName
									.toLowerCase());
						}
						return osName;
					}
						...
						if (Constants.FRAMEWORK_OS_VERSION.equals(key)) {
							// The version number of the operating system of the
							// hosting computer.
							...
							if (Constants.FRAMEWORK_PROCESSOR.equals(key)) {
								// The name of the processor of the hosting
								// computer.
								String procName = System.getProperty("os.arch");
								if (processorAliases.containsKey(procName
										.toLowerCase())) {
									procName = (String) processorAliases
											.get(procName.toLowerCase());
								}
								return procName;
							}
							else
								if (Constants.FRAMEWORK_EXECUTIONENVIRONMENT
										.equals(key)) {
									return System
	 ...
	 * 
	 * ENTRY (, ENTRY)* ENTRY = key (; key)* (; OP)* OP = param '=' value
	 * 
	 * @param s String to parse
	 * @param single If true, only allow one key per ENTRY and only allow unique
	 *        parmeters for each ENTRY.
	...
	static Iterator parseEntries(String s, boolean single) {
		...
		if (s != null) {
			...
			do {
				...
				if (key == null) {
					throw new IllegalArgumentException("Expected key at: "
							+ at.getRest());
				}
				if (!single) {
					keys.add(key);
