...
public class StartLevelImpl implements StartLevel {
	...
	public void setStartLevel(final int startlevel) {
		...
		if (startlevel < 1)
			throw new IllegalArgumentException("Start Level < 1");
		StartLevelJob job = new StartLevelJob() {
			public void execute() {
				...
				framework.listeners.frameworkEvent(new FrameworkEvent(
						FrameworkEvent.STARTLEVEL_CHANGED, new Object()));
			}
		};
		addJob(job);
	}
	...
	public void setBundleStartLevel(Bundle bundle, int startlevel) {
		...
		if (bundle.getBundleId() == 0)
			throw new IllegalArgumentException(
					"Illegal operation on System Bundle");
		if (startlevel < 1)
			throw new IllegalArgumentException("Bundle #"
					+ bundle.getBundleId() + ": startlevel < 1");
