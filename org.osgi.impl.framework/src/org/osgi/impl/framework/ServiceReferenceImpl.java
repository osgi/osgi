...
public class ServiceReferenceImpl implements ServiceReference {
	 ...
	 * ServiceReference, i.e., whose usage count for this service is greater
	 * than zero.
	 * 
	 * @return array of bundles whose usage count for the service wrapped by
	 *         this ServiceReference is greater than zero, or <tt>null</tt> if
	 *         no bundles currently are using this service
	 * 
	 * @since 1.1
	 */
	public Bundle[] getUsingBundles() {
		synchronized (registration) {
			if (registration.reference != null
					&& registration.dependents.size() > 0) {
	...
	Object getService(final BundleImpl bundle) {
		...
		synchronized (registration) {
			if (registration.bundle != null) {
				...
				if (ref == null) {
					String[] classes = (String[]) registration.properties
					...
					if (bundle.framework.checkPermissions) {
						...
						for (int i = 0; i < classes.length; i++) {
							try {
								acc.checkPermission(new ServicePermission(
										classes[i], ServicePermission.GET));
								perm = true;
								break;
							}
						...
						if (!perm) {
							throw new SecurityException(
									"Bundle has not permission to get service.");
						}
					}
						...
						try {
							s = AccessController
									.doPrivileged(new PrivilegedExceptionAction() {
										public Object run() {
											return ((ServiceFactory) registration.service)
													.getService(bundle,
															registration);
										}
									});
							...
							bundle.framework.listeners.frameworkError(
									registration.bundle, pe.getException());
							return null;
						}
						if (s == null) {
							return null;
	 ...
	 * @param checkRefCounter If true decrement refence counter and remove
	 *        service if we reach zero. If false remove service without checking
	 *        refence counter.
			...
			if (registration.reference != null) {
				...
				if (countInteger != null) {
					...
					if (checkRefCounter && count > 1) {
						registration.dependents.put(bundle, new Integer(
								count - 1));
						return true;
					}
					else {
						Object sfi = registration.serviceInstances
								.remove(bundle);
						if (sfi != null) {
							try {
								((ServiceFactory) registration.service)
										.ungetService(bundle, registration, sfi);
							}
							catch (Exception e) {
								bundle.framework.listeners.frameworkError(
										registration.bundle, e);
							}
						}
					}
				}
	 ...
	 * 
	 * @return Dictionary containing properties or null if service has been
	 *         removed.
	 */
	...
	//    
	/**
	 * Clone object. Handles all service property types and does this
	 * recursivly.
	 * 
									...
									if (val instanceof long[]) {
										...
										if (val instanceof short[]) {
											...
											if (val instanceof Vector) {
												Vector c = (Vector) ((Vector) val)
														.clone();
												for (int i = 0; i < c.size(); i++) {
													c.set(i, cloneObject(c
															.get(i)));
												}
												val = c;
											}
