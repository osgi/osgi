...
class BundleImpl implements Bundle, Comparable {
	 ...
	 * 
	 * @param bundlesDir Directory where to store the bundles all persistent
	 *        data.
	 * @param fw Framework for this bundle.
	 ...
	 * @exception SecurityException If we don't have permission to import and
	 *            export bundle packages.
	 * @exception BundleException If can not find native code that match this
	 *            JVM.
	 */
	BundleImpl(File bundlesDir, Framework fw, String loc, InputStream in)
		...
		String requiredEEs = (String) headers
		...
		if (framework.checkPermissions) {
			PermissionCollection pc = framework.permissions
					.getPermissionCollection(this);
	...
	/**
	 * Construct a new Bundle based on saved property data from last session.
	 * 
	 * @param bundlesDir Directory where to store the bundles all persistent
	 *        data.
	 * @param fw Framework for this bundle.
	...
	BundleImpl(File bundlesDir, Framework fw, String pfirst, Properties p)
		...
		String pbase = pfirst.substring(0, pfirst.indexOf('.', BUNDLE_BASE
				.length()));
		long tid = 0;
		try {
		...
		try {
			startLevel = Integer.parseInt(p.getProperty(pbase + ".startLevel",
					Integer.toString(fw.bundles.getInitialBundleStartLevel())));
			p.remove(pbase + ".startLevel");
		}
		catch (NumberFormatException e) {
			startLevel = fw.bundles.getInitialBundleStartLevel();
		}
		isPersistentlyStarted = Boolean.valueOf(
				p.getProperty(pbase + ".isPersistentlyStarted", "false"))
				.booleanValue();
		...
		switch (getUpdatedState()) {
			...
			case RESOLVED :
				if (startLevel <= framework.startLevel.activeStartLevel
						...
						AccessController
								.doPrivileged(new PrivilegedExceptionAction() {
									public Object run() throws BundleException {
										final String ba = archive
												.getAttribute(Constants.BUNDLE_ACTIVATOR);
										if (ba != null) {
											try {
												Class c = getClassLoader()
														.loadClass(ba.trim());
												bactivator = (BundleActivator) c
														.newInstance();
					...
					framework.listeners.bundleChanged(new BundleEvent(
							BundleEvent.STARTED, this));
				}
				// The bundle has been correctly started. It must be marked as
				// persitently started
			...
			case ACTIVE :
				return;
			case STARTING :
				// This happens if we call start from inside the
				// BundleActivator.start method.
				// We don't allow it.
				throw new BundleException(
						"Bundle.start called from BundleActivator.start");
			case STOPPING :
				// This happens if we call start from inside the
				// BundleActivator.stop method.
				// We don't allow it.
				throw new BundleException(
						"Bundle.start called from BundleActivator.stop");
			case UNINSTALLED :
				throw new IllegalStateException(
						"Bundle.start: Bundle is in UNINSTALLED state");
		}
	}
	...
	synchronized public void stop() throws BundleException {
		...
		switch (state) {
			...
			case ACTIVE :
				...
				isPersistentlyStarted = true;
				framework.bundles.save();
				Exception savedException = (Exception) AccessController
				...
				if (savedException != null) {
					throw new BundleException(
							"Bundle.stop: BundleActivator stop failed",
							savedException);
				}
				break;
			case STARTING :
				// This happens if we call stop from inside the
				// BundleActivator.start method.
				// We don't allow it.
				throw new BundleException(
						"Bundle.start called from BundleActivator.stop");
			case STOPPING :
				...
				// We don't allow it.
				throw new BundleException(
						"Bundle.stop called from BundleActivator.stop");
			case UNINSTALLED :
				throw new IllegalStateException(
						"Bundle.stop: Bundle is in UNINSTALLED state");
		}
	}
	 ...
	 * @see org.osgi.framework.Bundle#update
	 */
	synchronized public void update(final InputStream in)
		...
		try {
			...
			switch (getUpdatedState()) {
									...
									public Object run() throws BundleException {
										...
										try {
											...
											if (in == null) {
												...
												update = archive
														.getAttribute(Constants.BUNDLE_UPDATELOCATION);
												if (update == null) {
											...
											newHeaders = archive
													.getHeaderDictionary();
											String requiredEEs = (String) newHeaders
													.get(Constants.BUNDLE_REQUIREDEXECUTIONENVIRONMENT);
											verifyRequiredEEs(requiredEEs,
													update);
											...
											throw new BundleException(
													"Failed to get update bundle",
													e);
										}
										// Remove this bundles packages
										boolean allRemoved = bpkgs
												.unregisterPackages(false);
										// Loose old bundle if no exporting
										// packages left
										if (classLoader != null) {
											if (allRemoved) {
												classLoader.purge();
										...
										framework.listeners
												.bundleChanged(new BundleEvent(
														BundleEvent.UPDATED,
														thisBundle));
										// Restart bundle if previously stopped
										// in the operation
										if (wasActive) {
											try {
												thisBundle.start();
				...
				case STARTING :
					...
					throw new IllegalStateException(
							"Bundle.update: Bundle is in STARTING state");
				case STOPPING :
					// Wait for RESOLVED state, this doesn't happen now
					// since we are synchronized.
					throw new IllegalStateException(
							"Bundle.update: Bundle is in STOPPING state");
				case UNINSTALLED :
					throw new IllegalStateException(
							"Bundle.update: Bundle is in UNINSTALLED state");
			}
		}
		finally {
			if (in != null) {
	...
	synchronized public void uninstall() throws BundleException {
		...
		switch (state) {
			...
			case RESOLVED :
				...
				state = UNINSTALLED;
				framework.listeners.bundleChanged(new BundleEvent(
						BundleEvent.UNINSTALLED, this));
				break;
			case STARTING :
				// Wait for RUNNING state, this doesn't happen now
				// since we are synchronized.
				throw new IllegalStateException(
						"Bundle.uninstall: Bundle is in STARTING state");
			case STOPPING :
				// Wait for RESOLVED state, this doesn't happen now
				// since we are synchronized.
				throw new IllegalStateException(
						"Bundle.uninstall: Bundle is in STOPPING state");
			case UNINSTALLED :
				throw new IllegalStateException(
						"Bundle.uninstall: Bundle is in UNINSTALLED state");
		}
	}
	 ...
	 * 
	 * This bundle's <tt>ClassLoader</tt> is called to search for the named
	 * resource.
	 * 
	...
	synchronized public URL getResource(final String name) {
		if (state == UNINSTALLED) {
			throw new IllegalStateException(
					"Bundle.getResource: Bundle is in UNINSTALLED state");
		}
		try {
			framework.checkAdminPermission();
	...
	//
	/**
	 * Compare to bundles when comes to start order. This is only used for
	 * sorting bundles in start order and is inconsistent with the equals
	 * method.
	 * 
	 ...
	 * 
	 * @param isPersistentlyStarted Set to true if this bundle should be marked
	 *        as persistently started.
	 */
	...
	/**
	 * Get updated bundle state. That means check if an installed bundle has
	 * been resolved.
	 * 
	...
	/**
	 * Get class loader for this bundle. Create the classloader if we haven't
	...
	ClassLoader getClassLoader() {
		if (classLoader == null) {
			synchronized (this) {
				if (classLoader == null) {
					classLoader = (BundleClassLoader) AccessController
							.doPrivileged(new PrivilegedAction() {
								public Object run() {
	...
	/**
	 * We have lost one of our imported packages. Set state to INSTALLED and
	 * throw away our tainted classloader.
	 */
	...
	/**
	 * Added our properties to a Propeties object so that we can recreate this
	 * Bundle when we restart framework.
	 * 
	...
	void toProperties(Properties p) {
		...
		p.put(pbase + "isPersistentlyStarted", String
				.valueOf(isPersistentlyStarted));
	}
	...
	URL getBundleURL(int archiveIndex, String name) {
		try {
			return new URL(BundleURLStreamHandler.PROTOCOL, Long.toString(id),
					archiveIndex, name, new BundleURLStreamHandler(this));
		}
	...
	Iterator getExports() {
		if (oldClassLoaders != null) {
			...
			for (Iterator i = oldClassLoaders.values().iterator(); i.hasNext();) {
				for (Iterator j = ((BundleClassLoader) i.next()).getBpkgs()
						.getExports(); j.hasNext();) {
					res.add(j.next());
				}
			}
			if (bpkgs != null) {
	...
	private void filterGetServicePermission(Set srs) {
		...
		for (Iterator i = srs.iterator(); i.hasNext();) {
			...
			String[] classes = (String[]) sr.properties
					.get(Constants.OBJECTCLASS);
			boolean perm = false;
			for (int n = 0; n < classes.length; n++) {
				try {
					acc.checkPermission(new ServicePermission(classes[n],
	...
	private void initPackages() {
		bpkgs = new BundlePackages(this, archive
				.getAttribute(Constants.EXPORT_PACKAGE), archive
				.getAttribute(Constants.IMPORT_PACKAGE), archive
				.getAttribute(Constants.DYNAMICIMPORT_PACKAGE));
		bpkgs.registerPackages();
	}
	 ...
	 * @return A List of Strings with pathname to native code libraries or null
	 *         if input was null.
	...
	private List getNativeCode(Archive a) throws BundleException {
		...
		if (bnc != null) {
			...
			String fosLang = Framework
					.getProperty(Constants.FRAMEWORK_LANGUAGE);
			VersionNumber fosVer = new VersionNumber(Framework
					.getProperty(Constants.FRAMEWORK_OS_VERSION));
			for (Iterator i = Framework.parseEntries(bnc, false); i.hasNext();) {
				Map params = (Map) i.next();
				List pl = (List) params
						.get(Constants.BUNDLE_NATIVECODE_PROCESSOR);
				List ol = (List) params.get(Constants.BUNDLE_NATIVECODE_OSNAME);
				if ((containsIgnoreCase(pl, p) || containsIgnoreCase(pl, Alias
						.unifyProcessor(p)))
						&& (containsIgnoreCase(ol, o) || containsIgnoreCase(ol,
								Alias.unifyOsName(o)))) {
					List lang = (List) params
					...
					if ((best == null || (thisVer != null && (bestVer == null || bestVer
							.compareTo(thisVer) < 0)))
							|| (((bestVer != null && bestVer.compareTo(thisVer) == 0) || bestVer == thisVer)
			...
			if (best == null) {
				throw new BundleException(
						"Native-Code: No matching libraries found.");
			}
			...
			for (Iterator i = res.iterator(); i.hasNext();) {
				...
				try {
					...
					e = new BundleException(
							"Native-Code: File not found in bundle: " + s);
				}
	...
	private void removeBundleResources() {
		...
		for (Iterator i = s.iterator(); i.hasNext();) {
			((ServiceRegistrationImpl) i.next()).reference.ungetService(this,
					false);
		}
	}

	/**
	 * Create a class loader, that is used to load any class from within the
	 * bundles JAR file. If Bundle-ClassPath is present in the Manifest, it is
	 * used to describe the search order for classes. Path can be either dot
	 * ('.') which represents the bundle's JAR file or it can be the path of a
	 * JAR file contained in the bundle's JAR file. Default value is dot.
	 * 
	...
	static void verifyRequiredEEs(String requiredEEs, String loc)
			throws BundleException {
		if (requiredEEs == null || "".equals(requiredEEs))
			return;
		String availableEEs = Framework
				.getProperty(Constants.FRAMEWORK_EXECUTIONENVIRONMENT);
		if (availableEEs == null)
			availableEEs = "";
		...
		String message = "No match for any required excecution environment. "
				+ "Required: " + requiredEEs + "Available: " + availableEEs
				+ "Bundle: " + loc;
