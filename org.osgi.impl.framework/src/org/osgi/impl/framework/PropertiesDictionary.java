 ...
 * Checks that all the keys are strings and adds the class names. Note! Creation
 * of PropertiesDictionary must be synchronized.
 * 
...
public class PropertiesDictionary extends Dictionary {
	...
	PropertiesDictionary(Dictionary in, String[] classes, Long sid) {
		...
		props.put(Constants.SERVICE_ID, sid != null ? sid : new Long(
				nextServiceID));
		if (in != null) {
				...
				try {
					...
					if (props.containsKey(key)) {
						...
						throw new IllegalArgumentException(
								"Properties contains same key (" + key
