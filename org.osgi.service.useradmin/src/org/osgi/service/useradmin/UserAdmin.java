 ...
 * {@link Group}objects. In other words, the members of a <tt>Group</tt>
 * object are roles themselves.
 * <p>
 * Every User Admin service manages and maintains its own namespace of
 * <tt>Role</tt> objects, in which each <tt>Role</tt> object has a unique
 * name.
 * 
 * @version $Revision$
 */
public interface UserAdmin {
	/**
	 * Creates a <tt>Role</tt> object with the given name and of the given
	 * type.
	 * 
	 * <p>
	 * If a <tt>Role</tt> object was created, a <tt>UserAdminEvent</tt>
	 ...
	 * @param type The type of the <tt>Role</tt> object to create. Must be
	 *        either a {@link Role#USER}type or {@link Role#GROUP}type.
	 * 
	 * @return The newly created <tt>Role</tt> object, or <tt>null</tt> if a
	 *         role with the given name already exists.
	 * 
	 * @throws IllegalArgumentException if <tt>type</tt> is invalid.
	 * 
	 * @throws SecurityException If a security manager exists and the caller
	 *         does not have the <tt>UserAdminPermission</tt> with name
	 *         <tt>admin</tt>.
	 */
	public Role createRole(String name, int type);

	/**
	 * Removes the <tt>Role</tt> object with the given name from this User
	 * Admin service.
	 * 
	 * <p>
	 * If the <tt>Role</tt> object was removed, a <tt>UserAdminEvent</tt>
	 ...
	 * 
	 * @return <tt>true</tt> If a <tt>Role</tt> object with the given name
	...
	/**
	 * Gets the <tt>Role</tt> object with the given <tt>name</tt> from this
	 * User Admin service.
	 * 
	 * @param name The name of the <tt>Role</tt> object to get.
	 * 
	 * @return The requested <tt>Role</tt> object, or <tt>null</tt> if this
	 *         User Admin service does not have a <tt>Role</tt> object with
	 *         the given <tt>name</tt>.
	 */
	public Role getRole(String name);

	/**
	 * Gets the <tt>Role</tt> objects managed by this User Admin service that
	 * have properties matching the specified LDAP filter criteria. See
	 * <tt>org.osgi.framework.Filter</tt> for a description of the filter
	 * syntax. If a <tt>null</tt> filter is specified, all Role objects
	 * managed by this User Admin service are returned.
	 * 
	 * @param filter The filter criteria to match.
	 * 
	 * @return The <tt>Role</tt> objects managed by this User Admin service
	 *         whose properties match the specified filter criteria, or all
	 *         <tt>Role</tt> objects if a <tt>null</tt> filter is specified.
	 *         If no roles match the filter, <tt>null</tt> will be returned.
	 *  
	 */
	public Role[] getRoles(String filter) throws InvalidSyntaxException;

	/**
	 * Gets the user with the given property <tt>key</tt>-<tt>value</tt>
	 * pair from the User Admin service database. This is a convenience method
	 * for retrieving a <tt>User</tt> object based on a property for which
	 * every <tt>User</tt> object is supposed to have a unique value (within
	 ...
	 *         more than one matching users are found, <tt>null</tt> is
	 *         returned.
	 */
	public User getUser(String key, String value);

	/**
	 * Creates an <tt>Authorization</tt> object that encapsulates the
	 * specified <tt>User</tt> object and the <tt>Role</tt> objects it
	 * possesses. The <tt>null</tt> user is interpreted as the anonymous user.
	 * The anonymous user represents a user that has not been authenticated. An
	 * <tt>Authorization</tt> object for an anonymous user will be unnamed,
	 * and will only imply groups that user.anyone implies.
	 * 
	 * @param user The <tt>User</tt> object to create an
	 *        <tt>Authorization</tt> object for, or <tt>null</tt> for the
	 *        anonymous user.
	 * 
	 * @return the <tt>Authorization</tt> object for the specified
	 *         <tt>User</tt> object.
	 */
