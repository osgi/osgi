	...
	BundleActivator
{
	...
	/**
	   start. Gets a reference to the TestCaseLink to communicate with
	   the TestCase.
	*/
	...
	*/
		...
		throws Exception
	{
		...
		try {
			...
			// The order of the headers isn't specified, so we'd better sort them.
	...
	*/
	void testMissingManifestHeaders()
		throws Exception
	{
		Bundle  	tb;
		Dictionary  h;
	...
	*/
	void testBundleClassPath()
	{
		...
		log(".","");
		log("Trying to instansiate a class from a bundle class path declared in the manifest file","");
		try
		{
			BundleClass dummy=new BundleClass();
			log("We managed to instansiate a a class from the extended bundleclass path","OK");
		}
		catch (Exception e)
		{
			log("We didn't manage to instansiate a class from the extended bundle class path","FAIL");
	...
	*/
	void testBundleLocation()
	{
		...
		{
			log(".","");
			log("Tests if bundle location remains unchanged after an bundle update.","");
	...
	public synchronized void  frameworkEvent(FrameworkEvent fe)
	{
		try
		{
			...
			case FrameworkEvent.ERROR:				
				log("Testing FrameworkEvent:",
				fe.getThrowable().getClass() + ".");
				break;
	...
	*/
	synchronized void  testFrameworkListener()
		throws Exception
	{
		Bundle  tb;
		synced = false;
	...
	*/
		...
		throws Exception
	{
		...
		try
		{
			h = tb.getHeaders();
			log("",
			"The Import-Package header was " + h.get("Import-Package"));
			tb.uninstall();				
		}
		catch (Exception ex)
		{
			log("got exception while uninstalling test bundle i test double manifest tags","FAIL");
			ex.printStackTrace();
		}
 ...
 void testBundleZero()
	{
		...
		{
			log("BundleException thrown when excecuting bundle(0).uninstall","OK");
			e.printStackTrace();
		}
		
		catch (Exception e)
		{
			log("Unexpected exception when excecuting bundle(0).uninstall","Fail");
			e.printStackTrace();
		}			
	}
  ...
  void testEERequirement() {
    ...
    try {
      ...
      log("Install: No BundleException thrown when EE requirements not fulfilled","Fail");
    } catch(BundleException be) {
      log("Install: BundleException thrown when EE requirements not fulfilled.","OK");
      be.printStackTrace();
    }

    try {
      // Set available EE that does not match, install again, should fail
      System.getProperties().put(Constants.FRAMEWORK_EXECUTIONENVIRONMENT, oneAvailableEENoMatch);
      tb = _context.installBundle(_tcHome + "tb7.jar");
      log("Install: No BundleException thrown when EE requirements not fulfilled","Fail");
    } catch(BundleException be) {
      log("Install: BundleException thrown when EE requirements not fulfilled.","OK");
    ...
    try {
      ...
      System.getProperties().put(Constants.FRAMEWORK_EXECUTIONENVIRONMENT, oneAvailableEEOneMatch);
      tb = _context.installBundle(_tcHome + "tb7.jar");
      log("Install: No BundleException thrown when EE requirements fulfilled.","OK");
    } catch(BundleException be) {
      log("Install: BundleException thrown when EE requirements fulfilled: " + extractNestedMessages(be),"Fail");
    }

    try {
      // Set available EE that does not match, update, should fail
      System.getProperties().put(Constants.FRAMEWORK_EXECUTIONENVIRONMENT, twoAvailableEEsNoMatch);
      tb.update();
      log("Update: No BundleException thrown when EE requirements not fulfilled.","Fail");
    } catch(BundleException be) {
      log("Update: BundleException thrown when EE requirements not fulfilled.","OK");
    }
    try {
      // Set two available EEs with one match, update again, should succeed
      System.getProperties().put(Constants.FRAMEWORK_EXECUTIONENVIRONMENT, twoAvailableEEsOneMatch);
      tb.update();
      log("Update: No BundleException thrown when EE requirements fulfilled.","OK");
    } catch(BundleException be) {
      log("Update: BundleException thrown when EE requirements fulfilled: " + extractNestedMessages(be),"Fail");
    }
    System.getProperties().remove(Constants.FRAMEWORK_EXECUTIONENVIRONMENT);
    try {tb.uninstall();} catch(Throwable ignored){}
  }
}
