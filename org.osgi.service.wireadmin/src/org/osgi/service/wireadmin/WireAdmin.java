 ...
 * 
 * <p>
 * This service can be used to create <tt>Wire</tt> objects connecting a
 * Producer service and a Consumer service. <tt>Wire</tt> objects also have
 * wire properties that may be specified when a <tt>Wire</tt> object is
 * created. The Producer and Consumer services may use the <tt>Wire</tt>
 * object's properties to manage or control their interaction. The use of
 * <tt>Wire</tt> object's properties by a Producer or Consumer services is
 * optional.
 * 
 * <p>
 * Security Considerations. A bundle must have
 * <tt>ServicePermission[GET,WireAdmin]</tt> to get the Wire Admin service to
...
public interface WireAdmin {
	/**
	 * Create a new <tt>Wire</tt> object that connects a Producer service to a
	 * Consumer service.
	 * 
	 * The Producer service and Consumer service do not have to be registered
	 * when the <tt>Wire</tt> object is created.
	 * 
	 * <p>
	 * The <tt>Wire</tt> configuration data must be persistently stored. All
	 ...
	 * <tt>Wire</tt> object's persistent identity (PID). This value is
	 * generated by the Wire Admin service when a <tt>Wire</tt> object is
	 * created.</li>
	 ...
	 * 
	 * @param producerPID The <tt>service.pid</tt> of the Producer service to
	 *        be connected to the <tt>Wire</tt> object.
	 * @param consumerPID The <tt>service.pid</tt> of the Consumer service to
	 *        be connected to the <tt>Wire</tt> object.
	 ...
	 * 
	 * <p>
	 * The <tt>Wire</tt> object representing a connection between a Producer
	 * service and a Consumer service must be removed. The persistently stored
	 * configuration data for the <tt>Wire</tt> object must destroyed. The
	 ...
	 * <tt>filter</tt> must be returned. These <tt>Wire</tt> objects are not
	 ...
	 * 
	 * @param filter Filter string to select <tt>Wire</tt> objects or
