...
public class EventChannelImpl implements EventChannel, FrameworkListener, BundleListener, ServiceListener, Runnable
{
  ...
  {
    try {
      ServiceReference []references = bc.getServiceReferences( "org.osgi.service.event.ChannelListener", null );
      sendEvent( event, references );
    }catch( Exception e )
    {
      log( LogService.LOG_ERROR, "Exception occurred at sending the event!", e );
    }
  }
  ...
  public void sendEvent(ChannelEvent event, ServiceReference []references)
  {
    ...
    for( int i = 0; i != references.length; i++ )
    {
      ...
      try 
      {
        ...
        {
          ChannelListener listener = (ChannelListener)bc.getService( references[i] );
  ...
  public synchronized void postEvent(ChannelEvent event)
  {
    try {
      ...
      {  
        ServiceReference []references = bc.getServiceReferences( "org.osgi.service.event.ChannelListener", null );    
        asynchronEventQueue.addLast( new AsynchronQueueItem( event, references ) );
      }
      notify();
    }catch( Exception e )
    {
...
//  --------------   OSGI R4, OSGI R4, OSGI R4, OSGI R4, OSGI R4, OSGI R4 --------------------------
    ...
    {
      props.put( "exception.class",     event.getThrowable().getClass().getName() );
