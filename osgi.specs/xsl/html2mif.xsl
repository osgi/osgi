...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="table" mode="Tbl">
		...
		<Tbl TblID="{count(preceding::*)}" TblNumColumns="{count( tr[1]/th )}">
	...
	<xsl:template match="td|th" mode="Tbl">
		...
		<Cell>
			...
			<CellContent>
				...
				<!-- <xsl:call-template name="para"> <xsl:with-param name="inside" select="."/> 
					<xsl:with-param name="tag" select="$class"/> </xsl:call-template> -->
			</CellContent>
		</Cell>
	</xsl:template>
	...
	<xsl:template match="img" mode="Frames">
		<Frame FrameType="RunIntoParagraph" AnchorAlign="Left"
			ID="{count(preceding::*)}" ShapeRect="0 0 {@width} {@height}">
