...
public class PowerManagerImpl implements PowerManager {
	 ...
	 */
	public PowerManagerImpl(BundleContext context) {
		this.context = context;
        eventTracker = new ServiceTracker(	context, 
	...
	/**
	 * Indicates the service registration used to register this service
	 * 
	 * @serviceRegistration The service registration used to register this service
	 */
	 ...
	 */
	public synchronized ServiceReference[] setPowerState(int state, boolean urgent)
	{
    	...
    	try {
    		AccessController.checkPermission(new PowerPermission(PowerPermission.SET_SYSTEM_POWER, "system"));
    	}
    	...
    	try {
    		if ((state != PowerManager.S0) && !urgent) {
    			refs = context.getServiceReferences(PowerHandler.class.getName(), null);
    			...
    			for (int i=0; i<refs.length; i++) {
    				...
    				// Retrieve and check if a value is provided for this transition
    				// Check if the power state is supported by the device
    				if (!handler.handleQuery(state)) {
    					queryFailed = true;
		...
		props.put(EventConstants.TIMESTAMP, new Long(System.currentTimeMillis()));
    	
        switch (state) {
        	// The system is strating up or resuming
        	// Devices must be set to DO and event hanlders must be informed
        	case PowerManager.S0:
        		...
        		if (eventAdmin != null)
        			eventAdmin.sendEvent(new Event(eventTopic(state), props));
        		break;
	...
	private void setPowerStateToDevices(int systemPowerState) {
    	...
    	try  {
    		...
    		for (int i=0; i<refs.length; i++) {
    			...
    			try {
    				// Determine the right device power state and set it to the device
    				devicePowerState = determineDevicePowerState(refs[i], systemPowerState);
    				if ((devicePowerState != DevicePower.UNSPECIFIED_STATE) && (device != null))
    					device.setPowerState(devicePowerState);
    			}
    			catch (IllegalArgumentException iae) {
    				log(LogService.LOG_ERROR, "Error while setting a new power state to the device", iae);
	 ...
	 */
	private int determineDevicePowerState(ServiceReference ref, int systemPowerState) {
		int devicePowerState = DevicePower.UNSPECIFIED_STATE;
    	int[] mappings;
    	Integer capa;
		
		mappings = (int[])(ref.getProperty(DevicePower.DEVICE_POWER_MAPPING));
		capa = (Integer)(ref.getProperty(DevicePower.DEVICE_POWER_CAPABILITIES));
	...
	/**
	 * Create the event topic according to the transition that
	 * has occured or will occur.
	 * 
	 * @param systemPowerState The new system power state.
	...
	private String eventTopic(int systemPowerState) {
		...
		switch (systemPowerState) {
			case PowerManager.S0:
				eventTopic = "org/osgi/service/power/SystemEvent/S0";
				break;
			case PowerManager.S1:
				eventTopic = "org/osgi/service/power/SystemEvent/TO_S1";
				break;
			case PowerManager.S2:
				eventTopic = "org/osgi/service/power/SystemEvent/TO_S2";
				break;
			case PowerManager.S3:
				eventTopic = "org/osgi/service/power/SystemEvent/TO_S3";
				break;
			case PowerManager.S4:
				eventTopic = "org/osgi/service/power/SystemEvent/TO_S4";
				break;
			case PowerManager.S5:
				eventTopic = "org/osgi/service/power/SystemEvent/TO_S5";
				break;
			default:
				eventTopic = "unknown System Power State";
				break;
		}
