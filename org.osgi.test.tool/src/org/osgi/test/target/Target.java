...
This activator is loaded in the target framework. It has
a dual function. First, it will broadcast a message on the
local net to make the director aware of local targets. Second,
it will listen to incoming connections for application control.
This will allow the director to download bundles.
<p>
Downloaded bundles are managed by this bundle. If the connection
is closed or lost, this bundle will uninstall all installed
bundles.
<p>
If a second connection comes in, the first connection will be
closed and bundles will be uninstalled.
<p>When a bundle is installed via us, it should get a
TestCaseLink service. We are providing a factory with this. If this
service is obtained, we know the bundle is active. If the bundle
is tired and wants to go, it should unget this service. We
will then uninstall the bundle and inform the director.
 */
		...
		ITarget
{
	...
	public void start( BundleContext context ) throws Exception {
		...
		if ( period != null ) {
			try {
					...
					public void run() {
						try {
							while( true ) {
								...
								if ( ticks++ > MAX_TICKS ) {
									System.err.println("Watchdog is killing this process do to inactivity for " + (MAX_TICKS*WATCHDOG_PERIOD / 60000) + " minutes");
									System.err.println("(This option is controlled through the org.osgi.test.watchdog System property)");
									System.exit(-1);
								}
							}
						}
				...
				// too bad, no watchdog
				System.err.println("Watchdog requested, but value was not an integer " + period);
			}
		}
	}
	...
	Clean up any installed bundles, close the link (if not
	done yet) and reinitialize.
	 */
				...
				if ( old.get( now[i]) == null ) {
					try {
						if ( now[i].getLocation().indexOf( "~keep~" ) < 0 ) {
							now[i].uninstall();
							log( "Removing newly installed bundle " + now[i], null  );
						}
					}
	...
	We have a prefabricated service for each installed
	bundle and return this. If this method is called by
	bundles that were not installed by us, then we return
	null.
	...
	When a testbundle gives up its service, we assume
	it is ready and warn the director about it.
	 */
	public void ungetService(Bundle bundle, ServiceRegistration registration, Object trojan)
	{
		try {
			TestCaseLinkImpl	control = getTestCaseLink(bundle);
			...
			if ( run != null )
				...
				log( "TargetLink already close for ungetservice " + registration, null );
		}
		catch( IOException e ) {
			e.printStackTrace();
		}
	...
	We mainly listen to incoming connections. However, we
	timeout every now and then to broadcast an announcement
	on the net of our existence.
	 */
	public void run()
	{
		...
		try
		{
			...
			+   " comment=" +
				...
				+   " "
				+   getProperty( "org.osgi.framework.version", null )
				+   " "
				+   getProperty( "java.vendor", null )
				+   " "
				+   getProperty( "java.version", null )
				).replace(' ', '_' );
			
			String ipaddr = getProperty( "org.osgi.test.broadcast", "255.255.255.255" );
			...
			try { announce.send(packet); }
				...
				packet = new DatagramPacket(
					...
					2001 );
				try { announce.send(packet); } catch( Exception ee ) {}
			}
			...
			while (cont)
			{
				try
				{
					if ( getProperty( "org.osgi.test.seewait", null ) != null )
						System.out.print( status[ index++ % status.length ] + "\r" );
					...
					log( "Accepting connection from " + socket.getInetAddress(), null );
					...
					try { announce.send(packet); status=ok; } catch(Exception ioe) {
						status = problems;
					}
				}
		...
		catch( Throwable ee )
		{
			...
			try {
				...
				s.close();
			} catch( Exception e) { System.out.println("Cannot cleanup"); }
			if ( run != null )
				run.close();
	...
	/**
	Push an object to the test bundle on this target
	coming from the testcase.
	...
	/**
	Convenience method to find the corresponding test case link
	via its name.
	...
	void log( String msg, Throwable e )
	{
		...
		System.out.println( "Target: " + msg + extra + (e == null? "" : e.toString() ) );
		if ( e != null )
			e.printStackTrace();
		System.out.flush();
	}
	...
	/**
	Framework keys we want to have in the
	applet.
	 */
	...
	We will send some of the framework properties and
	all VM properties to the director.
	 */
	...
	/**
	 * Try to update the framework. However, this method is not
	 * very useful because it seems to run in many race conditions.
	 */
