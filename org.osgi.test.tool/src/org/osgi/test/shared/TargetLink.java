...
/**
	This class is the network link between the target
	framework and the director. This class is used by
	both! By reusing the class, all message types
	and parameter marshalling can be confined to a
	single class.
*/
		...
		Link 
{
	...
	public TargetLink( IRun run )   		{ super("run"); _run = run; }
	public TargetLink( ITarget target ) { super( "target"); _target = target; }
	...
	/**
		Called from director to target to install a new 
		bundle.
	...
	*/
	...
	/**
		Called from director to target to send an object to the
		testbundle.
	...
	*/
	...
	/**
		Called from target to director to send an object
		to the owning testcase.
	...
	*/
	...
	/**
		Called from target to director to log a message
	...
	*/
	...
	/**
		Called from target to director to indicate that the
		bundle has given up the test link service indicating that
		it is done.
	...
	*/
	...
	/**
		Called from the target to the director to indicate the
		properties of the target VM and Framework.
	...
	*/
	public void deliver( Message msg ) throws IOException
	{
		...
		try
		{
			...
			default:
				debug("Invalid cmd for target link " + msg + " "  + this, null);
			}
			msg.reply( OK, result );
		}
		catch( Exception e )
	...
	/**
		Convenience method to turn an inputstream
		into a byte array. The method uses a
		recursive algorithm to minimize memory
		usage.
