...
This is a base class to be used as a link between 
two processes. Serialization is used for communication
protocol.
<p>This class is in the domain of CORBA, RMI, Voyager etc.
These are not used so that no extra components are 
introduced which all have their issues.
E.g. CORBA/RMI creates lots and lots of classes
and have all kind of side effects. RMI is not supported
in Internet Explorer, Voyager needs licensing etc. So
therefore this is a simple point-to-point connection. It
is intended to be extended by a semantic class that
implements the methods and marshalling.
<p>The operation is quite simple. The link class is opened 
with a socket. The other side of the socket should also
be a socket. When the link is opened, it will start a thread
that waits for incoming messages. These are dispatched in
...
public class Link 
	...
	Thread 
{
	private DataOutputStream	  	_out;   				// -> to other link
	private DataInputStream   		_in;					// <- from other link
	Socket  				_socket;				// Socket to allow closing
	boolean 				_open;  				// Are we open?
	Hashtable   			_requests = new Hashtable(); // Outstanding requests
	Thread  				_delivery;
	Queue   				_queue;
	PrintStream 			_debug;
	...
	public int TIMEOUT 					= 1200000;    // ms 30 secs
	...
	public InetAddress getAddress() { return _socket.getInetAddress(); }
		...
		_delivery = new Thread() {
			public void run() {
				while( _open )
					...
					{
						System.out.println( "Error in open(Socket) in " + getName() );
						e.printStackTrace();
					}
			}
		};
	...
	This is a null operation if the link IS already closed.
	All threads waiting for a reply will be
	canceled. We ignore any exceptions here because we
	are cleaning up. If things, fail, tough luck.
	 */
	public void close()
	{
		if ( _open )
		{
			...
			catch(IOException e ) { debug( "close", e ); }
		}
	}
	
	
	/**
	Send a message to the other side. A message is defined
	as an integer for the type and some type of object that
	MUST be serializable. This is an asynchronous notification.
	...
	/**
	Perform a request to the other side and wait for 
	a reply. If the message on the other side threw
	an exception, then we will throw a LinkException
	which contains that remote exception. An exception
	is also thrown when the reply is not received within
	the timeout.
	...
	This run will keep on running as long as the _open
	flag is true. To quit the link, the _open flag should
	be false and the socket should be closed.
	<p>
	We deliver each message in a new thread. Not very efficient,
	but in this case not that relevant.
	 */
	public void run() {
		try {
			debug( "run", null );
			...
			while ( _open )
			{
				int sz = _in.readInt();
				if ( sz < 0 ) {
					System.err.println("Closing because we read "  + sz + " as sz" );
					close();
					break;
				}
		...
		catch( EOFException e ) { debug( "end of sessions other site", e ); }
	...
	/**
	This is when we get a response but nobody is
	waiting for it.
	...
	/**
	Key generator to make an object key from request/reply
	transaction id.
	 */
	Object key( int key ) {
	...
	to implement the message semantics.
