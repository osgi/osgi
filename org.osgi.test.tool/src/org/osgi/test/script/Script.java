 ...
 * Holds a script. 
 *
 * A script is a Tag (XML) holding the script. The tag is made from a 
 * text file that is obtained from a URL. It is compiled using 
 * an XML parser into the Tag object.
 * <p>XML is really easy to use as a script language because it provides
 * the recursive block oriented nature needed. 
 * <p>The command set of the script language is open ended. Any part can
 * add and remove providers. These are normal objects. Any public method
 * starting with a "_" is exported as a script command (without the "_").
 * Reflection is used to find these methods.
 */
public class Script {
	Hashtable   			variables;
	Tag 					result;
	...
	/**
	 * Execute a script with a context and the tag that represents the
	 * script.
	 */
	public Tag execute( ScriptContext ctxt, Tag tag ) throws Exception {
		...
		try {
			for ( Enumeration e=chain.elements(); e.hasMoreElements(); ) {
				...
				if ( method != null ) {
					return (Tag) method.invoke( provider, new Object[] { ctxt, tag } );
				}
			}
	...
	Method findMethod( Class clazz, String method ) {
		try {
			return clazz.getMethod( "_" + method, new Class[] { ScriptContext.class, Tag.class } );
		}
		catch( Exception e ) {
			return null;
		}
