...
/**
 * The Tag class represents a minimal XML tree. It consist of a named
 * element with a hashtable of named attributes. Methods are provided
 * to walk the tree and get its constituents.
 * The content of a Tag is a list that contains String objects or 
 * other Tag objects.
 */
public class Tag {
	Tag 			parent;							// Parent element
	String  		name;							// Name of the tag
	Hashtable   	attributes = new Hashtable();	// Attributes name -> value
	...
	/**
	 * Construct a new Tag with a name and a set of attributes. The
	 * attributes are given as ( name, value ) ...
	 */
	...
	/**
	 * Add a new date attribute. The date is formatted as
	 * the SimpleDateFormat describes at the top of this class.
	 */
	...
	/**
	 * Return a string representation of this Tag and all its
	...
	/**
	 * Return only the tags of the first level of descendants
	 * that match the name.
	 */
	...
	/**
	 * Convenience method to print a string nicely and does 
	 * character conversion to entities.
	 */
	void formatted( PrintWriter pw, int left, int width, String s ) {
		...
		for ( int i=0; i<s.length(); i++ ) {
			...
			switch( c ) {
			case '<':   	pw.print( "&lt;" ); pos+=4;  break;
			case '>':   	pw.print( "&gt;" ); pos+=4; break;
			case '&':   	pw.print( "&amp;" ); pos+=5; break;
			default:		pw.print( c );pos++; break;
			}
