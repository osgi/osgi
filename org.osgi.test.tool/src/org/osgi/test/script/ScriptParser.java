...
class ScriptParser extends DefaultHandler {
	...
	Tag parse( InputSource source ) throws Exception {
		...
		if ( current.getContents().size() != 1 ) 
			throw new RuntimeException( "Only 1 top level tag allowed (should be <script>)" );
	 ...
	 */
	public void startElement (String namespace, String local, String qname, Attributes atts ) {
