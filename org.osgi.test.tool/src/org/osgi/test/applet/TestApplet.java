...
/**
Applet that communicates with the handler to control
the testcases.

This applet is enclosed with the handler bundle and
registered on the local http server. When it is started
it will contact the codebase() on the AppletLink.PORT
(probably 4445). This will not work through firewalls
obviously.
<p>This is not a normal java Applet but instead it is
based on IFC. This is done for many reasons, it makes the
Applet significant smaller (this whole applet including
all the images is <50K, this class <17K) in Netscape and it will look
the same over all platforms (no portability problems like
AWT). IFC can be downloaded for free from the Netscape
site <code> http://developer.netscape.com/docs/manuals/ifc </code>
Obviously it has the disadvantage that it requires the
download of ifc112.jar in Internet Explorer. However, this
should be taken care of in the html file. As an extra feature,
this applet can be run standalone as well without a browser
when ifc112.jar is in the classpath.
<p>Some quick info about IFC. The UI is layed out with a
applet called Constructor (can be downloaded from Netscape). This
UI builder positions and parametrizes all the components. Event
handling is much simpler that AWT/Swing. Each UI component can
send a command string to the TargetChain.applicationChain(). All
interested parties can be on this chain and inspect this string
to see if they can implement the command. In this applet, the
necessary methods (canPerformCommand and performCommand) are
implemented using reflection. This means that a command
in constructor called doIt can be implemented by writing
a method public void doIt( Object o ) in this class. Commands
are normally executed in a single thread. Commands can be
executed in this thread by calling performCommandLater() to
allow synchronization. Important is to not draw components,
but to allow them to be redrawn when the command queue is
empty, ie use setDirty(true) instead of draw(). There exists
several books about IFC:
<pre>
...
Obviously you should start with the quite well written tutorial
that is part of the IFC download.
<p>The components are very easy to
<p>The applet contains two parts. On the upper part, the
list of testcases is shown. On the lower part is the log
output. Buttons are used to control the execution.
<p>This applet uses a proprietare protocol
to talk to the handler. This protocol is implemented
in the AppletLink class. This class is used on both
sides of the link.
<p>Security is implemented using Netscapes security model. Currently
the OSGi has no certificate which means that we must use codebase
principals. This means that the codebase is used as certificate.
Unfortunately, this must be explicitly enabled via a Netscape
property. See
<pre>
 ...
 */
public class TestApplet extends Application implements ExtendedTarget, IApplet, TextViewOwner {
	final static String NOHOST = "no host found";

	Timer   				tick;   			// Mouse emulator to prevent bug
	...
	Button  				debug;  			// Check for debug output
	Button  				local;  			// Only local hosts
	Button  				forever;			// No timeout
	Button  				xml;				// Show raw XML
	Button  				single; 			// Single stepping support
	TextField				prefix;				// Contains bundle name prefix.
	Dictionary  			targetProperties;   // Properties of target (VM + Framework)
	RemoteService   		target; 			// Current selected target ID
	boolean 				run;				// If running a test run
	TextField   			framework;  		// Framework info from targetProperties
	TextField   			os; 				// OS info from targetProperties (Framework)
	TextField   			vm; 				// VM info from targetProperties
	Properties  			cm; 				// Configuration Management properties
	...
	This method customizes the  window and it will read the plan file. The plan
	file is called "test.planb" and is assumed to be
	accessable through the local resources.
	 */
	...
	public void init() {
		...
		try {
			...
			cases   = (ListView) ((ScrollGroup)plan.componentNamed( "cases" )).contentView();
			TabListItem item = (TabListItem) cases.prototypeItem();
			item.setTabs( new int[] { 160 } );

			messages   = (TextView) ((ScrollGroup)plan.componentNamed( "messages" )).contentView();
			messages.setCommandForKey( "copy", null, 'C',KeyEvent.CONTROL_MASK, View.WHEN_IN_MAIN_WINDOW );
			...
			bundles = (ListView) ((ScrollGroup)plan.componentNamed( "bundles" )).contentView();
			...
			status.setStringValue( "v" + getVersion() + " OSGi (c) 2001 All Rights Reserved" );
	...
	/**
	Convenience method to read in the plan file. If the plan
	was already read in, we return the previous plan.
	...
	/**
	Netscape does not implement getResource() in class and
	therefore URLs for resources do not work. This is used
	to hack around this together with the PrivateBitmap class.
	This method is called when the decoder (deserializer)
	needs to create a new class. We just intercept the
	bitmap class and change it to our own implementation
	where we then use the right resource handling.
	 */
	...
	@param  name		full name ame of the resource (as defined in Class.getResourceAsStream())
	...
	/**
	Convenience method to get an image from the resources
	by name. This method assumes images are local to the
	package and stored in the images directory.
	...
	The targets are represented as ID objects and the
	hosts is a popup that holds this list. This method
	is called from the handler via the AppletLink.
	<p>Unfortunately, the popup requires at least one
	element so we have to check for an empty list.
	...
	public void _bundlesChanged(Object parm) throws Exception
	{
		...
		for ( int i=0; i<bs.length; i++ )
			names[i] = (bs[i].getHeaders().get("bundle-name") + "").toUpperCase();
	...
	public void _stopBundle( Object o ) throws Exception {
		...
		for ( netscape.util.Enumeration e= selected.elements(); e.hasMoreElements(); ) {
	...
	If we have a new target we stop a potential current run,
	set the testcase status to unknown and clear target
	info.
	 */
	...
	Called from the handler over the AppletLink. It will
	rebuild the list of testcases and will set status to
	unknown.
	 */
	...
	Depending on the current state, this will cancel
	or start a testrun.
	 */
	...
	public synchronized void _control( Object o ) throws Exception
	{
		...
		try
		{
			...
			testthread = new Thread() {
				public void run() {
					try {
						handler.startRun( applet.target.getHost(), applet.target.getPort(), options );
						for ( netscape.util.Enumeration e=all.elements(); e.hasMoreElements(); ) {
							ListItem	item = (ListItem) e.nextElement();
					...
					performCommandLater( TargetChain.applicationChain(), "showTestResult", lastRun );
					testthread = null;
				}
			};
			testthread.start();
	...
	/**
	Dummy target for emulating mouse events. When a
	view is dirtied in another thread, it is not
	executed until there is a new event. This will
	...
	Get a file name and install that file in the
	handler framework (NOT the target).
	 */
	public void _install( Object o ) throws Exception
	{

		FileChooser fc = new FileChooser(Application.application().mainRootView(),
			"Select bundle to install",
		...
		try {
			...
			if ( e instanceof BundleException) {
				...
				Alert.runAlertInternally("Could not install bundle" + dir + "/" + name,
					"Error msg: " + msg,"OK",null,null );
			}
		}
	}
	...
	public void _saveLog( Object o ) throws Exception
	{
		...
		&& dir != null )
		{
			...
			PrintWriter 	out = new PrintWriter( new FileWriter( new File( dir, name ) ) );
	...
	Update the testcase line in the window to show an icon for
	the result.
	...
	The testcases can contain an icon for each test
	case. This method sets the icon for one or for
	all when the ID is null.
	...
	/**
	Called from the handler to set the framework
	and VM properties from the target.
	
	This method will update the framework, os and
	vm properties on the window.
	...
	public void setTargetProperties( Dictionary properties )
	{
		...
		framework.setStringValue(
				properties.get( "org.osgi.framework.vendor" )
			+   " "
			+   properties.get( "org.osgi.framework.version" ));

		os.setStringValue(
				properties.get( "org.osgi.framework.os.name" )
			+   " "
	...
	Tag tag = (Tag) history.get( item.data() );
	show( tag, "Test run history", false );
	 */
	}
	...
	/**
	Convenience method to find a method from the reflection
	API.
	 */

	public Method find(String command) throws NoSuchMethodException, IllegalAccessException
	{
	...
	/**
	Implementation of @see ExtendedTarget interface.
	
	Lookup the command as a method with a '' prefix
	and x(String,Object). If the method exists, execute it
	with the given parameters, else throw an exception.
	
	@param command  Name of command, translated method: <command>(String,Object)
	...
	public void performCommand(String command, Object arg)
	{
		...
		catch( NoSuchMethodException e ) { throw new RuntimeException( "No Such Method " + command ); }
	}
	...
	Lookup the command as a method with a '' prefix
	and x(String,Object). If the method exists, return true,
	else return false.
