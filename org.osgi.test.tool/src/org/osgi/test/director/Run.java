...
/**
	...
	This class will be created by the handler on request
	of the ui or script.
	<p>When we start the testcase we handout the TestRun
	interface which is us. Through this interface, the testcase
	can create bundles in the target and send/receive
	objects with the target. When the testcase
	returns, we will clean up their mess.
	<p>During the run, we maintain a history in the Tag
	object.
*/
public class Run
	implements 
	...
	TargetLink  		target;			// Link to the target, only open during testcase exec.
	...
	long				lasttime;		// Separates testcases at least 2 secs
	boolean				hadProperties;	// Waits until properties are send after closing link.
	...
	*/
	...
	Run( Handler handler, IApplet applet, String host, int port, int options ) throws Exception {
		this.host = host;
		this.port = port;
	...
	/**
	 * Execute a testcase. A testcase can be a normal testcase
	 * or a script.
	 * The target is opened when it is a normal testcase
	 * and a testcase is initiated on the target.
	 */
	public Tag doTestCase( TestCase tc ) throws Exception {
		...
		System.out.println("\r\n----------------------------------------------\r\n"
		+	"Opening to " + host + ":" + port + " for testcase " + tc.getName()
		+	"\r\n----------------------------------------------" );
		//
		...
		if ( target.isOpen() ) {
			for ( Enumeration e = testbundles.elements(); e.hasMoreElements(); ) {
				...
				try { testbundle.uninstall(); } catch( Exception ee) {/* disaster recovery */}
			}
			for ( Enumeration e = testbundles.elements(); e.hasMoreElements(); ) {
				TestBundleImpl testbundle = (TestBundleImpl) e.nextElement();
	...
	/**
		...
		All testcases are aborted and the run finishes as quickly 
		as possible. Obviously we need support from the testcase
		to make this work.
	*/
	public void close() {
		try {
	...
	/**
		Report progress to the applet. We assume that the
		testcase will report progress relative to itself
		so we recalculate it to handle n testcases and
		then report it to the applet.
	...
	*/
	...
	/**
		Create a test bundle in the target and compare the
		reference output.
	...
	*/
	...
	public TestBundle createTestBundle(
		String name,
		InputStream jar,
	...
	*/
	...
	/**
		Called from the targetlink to set the properties as
		defined in the target for the VM and the framework.
	*/
	
	public synchronized void setTargetProperties( Dictionary properties ) throws IOException {
		if ( ! hadProperties ) {
			hadProperties = true;
	...
	*/
	 ...
	 *
	 * To avoid confusion, we wait until the target has send us
	 * its properties. This is synchronized.
	 */		
