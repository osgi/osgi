...
/**
 * Control the tests.
 * Originally, the director had an applet and this class was instantiated for each
 * applet session. In 2001, the applet was removed and the handler is now
 * instantiated only once in the director.
 * <p>The handler can start a test run (startRun). After this the getRun()
 * will return a valid Run object. The Run object can be used to do zero
 * or more testcases. At the end, the handle can stop the run (stopRun) and
 * will gather the test results. During a run, test results are stored in an
 * XML oriented object (Tag). Each aspect of the test run adds information
 * to this history object. The final result is registered on the web.
*/
public class Handler
	implements BundleListener
{
	BundleContext   		context;		// Framework interface
	TestCaseTracker 		caseTracker;	// Holds wrappers around the testcases
	RemoteServiceTracker	targetTracker;  // Wrappers around the targets
	HttpTracker				httpTracker;	// Registers a servlet for results + target
	...
	File					directory;		// Where test results are stored
	String					lastRun;		// Name of last made test result file.
	...
	*/
	Handler( BundleContext context ) throws Exception {
		...
		applicant 	= getProperty( "org.osgi.test.compliance.applicant", "[property org.osgi.test.compliance.applicant notset]" );
		program 	= getProperty( "org.osgi.test.compliance.program",   "[property org.osgi.test.compliance.program notset]" );
		campaign 	= getProperty( "org.osgi.test.compliance.campaign",  "[property org.osgi.test.compliance.campaign notset]" );
	}
	...
	*/
	...
	/**
	 * Stop the test run and create an XML file in the
	 * history directory with the time/date as name. These
	 * files are made available on the web in the HttpTracker.
	 */
	public String stopRun() {
		if ( run != null ) {
			try {
				java.text.SimpleDateFormat	sf= new java.text.SimpleDateFormat( "MMddHHmmss" );
				lastRun = sf.format(new Date()) + ".xml";
				File file = new File( directory, lastRun );
				PrintWriter pw = new PrintWriter( new FileWriter( file ) );
				pw.println( "<?xml version='1.0' encoding='ISO-8859-1'?>" );
				pw.println( "<?xml-stylesheet type='text/xsl' title='Compact' href='http://membercvs.osgi.org/xsl/testresult-compact-1.xsl'?>" );
				pw.println( "<?xml-stylesheet type='text/xsl' title='Full' href='http://membercvs.osgi.org/xsl/testresult-full-1.xsl'?>" );
	 ...
	 *
	 * Sometimes hosts are not able to broadcast and can thus not
	 * reach the director. In those cases it is possible to add
	 * a host manually from the UI.
	 */
	public void connectManual( String host )  {
		try {
			...
			if ( context.getServiceReferences(
				RemoteService.class.getName(),
				"(&(application=org.osgi.test)(host="+name+")(port="+port+"))" )
				== null ) {
	...
	/**
	 * Convenience method to return the file name part of a url
	 ...
	 *
	 * If the out parameters has a ':' in the 3 .. 10 th column,
	 * a URL is assumed, else a file name. If it is a URL,
	 * the output facility of the URL is used. This is not
	 * always possible but works for example for mailto:,
	 * http: and ftp:
	 */
	...
	*/
	...
	/**
		Called when the run is finished. We will update the
		applet.
	*/
	...
	public ScriptEditor newScriptEditor(String name, URL path) {
		...
		editor.registration = context.registerService(
	...
	/**
	 * Get the description of a testcase. This includes lots
	 * of information of the service.
	 */
	...
	/**
	 * Return the base url to the/a web server where the results
	 * can be found.
	 */
