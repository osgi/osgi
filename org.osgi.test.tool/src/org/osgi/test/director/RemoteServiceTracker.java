...
/**
 * Track the RemoteService services in the registry.
 * When such a service is discovered, add it to the list
 * and notify the UI.
 */
class RemoteServiceTracker extends ServiceTracker {
	...
	RemoteServiceTracker( Handler handler, BundleContext context ) throws InvalidSyntaxException {
		super( context, 
			context.createFilter(
				"(&(application=org.osgi.test)(objectClass=" + RemoteService.class.getName() + "))" ),
			null ); 	
