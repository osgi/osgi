...
/**
	...
	A testcase can download a bundle in the target
	framework. This class represents this remote
	bundle in the local framework. It implements the
	org.osgi.test.service.TestBundle interface for this
	purpose.
*/
class TestBundleImpl 
	implements 
		TestBundle
{   
	...
	Vector  					input = new Vector();  // Look ahead for reference output
	BufferedReader			 	ref;
	InputStream 				refIn;
	...
	public TestBundleImpl( Run run, String name, InputStream ref )
		...
		this.refIn = ref;
		if ( ref != null )
			this.ref = new BufferedReader( new InputStreamReader(ref, "ISO-8859-1") );
		else
	...
	/**
	 * Processes the remaining reference log and
	 * reports the number of lines left.
	 */
	 ...
	 *
	 * Log messages are compared to the reference output. The
	 * comparing will do a lookahead so it can skip lines that do
	 * not match.
	 */
	...
	/**
	 * Get the next line of the reference input, taking prior lookahead
	 * into calculation.
	 */
