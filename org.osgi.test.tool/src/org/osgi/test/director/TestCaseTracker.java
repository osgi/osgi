...
class TestCaseTracker extends ServiceTracker {
	...
	/**
	 * Convenience method to create an HTML description
	 * of a testcase. Uses the bundle manifest to find
	 * many details of the testcase.
	 */
	String getDescription( ServiceReference ref ) {
		...
		append( result, "<em>Version</em>   		  ",  (String) b.getHeaders().get( "bundle-version"), "<br>" );
		append( result, "<em>Copyright</em> 		  ",  (String) b.getHeaders().get( "bundle-copyright"), "<br>" );
		append( result, "<em>Vendor</em>			  ",  (String) b.getHeaders().get( "bundle-vendor"), "<br>" );
		String docurl = (String) b.getHeaders().get( "bundle-docurl");
		append( result, "<em>Documentation</em> 	  <a href='", docurl , "'>" + docurl + "</a><br>" );
		append( result, "<em>Contact address</em>   ",  (String) b.getHeaders().get( "bundle-contactaddress"), "<br>" );
		append( result, "<em>Location</em>  		  ",  (String) b.getLocation(), "<br>" );
	 ...
	 *
	 * We build an XML structur of all the information we can
	 * find about the testcase's bundle.
	 */
	Tag getDescriptionTag( TestCase tc ) {
		...
		try {
			...
			try { md = java.security.MessageDigest.getInstance("MD5"); } catch( Exception e ){}
			...
			System.out.println( "Bundle location is not a url, no MD5 digest possible, " + bundle.getLocation() );
		}
		catch( Exception e ) {
			e.printStackTrace();
		}
	...
	TestCase getTestCase( String name ) { return (TestCase) testcases.get( name ); }
}
