...
/**
 *	The Discovery class keeps track remote services on
 *	the local net and registers these in the registry.
 *	
 *	A local net service broadcasts regularly a message
 *	with:
 *	<pre>
 *		application=<app> host=<host> port=<port>
 *	</pre>
 *	This class picks these messages up and parses them. If
 *	a service is detected it is registered as a RemoteService
 *	object in the framework registry. As a property, the
 *	application is set to the received application. When a
 *	broadcast has not been received for a certain amount of time,
 *	the service is removed again. This
 *	allows interested parties to listen to remote services
 *	in the framework.
 */
		...
		Thread
{
	...
	boolean 				cont 	   	= true;
	Hashtable   			services   	= new Hashtable();  // RemoteService -> RemoteServiceImpl
	...
	*/
	...
	/**
		...
		If the service already exists, the time is
		updated. Else it is registered in the
		framework. The parameter is a newly created
		service and is used to find the same
		service in the local hashtable.
	...
	*/
	...
	/**
		A service was not found during a certain time, remove
	...
	*/
	...
	/**
		...
		Listen to datagrams on port 2001 and parse these.
		The listening is done with a time out. In this timeout
		the existing services are checked and removed if their
		lease expired.
	*/
	public void run() {
		try {
			...
			while ( cont ) {
				try {
					...
					&&   application != null
					&&   port > 0 ) {
						RemoteServiceImpl   service = new RemoteServiceImpl( application, host, port, comment );
						updateService( service );
					}
					else
						log( "Invalid remote service request from " + packet.getAddress(), null );
				}
