...
class HttpTracker extends ServiceTracker implements HttpContext {
	 ...
	 */
	public boolean handleSecurity(HttpServletRequest rq, HttpServletResponse rsp) { return true; }
 ...
 *
 * Test results are stored as XML files but can be expanded using
 * the included XSLT processor. The following parameters
 * are supported in the URL:
 * <pre>
...
class TestServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws IOException {
		...
		try {
			if ( source == null ) {
				...
				out.println( "<html><head><title>Test Results</title></head><body>" );
				out.println( "<h1>Test Results</h1>" );
				File		result[] = handler.getResults();
				for ( int i=0; result != null && i < result.length; i++ ) {
					out.println( 
						"<a name='" + result[i].getName() + "'>" +
						"<a href='" + Handler.ALIAS + "?source=" + result[i].getName() + "'>" + result[i].getName() + "</a> <a href='" + Handler.ALIAS + "?source=" + result[i].getName() + "&style=testresult.xsl'>FORMATTED</a><br>" );
				}
	 ...
	 */
	void copyraw( String source, HttpServletRequest rq, HttpServletResponse rsp ) throws Exception {
	 ...
	 */
	void apply( String style, String source, HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			transformer.transform(new StreamSource(handler.getResult( source )), new StreamResult(out));
		} catch (Exception err) {
