...
/**
	...
	This bundle is installed in a framework and is responsible
	for provding a test harness for testcases. The framework
	registry is used to find target frameworks (the once that
	need to be tested) and testcases. Testcases are installed
	as other bundles wich register a testcase service.
	<p>When the bundle is started, a UI will pop up.
	<p>The osgi-target.jar should be installed in the target framework. It will
	broadcast its net address which will be picked up by the
	Discovery class here. This will register a service that the
	handlers are listening to.
 */
		...
		BundleActivator
{
	...
	BundleActivator 		activator;  		// Activator of applet start code
	public static Handler   handler;			// The handler for the applet/script
	 ...
	 *	
	 *	Initialize and start the correct configuration. Properties
	 *	are used to start a script or a UI or both.
	 */
	...
	public void start(BundleContext context) throws Exception {
		...
		commands 		= new TestCommands( handler, context ); // Interface for script/console
		...
		if ( script != null ) {
			if ( ui == null )
				ui = "false";
			Thread thread = new Thread() { public void run() { doScript(); } };
			thread.start();
		}
	...
	public void doScript() {	
		...
		try {
			System.out.println( "Running script on " + host + ":" + port + " " + new URL(script)  );
		...
		try {
			...
			if ( out.endsWith( ".html" ) ) {
				String url = handler.getBaseURL() + "?source=" + result + "&style=testresult.xsl";
