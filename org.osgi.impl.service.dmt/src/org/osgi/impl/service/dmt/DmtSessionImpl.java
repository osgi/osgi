...
class EventStore {
    ...
    synchronized void add(int type, Node node, Node newNode, Acl acl,
        ...
        if (isAtomic) { // add event to event store, for delivery at commit
            ...
            DmtEventCore event = events.size() > 0 ? (DmtEventCore) events.getLast() : null;
            ...
            boolean sameNode = eventNodes != null && eventNodes.size() > 0	&& node.equals(eventNodes.get(eventNodes.size() - 1)); 
            if (!sameNode) 
