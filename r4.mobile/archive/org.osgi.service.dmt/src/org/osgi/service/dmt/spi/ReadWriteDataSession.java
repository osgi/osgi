 ...
 * <p>
 * When a tree manipulation command is called on the Dmt Admin service, it must
 ...
 * etc.  All errors described by the error codes
...
public interface ReadWriteDataSession extends ReadableDataSession {
     ...
     *         to a non-existing node, or if <code>newNodePath</code> points to
     *         a node that cannot exist in the tree
     *         <li><code>NODE_ALREADY_EXISTS</code> if <code>newNodePath</code>
     ...
     *         a sibling of <code>nodePath</code> with the name 
     *         <code>newName</code>
     *         <li><code>METADATA_MISMATCH</code> if the node could not be
     ...
     *         <li><code>NODE_NOT_FOUND</code> if <code>nodePath</code> points
     *         to a non-existing node
     *         <li><code>METADATA_MISMATCH</code> if the type could not be set
     *         because of meta-data restrictions
     *         <li><code>DATA_STORE_FAILURE</code> if an error occurred while
     *         accessing the data store
     *         <li><code>COMMAND_FAILED</code> if some unspecified error is 
     ...
     *         <li><code>NODE_NOT_FOUND</code> if <code>nodePath</code> points
     *         to a non-existing node
     *         <li><code>METADATA_MISMATCH</code> if the value could not be set
     *         because of meta-data restrictions
