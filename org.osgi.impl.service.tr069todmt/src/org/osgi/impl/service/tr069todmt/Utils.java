...
public class Utils {
  ...
  /*
   FORMAT_DATE - value must be parsable to an ISO 8601 calendar date in complete representation, basic format (pattern {@code CCYYMMDD})
   TODO - CC stands for?!? May be Century?
   */
  ...
  static String getDmtValueAsString(Node node) throws DmtException {
    ...
    switch (dmtValue.getFormat()) {
      case DmtData.FORMAT_INTEGER: {
        ...
        /* The FORMAT_TIME must be treated as a relative time for TR-069.
           
           If absolute time is not available to the CPE, it SHOULD instead indicate the relative time since boot, where the boot
           time is assumed to be the beginning of the first day of January of year 1, or 0001 01 01T00:00:00. Relative time since
           boot MUST be expressed using an untimezoned representation.
         */
  ...
  static int getTR069Type(DmtData dmtValue, String[] mimeTypes) {
    ...
    switch (dmtType) {
      ...
      case DmtData.FORMAT_TIME:
      case DmtData.FORMAT_DATE_TIME: {
        /* datetime (default for FORMAT_DATE and FORMAT_DATE_TIME), string (default for FORMAT_TIME) */
        if (mimeTypeDefinedType == TR069Connector.TR069_DEFAULT || 
      ...
      case DmtData.FORMAT_INTEGER:
      case DmtData.FORMAT_LONG: {
        /* int (default for FORMAT_INTEGER), long (default for FORMAT_LONG and FORMAT_FLOAT), string, unsignedInt, unsignedLong*/
        if (mimeTypeDefinedType == TR069Connector.TR069_DEFAULT || 
      ...
      case DmtData.FORMAT_NULL: {
        /* boolean, datetime, int, long, string (default), unsignedInt, unsignedLong */
        if (mimeTypeDefinedType == TR069Connector.TR069_DEFAULT || 
            mimeTypeDefinedType == TR069Connector.TR069_BOOLEAN || 
            mimeTypeDefinedType == TR069Connector.TR069_DATETIME ||
            mimeTypeDefinedType == TR069Connector.TR069_INT ||
  ...
  static String getTR069TypeName(int type) {
    switch (type) {
      case TR069Connector.TR069_INT: return "int";
      case TR069Connector.TR069_UNSIGNED_INT: return "unisgnedInt";
      case TR069Connector.TR069_LONG: return "long";
      case TR069Connector.TR069_UNSIGNED_LONG: return "unsignedLong";
      case TR069Connector.TR069_STRING: return "string";
      case TR069Connector.TR069_BOOLEAN: return "boolean";
      case TR069Connector.TR069_BASE64: return "base64";
      case TR069Connector.TR069_HEXBINARY: return "hexBinary";
      case TR069Connector.TR069_DATETIME: return "dateTime";
