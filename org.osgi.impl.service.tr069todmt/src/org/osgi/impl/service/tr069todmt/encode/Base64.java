...
public class Base64 {
  ...
  /**
   * Encodes binary data to Base 64 ASCII string. Result is with 76 symbols per
   * line.
   * 
  ...
  /**
   * Encodes binary data to Base 64 ASCII string. The function can break the result into lines
   * with specified numbers of characters.
   * 
   * @param data
   *            byte array data for encoding.
   * @param off1
  ...
  public static byte[] decode(byte[] data, int off, int length) throws Exception {
    ...
    if (data[len - 1] == PADDING) padding++ ; // 1 or 2 chars
    if (data[len - 2] == PADDING) padding++ ; // 1 char, otherwise - 2
  ...
  private static final void decodeAtom(byte[] b_out, byte[] b_in, int off1, int off2) throws Exception {
    if ((off1 < 0) || (b_in.length <= off1)) throw new Exception("decodeAtom exception: invalid offset");
