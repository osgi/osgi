...
public class TBC extends Thread
{
	...
	*/
		...
		throws Exception
	{
		...
		//ServiceReference sr = context.getServiceReference("org.osgi.service.packageadmin.PackageAdmin");
				...
				|| (pa.getExportedPackage(TP3) != null))
		{
			...
			//Call ExportedPackage.getSpecificationVersion() for ...testpackage1
			log("The specification version for TestPackage 1: 1.0.1 : '"+pa.getExportedPackage(TP1).getSpecificationVersion()+"'");
			//Install tb3
			...
			log("Testbundle 1 exports only TestPackage 1: true : '"+(eps.length == 1)+"'");
			...
			//Is it tb1? (yes)
			log("What bundle is exporting TestPackage 1. Is it Testbundle 1: true : '"+(eb.equals(tb1))+"'");
			//Is it tb3? (no)
			log("What bundle is exporting TestPackage 1. Is it Testbundle 3: false : '"+(eb.equals(tb3))+"'");
			...
			for(int i = 0; i<ib.length;i++){
				if(ib[i].equals(tb2))
					i2=true;
				else if(ib[i].equals(tb3))
					i3=true;
				else  if(ib[i].equals(tb1))
					i1=true;
			}
			log("What bundles is importing TestPackage 1. Is it Testbundle 1: true : '"+i1+"'");
			log("What bundles is importing TestPackage 1. Is it Testbundle 2: true : '"+i2+"'");
			log("What bundles is importing TestPackage 1. Is it Testbundle 3: true : '"+i3+"'");
            link.send("50");
			//Uninstall tb1
			tb1.stop();
			tb1.uninstall();
			...
			if(eb!=null){
				//Is it tb1? (no)
				log("What bundle is exporting TestPackage 1. Is it Testbundle 1: false : '"+(eb.equals(tb1))+"'");
				//Is it tb3? (yes)
				log("What bundle is exporting TestPackage 1. Is it Testbundle 3: true : '"+(eb.equals(tb3))+"'");
			}else
				log("Nobody is exporting TestPackage 1.");

			//Call ExportedPackage.getSpecificationVersion() for ...testpackage1
			log("The specification version for TestPackage 1: 1.0.2 : '"+pa.getExportedPackage(TP1).getSpecificationVersion()+"'");
			//Call ExportedPackage.getName() for ...testpackage1
			log("What is TestPackage 1's name: org.osgi.test.cases.packageadmin.tc3.tb1 : '"+pa.getExportedPackage(TP1).getName()+"'");
            link.send("75");
			tb2.stop();
			tb2.uninstall();
			tb3.stop();
