...
public class GetNodeTitle {
	...
	private void testGetNodeTitle001() {
		...
		try {
			...
			String title = session.getNodeTitle(TestReadOnlyDataPluginActivator.TEST_READ_ONLY_PLUGIN_ROOT);
			tbc.assertEquals("Asserts that DmtAdmin fowarded "+ TestReadOnlyDataPlugin.GETNODETITLE+" to the correct plugin",TestReadOnlyDataPlugin.GETNODETITLE,title);
		} catch (Exception e) {
	...
	private void testGetNodeTitle002() {
		...
		try {
			...
			session = tbc.getDmtAdmin().getSession(TestReadOnlyDataPluginActivator.TEST_READ_ONLY_PLUGIN_ROOT,
					DmtSession.LOCK_TYPE_EXCLUSIVE);
			session.getNodeTitle(TestReadOnlyDataPluginActivator.INTERIOR_NODE_EXCEPTION);
			...
			tbc.assertTrue("Asserts that DmtAdmin fowarded the DmtException with the correct message. ", 
					e.getMessage().indexOf(TestReadOnlyDataPlugin.GETNODETITLE)>-1);			
		} catch (Exception e) {
