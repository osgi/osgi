 ...
 * @author Andre Assad
 * 
 * A test implementation of DmtExecPlugin. This implementation validates the
 * DmtSession calls to a subtree handled by a DmtExecPlugin.
 */
public class TestExecPlugin implements DmtExecPlugin, DmtDataPlugin {

	public static final String EXECUTE = "DmtExecPlugin.execute";
	...
	public void execute(DmtSession session, String nodeUri, String correlator,String data)
			throws DmtException {
		if (nodeUri.equals(TestExecPluginActivator.INTERIOR_NODE_EXCEPTION)) {
			...
			if (null == session) {
				...
				tbc
						.assertEquals(
								"Asserts that DmtSession is fowarded to DmtExecPlugin with the same lock type",
								DmtSession.LOCK_TYPE_EXCLUSIVE, session
										.getLockType());
				tbc
						.assertEquals(
								"Asserts that DmtSession is fowarded to DmtExecPlugin with the same subtree",
								TestExecPluginActivator.TEST_EXEC_PLUGIN_ROOT, session
										.getRootUri());
			}
			DmtTestControl.TEMPORARY = EXECUTE;
			DmtTestControl.PARAMETER_3 = data;
