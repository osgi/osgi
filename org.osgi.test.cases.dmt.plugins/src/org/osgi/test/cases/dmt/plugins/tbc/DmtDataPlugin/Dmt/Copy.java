 ...
 * @author Andre Assad
 * 
 * @methodUnderTest org.osgi.service.dmt.Dmt#copy
...
public class Copy {
	...
	private void testCopy001() {
		...
		try {
			...
			tbc.assertEquals("Asserts that DmtAdmin fowarded "
					+ TestDataPlugin.COPY + " to the correct plugin",
					TestDataPluginActivator.INEXISTENT_NODE,
	...
	/**
	 * @testID testCopy002
	 * @testDescription Forces a DmtException in order to check if Dmt#copy
		...
		try {
			...
			tbc
					.assertTrue(
							...
							e.getMessage().indexOf(TestDataPlugin.COPY) > -1);
		} catch (Exception e) {
