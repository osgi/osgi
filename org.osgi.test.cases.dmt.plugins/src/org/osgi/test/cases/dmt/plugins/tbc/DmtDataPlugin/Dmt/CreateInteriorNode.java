...
public class CreateInteriorNode {
	 ...
	 *                  createInteriorNode to the correct plugin.
	 */
	private void testCreateInteriorNode001() {
		...
		try {
			...
			session
					.createInteriorNode(TestDataPluginActivator.INEXISTENT_NODE);

			tbc.assertEquals("Asserts that DmtAdmin fowarded "
					+ TestDataPlugin.CREATEINTERIORNODE_1
	 ...
	 *                  Dmt#createInteriorNode correctly forwards the exception
	 *                  to DmtAdmin.
	 */
	private void testCreateInteriorNode002() {
		...
		try {
			...
			session = tbc.getDmtAdmin().getSession(
					...
					DmtSession.LOCK_TYPE_EXCLUSIVE);
			session
					.createInteriorNode(TestDataPluginActivator.INEXISTENT_NODE_EXCEPTION);
			tbc.failException("#", DmtException.class);
		} catch (DmtException e) {
			tbc
					.assertEquals(
							"Asserts that DmtAdmin fowarded the DmtException with the correct subtree: ",
							TestDataPluginActivator.INEXISTENT_NODE_EXCEPTION, e
									.getURI());
			tbc
					.assertEquals(
			...
			tbc
					.assertTrue(
							...
							e.getMessage().indexOf(
									TestDataPlugin.CREATEINTERIORNODE_1) > -1);
		} catch (Exception e) {
	 ...
	 *                  java.lang.String type) to the correct plugin.
	 */
	private void testCreateInteriorNode003() {
		...
		try {
			...
			tbc.assertEquals("Asserts that DmtAdmin fowarded "
					+ TestDataPlugin.CREATEINTERIORNODE_2
					+ " to the correct plugin",
					TestDataPlugin.CREATEINTERIORNODE_2,
					DmtTestControl.TEMPORARY);
		} catch (Exception e) {
		...
		try {
			...
			tbc
					.assertTrue(
							"Asserts that DmtAdmin fowarded the DmtException with the correct message. ",
							e.getMessage().indexOf(
									TestDataPlugin.CREATEINTERIORNODE_2) > -1);
		} catch (Exception e) {
			tbc.fail("Expected " + DmtException.class.getName() + " but was "
					+ e.getClass().getName());
		} finally {
