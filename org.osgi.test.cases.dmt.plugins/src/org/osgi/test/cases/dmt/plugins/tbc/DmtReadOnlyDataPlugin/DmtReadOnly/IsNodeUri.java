 ...
 * @author Andre Assad
 * 
 * @methodUnderTest org.osgi.service.dmt.DmtReadOnly#isNodeUri
...
public class IsNodeUri {
	 ...
	 *                  isNodeUri to the correct plugin.
	 */
	private void testIsNodeUri001() {
		...
		try {
			...
			tbc.assertTrue("Asserts that DmtAdmin fowarded "+ TestReadOnlyDataPlugin.ISNODEURI
					+" to the correct plugin",session.isNodeUri(TestReadOnlyDataPluginActivator.INTERIOR_NODE));
	 ...
	 * @testDescription Asserts that DmtAdmin correctly forwards the call of
	 *                  isNodeUri to the correct plugin, by passing an invalid
	...
	private void testIsNodeUri002() {
		...
		try {
			...
			session = tbc.getDmtAdmin().getSession(TestReadOnlyDataPluginActivator.TEST_READ_ONLY_PLUGIN_ROOT,
					DmtSession.LOCK_TYPE_EXCLUSIVE);
			tbc.assertTrue("Asserts that DmtAdmin fowarded "+ TestReadOnlyDataPlugin.ISNODEURI
					+" to the correct plugin",!session.isNodeUri(TestReadOnlyDataPluginActivator.INEXISTENT_NODE));
