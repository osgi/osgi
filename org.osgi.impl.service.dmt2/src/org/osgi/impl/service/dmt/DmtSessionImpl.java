...
//            context.getPluginDispatcher().getDataPlugin(node);
    	...
    	ServiceReference pluginReference = context.getPluginDispatcher().getPluginReference(node.getPath());
        Node root = getRootForPlugin(pluginReference, node);
        ...
        DataPlugin plugin = (DataPlugin) context.getBundleContext().getService(pluginReference);
        ...
        wrappedPlugin = new PluginSessionWrapper(pluginReference,
    ...
    private Node getRootForPlugin(ServiceReference plugin, Node node) {
//        Node[] roots = plugin.getDataRoots();
    	String[] roots = (String[]) plugin.getProperty( "dataRootURIs");
        
        for(int i = 0; i < roots.length; i++) {
        	Node root = new Node( Uri.toPath(roots[i]));
            if(root.isAncestorOf(node))
                return root.isAncestorOf(subtreeNode) 
                    ? subtreeNode : root;
        }
